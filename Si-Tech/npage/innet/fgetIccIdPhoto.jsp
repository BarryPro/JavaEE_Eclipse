...
<%
	System.out.println("----------->fgetIccIdPhoto.jsp BEGIN");
	String retMsg = "";
	/*noPhoto 公安局无客户图片时默认图片base64码*/
...
"2Q==";
	String picPath = "/npage/picTemp/";
	String path = request.getContextPath();
	String basePath = request.getScheme()+"://"+request.getServerName()+":"+request.getServerPort()+path+"/";
	System.out.println("basePath="+basePath+"");
	//out.println("<img name='basePic2' src='"+trace+"'></img> ");
	String iccIdString = request.getParameter("idIccid")==null?"":request.getParameter("idIccid");
	System.out.println("iccIdString="+iccIdString);
	boolean ok = false;
	String fileName="";
	String trace ="";
	String picHTMLPath = "";
	if(!"".equals(iccIdString))
	{
//		long idLength  = Long.parseLong(request.getParameter("idIccid"));
		String idLength = request.getParameter("idIccid");
		fileName = idLength+".jpeg";
		trace = request.getRealPath(picPath)+"/"+fileName;
		System.out.println("trace="+trace+"");
		//DataSource ds =DataSourceUtils.getDataSource() ;
		DataSource ds = null;
		try {
			Context initCtx = new InitialContext();
			String dsJndiName = "GZTDS";
			ds = (DataSource)initCtx.lookup(dsJndiName);
		}
		catch (Exception ex)
        {
        System.out.println("取数据库连接池错误:"+ex);
        }
		System.out.println("获得DataSource="+ds+"");
		Connection conn = ds.getConnection();
		System.out.println("获得Connection="+conn+"");
		String sql = "select iccid_image from dCustIccIdChkMsg where id_iccid = ?";
		PreparedStatement ps = conn.prepareStatement(sql);
		ps.setString(1, idLength);
		System.out.println("获得PreparedStatement="+ps+"");
		ResultSet rs = ps.executeQuery();
		System.out.println("获得ResultSet="+rs+"");
		Clob photoClob =null;
		boolean hasRec = rs.next();
		
		OutputStream os=null;
		response.reset();//重置缓冲区,很关键，如果不写在一些容器中不能正常执行，比如weblogic
		response.setContentType("image/jpeg");
		os = response.getOutputStream();
		System.out.println("获得是否有记录os="+os);
		System.out.println("获得是否有记录="+hasRec);
		
		if(hasRec)
		{
			photoClob = rs.getClob(1);
			InputStream is = null ;
			byte[] photo64 = new byte[8192];
			if(photoClob!=null)
			{
				is = photoClob.getAsciiStream();
				System.out.println("获得photoClobInputStream="+is+"");
				byte[] bb = new byte[1];
				int b = -1;
				int i=0;
				while((b=is.read(bb))!=-1)
				{
					photo64[i++]=bb[0];
				}
			}
			
			System.out.println("photo64.length()="+new String(photo64).trim().length());
			
			if(new String(photo64).trim().length()<100)
			{
				retMsg="无此客户照片信息！";
				os.write( new sun.misc.BASE64Decoder().decodeBuffer(noPhoto));
				os.flush();
			}
			else
			{
				byte[] photo = new sun.misc.BASE64Decoder().decodeBuffer(new String(photo64));
				os.write(photo);
				os.flush();
				ok=true;
			}
	...
	if(ok){
			picHTMLPath = "fgetIccIdPhoto.jsp";
	}else{
			picHTMLPath="";
			if("".equals(retMsg))retMsg="取身份证照片信息错误!";
	}
	System.out.println("picHTMLPath="+picHTMLPath+"\tretMsg="+retMsg);
	System.out.println("----------->fgetIccIdPhoto.jsp END");
...
<FORM method=post name="fPubSimpSel">
	...
	<table cellspacing="0">
		<tr>
			<td align=center>
				<img name="custPhoto" width=200 height=130 ></img>
			</td>
		</tr>
	</table>
