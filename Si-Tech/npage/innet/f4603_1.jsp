...
function doProcess(packet)
{
   ...
   else if(retType=="4603chgSim"){
   //alert(retCode);
   //alert(retMessage);
   	if(retCode=="000000")
        {
			
		var out_flag=packet.data.findValueByName("out_flag");
                var jf_flag=packet.data.findValueByName("jf_flag");
                var contactPerson=packet.data.findValueByName("contactPerson");
                var contactPhone=packet.data.findValueByName("contactPhone");
                var contactFax=packet.data.findValueByName("contactFax");
                var contactMail =packet.data.findValueByName("contactMail");
                ...
                var contactAddr=packet.data.findValueByName("contactAddr");
                var contactPost=packet.data.findValueByName("contactPost");
                var zyjf=packet.data.findValueByName("zyjf");
                var zyxyd=packet.data.findValueByName("zyxyd");
                var old_phone=packet.data.findValueByName("old_phone");
                var vip_grade=packet.data.findValueByName("vip_grade");

		//document.all.WORK_FLOW_DIRECTION.value = WORK_FLOW_DIRECTION;
		document.all.contactPerson.value = contactPerson;
		document.all.contactPhone.value = contactPhone;
		document.all.contactFax.value = contactFax;
		document.all.contactMail.value = contactMail;
		
		document.all.idType.value = ID_TYPE;
		document.all.idIccid.value = ID_ICCID;
		document.all.custName.value = CUST_NAME;
		document.all.idAddr.value = idAddr;
		document.all.contactAddr.value = contactAddr;
		document.all.contactPost.value = contactPost;
		document.all.contactMAddr.value=idAddr;
		document.all.custAddr.value = idAddr;
		
		
		document.all.out_flag.value = out_flag;
		document.all.jf_flag.value = jf_flag;
		document.all.zyjf.value = zyjf;
		document.all.zyxyd.value = zyxyd;
		
		document.all.transJf.value = zyjf;
		document.all.transXyd.value = zyxyd;
		
		
		document.all.old_phone.value = old_phone;
		document.all.ownerType.value=vip_grade;
...
{       
        ...
        { 	
			...
			{
              document.all.opNote.value="<%=workName%>"+"对客户["+(document.all.custName.value).trim()+"]进行跨区入网业务。";
			}
	        ...
	        if(rdShowConfirmDialog("确认要提交客户开户信息吗？")==1)
       {
		...
		if(frm1100.vudyXl.value=""){
			frm1100.vudyXl.value="00";
		}
		
		
		
		
		document.all.ownerType.value = "01";  //临时 hjw
		//document.all.idType.value = "1"; //临时 hjw
		
		document.all.sysNote.value = document.all.opNote.value; 
	       document.frm1100.action="f4603_2.jsp";
...
function getInfo_withId()
{
    ...
    var getIdPacket = new RPCPacket("f1100_rpc.jsp","正在获得上级客户信息，请稍候......");
...
function getInfo_Cust()
{
    ...
    var getAccountId_Packet = new AJAXPacket("QrySimNo_4603.jsp","正在获得信息，请稍候......");
...
<%@ include file="/npage/include/header.jsp" %>                         
	<div class="title">
        ...
        try
        {
                String sqlStr ="select card_code,card_name from sGrpBigCardCode";
%>
...
%>
                  ...
                  <td width=34%> 
                    <input name=idIccid  v_minlength=5 v_maxlength=20 v_type="string" v_name="证件号码" maxlength="18"  index="11" onblur="rpc_chkX('idType','idIccid','A')">
                    <font class="orange">*</font> </td>
                </tr>
                <tr> 
                  <td class="blue"> 
                  ...
                  <td> 
                    <input name=idAddr  v_must=1 v_type="string" v_name="证件地址" maxlength="60" v_maxlength=60 size=35 index="12" onblur="document.all.custAddr.value=this.value;document.all.contactAddr.value=this.value;document.all.contactMAddr.value=this.value;">
                    <font class="orange">*</font> </td>
                  <td class="blue"> 
                    <div align="left">证件有效期</div>
                  </td>
                  <td> 
                    <input  name=idValidDate v_must=0 v_maxlength=8 v_type="date"  v_format="yyyyMMdd" v_name="证件有效期" maxlength=8 size="8" index="13" onblur="chkValid();">
                ...
                <tr> 
                  <td class="blue"> 
                    <div align="left">转移积分</div>
                  </td>
                  <td> 
                    <input name=zyjf  v_must=1 v_type="string" v_name="转移积分" maxlength="60" readonly v_maxlength=60 size=20 index="12" >
                    <font class="orange"></font> </td>
                  <td class="blue"> 
                    <div align="left">转移信誉度</div>
                  </td>
                  <td> 
                    <input  name=zyxyd v_must=0 v_maxlength=8 v_type="string"  v_name="转移信誉度" maxlength=20 readonly size="20" index="13" >
...
%>
                ...
                <TR> 
                  ...
                  <TD class="blue"> 
                    <input name=contactPerson  v_type="string" v_name="联系人姓名" maxlength="20" size=20 index="19" v_must=0 v_maxlength=20>
                  </TD>
                  <TD class="blue"> 
                ...
                <TR> 
                  ...
                  <TD class="blue"> 
                    <input name=contactPost  v_type="zip" v_name="联系人邮编" maxlength="6"  index="22" size="20">
                  </TD>
                </TR>
                <TR> 
                  <TD class="blue"> 
                  ...
                  <TD class="blue"> 
                    <input name=contactFax  v_must=0 v_type="phone" v_name="联系人传真" maxlength="20"  index="23" size="20">
                  </TD>
                  <TD class="blue"> 
                    <div align="left">联系人E_MAIL</div>
                  </TD>
                  <TD> 
                    <input name=contactMail  v_must=0 v_type="email" v_name="联系人E_MAIL" maxlength="30" size=30 index="24">
                  </TD>
                </TR>
                <TR> 
                  <TD class="blue"> 
              ...
              <TABLE id=tb0 width=100% border=0 align="center" cellSpacing=1 >
                ...
                <TR> 
                  <TD width=16% class="blue"> 
                    <div align="left">客户性别1</div>
                  </TD>
                  <TD width=34%> 
                  	<input type="test" name="custSex"   value="">
                  ...
                  <TD width="34%"> 
                    <input  name=birthDay maxlength=8 index="27" v_format="yyyyMMdd" v_must=0 v_maxlength=8 v_type="date" v_name="出生日期" size="8">
...
%>
                  ...
                  <TD class="blue"> 
                    <div align="left">学历1</div>
                  </TD>
                  <TD> 
                  	<input type="test" name="vudyXl"   value="">
...
%>
 		<wtc:pubselect name="sPubSelect" outnum="" retmsg="msg" retcode="code" routerKey="region" routerValue="<%=regionCode%>">
  	 <wtc:sql><%=sqlStr%></wtc:sql>
 	  </wtc:pubselect>
	 <wtc:array id="result_t" scope="end"/>
...
%>
                ...
                <TR> 
                  ...
                  <TD> 
                    <input name=yzrq   index="35" v_must=0 v_maxlength=8 v_format="yyyyMMdd" v_type="date" v_name="营业执照有效期" maxlength=8 size="8">
                  </TD>
                </TR>
                <TR> 
                  <TD class="blue"> 
        ...
        <TABLE cellSpacing=0>
          <TBODY>
            <TR> 
                  <TD align=center> 
                    <input  class="b_foot" name=print  onmouseup="printCommit()" onkeyup="if(event.keyCode==13)printCommit()"  type=button value=下一步  index="39" disabled>
  ...
  <input type="hidden" id="transJf"  name="transJf" value="">
  <input type="hidden" id="transXyd"  name="transXyd" value="">
  
    
		
		  
<input type="hidden" name="out_flag" value="">
<input type="hidden" name="jf_flag" value="">
<input type="hidden" name="old_phone" value=""> 
