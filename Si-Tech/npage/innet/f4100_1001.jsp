    ...
    if(retType == "ClientId")
    {
     ...
     if(retType == "4255")
     {
        ...
        if(retCode=="0000")
		{
            ...
            {
			    rdShowMessageDialog("鉴权失败 错误代码:"+retCode);
			}
		}
		else
		{
...
{       
        ...
        { 
			...
			{
              document.all.opNote.value="<%=workName%>"+"对["+(document.all.custName.value).trim()+"]("+document.all.ownerType.value+")进行跨区入网";
			}
...
function commitJsp()
	{
	   with(document.all)
	   {
	   		if(phone_No.value.length == 0)    //校验客户手机号码是否为空
			{
			    rdShowMessageDialog("原归属地号码不能为空！");
			    return false;
			}
			if(check_Type.value == "0")       //判断验证方式
			{
				if(sid_No.value.length == 0)
				{
					rdShowMessageDialog("您选择证件验证，证件号码不能为空！");
					return false;
				}
				if(document.all.paper_Type.value == "00")
	   			...
	   			else if(document.all.paper_Type.value == "01")
	   			{
		   			...
		   			{
		       			rdShowMessageDialog("VIP卡号码必须为16位的数字！");
			   			return false;
		   			}
	       				document.all.sid_No.v_type = "0_9";		
	  			}
	  		//增加密码校验，要么不输入，要么必须输入6位数字
	  		//if(cust_Pass.value.length == 0)
				...
				//}else{
					...
					if(cust_Pass.value.length != 6){
						rdShowMessageDialog("客户密码只能为6位数字");
						return false;
					}
			...
			else 
			{
				if(cust_Pass.value.length == 0)
				{
				 	rdShowMessageDialog("您选择密码验证，客户密码不能为空！");
					return false;
				}
				//输入了，就必须6位数字
					if(cust_Pass.value.length != 6){
	   ...
	   var myPacket = new AJAXPacket("/npage/innet/s4255_cfm.jsp?Name="+Name,"正在获得信息，请稍候......");
...
function getInfo_Cust()
{
    ...
    var getAccountId_Packet = new RPCPacket("QrySimNo_4100.jsp","正在获得信息，请稍候......");
...
<FORM method=post name="frm1100" action="f1100_2.jsp"  onKeyUp="chgFocus(frm1100)">
 ...
 <div class="title">
		<div id="title_zi">跨区入网开户</div>
	</div>
              <TABLE cellspacing="0">
                <TBODY> 
                    <TR> 
                        <TD width=16% class="blue"> 
                            <div align="left">原归属地号码</div>
                        </TD>
                        <TD width=34%> 
                  	        <input name="phone_No" type="text" size="13"  v_name="原归属地号码" v_type="mobphone">
                        </TD>
                        <td width="16%" class="blue"><div align="left">验证方式</div></td>
                        <td width="34%">
                            <div align="left"> 
                            <select id="check_Type" name="check_Type" size="1" onChange="change_check_type()">
                                <option value="0">证件验证</option>
                            </select>
                            </div>
                        </td>
                    </TR>
                    <TR>
                        <td width="16%" class="blue"><div align="left">客户姓名</div></td>
                        <td width="34%"> 
                            <div align="left"> 
                                <input name="Cust_Name" type="text" size="20"  maxlength="20">
                            </div>
                        </td>
                        <td width="16%"  id="sid_style_3" class="blue"><div align="left">证件类型</div></td>
...
<%				                    
                    ...
                    <TR>
                        <td width="16%" id="sid_style_1"  class="blue"><div align="left"><font size="2">证件号码</font></div></td>
                        <td width="34%" id="sid_style_2"> 
                            <div align="left" > 
                                <input name="sid_No" type="text" size="20"  maxlength="18" v_type="" v_name="证件号码">
                            </div>
                        </td>
                        <td width="16%" id="pass_style_1" class="blue"><div align="left">客户密码</div></td>
                        ...
                        <td colspan="4" id=="footer"> 
                            <div align="center"> 
                                <input name="confirm" class="b_foot" type="button"  value="确认" onClick="commitJsp()">
                                    &nbsp; 
                                <input name="reset"  class="b_foot" type="button"  value="清除" onClick="frm1100.Reset.click();"> 
                                    &nbsp; 
	      ...
	      <table  cellspacing="0" >
                	<TR bgcolor="#649ECC">
                        <TD height="30" width='16%'>消费积分</TD><TD width='16%'>品牌奖励积分</TD><TD width='16%'>网龄奖励积分</TD><TD width='16%'>专项转移积分</TD><TD width='16%'>已兑换积分</TD><TD width='16%'>可兑换积分</TD>
                    </TR>
         </table>
...
%>
                                    ...
                                    <TD class="blue"> 
                    <div align="left">客户入网时间</div>
                  </TD>
                  <TD class="blue"> 
                    <input name=ruwangtime   v_name="客户入网时间" maxlength="20"  index="20" size="20" class="InputGrey" readonly>
                     </TD>
                </TR>
                <TR> 
                  <TD class="blue"> 
                  ...
                  <TD colspan="3"> 
                    <input name=custAddr  v_must=1  v_type="string" v_must=0 v_maxlength=60 v_name="客户地址" onchange="change_ConAddr()" maxlength="60" size=35 index="18">
                    <font class="orange">*</font>
                  </TD>
                </TR>
                <TR> 
...
%>
                  ...
                  <TD width="34%"> 
                    <input  name=birthDay maxlength=8 index="27"  v_must=0 v_maxlength=8 v_type="date"  v_format="yyyyMMdd"  v_name="出生日期" size="8">
...
%>
                ...
                <TR> 
                  ...
                  <TD> 
                    <input name=yzrq   index="35" v_must=0 v_maxlength=8 v_type="date"  v_format="yyyyMMdd"  v_name="营业执照有效期" maxlength=8 size="8">
                  </TD>
                </TR>
                <TR> 
                  <TD class="blue"> 
        ...
        <TABLE   cellSpacing=0>
          <TBODY>
            <TR> 
                  <TD align=center> 
                    <input  name=print   class="b_foot" onmouseup="printCommit()" onkeyup="if(event.keyCode==13)printCommit()"  type=button value=下一步  index="39" disabled>
						        <input  name=reset1 type=button  class="b_foot" onclick="frm1100.Reset.click();" value=清除 index="40">
						        <input  name=back type=button  class="b_foot" onclick="removeCurrentTab()" value=关闭 index="41">
