...
%>
    <wtc:service name="sGetPhoneInfo" routerKey="region" routerValue="<%=regionCode%>" retcode="retCode" retmsg="retMsg" outnum="4" >
    	<wtc:param value="0"/>
    	<wtc:param value=""/>
      		    ...
      		    for(int j=0;j<Integer.parseInt(fieldNum)+1;j++)
      		    {
                    ...
                    if(j==3)  {
                    	...
                    	distyle="block";
          		        ...
          		        inputStr = inputStr + "<TD title=\'"+(result[i][j]).trim()+"\' class='"+tdClass+"' style='display:"+distyle+"'>&nbsp;" + (result[i][j]).trim() + "<input type='hidden' " +
          		            " id='Rinput" + i + j + "'  value='" + 
          		            (result[i][j]).trim() + ","+(result[i][3]).trim()+"' readonly></TD>";          		            
                    }          		            
      		    }
