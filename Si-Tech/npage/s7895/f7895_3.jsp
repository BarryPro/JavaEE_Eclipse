...
<%
    ...
    //wuxy alter 20140127 集团门户项目更改分隔符
...
<script language="JavaScript">
    function print_xls(){
        window.open('f7895_printxls.jsp?phoneNo=<%=errPhoneList%>&returnMsg=<%=errMsgList%>&unitID=<%=iUnitId%>&loginAccept=<%=iLoginAccept%>&grpName=<%=iGrpName%>&op_Code=<%=opCode%>&orgCode=<%=orgCode%>');
    }
</script>
<BODY>
...
<TABLE cellSpacing="0">
    <TR>
        <TH width='50%' align='center'>未删除成功号码</TH>
        <TH width='50%'>失败原因</TH>
    </TR>
    <%
...
<TABLE cellspacing="0">
    <tr id="footer">
        <td>
            ...
            <input class="b_foot" name=back onClick="window.location='f7895_1.jsp'" style="cursor:hand" type=button value=返回>
        </td>
    </tr>
</TABLE>
