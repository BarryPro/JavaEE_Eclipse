...
<%
	...
	//xl add for hxf id_no入库
	String s_id_no="";
	System.out.println("if之外的 smphoneNo is "+smphoneNo);
    if (smop_code.equals("zg62") ||smop_code.equals("e007") ||smop_code.equals("zg63") ) {
	...
	inParam[0] ="select decode(sm_code,'zn','神州行','gn','全球通','dn','动感地带','kd','宽带','kh','铁通宽带'),to_char(id_no) from dcustmsg where phone_no=: smphoneNo  and substr(run_code,2,1)<'a'";
	inParam[1] = "smphoneNo="+smphoneNo;
	System.out.println("QQQQQQQQQQQQQQQQQQQQQQQQQQQ smphoneNo is "+smphoneNo);
%>
...
<%
			...
			s_id_no=sm_name_arr[0][1];
			%>
				<script language="javascript">//alert("tets s_id_no is "+"<%=s_id_no%>");</script>
			<%
        }
	...
	String returnPage ="";
	System.out.println("aaaaaaaaaaaaaaaaaaaaaaa smop_code is "+smop_code);
	if(smop_code.equals("zg62") || smop_code=="zg62")
	{
		returnPage="../zg62/zg62_1.jsp";
	}
	else if(smop_code.equals("zg63") || smop_code=="zg63")
	{
...
<SCRIPT language="JavaScript">
    ...
    <%if (printtype.equals("0")) {%>
        printctrl.Print(53, 13, 9, 0, "业务名称：<%=result[0][2]%>");
    <%} else {%>
        printctrl.Print(53, 13, 9, 0, "业务名称：补打<%=result[0][2]%>");
    <%}%>
        ...
        printctrl.Print(13, 17, 9, 0, "合计金额：(大写)<%=result[0][8]%>");
        printctrl.Print(53, 17, 9, 0, "(小写)￥<%=result[0][9].trim()%>元整");
		
        printctrl.Print(13, 19, 9, 0, "(项目)");
		<%
		//System.out.println("WWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWw smop_code is "+smop_code);	
			if(smop_code.equals("zg62") || smop_code.equals("zg63"))
			{
			%>
				//printctrl.Print(20, 15, 9, 9, "预存款余额：<%=prepayFee_fp%>元");
				printctrl.Print(13, 20, 9, 0, "预存款余额：<%=curbalanace%>元");
				printctrl.Print(53, 20, 9, 0, "本次交费金额：<%=payMoney_fp%>元");
				printctrl.Print(13, 22, 9, 0, "未出帐宽带费：<%=ubill_fp%>元");
				printctrl.Print(53, 22, 9, 0, "宽带帐户剩余金额：<%=kdzhsy%>元");
				printctrl.Print(13, 24, 9, 0, "客服热线：10050");
				printctrl.Print(13, 26, 9, 0, "网址：http://www.10050.net");	
			<%
			}
			if(smop_code.equals("e007")  )
			{
			%>
				printctrl.Print(13, 20, 9, 0, "交费冲正金额：<%=result[0][9]%>元");	
				printctrl.Print(13, 22, 9, 0, "客服热线：10050");
				printctrl.Print(13, 23, 9, 0, "网址：http://www.10050.net");
			<%
			}
		%>
