...
if(!i.attr("data-target")){j.preventDefault();}var f=c(i);var h=f.data("bs.collapse");var g=h?"toggle":i.data();b.call(f,g);});}(jQuery);+function(e){var f=function(h,g){this.$element=e(h);
this.options=e.extend({},f.DEFAULTS,g);this.$trigger=e('[data-toggle="sidecollapse"][href="#'+h.id+'"],[data-toggle="sidecollapse"][data-target="#'+h.id+'"]');
this.transitioning=null;if(this.options.parent){this.$parent=this.getParent();}else{this.addAriaAndCollapsedClass(this.$element,this.$trigger);}if(this.options.toggle){this.toggle();
}};f.VERSION="3.3.5";f.TRANSITION_DURATION=350;f.DEFAULTS={toggle:true};f.prototype.dimension=function(){var g=this.$element.hasClass("width");return g?"width":"height";
};f.prototype.show=function(){if(this.transitioning||this.$element.hasClass("in")){return;}var i;var l=this.$parent&&this.$parent.children(".panel").children(".in, .sidecollapsing");
if(l&&l.length){i=l.data("bs.sidecollapse");if(i&&i.transitioning){return;}}var h=e.Event("show.bs.sidecollapse");this.$element.trigger(h);if(h.isDefaultPrevented()){return;
}if(l&&l.length){c.call(l,"hide");i||l.data("bs.sidecollapse",null);}var m=this.dimension();this.$element.addClass("sidecollapsing in").attr("aria-expanded",true);
this.$trigger.removeClass("collapsed").attr("aria-expanded",true);var k=this.$element.width();if(a=="right"){e("body").addClass("sideopening").css({right:k+15});
}else{e("body").addClass("sideopening").css({left:k+30});}e("html").css("overflow-x","hidden");this.transitioning=1;var g=function(){this.$element.removeClass("sidecollapsing").addClass("sidecollapse");
this.transitioning=0;this.$element.trigger("shown.bs.sidecollapse");};if(!e.support.transition){return g.call(this);}var j=e.camelCase(["scroll",m].join("-"));
this.$element.one("bsTransitionEnd",e.proxy(g,this)).emulateTransitionEnd(f.TRANSITION_DURATION);};f.prototype.hide=function(){if(this.transitioning||!this.$element.hasClass("in")){return;
}var h=e.Event("hide.bs.sidecollapse");this.$element.trigger(h);if(h.isDefaultPrevented()){return;}var i=this.dimension();this.$element.addClass("sidecollapsing").removeClass("in").attr("aria-expanded",false);
this.$trigger.addClass("collapsed").attr("aria-expanded",false);if(a=="right"){e("body").css({right:0});}else{e("body").css({left:0});}e("html").css("overflow-x","auto");
this.transitioning=1;var g=function(){this.transitioning=0;this.$element.removeClass("sidecollapsing").trigger("hidden.bs.sidecollapse");};if(!e.support.transition){return g.call(this);
}this.$element.one("bsTransitionEnd",e.proxy(g,this)).emulateTransitionEnd(f.TRANSITION_DURATION);};f.prototype.toggle=function(){this[this.$element.hasClass("in")?"hide":"show"]();
};f.prototype.getParent=function(){return e(this.options.parent).find('[data-toggle="sidecollapse"][data-parent="'+this.options.parent+'"]').each(e.proxy(function(j,h){var g=e(h);
this.addAriaAndCollapsedClass(d(g),g);},this)).end();};f.prototype.addAriaAndCollapsedClass=function(h,g){var i=h.hasClass("in");h.attr("aria-expanded",i);
g.toggleClass("collapsed",!i).attr("aria-expanded",i);};function d(g){var h;var i=g.attr("data-target")||(h=g.attr("href"))&&h.replace(/.*(?=#[^\s]+$)/,"");
return e(i);}function c(g){return this.each(function(){var j=e(this);var i=j.data("bs.sidecollapse");var h=e.extend({},f.DEFAULTS,j.data(),typeof g=="object"&&g);
if(!i&&h.toggle&&/show|hide/.test(g)){h.toggle=false;}if(!i){j.data("bs.sidecollapse",(i=new f(this,h)));}if(typeof g=="string"){i[g]();}});}var b=e.fn.sidecollapse;
e.fn.sidecollapse=c;e.fn.sidecollapse.Constructor=f;e.fn.sidecollapse.noConflict=function(){e.fn.sidecollapse=b;return this;};e(document).on("click.bs.sidecollapse.data-api",'[data-toggle="sidecollapse"]',function(k){var j=e(this);
if(!j.attr("data-target")){k.preventDefault();}var g=d(j);var i=g.data("bs.sidecollapse");var h=i?"toggle":j.data();c.call(g,h);});var a;e(function(){var k=e('[data-toggle="sidecollapse"]');
if(k.length>0){for(var h=0;h<k.length;h++){var g=e(k[h]).attr("data-dir");var j=e(k[h]).attr("data-target");if(g&&g=="right"){a="right";e("body").addClass("sideopen right");
e(j).addClass("sidecollapse right");}else{a="left";e("body").addClass("sideopen left");e(j).addClass("sidecollapse left");}}}});}(jQuery);+function(h){var e=".dropdown-backdrop";
...
}if(e.offsetTop!=null){e.offset.top=e.offsetTop;}b.call(f,e);});});}(jQuery);+function(b){function a(d){var c={freeMode:true,setWrapperSize:true,slidesPerView:"auto"};
var e=b.extend({},c,d);return this.each(function(){var g=b(this);var f=g.data("bs.swiper");if(!f){g.data("bs.swiper",(f=new Swiper(g,e)));}});}b.fn.swiper=a;
b(window).on("load",function(){b(".swiper-container").each(function(){var c=b(this);a.call(c);});});}(jQuery);