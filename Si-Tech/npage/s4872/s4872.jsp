...
<HTML>
	<HEAD>
		<TITLE>黑龙江BOSS- 区县转账退费限额设置 </TITLE>
		   ...
		   //  	}
		    ...
		    var phone_no = document.form1.msg_no.value;
				...
				if(phone_no.length>11 || phone_no.length<11 || isNumberString(phone_no,"1234567890")!=1 ) {
				 ...
				 document.form1.action="s4872_3.jsp";
			...
			function doquery(){
				...
				var group_id=document.form1.org_id.value;
				var myPacket = new RPCPacket("s4872_2.jsp","正在获得信息，请稍候......");
				//var sqlStr = "select  to_char(nvl(limit_day_def,0)),to_char(nvl(limit_month_def,0)),to_char(nvl(LIMIT_DAY_BEG,0)),to_char(nvl(LIMIT_DAY_END,0)),to_char(nvl(LIMIT_DAY_SPE,0)),to_char(nvl(LIMIT_MONTH_BEG,0)),to_char(nvl(LIMIT_MONTH_END,0)),to_char(nvl(LIMIT_MONTH_SPE,0)),to_char(phone_no) from slimitpay_qx	where region_code=(select distinct substr(org_code,1,2) from dloginmsg where group_id = '"+group_id+"') and 	limit_type ='"+limit_type+"' and group_id="+group_id;
				var sqlStr = "54";
				var param1 = "group_id="+group_id+",limit_type="+limit_type+",group_id="+group_id;
				//alert(sqlStr);
            ...
            <table id=tbs9 width=100% height=25 border=0 align="center" cellspacing=2>
            	<tr bgcolor="#E8E8E8" >
                  ...
                  &nbsp;&nbsp;限额类型：
                    ...
                    <select name="org_id">
                      <%
				                  sqlStr= "select to_char(a.group_id),a.group_id||'-->'||a.group_name from dchngroupmsg a, dchngroupinfo b,dloginmsg c where c.login_no='?' and c.group_id = b.parent_group_id and b.group_id= a.group_id and b.denorm_level = '1' order by a.group_id";
