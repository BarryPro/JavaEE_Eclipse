...
<script>
	...
	function call_ADDCode()
	{
		 ...
		 //alert(document.all.EndTime.value);
		 //alert(document.all.StartTime.value);
    ...
    function queryAddAllRow(add_type,StartTime,EndTime)
    {
      ...
      if(tmp_flag == false)
      {
        rdShowMessageDialog("已经有一条相同的时间段或时间段有重叠!");
        return false;
      }
        ...
        for(var a = document.all.dyntb.rows.length-2 ;a>0; a--)//删除从tr1开始，为第三行
        {
            ...
            if( op_type == oprType_Add)
            {
              ...
              if( ((StartTime.trim()) == (tmp_StartTime.trim()))
                ...
                else if((StartTime.trim())>(tmp_StartTime.trim())&& (StartTime.trim())<(tmp_EndTime.trim()))
              {
              	
              	return false;
              	
              }
              else if((EndTime.trim())>(tmp_StartTime.trim())&& (EndTime.trim())<(tmp_EndTime.trim()))
              {
              	return false;
              }
              else if((StartTime.trim())>(tmp_StartTime.trim())&& (EndTime.trim())<(tmp_EndTime.trim()))
              {
              	return false;
              }
              else if((StartTime.trim())<(tmp_StartTime.trim())&& (EndTime.trim())>(tmp_StartTime.trim()))
              {
              	return false;
              }
