	...
	function saveTo()
	{
	  ...
	  for(i=0;i<document.fPubSimpSel.elements.length;i++)
	  {
	    ...
	    {    //判断是否是单选或复选框
		   ...
		   {  
		   		//判断是否被选中
		   		if(document.fPubSimpSel.elements[i].v_status != "1"){ //未占用，并只传递新选中的
...
{   //取消复选框全部选中
    for(i=0;i<document.fPubSimpSel.elements.length;i++)
    {
        if(document.fPubSimpSel.elements[i].type =="checkbox")
        {    
        	if(document.fPubSimpSel.elements[i].v_status != "1"){
        		//判断是否是单选或复选框
    ...
    //根据传人的Sql查询数据库，得到返回结果
      		...
      		for(int i=0;i<recordNum;i++)
      		{
      		   String tdClass = ((i%2)==1)?"Grey":"";
      		    ...
      		    inputStr = "";
      		    out.print("<TR>");
      		    for(int j=1;j<Integer.parseInt(fieldNum);j++)
      		    {
                    if(j==1)
                    {
                        ...
                        {
                        	if((selType.compareTo("checkbox") == 0) && ("1".equals(result[i][0]))){ //当status为0时，复选框未选可改；当status为1时，复选框已选不可改
                        		typeStr = typeStr + "<input type='" + selType +
		          		            "' name='List' style='cursor:hand' RIndex='" + i + "'" +
		          		            "onkeydown='if(event.keyCode==13)saveTo();'" + " checked = 'checked' disabled  v_status='"+(result[i][0]).trim()+"' >";
	                        }else{
	                        	typeStr = typeStr + "<input type='" + selType +
		          		            "' name='List' style='cursor:hand' RIndex='" + i + "'" +
		          		            "onkeydown='if(event.keyCode==13)saveTo();' v_status='"+(result[i][0]).trim()+"'>";
	                        }
												}
