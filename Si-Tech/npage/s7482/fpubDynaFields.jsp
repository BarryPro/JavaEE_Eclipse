...
<%
	...
	try
	{
		sqlStr= "select field_code, ref_field_code,ref_field_value, nvl(formula,'')"
			...
			System.out.println("liujian7482--sql=" + sqlStr);
			System.out.println("liujian7482--=" + "userType="+iSmCode);
...
<%
			System.out.println("liujian7482--retArr1.length=" + retArr1.length + "--retCode1=" + retCode1);
...
<%
			...
			//动态下拉列表
			sqlStr = "select field_code,param_sql from sUserFieldParamCode where user_type=:userType and param_code='99' order by field_code,param_code asc";
			System.out.println(sqlStr );
				...
				for (int colNum = 0; (colNum < 2) && (i < fieldCount)||!fieldGroupNo[i].equals("0"); colNum ++)
				{
					...
					else
					{
    					 ...
    					 else
    					 {
    						...
    						/* yuanqs add 2011/6/22 10:04:00 下月套餐资费生效日期 */
							if ("80004".equals(fieldCodes[i])||"80006".equals(fieldCodes[i])) {
					...
					else if (dataTypes[i].equals("80")&&(!"".equals(feeCode))){
						/* yuanqs add 2011/6/21 10:23:15 下月套餐资费生效日期 style='display:none' */

						if ("80004".equals(fieldCodes[i])) {
							out.print("<TD width='32%' colspan="+colspan+" style='display:none'><input id='F"+fieldCodes[i]+"' name='F"+fieldCodes[i]+"' class='button' type='text' datatype="+dataTypes[i]+" maxlength='"+fieldLengths[i]+"' value='"+feeCode+"' readOnly />");
							out.print("<TD width='32%' colspan="+colspan+"></td>");
						} else {
							out.print("<TD width='32%' colspan="+colspan+"><input id='F"+fieldCodes[i]+"' name='F"+fieldCodes[i]+"' class='button' type='text' datatype="+dataTypes[i]+" maxlength='"+fieldLengths[i]+"' value='"+feeCode+"' readOnly />");
						}
						
					}
                    else if (dataTypes[i].equals("80")&&("".equals(feeCode)))//vp - 当月套餐类型
					{
						out.print("<TD width='32%' colspan="+colspan+">");
						...
						try
                        {
							...
							sqlBufSqlStr73.append( "where a.offer_attr_type='VpG0' and a.offer_id=d.offer_id");
							sqlBufSqlStr73.append( "and d.group_id=e.group_id");
							sqlBufSqlStr73.append( "and e.region_code='+regionCode+' and a.exp_date>=sysdate and a.eff_date<=sysdate");
							sqlBufSqlStr73.append( "and ( to_number(d.right_limit) <= "+ powerRight +") ");
							...
							sqlBufSqlStr73.append( "and a.offer_id = b.pkg_code(+)");
                           ...
                           /* sqlBufSqlStr73.append( "select trim(a.pkg_code),trim(a.pkg_code)||'-->'||trim(pkg_name) from svpmnpkgcode a,svpmnpkgcodeconfig b where a.region_code='"+regionCode+"' and stop_time>=sysdate and " + powerRight + " >= power_Right");
                            ...
                            %>
                                <wtc:pubselect name="sPubSelect" outnum="2" retmsg="msg" retcode="code" routerKey="region" routerValue="<%=regionCode%>">
                                    <wtc:sql><%=sqlStr73%></wtc:sql>
                                </wtc:pubselect>
                                <wtc:array id="result_tu73" scope="end"/>               
                            <%
                            /* yuanqs add 2011/6/22 10:32:37 
					...
					else if (dataTypes[i].equals("82"))//ap - 漫游类型
					{
						...
						/* yuanqs add 2011/6/22 10:03:27 下月套餐资费生效日期 style='display:none' */
