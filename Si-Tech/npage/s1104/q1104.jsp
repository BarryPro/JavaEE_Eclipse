...
<%
	...
	String myActivePhone = WtcUtil.repNull(request.getParameter("activePhone"));
	System.out.println("gaopengSeeLog1104----myActivePhone==="+myActivePhone);
%>
...
<%
	if("000000".equals(m2mrc)){
		...
		/*判断普通开户(1104)”和“前台预开户(g784)”，如果用户办理的主资费是“超级流量和卡39元(50227)”，则调用次服务wanghyd 20160126*/
    if("1104".equals(opCode) || "g784".equals(opCode)) {
 
 
%> 

		<wtc:service name="sXYYXInnetChk" outnum="1"
...
<%
		...
		/*** begin  判断资费是否是 4G 资费 @2013/11/26 ***/
    
    String mailFlag = "N";
    String miguFlag = "N";
    /*2013/12/25 13:48:45 gaopeng 关于2013年9月第一批业务支撑系统市场专业需求（关于主资费捆绑数据业务功能优化的需求）
    	加入139邮箱和咪咕特殊会员的查询,返回标志位。
    */
    String  inParamsMail [] = new String[2];
    inParamsMail[0] = "select count(1) from product_offer_attr where offer_id = :vofferId1 and offer_attr_seq = '80005' ";
    inParamsMail[1] = "vofferId1="+offerId;
    
    String  inParamsMigu [] = new String[2];
    inParamsMigu[0] = "select count(1) from product_offer_attr where offer_id = :vofferId2 and offer_attr_seq = '80006'";
    inParamsMigu[1] = "vofferId2="+offerId;
    /*gaopeng 2014/05/28 9:15:18 物联网行车卫士 通过offerId 查询是否需要展示物联网行车信息 返回标志位*/
    
    String  inParamsCarSave [] = new String[2];
    inParamsCarSave[0] = "SELECT count(1) FROM product_offer WHERE offer_attr_type ='YnPC' and offer_id= :vofferId3";
    inParamsCarSave[1] = "vofferId3="+offerId;
    System.out.println("gaopengSeeLog===inParamsCarSave[0]=:"+inParamsCarSave[0]);
    System.out.println("gaopengSeeLog===inParamsCarSave[1]=:"+inParamsCarSave[1]);
    
    String notessss="工号"+workNo+"对custid="+gCustId+"进行查询";
    
    /*查询*/
    String  inParamsMail22 [] = new String[2];
    inParamsMail22[0] = "select element_id from product_offer_detail where offer_id =:vofferId1 and sel_flag =0 and element_type ='10C' ";
    inParamsMail22[1] = "vofferId1="+offerId;
...
%>
  ...
  <wtc:array id="result_mail"  scope="end"/>
  	...
  	<wtc:service name="TlsPubSelCrm" routerKey="region" routerValue="<%=regionCode%>" retcode="retCode_migu" retmsg="retMessage_migu" outnum="1"> 
    <wtc:param value="<%=inParamsMigu[0]%>"/>
    <wtc:param value="<%=inParamsMigu[1]%>"/> 
  </wtc:service>  
  <wtc:array id="result_migu"  scope="end"/>
  	<wtc:service name="TlsPubSelCrm" routerKey="region" routerValue="<%=regionCode%>" retcode="retCode_carsave" retmsg="retMessage_carsave" outnum="1"> 
    <wtc:param value="<%=inParamsCarSave[0]%>"/>
    <wtc:param value="<%=inParamsCarSave[1]%>"/> 
  </wtc:service>  
  <wtc:array id="result_carsave"  scope="end"/>
  	
  <wtc:service name="TlsPubSelCrm" routerKey="region" routerValue="<%=regionCode%>" retcode="retCode_mail22" retmsg="retMessage_mail22" outnum="1"> 
    <wtc:param value="<%=inParamsMail22[0]%>"/>
    <wtc:param value="<%=inParamsMail22[1]%>"/> 
  </wtc:service>
<wtc:array id="result_kxbxs"  scope="end"/>
<%
...
var kxbxArray = new Array();
</script>
<%
	/*放入js数组主资费绑定的必选附加销售品id*/
	if(result_kxbxs.length > 0 && "000000".equals(retCode_mail22)){
		for(int i=0;i<result_kxbxs.length;i++){
			%>
			<script language=JavaScript> 
			kxbxArray[<%=i%>] = <%=result_kxbxs[i][0]%>
			</script>
			<%
		}
	}

    /*** end 判断资费是否是 4G 资费 @2013/11/26  ***/
    
    if("000000".equals(retCode_mail)){
      if(result_mail.length>0){
        if(Integer.parseInt(result_mail[0][0]) > 0){
        	mailFlag = "Y";
        }else{
        	mailFlag = "N";
        }
    ...
    String carSaveFlag ="";
    /*2014/05/28 9:18:02 gaopeng 物联网行车卫士&liyana*/
    System.out.println("gaopengSeeLog===retCode_carsave=:"+retCode_carsave);
    if("000000".equals(retCode_carsave)){
      if(result_carsave.length>0){
      System.out.println("gaopengSeeLog===result_carsave[0][0]=:"+result_carsave[0][0]);
        if(Integer.parseInt(result_carsave[0][0]) > 0){
        	carSaveFlag = "Y";
        }else{
        	carSaveFlag = "N";
        }
    ...
    System.out.println("gaopengSeeLog==================mailFlag:"+mailFlag+";miguFlag:"+miguFlag+";carSaveFlag:"+carSaveFlag);
...
%>
			...
			//	alert("<%=retOrdAryVal.getUtype("2."+i).getValue(j)%>");
			</script>	 
<%		 	
			}
		}	
...
String beizhussdese1="根据custid=["+gCustId+"]进行查询";
    ...
    //TE不调用服务sCustIdIccIdChk  dn gn其他不变
    if(!"TE".equals(sm_Code)){
%> 
...
<%
	 /*
		* 关于申请流量伴侣卡的需求
		* hejwa add 2016年12月17日15:53:17
		* wangzc 提供服务
		*/
	String main_k_flag      = "";
	String main_k_type      = "";
	String main_k_sql       = "select to_char(count(*)) as flag from product_offer_restrication where offer_id =:offerId and RESTRICATION_DOMAIN_TYPE ='BL' ";
	String main_k_sql_param = "offerId="+offerId; 
%>
...
<%
	...
	System.out.println("-------hejwa1104---------------main_k_flag---------->"+main_k_flag);
	System.out.println("-------hejwa1104---------------main_k_type---------->"+main_k_type);
	
	
	String orange_flag = "0";
	String orange_sql  = "select to_char( count(*)) as result  from product_offer_restrication where restrication_domain_type in ('JZ')  and offer_id=:offerId ";
	String orange_prm  = "offerId="+offerId;
%>
  <wtc:service name="TlsPubSelCrm" outnum="1" retmsg="msg" retcode="code" routerKey="region" routerValue="<%=regionCode%>">
		<wtc:param value="<%=orange_sql%>" />
		<wtc:param value="<%=orange_prm%>" />	
	</wtc:service>
	<wtc:array id="result_orange" scope="end"   />
		
<%
	if(result_orange.length>0){
		orange_flag = result_orange[0][0];
	}
	
	System.out.println("-------hejwa1104---------------orange_flag---------->"+orange_flag);
%>		
...
var offerTrafficflag="0"; //0是订购的主资费流量清零，非0为订购的主资费流量不清零
...

{
    if(v_hiddenFlag=="Y"){ //当为Y时，进入新版小区代码展示页面
      ...
      packet1.data.add("myActivePhone" ,"<%=myActivePhone%>");
...
function jdugeAreaHiddenKx(offer_id,offer_name){
	
  var packet = new AJAXPacket("ajax_jdugeAreaHidden.jsp","请稍后...");
	packet.data.add("offerId",offer_id);
	packet.data.add("offerName",offer_name);
 	packet.data.add("phoneNo","");
	packet.data.add("opCode","<%=opCode%>");
	core.ajax.sendPacket(packet,doJdugeAreaHiddenKx);
	packet =null;	
} 
...
function doJdugeAreaHiddenKx(packet){
  v_hiddenFlag = "";
  v_code = new Array();
  v_text = new Array();
  var retCode = packet.data.findValueByName("retCode");
  ...
  var hiddenFlag =  packet.data.findValueByName("hiddenFlag");//是否显示小区代码标识
  var offer_id =  packet.data.findValueByName("offerId");//资费代码
  var offerName =  packet.data.findValueByName("offerName");//资费代码
  ...
  if(retCode == "000000"){
    ...
    getOfferAttrKx(offer_id,offerName);
	}else{
		rdShowMessageDialog(retCode + ":" + retMsg,0);
		//getOfferAttr();
		return false;
...
function getOfferAttrKx(offer_id,offerName)

{
    if(v_hiddenFlag=="Y"){ //当为Y时，进入新版小区代码展示页面
    	$("#OfferAttributeKxTitle").show();
      var packet1 = new AJAXPacket("getOfferAttrNewKx.jsp","请稍后...");
      packet1.data.add("v_code" ,v_code);
      packet1.data.add("v_text" ,v_text);
      packet1.data.add("myActivePhone" ,"<%=myActivePhone%>");
      ...
      var packet1 = new AJAXPacket("getOfferAttr.jsp","请稍后...");
		...
		core.ajax.sendPacketHtml(packet1,doGetOfferAttrKx);
		packet1 =null;
}
...
function chgSimTypeOne(){
	...
	*/
	var simTypeOne = $.trim($("select[name='simTypeOne']").find("option:selected").val());
...
function doGetMajorProd(data)
{
	  $("#offer_component").html(data);
		...
		if(typeof(majorProductArr) != "undefined" && majorProductArr.length > 0)
		{
		  ...
		  if("<%=opCode%>" == "1104"){
		  	if("<%=sm_Code%>" == "TE"){
		  		$("#simTypeOneShowAndHide").show();
		  	}else{
		  		$("#simTypeOneShowAndHide").hide();
		  	}
		  	chgSimTypeOne();
			}
...
{
	$("#offer_component").html(data);
	/* begin add 对报错信息进行提示，方便定位 for 【申告】关于主资费绑定的附加资费未开通问题的优化需求@2015/4/3 */
	var offerDetailQry_valid = $("#offerDetailQry_valid").val();
	var offerDetailQry_retCode = $("#offerDetailQry_retCode").val();
  var offerDetailQry_retMsg = $("#offerDetailQry_retMsg").val();
	if(offerDetailQry_valid != 0){
		if(offerDetailQry_valid == 1){
			rdShowMessageDialog("错误代码："+offerDetailQry_retCode+"<br>"+offerDetailQry_retMsg,0);
			removeCurrentTab();
			return false;	
		}else{
			rdShowMessageDialog(offerDetailQry_retMsg,0);
			removeCurrentTab();
			return false;	
		}
	}
	/* end add 对报错信息进行提示，方便定位 for 【申告】关于主资费绑定的附加资费未开通问题的优化需求@2015/4/3 */
...
/*选择的可选资费进行小区查询*/
function kxzfOfferIdXqSearch(){
	
	$("#OfferAttributeKx").empty();
	$("#tab_order tr:gt(0)").each(function(){
		var kxzfOfferId = $(this).find("td").eq(0).html();
		var kxzfOfferName = $(this).find("td").eq(1).html();
		
		jdugeAreaHiddenKx(kxzfOfferId,kxzfOfferName);
	});
		...
		if(typeof(prodCompInfo) != "undefined"){
			...
			for(var i=0;i<prodCompInfo.length;i++){
				...
				if(simTypeOne == "1" && "<%=sm_Code%>" == "TE"){
					...
					if((prodCompInfo[i][2].split(":")[0]) == "2049" || (prodCompInfo[i][2].split(":")[0]) == "1038")
					{
						relationType = "1";
					}
				}else if(simTypeOne == "0" && "<%=sm_Code%>" == "TE"){
            ...
            if($("#flag").val() == "TD")
            {
         		...
         		$("#flag").val("true");
					...
					*/
					...
					{
						//relationType = "3";
						var packet33 = new AJAXPacket("/npage/bill/check184SuperTD.jsp","正在验证，请稍后。。。");
            packet33.data.add("phoneNo",document.prodCfm.selNum.value);
            core.ajax.sendPacket(packet33,doPro2);
            packet33 =null;
...
function doGetOfferAttrKx(data)
{
	
  if(v_hiddenFlag=="Y"){ //当为Y时，进入新版小区代码展示页面
    $("#OfferAttributeKx").append(data);
    
  }else{
  	//alert(2066);
    //$("#OfferAttributeKx").html(data);
  }
	$("#OfferAttributeKx :input").not(":button").keyup(function stopSpe(){
			var b=this.value;
			if(/[^0-9a-zA-Z\.\@\u4E00-\u9FA5]/.test(b)) this.value=this.value.replace(/[^0-9a-zA-Z\u4E00-\u9FA5]/g,'');
	});
}
      ...
      var retMessage = packet.data.findValueByName("retMessage");	
     	...
     	if(retCode=="000000")
    	{
    	    var getAccountId_Packet = new AJAXPacket("ajaxContractNo.jsp","正在获得帐户ID，请稍候......");
					...
					var usernamesss="";
					if(document.all.userName11.value.trim()==document.all.userName22.value.trim()) {
						usernamesss=document.all.userName.value;
					}else {
						usernamesss=document.all.userName11.value;
					}
					...
					getAccountId_Packet.data.add("accountName",usernamesss);
...
function cfmOfferf(){
	...
	var flags0="";
	var flagsN="";
	$("#div_offerComponent :checkbox").each(
		function ()
		{
			if ( this.name.substr(0,5)=="offer" )
			{
				if ( this.checked==true )
				{
					ofrIds=ofrIds+this.id+"#";
					flags0=flags0+"0#";
					flagsN=flagsN+"N#";
				}
			}
	 ...
	 var myPacket = new AJAXPacket("f1104Ajax.jsp", "正在提交，请稍候......");
	myPacket.data.add("iLoginAccept", "<%=sysAcceptl%>");	
	...
	myPacket.data.add("iPhoneNo", $("input[name='selNum']").val());									
	myPacket.data.add("iUserPwd", document.all.userpwd.value);									
	myPacket.data.add("iOfferId","<%=offerId%>" );									
	myPacket.data.add("iOfferIdStr", ofrIds);			
	myPacket.data.add("iFlagStr",   flagsN);	
		myPacket.data.add("iOfferStateStr",  flags0);
									
	core.ajax.sendPacket(myPacket,doWLW);
	myPacket=null;
	}
	
	function doWLW( packet )
	{
		...
		{
			rdShowMessageDialog(oRetCode+":"+oRetMsg);
			return false;
		}
...
if("<%=m2mFlag%>"!="Y"){    
    ...
    if("<%=brandID%>"=="101"){
    	var myPacket = new AJAXPacket("/npage/public/pubGetWLWPhoneNo.jsp","正在查询信息，请稍候......");
			  ...
			  myPacket.data.add("phoneNo",giveliyana);
			  ...
			  core.ajax.sendPacket(myPacket,function(packet){
				  ...
				  if(retCode == "000000"){
				  	giveliyana = phoneNoRet;
				  	//alert("ajaxOrderCmtChk 物联网转换后号码："+giveliyana);
				 	}else{
   ...
   if(iccidtypequery == "8" || iccidtypequery == "A" || iccidtypequery == "B" || iccidtypequery == "C"){
	  ...
	  var myPacket = new AJAXPacket("checkRealUserNum.jsp","正在查询信息，请稍候......");
	  myPacket.data.add("g_CustId","<%=gCustId%>"); 
	  myPacket.data.add("opCode","<%=opCode%>");
	  myPacket.data.add("phoness",giveliyana);
 ...
 var  orderCmtChk_Packet = new AJAXPacket("ajaxOrderCmtChk.jsp","正在获得帐户ID，请稍候......");
	...
	/* 在这里校验是否4G传serviceType，原来的根据offerid不行了。 */
	var selectSql = "";
	var params = "";
	var nuCardType = document.all.cardTypeN.value;
	if(nuCardType=="1"){
		//空卡开户
		var simType = document.all.simTypeCfm.value;
		selectSql = "90000013";
		params = simType + "|";
	}else{
		//实卡开户
	...
	var getServiceType_Packet = new AJAXPacket("/npage/public/pubSelectBySql.jsp","正在校验请稍候......");
	getServiceType_Packet.data.add("selectSql",selectSql);
	getServiceType_Packet.data.add("params",params);
	getServiceType_Packet.data.add("wtcOutNum","1");
	core.ajax.sendPacket(getServiceType_Packet,doQrySrvTypeBack);
	getServiceType_Packet = null;
}
function doQrySrvTypeBack(packet){
	var retcode = packet.data.findValueByName("retcode");
	var retmsg = packet.data.findValueByName("retmsg");
	var result = packet.data.findValueByName("result");
	if(retcode != "000000"){
...
function mySub()
{
	...
	var havenum = 0;
	for(var i=0;i<kxbxArray.length;i++){
			var offerid = kxbxArray[i];
			$("#tab_order tr:gt(0)").each(function(){
				var kxzfOfferId = $(this).find("td").eq(0).html();
				if(kxzfOfferId == offerid){
					havenum ++;
				}
			});
	}
	
	if(havenum != kxbxArray.length){
		rdShowMessageDialog("主资费必绑的附加资费信息异常，请重新操作！");
		return false;
	}
...
if("<%=m2mFlag%>"!="Y"){
	...
	if(document.all.Good_PhoneDate_GSM.style.display == "")
	{
		...
		//hejwa add 关于物联网14765语音功能开放及结算原则调整的通知 2014年4月1日 后台 yull
		//3010:语音功能（物联网） 3011:国际漫游（物联网） 这两个特服时  判断开户的号码 是否是20665 号段 只有20665 号段才可以选择
		var brandID101ProdFlag = 0;
		if("<%=brandID%>"=="101"){
			$("#tab_addprod :checked").each(function(){
				var prodId = this.id;
				if(prodId=="3010"||prodId=="3011"||prodId=="3022"){//判断产品
					var phoneNo = $("input[name='selNum']").val().trim();
					if(phoneNo.substring(0,5) != "14765"&&phoneNo.substring(0,5) != "14766"&&phoneNo.substring(0,5) != "20665"&&phoneNo.substring(0,5) != "20666"&&phoneNo.substring(0,4) != "1849"&&phoneNo.substring(0,4) != "1789"){
						//不包含，找到一个就可以退出了
						brandID101ProdFlag = 1;
						return false;
					}
				}
			});
			
			 //还要再加一个限制 如果开户时选择的资费中有 38972 资费 
			 //看一下用户的品牌 如果是物联网 则判断号段 如果号段不是20665 就提示不让办理
			 $("#div_offerComponent :checked").each(function(){
			 		var offerId = this.id;
			 		if("38972"==offerId.trim()){
			 			var phoneNo = $("input[name='selNum']").val().trim();
						if(phoneNo.substring(0,5) != "14765"&&phoneNo.substring(0,5) != "14766"&&phoneNo.substring(0,5) != "20665"&&phoneNo.substring(0,5) != "20666"&&phoneNo.substring(0,4) != "1849"&&phoneNo.substring(0,4) != "1789"){
							//不包含，找到一个就可以退出了
							brandID101ProdFlag = 2;
							return false;
						}
			 		}
			 });
		...
		if(brandID101ProdFlag==2){
			rdShowMessageDialog("38972销售品对物联网用户只允许14765、14766、1849、1789号段订购！");
		 	return false;	
		}
		if(brandID101ProdFlag==1){
			rdShowMessageDialog("3010或3011或3022产品对物联网用户只允许14765、14766、1849、1789号段订购！");
		 	return false;	
		}
...
<%
if("Y".equals(main_k_flag)){
%>
 	if($("#ipt_PhoneID").val().trim()==""){
			rdShowMessageDialog("请输入主卡号码");
			return;
	}
	
	if($("#ipt_PhoneID").attr("v_check_result")!="1"){
			rdShowMessageDialog("请校验主卡号码");
			return;		
	}
...
function doCheckPwdEasy(packet) {
	...
	if (retResult == "1") {
		...
		if($("input[name='op_note']").val() == ""){
			...
			if("<%=opCode%>" == "g784"){
				//alert(2109);
				$("input[name='op_note']").val("操作员"+"<%=workNo%>"+"对客户"+"<%=gCustId%>"+"进行前台预开户!");	
			}
			else if("<%=opCode%>" == "g785"){
				$("input[name='op_note']").val("操作员"+"<%=workNo%>"+"对客户"+"<%=gCustId%>"+"进行前台校园营销预开户!");	
			}
			else if("<%=opCode%>" == "m028"){
				$("input[name='op_note']").val("操作员"+"<%=workNo%>"+"对客户"+"<%=gCustId%>"+"进行天猫开户!");	
			}
			else if("<%=opCode%>" == "m094"){
				$("input[name='op_note']").val("操作员"+"<%=workNo%>"+"对客户"+"<%=gCustId%>"+"进行移动商城开户!");	
			}
		...
		var offerAttrStrKx = ""; 		//可选销售品属性串
		$("#OfferAttribute :input").not(":button,[type='hidden']").each(function(i){
			if(i > 0){
		...
		$("select[name='s_60001_Kx']").each(function(){
				offerAttrStrKx+=$(this).attr("offerId")+"|";
				offerAttrStrKx+="60001";
				offerAttrStrKx+="~";
				offerAttrStrKx+=$(this).val()+" $"+"^";
		...
		if(offerAttrStrKx != ""){
			offer_productAttrInfo += offerAttrStrKx;
		}
		
		//alert(offer_productAttrInfo);
...
//		} 
		...
		$("#cfmBtn").attr("disabled",true);
		/*2015/7/9 9:37:22 gaopeng 关于讨论客户号码销户后重新放号提醒问题的会议纪要 
			增加提示语
		*/
		var useSmCode = document.all.newSmCode.value;
		if(useSmCode == "dn" || useSmCode == "gn" || useSmCode == "zn"){
			rdShowMessageDialog("如您收到原机主捆绑的银行或其它服务行业等第三方公司信息，建议您直接联系第三方公司客服解除捆绑关系！");
		}		
...
function conf(path){
			...
			var myPacket = new AJAXPacket("savePrintInfos.jsp","正在查询信息，请稍候......");
...
function printInfo(printType)
{
    ...
    cust_info+="客户地址：	"+"<%=custAddr%>"+"|";
		...
		if("<%=opCode%>" == "g784" || "<%=opCode%>" == "g785" || "<%=opCode%>" == "m028" || "<%=opCode%>" == "m094"){
			if((document.all.modedxpay.value).trim().length!=0){
				opr_info+="办理业务：<%=opName%>"+"  "+"操作流水："+document.all.sysAcceptl.value+"   底线消费金额"+document.all.modedxpay.value+"元"+"|";
			}else{
				opr_info+="办理业务：<%=opName%>"+"  "+"操作流水："+document.all.sysAcceptl.value+"|";
			}
		}else{
	  ...
	  ajaxGetEPf('<%=offerId%>',xqdm);
		...
		for(int hhh=0;hhh<result_t33.length;hhh++){
		  ...
		  if(document.all.dECode.value!="") {
			     opr_info+="  主资费二次批价："+document.all.dECode.value+"-"+document.all.xq_name.value+"|";
	  	}else{
		...
		for(var h=1;h<tempArray1.length;h++){
			...
			if(node!=""&&node!="undefined"){
				...
				if("<%=dateStr2%>"==node.begTime.substring(0,8)){		  		
					...
					emergencyKxOfferContent += "("+node.getId()+"、"+node.getName()+")";			
    ...
    if("<%=orange_flag%>"=="1"){
		...
		if(jmz.getlength(retResultStr1) > 1024){
			note_info1+="可选资费描述：|"+emergencyKxOfferContent+"|";	
		}else{
    ...
    if(offerTrafficflag!="0") {
     note_info1+="已默认为您开通流量不清零服务。您<%=offerName %>套餐内当月剩余流量可结转至次月使用，结转流量次月月底失效，同一类型流量优先扣减上月结转流量。若您当月变更主套餐、销户、携号转网等，原套餐内剩余流量不结转至次月。"+"|";
    }

		/*2013/12/25 13:56:33 gaopeng 关于2013年9月第一批业务支撑系统市场专业需求（关于主资费捆绑数据业务功能优化的需求） 修改免填单内容*/
...
// note_info4+="4、4G业务仅在4G网络所覆盖的范围内提供，中国移动将不断扩大4G覆盖区域、提高服务质量。"+"|";
 ...
 if("<%=backCode%>" == "100001" && printAddFlag){
 	note_info4+= "尊敬的客户，为感谢您的支持，我公司将自本月起为您赠送“和阅读精品畅读包体验版”免费体验服务三个月，体验到期当月25号系统将自动为您退订业务，体验期间您也可以发送短信QXCDBTY到10086取消。"+"|";
 }
 
 /*彩铃提示信息*/
 if("<%=backCode%>" == "100002" && printAddFlag){
 	note_info4+= "尊敬的客户，为感谢您的支持，我公司赠送您“彩铃0元体验包”及“铃听天下0元包”免费体验至2016年3月15日，办理成功48小时内生效，体验到期系统自动为您退订业务，体验期间您也可以发送短信QXCLTY到10086取消。体验彩铃与其他彩铃产品不能同时订购，系统以客户最后一次订购产品为准。"+"|";
 }


<%
if("BL".equals(main_k_type)||"bl".equals(main_k_type)){
%>
    note_info4 += "1.流量伴侣卡号码为"+$("input[name='selNum']").val().trim()+"，流量伴侣卡主卡号码为"+$("#ipt_PhoneID").val()+"；|";
    note_info4 += "2.流量伴侣卡所产生的费用由流量伴侣卡主卡客户进行支付。 |";
    note_info4 += "3.流量伴侣卡不允许办理其他流量可选套餐（不能办理剩余流量转赠和流量赠予业务），也不允许变更其他主套餐。  |";
    note_info4 += "4.流量伴侣卡主卡只允许办理18元及以上一体化资费。  |";
    note_info4 += "5.流量伴侣卡停机后，需要流量伴侣卡主卡号码缴费开机后，副卡的号码才能开机。流量伴侣卡不能单独缴费，不能使用其账户中的费用。  |";
    note_info4 += "6.流量伴侣卡主卡需要在副卡销户后才能办理销户和过户业务，流量伴侣卡不允许办理过户业务。  |";
    note_info4 += "7.流量伴侣卡不允许办理营销活动。 |";
<%
}
else if("CH".equals(main_k_type)||"ch".equals(main_k_type)){
%>
	note_info4 += "1.超和卡副卡号码为"+$("input[name='selNum']").val().trim()+"，对应的主卡号码为"+$("#ipt_PhoneID").val()+"；|";
	note_info4 += "2.超和卡副卡所产生的费用由对应的主卡客户进行支付。 |";
	note_info4 += "3.超和卡副卡的主卡号码进入停机、报停、挂失等异常状态时，超和卡副卡同步触发停机。主卡号码恢复正常状态时，副卡自动恢复正常状态。 |";
	note_info4 += "4.超和卡副卡不允许办理其他可选套餐，也不允许变更其他主套餐。|";
	note_info4 += "5.超和卡副卡的主卡客户仅允许变更为套餐价值属性大于当前主卡套餐价值属性的统一套餐。|";
<%
}
else if("BL1".equals(main_k_type)||"bl1".equals(main_k_type)){
%>
    note_info4 += "1.30元流量卡号码为"+$("input[name='selNum']").val().trim()+"，流量伴侣卡主卡号码为"+$("#ipt_PhoneID").val()+"|"; 
    note_info4 += "2.30元流量卡所产生的费用由流量伴侣卡主卡客户进行支付，30元流量卡不能单独缴费。30元流量卡停机后，需要流量伴侣卡主卡号码缴费开机后，30元流量卡才能开机。 |"; 
    note_info4 += "3.流量伴侣卡主卡号码进入停机、报停、挂失等异常状态时，30元流量卡同步触发停机。流量伴侣卡主卡号码恢复正常状态时，30元流量卡自动恢复正常状态。 |"; 
    note_info4 += "4.30元流量卡可以正常办理可选套餐，但不能变更其他主资费。 |"; 
    note_info4 += "5.流量伴侣卡主卡客户只允许变更其他18元及以上档位一体化套餐，不允许变更为其他主套餐；流量伴侣卡主卡可在30元流量卡销户之后变更其他主套餐。 |"; 
    note_info4 += "6.流量伴侣卡主卡需要在30元流量卡销户后才能办理销户业务。 |"; 
    note_info4 += "7.30元流量卡不允许办理流量共享、剩余流量转赠和流量求赠赠予业务。|"; 

<%
}else if("DW".equals(main_k_type)||"dw".equals(main_k_type)){
%>
	note_info4 += "1.咪咕大王卡号码为"+$("input[name='selNum']").val().trim()+"，咪咕大王卡主卡号码为"+$("#ipt_PhoneID").val()+"；|";
	note_info4 += "2.咪咕大王卡所产生的费用由咪咕大王卡主卡客户进行支付，咪咕大王卡不能单独缴费。咪咕大王卡停机后，需要流量咪咕大王卡号码缴费开机后，咪咕大王卡的号码才能开机。|";
	note_info4 += "3.咪咕大王卡主卡号码进入停机、报停、挂失等异常状态时，咪咕大王卡同步触发停机。咪咕大王卡主卡号码恢复正常状态时，咪咕大王卡自动恢复正常状态。|";
	note_info4 += "4.咪咕大王卡不允许办理其他流量套餐，也不允许变更其他主资费。|";
	note_info4 += "5.咪咕大王卡主卡客户只允许变更其他18元及以上档位一体化套餐，不允许变更为其他主套餐。|";
	note_info4 += "6.咪咕大王卡主卡需要在咪咕大王卡销户后才能办理销户业务。|";
	note_info4 += "7.咪咕大王卡不允许办理流量共享、剩余流量转赠和流量求赠赠予业务。|";
<%
}else  if("PG".equals(main_k_type)||"pg".equals(main_k_type)){
%>
	note_info4 += "1.苹果套卡号码为"+$("input[name='selNum']").val().trim()+"，对应的主卡号码为"+$("#ipt_PhoneID").val()+"；|";
	note_info4 += "2.苹果套卡所产生的费用由对应的主卡客户进行支付；苹果套卡不能单独缴费，不能使用苹果套卡账户中的费用；|";
	note_info4 += "3.苹果套卡停机后，需要主卡号码缴费开机后，苹果套卡的号码才能开机；|";
	note_info4 += "4.苹果套卡可以订购不判断预存款的可选资费，不允许变更为任何其他主资费，苹果套卡的主卡只允许变更为指定范围内的主资费；|";
	note_info4 += "5.苹果套卡的主卡需要在苹果套卡销户后才能办理销户和过户业务，苹果套卡不允许办理过户业务；|";
	note_info4 += "6.苹果套卡不允许办理剩余流量转赠和流量求赠赠予业务。|";
	note_info4 += "7.苹果套卡不参与流量共享业务。|";
...
function ajaxQueryOfferTraffic(offerids) {
		var funtionnames_Packet = new AJAXPacket("queryOfferTraffic.jsp","正在查询，请稍后......");
...
function checksim(){
    ...
    if("<%=brandID%>"=="101"){
    	var myPacket = new AJAXPacket("/npage/public/pubGetWLWPhoneNo.jsp","正在查询信息，请稍候......");
			  ...
			  core.ajax.sendPacket(myPacket,function(packet){
				  ...
				  if(retCode == "000000"){
				  	giveliyana = phoneNoRet;
				  	//alert("sim卡号校验 物联网转换后号码："+giveliyana);
				 	}else{
    ...
    /* ningtn 4G项目增加*/
    /* 不要以offerId为查询条件了，是否选中4G特服作为条件 */
    var choose4GFlag = $("#tab_addprod :checkbox[id='4001']").attr("checked");
    if(typeof(choose4GFlag) != "undefined" && (choose4GFlag == true || choose4GFlag == "true")){
    	/*选中，传1*/
    	checkResource_Packet.data.add("offerId","1");
    }else{
    	/*未选中，传0*/
    ...
    var nuCardType = document.all.cardTypeN.value;
		...
		if("<%=sm_Code%>" == "TE" && "<%=opCode%>" == "1104"){
			checkResource_Packet.data.add("simTypeOne",$.trim($("select[name='simTypeOne']").find("option:selected").val()));
		}else{
...
function chaCardType(){  //空卡开户
	...
	if(nuCardType=="1"){
       ...
       if("10073"==document.all.simTypeCfm.value){
       	rdShowMessageDialog("请去‘m404城市通开卡代收费界面’收取公交费”。");
       }
...
function writechg(){
	...
	if(document.all.cardtype_bz.value=="k"){
		var phone = $("input[name='selNum']").val().trim();
  			document.all.b_write.disabled=true;
...
function setPtPDate(){
		if(document.all.ptPhoneFlag.value=="Y"){
			...
			$("#ptPhoneDate").val("");//防止营业员先选择不允许过户后，填过可办理过户时间，提交后没有校验的问题。
		}else{
...
function go_sPhoneIDCheck(){
		if($("#ipt_PhoneID").val().trim()==""){
			rdShowMessageDialog("请输入主卡号码");
			return;
		}
	  var packet = new AJAXPacket("go_sPhoneIDCheck.jsp","请稍后...");
        packet.data.add("opCode","<%=opCode%>");//
        packet.data.add("g_CustId","<%=gCustId%>"); 
        packet.data.add("sPhoneID",$("#ipt_PhoneID").val().trim());//
        packet.data.add("main_k_type","<%=main_k_type%>");//
		packet.data.add("iOfferId","<%=offerId%>");//
    core.ajax.sendPacket(packet,do_sPhoneIDCheck);
    packet =null;
}
function do_sPhoneIDCheck(packet){
    ...
    if(error_code!="000000"){//调用服务失败
      ...
      $("#ipt_PhoneID").val("");
	    ...
	    $("#ipt_PhoneID").attr("readOnly","readOnly");
	    $("#ipt_PhoneID").addClass("InputGrey");
	    $("#ipt_PhoneID").attr("v_check_result","1");
	    $("#btn_sPhoneIDCheck").attr("disabled","disabled");
...
/*
57575 集团客户3元底线消费-物联网专用   哈尔滨     01	哈尔滨              
57576 集团客户3元底线消费-物联网专用   齐齐哈尔   02	齐齐哈尔            
57577 集团客户3元底线消费-物联网专用   牡丹江     03	牡丹江              
57578 集团客户3元底线消费-物联网专用   佳木斯     04	佳木斯              
57579 集团客户3元底线消费-物联网专用   双鸭山     05	双鸭山              
57580 集团客户3元底线消费-物联网专用   七台河     06	七台河              
57581 集团客户3元底线消费-物联网专用   鸡西       07	鸡西                
57582 集团客户3元底线消费-物联网专用   鹤岗       08	鹤岗                
57583 集团客户3元底线消费-物联网专用   伊春       09	伊春                
57584 集团客户3元底线消费-物联网专用   黑河       10	黑河                
57585 集团客户3元底线消费-物联网专用   绥化       11	绥化                
57586 集团客户3元底线消费-物联网专用  大兴安岭    12	大兴安岭            
57587 集团客户3元底线消费-物联网专用   大庆       13	大庆                
*/
...
$(document).ready(function(){
		...
		if(
				(offer_id=="57575"&&regionCode=="01")||
				(offer_id=="57576"&&regionCode=="02")||
				(offer_id=="57577"&&regionCode=="03")||
				(offer_id=="57578"&&regionCode=="04")||
				(offer_id=="57579"&&regionCode=="05")||
				(offer_id=="57580"&&regionCode=="06")||
				(offer_id=="57581"&&regionCode=="07")||
				(offer_id=="57582"&&regionCode=="08")||
				(offer_id=="57583"&&regionCode=="09")||
				(offer_id=="57584"&&regionCode=="10")||
				(offer_id=="57585"&&regionCode=="11")||
				(offer_id=="57586"&&regionCode=="12")||
				(offer_id=="57587"&&regionCode=="13")
		){
			
			//1009:短消息 取消选中
			$("#tab_addprod :checkbox[id='1009']").removeAttr("checked"); 
		}
...
<input type="hidden" id="main_k_type" name="main_k_type" value="<%=main_k_type%>"/>
<input type="hidden" id="main_k_flag" name="main_k_flag" value="<%=main_k_flag%>"/>
...
<div id="tabsJ">
	<ul>
		...
		<li id="tb_2"><input type='radio' name='changeSelh' value='selStruc' onclick="HoverLi(2,2);">销售品构成<font color="red">(办理可选资费)</font></li>
	</ul>
</div>
<!-- tab end -->
<!-- 基本销售品 开始-->
...
<DIV id="OfferAttributeKxTitle" style="display:none" class=title><div id="title_zi">已选择附加销售品小区信息</div></DIV>
<div id="OfferAttributeKx"></div><!--可选销售品小区属性-->
...
<table cellSpacing=0 id="serviceNoInfo">	
	...
	<tr id="simTypeOneShowAndHide" >
		<td class="blue">SIM卡类型</td>
		<td colspan="3">
			<select name="simTypeOne" onchange="chgSimTypeOne();">
...
<% 
/*2014/06/17 13:45:25 gaopeng 关于齐齐哈尔分公司开发“营销预开户”系统的需求 */
if("g784".equals(opCode) ){%>							
							<tr>
								<td class="blue">是否赠费</td>
								<td colspan="3">
									<select name="gFeeFlag">
										<option value="0" selected>否</option>	
										<option value="1">是</option>	
									</select>	
								</td>
...
<SCRIPT language=JavaScript>
	...
	function doIsGoodPhoneF(packet){
			...
			if(countGoodNo!=0){  //靓号
					...
					}else if(innetType=="04" && selNum != ""){ //188号段的非特殊号码
						var selNum3 = selNum.substring(0,3);//取出号码前3位
						if(selNum3 == "188"){
							document.all.pt_PhoneDate_GSM.style.display = "";		
						}
...
$(document).ready(function () {
	...
	/*c关于开发智能终端CRM模式APP的函 - 第三批@2015/4/6 */
	if("<%=opCode%>" == "1104" || "<%=opCode%>" == "m028"){ //普通开户、特殊号码、一卡双号都为1104，天猫开户为m028
...
<table cellSpacing=0>	
	<TR>
	  ...
	  <TD width="35%"><input name="contractNo" id="contractNo" class="required for0_9" size="16" maxlength="14" readonly>
...
%>
		...
		<td>
		<input type="text" name="ipt_PhoneID" id="ipt_PhoneID" v_check_result="0" maxlength="11"  />
		<input type="button" class="b_text" value="校验" id="btn_sPhoneIDCheck"   onclick="go_sPhoneIDCheck()" />
...
<input type="hidden" id="m2mFlag" name="m2mFlag" value="<%=m2mFlag%>"/>
...
<!--2014/07/09 15:10:07 gaopeng 物联网行车卫士隐藏域 -->
<input type="hidden" id="carSaveFlag" name="carSaveFlag" value="<%=carSaveFlag%>"/>
