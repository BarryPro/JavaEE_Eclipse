...
$().ready(function(){
  ...
  %>  
	...
	<%if(opCode.equals("g719")||opCode.equals("1272")){%>
	myFavorite();
 	hotOffer();	
 	$("#show1270").show();		
	<%}else{%>	
...
function addOffer(offer_id){
  jdugeAreaHidden(offer_id);//diling add for 判断是否显示新版小区代码@2013/3/14
}
  ...
  for(var i=0;i<document.all.proOptFlag.length&&(typeof(document.all.proOptFlag.length)!="undefined");i++){
	...
	if(errorCode == 0){
		...
		$.each(retAry,function(i,n){
			...
			}else if(offerStatus == 3 && offerOrProd == 0){
			...
			if(oldSmCode == "21"){
				...
				//rdShowMessageDialog("该操作涉及到品牌变更，您的现有积分或M值将于品牌变更生效时失效，请您在新资费生效前及时兑换。");
		  			...
		  			if(smzvalues=="3") {//非实名制全球通、动感地带客户转移为神州行客户
		  				rdShowMessageDialog("<%=readValue("g719","ps","jf",Readpaths)%>");
		  			}
		  		}
		}
	}
...
{	
	...
	<%}else if(opCode.equals("g719")){%>
		retInfo = printInfog719();
...
function printInfo1253()
{
      ...
      opr_info+="资费（代码、名称）："+document.all.offerIdHv.value+" "+document.all.offerNameHv.value+"|";
...
function printInfo1255()
{
    ...
    opr_info += "到期后执行资费：" + document.all.dOfferId.value+" "+document.all.dOfferName.value + "|";
		if(document.all.dECode.value!="") {
			opr_info+="到期后二批代码："+document.all.dECode.value+"|";
			}
		if(zdxq=="1")
		{
...
function printInfog719()
{
  ...
  for(var iw=1;iw<countBaseOffer;iw++){
    ...
    opr_info+="新申请主资费二次批价："+ document.all.newZOfferECode.value+"*"+"新资费对应品牌："+ sm_code+"|" ;
	...
	else
	{
		if(newSmCode != oldSmCode)
		{
	    				  			...
	    				  			var smzvalues =document.all.smzvalue.value.trim();
		  			if(smzvalues=="3") {//非实名制全球通、动感地带客户转移为神州行客户
		  					note_info4+="<%=readValue("g719","ps","jf",Readpaths)%>"+"|";
		  			}
