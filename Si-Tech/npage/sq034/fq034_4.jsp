...
%>
<%			  
System.out.println("------------mylog-----------------quevalue-----------"+quevalue);
	...
	if(quetypes.equals("21")){
		if(orderSelectId!=null&&!orderSelectId.equals("")){
			...
			if(opcodestr!=null&&opcodestr.equals("2")){
				...
				System.out.println("------------mylog---------"+sername+"--------quevalue----------------"+quevalue);
				System.out.println("------------mylog---------"+sername+"--------orderSelectId-----------"+orderSelectId);
				%>
		...
		if(opcodestr!=null&&opcodestr.equals("2")){
			...
			System.out.println("------------mylog---------"+sername+"--------quetype----------------"+quetype);
			System.out.println("------------mylog---------"+sername+"--------quevalue---------------"+quevalue);
			%>
...
if(quetype!=null&&!quetype.equals("")&&quevalue!=null&&!quevalue.equals("")&&(!(quetype.equals("2")&&!quetypes.equals("21")))||quetype2!=null&&quetype2.equals("2")){
	...
	System.out.println("------------mylog---------"+sername+"--------parm1---------------"+parm1);
	System.out.println("------------mylog---------"+sername+"--------parm2---------------"+parm2);
	System.out.println("------------mylog---------"+sername+"--------parm3---------------"+parm3);
	System.out.println("------------mylog---------"+sername+"--------parmtype1-----------"+parmtype1);
	System.out.println("------------mylog---------"+sername+"--------parmtype------------"+parmtype);
      ...
      strArray = CreatePlanerArray.createArray("arrMsg",endRow-startRow);				
		...
		for(int i=startRow;i<endRow;i++){		
		  ...
		  if(opcodestr!=null&&opcodestr.equals("2")&&(retVal.getSize("2."+i)>2)){
			...
			System.out.println("------------mylog---------sGetOrdeArray--------parmtype------------");
			%>
			<wtc:utype name="sGetOrdeArray" id="retVal2" scope="end" >
		...
		for(int i=startRow;i<endRow;i++){	
			...
			System.out.println("------------mylog---------sGetOrdeArray--------1111111------------");
			%>
			<wtc:utype name="sGetOrdeArray" id="retVal2" scope="end" >
				 <wtc:uparam value="<%=sendInfo %>" type="UTYPE"/> 
			</wtc:utype>
			...
			//if(retVal.getUtype("2")!=null&&retVal.getSize(2)>0)
			%>

			<%		
			for(int j=0;j<retVal2.getSize("2.0");j++){
				for(int a=1;a<8;a++){
					String rtv2=retVal2.getUtype("2.0").getUtype(j).getValue(a-1);
					realPhone=retVal2.getUtype("2.0").getUtype(j).getValue(2);
			    System.out.println("value=========="+rtv2);
...
<wtc:service name="se083Check" outnum="1" retmsg="se083Msg" retcode="se083Code" routerKey="region" routerValue="<%=regionCode%>">
		<wtc:param value="" />
		<wtc:param value="01" />	
		<wtc:param value="<%=opCode%>" />	
		<wtc:param value="<%=work_no%>" />
		<wtc:param value="<%=password%>" />
		<wtc:param value="<%=realPhone%>" />
		<wtc:param value="" />
		<wtc:param value="" />
</wtc:service>
<wtc:array id="se083Result" scope="end"/>
<%
	if("000000".equals(se083Code)){
