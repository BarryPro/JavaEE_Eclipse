...
function segmentAdd_grp<%=jsi%>(){
	...
	tr1.insertCell().innerHTML = '<td><input class=b_text type="button" name="del" value="删除" onclick="deleteIt_grp<%=jsi%>(this)"></td>';
		...
		if (resultList != null)
		{
			while(i<fieldCount)
			{
				...
				if(trFlag||i==0)
				{
    				...
    				<TR>
    				<TD><input type="button" name="addSegment<%=fieldGroupNo[i]%>" class="b_text"  value="增加" onclick="segmentAdd_grp<%=fieldGroupNo[i]%>()"></TD>
    				<%
					}
					else
					{
