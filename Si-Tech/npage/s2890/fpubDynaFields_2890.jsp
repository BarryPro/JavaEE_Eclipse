...
%>
		...
		if(!checkValues_flag)
		{
		//wuxy alter 20090825 端到端　类型更换　从01变更为10
		...
		//wuxy alter 20090825 端到端　类型更换　从02变更为11    
		...
		//wuxy alter 20090825 端到端　类型更换　从03变更为12
...
%>   
     ...
     %>
				...
				{	
					...
					//wuxy alter 20090825 端到端　类型更换　从01变更为10
					else if(frm[eval('v')].datatype=='10'){//整形
					...
					//wuxy alter 20090825 端到端　类型更换　从02变更为11
					else if(frm[eval('v')].datatype=='11'){//浮点型
					...
					//wuxy alter 20090825 端到端　类型更换　从03变更为12
					else if(frm[eval('v')].datatype=='12'){//字符型
				     	if (JHshStrLen(frm[eval('v')].value) > "<%=slGroupItem[m]%>"){
					...
					//wuxy alter 20090825 端到端　类型更换　从07变更为14				
					else if(frm[eval('v')].datatype=='14'){//日期
				  ...
				  //wuxy alter 20090825 端到端　类型更换　从08变更为15
					else if(frm[eval('v')].datatype=='15'){//日期时间
					...
					//wuxy alter 20090825 端到端　类型更换　从09变更为13
					else if(frm[eval('v')].datatype=='13'){//金额
				         	if(!(parseFloat(frm[eval('v')].value )==parseInt(frm[eval('v')].value ))){
					     //rdShowMessageDialog("参数类型为浮点型!",0);
					     frm[eval('v')].focus();
					     return false;					
					...
					//wuxy alter 20090825 端到端　类型更换　从14变为23
					else if(frm[eval('v')].datatype=='23'){//时间
...
<TABLE cellspacing="0" style="display:<%=listShow%>">
	<TR>
		<TD colspan=20>
			<font color=red><!--wuxy alter 20090902 MAS短信服务号码长度改为5-14 -->
				短消息服务代码为：基本接入号<%=resultCheckdocId[0][0]%> +XXXX  提醒：MAS短信服务号码长度必须为5-14位,ADC短信服务号码长度必须为14位!请核对好再确认。
			</font>
		</TD>
	</TR>
		...
		while (i < fieldCount) {
				...
				for (int colNum = 0; (colNum < ColNumInARow) && (i < fieldCount); colNum ++)
				{
				  ...
				  /*
					...
					//wuxy alter 20090824　端到端修改了param_type　做相应调整，从10改为17
					...
					//wuxy alter 20090824　端到端修改了param_type　做相应调整，从20改为21
					...
					//wuxy alter 20090824　端到端修改了param_type　做相应调整，从12改为22　
					else if(dataTypes[i].equals("22"))//密码
					{
