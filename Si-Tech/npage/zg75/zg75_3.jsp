...
<%
	String[] inParas0 = new String[29];
		...
		inParas0[10] ="";//品牌
		inParas0[11] =invoice_money;//小写金额
		inParas0[12] = mon.NumToRMBStr(Double.parseDouble(invoice_money));//大写金额
		inParas0[13] = "通信服务费:"+txfwf+"<p>"+"销售折扣"+xszk+"<p>" ;//备注
...
<%
		...
		if(RetResult.length > 0)
		{
			...
			if((result2[0][0]!="000000")&&!result2[0][0].equals("000000"))
			{
				%>
					<script language="JavaScript">
					    rdShowMessageDialog("111电子发票入库失败,错误代码"+"<%=result2[0][0]%>"+",错误原因:"+"<%=result2[0][1]%>".",0);
					    document.location.replace("<%=returnPage%>");
					</script>
        <%			
			}
...
%>
					<script language="JavaScript">
					    rdShowMessageDialog("222电子发票入库失败,szg12InDB_pt服务返回结果为空.",0);
					    document.location.replace("<%=returnPage%>");
					</script>
<%			
		}
