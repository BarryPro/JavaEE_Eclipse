...
/****************由移动号码得到密码、机主姓名、 等信息 s1251Init***********************/
  ...
  //retList = impl.callFXService("sopengiftinit", paraAray1, "9","phone",phoneNo);
%>
	<wtc:service name="sopengiftinit" routerKey="phone" routerValue="<%=phoneNo%>" retcode="retCode1" retmsg="retMsg1" outnum="9">			
...
<!--
  ...
  //***
  function frmCfm(){
	document.frm.action = "fopengiftcomit.jsp";
	document.frm.submit();
	return true;
  }
  //***//校验是否选中领奖纪录
...
{ 
    ...
    var fieldName = "礼品类别|礼品代码|礼品名称|";//弹出窗口显示的列、列名
   	var sqlStr ="select award_id,award_no,award_info from sopengiftinfo where  award_id >= '"+document.frm.awardId.value+"' and region_code=substr('"+document.all.orgCode.value+"',1,2) order by award_id,award_no";
	//	alert(sqlStr); 
