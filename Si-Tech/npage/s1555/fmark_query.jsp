...
/****************由移动号码得到密码、机主姓名、 等信息 s1251Init***********************/
  ...
  //retList = impl.callFXService("s1557markinit", paraAray1, "9","phone",phoneNo);
%>
	<wtc:service name="s1557markinit" routerKey="phone" routerValue="<%=phoneNo%>" retcode="retCode1" retmsg="retMsg1" outnum="9">			
...
<%
  ...
  if(tempArr.length==0)
  {
	 ...
	 {
	   retFlag = "1";
	   retMsg = "s1557markinit查询号码基本信息为空!<br>" + "errCode: " + errCode + "<br>errMsg: " +  errMsg;
     }
  }else if(tempArr.length>0)
  {
	if (errCode.equals("000000")){
	  ...
	  passwordFromSer=tempArr[0][3];//密码
	    ...
	    {
	         retFlag = "1";
	         retMsg = "s1557openinit查询号码基本信息失败!<br>" + "errCode: " + errCode + "<br>errMsg: " +  errMsg;
        }
	}
  }
...
<!--
  ...
  //***
  function frmCfm(){
	document.frm.action = "fmark_2.jsp";
	document.frm.submit();
	return true;
  }
  //***//校验是否选中领奖纪录
...
{ 
    ...
    var fieldName = "礼品类别|礼品代码|礼品名称|";//弹出窗口显示的列、列名
    //alert("regionCode="+regionCode);
   	var sqlStr ="select gift_no,gift_code,gift_name from smarkgift where  gift_no >= '"+document.frm.awardId.value+"' and region_code='"+document.frm.regionCode.value+"' order by gift_no,gift_code";
	//alert(sqlStr); 
      ...
      <table cellspacing="0">
	        <tr> 
				...
				<td colspan="3">
			        ...
			        <input type="text" class="button" name="awardInfo" size="30" v_must=1 v_name=奖品名称  >&nbsp;&nbsp;
