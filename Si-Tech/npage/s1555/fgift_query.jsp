...
/****************由移动号码得到密码、机主姓名、 等信息 s1251Init***********************/
  ...
  //retList = impl.callFXService("s1557giftinit", paraAray1, "9","phone",phoneNo);
%>
	<wtc:service name="s1557giftinit" routerKey="phone" routerValue="<%=phoneNo%>" retcode="retCode1" retmsg="retMsg1" outnum="9">			
  ...
  }else if(tempArr.length>0)
  {
	if (errCode.equals("000000")){
	  ...
	  passwordFromSer=tempArr[0][3];//密码
	    ...
	    {
	         retFlag = "1";
	         retMsg = "s1557giftinit查询号码基本信息失败!<br>" + "errCode: " + errCode + "<br>errMsg: " +  errMsg;
        }
	}
  }
  /****得到打印流水****/
...
<!--
  ...
  //***
  function frmCfm(){
	document.frm.action = "fgift_2.jsp";
	document.frm.submit();
	return true;
  }
  //***//校验是否选中领奖纪录
...
{ 
    ...
    var fieldName = "礼品类别|";//弹出窗口显示的列、列名
   	var sqlStr ="select award_id from sopenlikeinfo where  award_id >= '"+document.frm.awardId.value+"' and region_code=substr('"+document.all.orgCode.value+"',1,2) order by award_id";
	//	alert(sqlStr); 
    var selType = "S";    //'S'单选；'M'多选
    var retQuence = "1|0|";//返回字段
    var retToField = "awardNo|";//返回赋值的域
