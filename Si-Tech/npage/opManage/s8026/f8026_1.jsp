   ...
   <div id=tbs1 style=display="">
		...
		<TABLE cellspacing="0"  >
			<tr> 
				<td align = center height="30" id="footer">		
						<input  type="button" id="bSubmit" name="bSubmit" onclick="if(check(form1)) fsubmit()" value="增加" class="b_foot">&nbsp;
	...
	<div id=tbs3 style=display="none">
		...
		<TR  > 			
			<TD  height = 20 class="blue">角色代码</TD>
			<TD  colspan="3" height = 20>
		 		<input type=text  readOnly v_type="string"  v_must=1 v_minlength=1 v_maxlength=100 v_name="角色代码"  name=role_code maxlength=100 onkeydown="if(event.keyCode==13)selectrole()" >
		...
		<TABLE  cellspacing="0">
			<tr> 
				<td align = center height="30" id="footer">		
					<input  type="button"  id="bSubmit4"  name="bSubmit4" onclick="if(check(form4)) fsubmit4()" value="修改" class="b_foot">&nbsp;
		...
		<TABLE  cellspacing="0">
			<form name="form3" method="post" action="">
				...
				<TR  > 			
					<TD  height = 20 class="blue">角色代码</TD>
					<TD  colspan="3" height = 20>
						<input type=text v_type="string" readOnly v_must=1 v_minlength=1 v_maxlength=100 v_name="角色代码"  name=role_code maxlength=100 onkeydown="if(event.keyCode==13)selectrole1()" >
...
$(document).ready(function(){
	$("#bSubmit4").bind("mousemove", function(){
		$("#ts").show();
		var dd=document.all.ts;
		dd.style.top=event.clientY;
		dd.style.left=event.clientX;
	}) ;
	$("#bSubmit4").bind("mouseout", function(){
		$("#ts").hide();
	}) ;
	
	$("#bSubmit").bind("mousemove", function(){
		$("#ts").show();
		var dd=document.all.ts;
		dd.style.top=event.clientY;
		dd.style.left=event.clientX;
	}) ;
	$("#bSubmit").bind("mouseout", function(){
		$("#ts").hide();
	}) ;
});
$(document).keydown(function(event){
	if (event.keyCode == 13 && event.ctrlKey){
		if(tbs1.style.display==""){
			fsubmit();
		}
		if(tbs3.style.display==""){
			fsubmit4();
		}
	}
});
