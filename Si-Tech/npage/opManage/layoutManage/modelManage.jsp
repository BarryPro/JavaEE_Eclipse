	...
	var roleStr = "";
		...
		function g(o){
	 	...
	 	function doSubmit(){
	 		...
	 		if(optLength==0){
	 			rdShowMessageDialog("当前布局为空！",1);
	 			return;
	 		}
	 		if(rdShowConfirmDialog("确认要设置默认布局为“"+$("#moduleHide1 option:selected").text()+"”吗？")==1)
	       	{	       	
		 		//对当前布局进行组装，用~隔开
		 		var optVal = "";
		 		var defaultVal = "";
		 		for(var i=0;i<optLength;i++){
		 			...
		 			if(val==g("moduleHide1").value){
		 				...
		 				var submitPacket = new AJAXPacket("layout_op.jsp","正在执行,请稍后...");
	 	...
	 	function doProcess(packet)      
 		{
	       ...
	       if(opType=="setRole"){
       			if(retCode=="000000"){
       				rdShowMessageDialog("布局角色赋权成功！",2);
       				g("doSet").disabled = "";
       				window.opener.queryTemplate();
       				window.close();
       			}else{
       				rdShowMessageDialog("布局角色赋权失败!<br>错误代码："+retCode+"<br>错误信息："+retMsg,0);
       				return false;
       			}
       	   }
       }
