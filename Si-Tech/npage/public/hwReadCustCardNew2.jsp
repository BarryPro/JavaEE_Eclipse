...
<table cellspacing="0">
	<tr>
		...
		<input type="button" id="photoCollect3" name="photoCollect3" class="b_text"   value="人像采集-高拍仪" onClick="photoCollect22(1)" >
		<input type="button"  class="b_text"   value="上传身份证图像" onClick="camsfztpsc2()">
		</td>
	</tr>
...
<script language="JavaScript">	
  ...
  function photoCollect22(cardnum){
		...
		alert("返回文件名--------"+authinfo);
		authinfo = $.trim(authinfo);
		campicname2=authinfo;
		/*增加全路径*/
		authinfo = "C:/bmp/"+authinfo;
		//$("#accInfoHid2").val(authinfo);
		campicpath_n2=authinfo;
	 ...
	 //将图片上传到数据库中
		function camsfztpsc2() {
			alert("<%=labelName%>");
			if("1993"=="<%=sopcode%>"&&"submitFlag2"=="<%=labelName%>"&&document.all.upbut_flag2.value=="0"){
			...
			window.open("/npage/public/camCardInfoSave.jsp?labelName=<%=labelName%>&idType="+sfzleixing+"&regionCode=<%=regionCode%>&workno=<%=workNo%>&name="+sfznamess+"&opcodes="+sfzcodess+"&picpath_n="+campicpath_n2+"&idIccid="+sfzhaoma,"","width="+(screen.availWidth*1-900)+",height="+(screen.availHeight*1-570) +",left=450,top=240,resizable=yes,scrollbars=yes,status=yes,location=no,menubar=no");
	 }
