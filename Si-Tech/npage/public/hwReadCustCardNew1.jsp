...
<table cellspacing="0">
	<tr>
		...
		<input type="button" id="photoCollect3" name="photoCollect3" class="b_text"   value="人像采集-高拍仪" onClick="photoCollect11(1)" >
		<input type="button"  class="b_text"   value="上传身份证图像" onClick="camsfztpsc1()">
		</td>
	</tr>
...
<script language="JavaScript">	
  ...
  var campicpath_n1="";
  var campicname1="";
  function photoCollect11(cardnum){
		...
		alert("返回文件名--------"+authinfo);
		authinfo = $.trim(authinfo);
		campicname1=authinfo;
		/*增加全路径*/
		authinfo = "C:/bmp/"+authinfo;
		//$("#accInfoHid1").val(authinfo);
		campicpath_n1=authinfo;
		
  }
	 
	 //将图片上传到数据库中
		function camsfztpsc1() {
			alert("<%=labelName%>");
			if("1993"=="<%=sopcode%>"&&"submitFlag1"=="<%=labelName%>"&&document.all.upbut_flag1.value=="0"){
				rdShowMessageDialog("请先上传身份证头像后再上传身份证图像!");
				return;
			}
			alert($("#gestoresIdType").val());
		 	alert(document.all.gestoresIdType.value);
			if("1993"=="<%=sopcode%>"&&"submitFlag1"=="<%=labelName%>"){
		 		sfzhaoma = $("#gestoresIccId").val();
				sfznamess = $("#gestoresName").val();
				sfzleixing = document.all.gestoresIdType.value.split("|")[0];
		 	}
