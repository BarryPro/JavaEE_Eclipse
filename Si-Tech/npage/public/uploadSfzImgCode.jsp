...
<%
	...
	System.out.println("---chenlei-------filePath---------"+filePath);
	...
	//		System.out.println("---liangyl---------------sSaveNameTxt-----------"+sSaveNameTxt);
			...
			System.out.println("---chenlei-------sSaveName_db---------"+sSaveName_db);
        ...
        System.out.println("---liangyl-----------su.save(savapic)----------------");        
		   ...
		   File imgfile = new File(sSaveName);
				...
				if(imgfile.length()>100000){
					System.out.println("---chenlei-----------reduceImg--begin----------------");
					float abc =(float) ((float)1/(float)(imgfile.length()/100000));
					reduceImg(sSaveName,sSaveName, 0, 0,abc);
					System.out.println("---chenlei-----------reduceImg--end----------------"+abc);
				}
				
        String sbasestr = GetImageStr(sSaveName_db);
        String sbasestrs[] ={sbasestr};
...
//				fos.close();
	    ...
	    String sign_date =new SimpleDateFormat("yyyy-MM-dd").format(new java.util.Date()).toString();
 		...
 		String idIccid = "230101199201011111";
		String idRegion_S = idIccid.substring(0,2);
		String idRegion = idIccid.substring(2,4);
		String dirName = idIccid.substring(0,6)+"/";
		
		
		System.out.println("chenlei---idRegion_S|"+idRegion_S);//身份证号前2位，省份标志 23
		System.out.println("chenlei---idRegion|"+idRegion);//身份证3-4 位，地市标志 01
		
		String serverIP = "10.110.2.241";//生产环境用
		serverIP="10.110.13.52";//测试环境用
		int serverPORT =21;// 6790;
		String idPath = "";
		
	if("23".equals(idRegion_S)){
		...
		String ftpsPassword =getPas();//生产环境用
		ftpsPassword ="cXdlcjQzMjE="; //测试环境用
		System.out.println("chenlei----ftpsPassword|"+ftpsPassword);
	...
	inputParsm[8] = "1";
	inputParsm[9] = legalPerson;
	inputParsm[10] = startDate;
	inputParsm[11] = endDate;
	
	inputParsm[12] = foundDate;
	...
	*/
	System.out.println("chenlei---身份证类型----"+idType);
	if(!"0".equals(idType)){
	%>
		<wtc:service name="sUnitImageComp" routerKey="region" routerValue="<%=regionCode%>" retcode="trueCode" retmsg="trueMsg" outnum="1">
...
System.out.println("---chenlei-------校验返回结果-----------"+trueResult[0][0]);
	if("000000".equals(trueCode)){
	    ...
	    <%
			...
			opener.returnSubState("2222222","0");
	    	</script>
	    <%
		}
	}else{
		...
		<script language="javascript">
			...
			opener.returnSubState("<%=labelName%>","1");--%>
			rdShowMessageDialog("记录失败，错误代码：<%=trueCode%> ，错误信息：<%=trueMsg%>",0);
			opener.returnSubState("3333333","1");
		</script>
		<%
	}
	}
...
catch(Exception e)
{
 			  ...
 			  System.out.println("---chenlei-------------------------dpa-----------------------------"+dpa);
			  System.out.println("---chenlei------------------------up_fa--------------------------"+up_fa);
	   if(up_fa.equals("1")){
	   %>
	 ...
	 <%}else{%>
	  		<script language="JavaScript">
	  		function delete_pic(){
            	$('#foundDate').css('readonly','readonly');
            	alert('readonly');
	  			try{
            			var fso = new ActiveXObject("Scripting.FileSystemObject");  //取系统文件对象
            		}catch(e){
