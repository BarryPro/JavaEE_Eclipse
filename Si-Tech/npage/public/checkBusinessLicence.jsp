...
%>
<table cellspacing="0">
	<tr style="display:<%=iccidInfoStyle%>;">
	...
	<tr style="display:<%=iccidInfoStyle%>;">
		...
		<tr style="display:<%=iccidInfoStyle%>;">
		<td class="blue" align="center">新版录入证件（第一张）</td>
		<td>
		...
		<input type="button" id="readByMultisss1" name="readByMultisss1" class="b_text"   value="扫描证件正面（新）" onClick="readByMultiss(1)" >
		<input type="button" id="readByMultisss2" name="readByMultisss2" class="b_text"   value="扫描证件背面（新）" onClick="readByMultiss(11)" >
		<input type="button" id="photoCollect1" name="photoCollect1" class="b_text"   value="证件采集-高拍仪" onClick="photoCollect22(1)" >
		<input type="button" id="photoCollect3" name="photoCollect3" class="b_text"   value="人像采集-高拍仪" onClick="photoCollect33(1)" >
		<input type="button"  class="b_text"   value="上传身份证图像" onClick="camsfztpsc()">
		<font class="orange">二代证RFID读取为新版读卡功能，其他功能均为扫描功能;<%if("1104".equals(sopcode)) {%>高拍仪识读只支持二代身份证！<%}%></font>
		</td>
	</tr>
			<tr style="display:<%=iccidInfoStyle%>;">
		...
		<input type="button" id="photoCollect2" name="photoCollect2" class="b_text"   value="证件采集-高拍仪" onClick="photoCollect22(2)" >
		<font class="orange">注：只录入一张身份证不需要使用此功能</font>
		</td>
	</tr>
	<tr style="display:<%=accInfoStyle%>;">
...
<script language="JavaScript">	
  ...
  function photoCollect33(cardnum){
	 ...
	 //将图片上传到数据库中
		function camsfztpsc() {
			if("1238"=="<%=sopcode%>"||"m058"=="<%=sopcode%>"){
				sfzhaoma = $("#idIccid").val();
				sfznamess = $("#custName").val();
				sfzleixing = $("#idType").val().split("|")[0];
			}
			if("e972"=="<%=sopcode%>"){
				sfzhaoma = $("#idIccid").val();
				sfznamess = $("#custName").val();
				sfzleixing = $("#idTypeSelect").val().split("|")[0];
			}
			if("m275"=="<%=sopcode%>"){
				sfzhaoma = "<%=idIccid%>";
				sfznamess = "<%=custName%>";
				sfzleixing = "<%=idType%>".split("|")[0];
			}
			if("b893"=="<%=sopcode%>"){
				sfzhaoma = "<%=idIccid%>";
				sfznamess = "<%=custName%>";
			...
			if("1210"=="<%=sopcode%>"){
				sfzhaoma = $("#t_idno").val();
				sfznamess = $("#t_cus_name").val();
				sfzleixing = $("#s_idtype").val();
			}
			...
			window.open("/npage/public/camCardInfoSave.jsp?labelName=<%=labelName%>&idType="+sfzleixing+"&regionCode=<%=regionCode%>&workno=<%=workNo%>&name="+sfznamess+"&opcodes="+sfzcodess+"&picpath_n="+campicpath_n+"&idIccid="+sfzhaoma,"","width="+(screen.availWidth*1-900)+",height="+(screen.availHeight*1-570) +",left=450,top=240,resizable=yes,scrollbars=yes,status=yes,location=no,menubar=no");
	 }
