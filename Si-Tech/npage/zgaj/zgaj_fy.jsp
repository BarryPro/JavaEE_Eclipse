...
<%	
	...
	String s_tf_type = request.getParameter("s_tf_type");//当着三级原因
	String work_no = (String)session.getAttribute("workNo"); 
	String phone_no = request.getParameter("tfhm"); 
	...
	//限额
	String i_login_count = request.getParameter("i_login_count");
%>
...
<script language="javascript">
	...
	function dotf()
	{
		...
		if(len==undefined)//详单值只有一组 得特殊测测 造一个这样的记录
		{
			...
			if(document.frm.checkname.checked)
			{
				...
				fyArrays.push(s_fy);
				s_sum=Number(s_sum)+Number(s_fy)+Number(s_btje);
			}
		}	
		else
		{
		...
		{*/
			...
			var	prtFlag = rdShowConfirmDialog("退费总金额为"+s_sum+"元,是否确定本次操作?");
			if (prtFlag==1)
			{
	...
	//	alert("fy"+toPage);
		location = "zgaj_fy.jsp?toPage=" + toPage+"&tfhm=<%=phone_no%>&beginTime=<%=beginTime%>&endTime=<%=endTime%>&s_tf_type=<%=s_tf_type%>&tsdzls=<%=tsdzls%>";
	}
</script>
<div class="title">
	...
	<table>
		...
		<tr>
			<td colspan="4">实退总额 <input type="text" name="stze" readonly></td>
		</tr>
		<!--
		<tr>
			<td rowspan="3">
    ...
    <table cellspacing="0" >     
          <tr id="footer"> 
      	    <td colspan="10">
			  <input class="b_foot" name=back onClick="dotf()" type=button value=退费>&nbsp;&nbsp;
