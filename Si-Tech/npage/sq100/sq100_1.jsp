...
onload=function(){
  ...
  */
  ...
  //实际使用人信息
  	...
  	var checkIdType33 = $("select[name='realUserIdType']").find("option:selected").val();
  	/*身份证加入校验方法*/
  	if(checkIdType33.indexOf("身份证") != -1){
  		document.frm1100.realUserIccId.v_type = "idcard";
  	}else{
  		document.frm1100.realUserIccId.v_type = "string";
  	}
...
{   
	...
	if("<%=opCode%>"=="1993"){
		...
		if(!checkAddrFunc(document.all.realUserAddr,6,1)){
			everyuserFlag=true;
		}
		...
		var gestoresName = $("#gestoresName").val().trim();
		...
		else if($("#gestoresIccId").val().trim()==$("#realUserIccId").val().trim()){
			rdShowMessageDialog("经办人的证件号码与使用人的证件号码不可以相同!",0);
			everyuserFlag=true;
		}
		else if(document.all.gestoresIdType.value.indexOf("0")!=-1&&document.all.up_flag1.value!=2){
			rdShowMessageDialog("请上传经办人身份证头像照片!",0);
			everyuserFlag=true;
		}
		else if(document.all.responsibleType.value.indexOf("0")!=-1&&document.all.up_flag2.value!=2){
			rdShowMessageDialog("请上传责任人身份证头像照片!",0);
			everyuserFlag=true;
		}
		else if(document.all.realUserIdType.value.indexOf("0")!=-1&&document.all.up_flag3.value!=2){
			rdShowMessageDialog("请上传使用人身份证头像照片!",0);
			everyuserFlag=true;
		}
//		else if(document.all.gestoresIdType.value.indexOf("0")!=-1&&$("#submitFlag1").val()=="1"){
//			 rdShowMessageDialog("经办人人像校验没通过不允许提交!");
//			 everyuserFlag=true;
//		}
//		else if($("#responsibleType").val().indexOf("0")!=-1&&$("#submitFlag2").val()=="1"){
//			 rdShowMessageDialog("责任人人像校验没通过不允许提交!");
//			 everyuserFlag=true;
//		}
//		else if($("#realUserIdType").val()=="0"!=-1&&$("#submitFlag3").val()=="1"){
//			 rdShowMessageDialog("使用人人像校验没通过不允许提交!");
//			 everyuserFlag=true;
//		}
		else {
			everyuserFlag=false;
      ...
      if(rdShowConfirmDialog("确认要提交客户开户信息吗？")==1) {
        ...
        if($("#cust_M_workNO").attr("readyOnly")=="readyOnly"){
        	...
        	//进行拼串处理   xxxx|xxxxx的格式，最为新下标49的入参格式。注：下标49应只涉及1993页面的调整。原1100功能应该不涉及。
        		...
        		param_s1100Cfm_49 = $("#input_accept").val()+"|";
        }
        
        if("1993"=="<%=opCode%>"){
        	param_s1100Cfm_49 = param_s1100Cfm_49 +"|"+document.all.Tax_identi_num.value;
        }
        ...
        +"&groupNo="+$("#groupNo").val()+"&input_accept="+param_s1100Cfm_49+"&xsjbrxx="+xsjbrxx;
	        	if("1993"=="<%=opCode%>"){
						    	actionStr+="&realUserName="+document.all.realUserName.value
							   +"&realUserAddr="+document.all.realUserAddr.value.replace(new RegExp("#","gm"),"%23")
...
function valiRealUserIdTypes(idtypeVal){
	...
	if(idtypeVal == "0"||idtypeVal == "D"){ //身份证
...
/*2017/5/27 add 证件类型为外国公民护照 姓名中间可以为空格*/
function checkNameStr1(code){
    ...
    if(re6.test(code)||re7.test(code)||re8.test(code)) return "KR"; //韩国
    if(forHanZi1(code)) return "CH"; //汉字
    if(forKuoHao16New1(code)) return "EH";//空格符号
...
%>
                ...
                <TR> 
                  ...
                  <TD> 
                    ...
                    <font class=orange>*</font> </TD>
                  <TD class="blue" > 
                    <%if(!opCode.equals("1100")){%><div align="left">联系人邮编</div><%}%>
                  </TD>
                  <TD> 
                    <input <%if(opCode.equals("1100")){%>type="hidden"<%}%> name=contactPost class="button" v_type="zip" v_name="联系人邮编" maxlength="6"  index="22" size="20">
                  </TD>
                </TR>
                <TR <%if(opCode.equals("1100")){%>style="display:none"<%}%>>
                  <TD class="blue" > 
                ...
                <%if("1993".equals(opCode)){%>
                	<%@ include file="/npage/sq100/gestoresInfo1.jsp" %>  
                <%}else{%>
                ...
                <%if("1993".equals(opCode)){%>
                	<%@ include file="/npage/sq100/responsibleInfo1.jsp" %>
                	<%@ include file="/npage/sq100/realUserInfo1.jsp" %>
                <%}else{%>
              ...
              <TABLE id=tb0 cellSpacing="0">
                <TBODY <%if(opCode.equals("1100")){%>style="display:none"<%}%>> 
                <TR> 
                  <TD width=16% class="blue" > 
...
%>  <TR <%if(!opCode.equals("1993")){%>style="display:none"<%}%>> 
      <TD width="16%" class="blue" > 
        <div align="left">纳税人识别号</div>
      </TD>
      <TD> 
        <input type="text" id="Tax_identi_num" name="Tax_identi_num"   maxlength="30" v_type="string" v_minlength="15"  v_maxlength="30" />
      </TD>
    </TR>
...
<TABLE cellSpacing="0">
  ...
  <input type="hidden" name="pic_name1" value=""><!--标识上传文件的名称-->
  <input type="hidden" name="picbase64_name1" value=""><!--标识上传图片base64文件名-->
  <input type="hidden" name="but_flag1" value="0"> <!--按钮点击标志-->
  <input type="hidden" name="card_flag1" value="">  <!--身份证几代标志-->
  <input type="hidden" name="upbut_flag1" value="0"> <!--上传按钮点击标志-->
  <input type="hidden" name="up_flag1" value="0"><!--是否上传标志-->
  <input type="hidden" name = "submitFlag1" id="submitFlag1" value="1"/>
  
  
  
  <input type="hidden" name="pic_name2" value=""><!--标识上传文件的名称-->
  <input type="hidden" name="picbase64_name2" value=""><!--标识上传图片base64文件名-->
  <input type="hidden" name="but_flag2" value="0"> <!--按钮点击标志-->
  <input type="hidden" name="card_flag2" value="">  <!--身份证几代标志-->
  <input type="hidden" name="upbut_flag2" value="0"> <!--上传按钮点击标志-->
  <input type="hidden" name="up_flag2" value="0"><!--是否上传标志-->
  <input type="hidden" name = "submitFlag2" id="submitFlag2" value="0"/>
  
  
  <input type="hidden" name="pic_name3" value=""><!--标识上传文件的名称-->
  <input type="hidden" name="picbase64_name3" value=""><!--标识上传图片base64文件名-->
  <input type="hidden" name="but_flag3" value="0"><!--按钮点击标志-->
  <input type="hidden" name="card_flag3" value=""><!--身份证几代标志-->
  <input type="hidden" name="upbut_flag3" value="0"><!--上传按钮点击标志-->
  <input type="hidden" name="up_flag3" value="0"><!--是否上传标志-->
  <input type="hidden" name = "submitFlag3" id="submitFlag3" value="0"/>
