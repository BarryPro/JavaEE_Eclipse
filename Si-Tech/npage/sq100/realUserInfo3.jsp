 ...
 <tr id="realUserInfo2" > 
  ...
  <td> 
    ...
    <input type="button" name="scan_idCard_two22" id="scan_idCard_two22" class="b_text"   value="读卡(2代)" onClick="Idcard2('22')" />
  	<input type="button" id="sendProjectList3" name="sendProjectList3" class="b_text" value="下发工单" onclick="sendProLists3()"/>
        <input type="button" id="qryListResultBut3" name="qryListResultBut3" class="b_text" value="工单结果查询" onclick="qryListResults3()"/>
  </td>
  <td class="blue" > 
  ...
  <td> 
    ...
    <input type="hidden" name="isSendListFlag3" id="isSendListFlag3" value="N" />
    <input type="hidden" id="isQryListResultFlag3"  name="isQryListResultFlag3"  value="N"  v_minlength=0 v_maxlength=14  v_type="0_9" maxlength="14" />
    </td>
</tr>
			...
			{              
			  ...
			  var name = IdrControl1.GetName();
				...
				if(flag == "real"){ //实际使用人
					...
					doucment.all.isQryListResultFlag1.value="Y";
					doucment.all.isQryListResultFlag2.value="Y";
					doucment.all.isQryListResultFlag3.value="Y";
		  ...
		  //工单结果查询
			function qryListResults3(){
				...
				if(typeof(ret) == "undefined"){
					...
					$("#isQryListResultFlag3").val("N");//选择了工单查询结果
				}else if(ret!=null && ret!=""){
					$("#isQryListResultFlag3").val("Y");//选择了工单查询结果
					$("#realUserName").val(ret.split("~")[0]); //客户姓名
					$("#realUserIccId").val(ret.split("~")[1]); //证件号码
					if($("#realUserIccId").val() != ""){
						checkIccIdFunc16New(document.all.idIccid,0,0);
						rpc_chkX('realUserIdType','realUserIccId','A');
					}
					$("#realUserAddr").val(ret.split("~")[2]);  //证件地址
					
					$("#realUserIccId").attr("class","InputGrey");
		  		$("#realUserIccId").attr("readonly","readonly");
		  		$("#realUserName").attr("class","InputGrey");
		  		$("#realUserName").attr("readonly","readonly");
		  		$("#realUserAddr").attr("class","InputGrey");
		  		$("#realUserAddr").attr("readonly","readonly");	
				}
			}
</script>