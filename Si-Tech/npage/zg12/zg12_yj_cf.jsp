		...
		*/
		...
		if(showMesgInfoDetail01("sidmsgDom.toString()",sidType)=="SID01_true")
		{
			...
			if(isMulti==0)//单税率
			{
				if(ListGoodsName6.length>0)
				{
					...
					*/
					...
					/*3333333333333333obAISINO.SafeInvoke(strREQ);
					var strSIDMsg =obAISINO.SIDMsg; 
					var sidmsgDom = decode64(strSIDMsg);
					*/
			...
			//	alert("跳转到打印 SID14_flag is "+SID14_flag);
				if(SID14_flag=="Y")
				{
					var s="zg12_yjkj.jsp?List_tax_invoice_number="+List_tax_invoice_number+"&List_tax_invoice_code="+List_tax_invoice_code+"&List_tax_rate="+List_tax_rate+"&kpd="+ClientNo+"&ClientNo="+ClientNo+"&yj_date="+yj_date+"&phone_no="+phone_no+"&print_sum_money="+print_sum_money+"&print_accept="+print_accept+"&print_ym="+print_ym+"&s_all_spmc="+s_all_spmc+"&s_all_je="+s_all_je+"&s_all_se="+s_all_se+"&yj_end_date="+yj_end_date+"&tax_no1="+gf_tax_code+"&opCode="+"<%=opCode%>"+" and seller_name is "+"<%=seller_name%>";
					alert("s is "+s);
