...
<%
	...
	inParas2[0]="select to_char(phone_no),to_char(begin_dt),to_char(end_dt),tax_name,tax_no,tax_address,tax_phone,tax_khh,tax_contract_no,spr,to_char(s_type_flag),nvl(notes,''),to_char(xf_info) from DINVOICE_TAX_SP where login_accept=:s_accept";
	inParas2[1]="s_accept="+login_accept;
	%>
		<wtc:service name="TlsPubSelBoss" retcode="retCode_ch" retmsg="retMsg_ch" outnum="13">
				...
				function doProcess(packet)
				{
					...
					else
					{
						if(s_flag=="0")
						{
							...
							if(s_count<1)
							{
								rdShowMessageDialog("发票未发起审批操作,请先在zg23发起开具审批!",0);
								history.go(-1);
							}
							else
							{
								...
								{ 
									//alert("月结类"); 
									document.frm1508_2.action="zg12_yj.jsp?q_value="+q_value;
									document.frm1508_2.submit();
								} 
								else
								{
					...
					<tbody>
						...
						<tr>
							...
							<!--<input type="text" name="phone_no"   maxlength=11 value="20210058797">-->
								<input type="text" name="phone_no" readonly    value="<%=phone_no%>">
