...
<%
	...
	inParas2[0]="select to_char(a.contract_no),a.bank_cust,d.type_name,e.pay_name,to_char(a.owe_fee), b.user_passwd,to_char(b.phone_no) from dConMsg  a,dCustMsg  b, dConUserMsg  c,sAccountType  d,sPayCode  e where a.contract_no=c.contract_no and c.serial_no=0 and b.cust_id=:s_cust_id and substr(b.run_code,2,1) < 'a' and b.id_no = c.id_no and a.account_type=d.account_type and e.region_code=substr(a.belong_code,1,2) and e.pay_code=a.pay_code and b.phone_no like '20%' ";
...
%>  		
		<SCRIPT LANGUAGE="JavaScript">
			window.returnValue="<%=result1[0][6].trim()%>";
			window.close(); 
		</SCRIPT>
<%  		
		}else{
...
<script language="JavaScript">
window.returnValue='';
//改为传多个值 只有一个记录的用不用改?明天测试下
function selAccount()
{	
      var  phoneArrays=[];
	  var  contractArrays=[];
	  var  len=document.frm.account.length;
	  var i_count=0;
      for(i=0;i<document.frm.account.length;i++) 
	  {		    
		      if (document.frm.account[i].checked==true)
		      {
 					 var phones = document.getElementById("phone"+i).value;
					 phoneArrays.push(phones);
					 i_count++;
					// contractArrays.push(contracts);
					  
				//	 alert("test is "+window.returnValue);
					// break;
			  }
	   } 
	  // alert("i_count is "+i_count+" and phoneArrays is "+phoneArrays);
	   if(i_count>10)
	   {
		   rdShowMessageDialog("勾选记录不能超过10条!");
		   return false;
	   }
	   else
	   {
		   rdShowMessageDialog("勾选号码:"+phoneArrays);
		   window.returnValue=phoneArrays; 
 		   window.close(); 
	   }	
    ...
    row = (String[])result.get(i);
		%>
			<tr align="center"> 
			  <td class="<%=tbClass%>"> 
				<div align="center"> 
				  <input type="checkbox" name="account" value="<%=row[6].trim()%>" id="jtCheck<%=i%>" <%=checkflag%>>
				</div>
			  </td>
			  <input type="hidden" id="phone<%=i%>" value="<%=row[6]%>">
