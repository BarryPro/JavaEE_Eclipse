...
<%
	...
	<%
			...
			<%
			}else{%>//如果字段不可空，但是被隐藏了 也return true ;wangzn 2011/6/10 10:24:06
				if($("#F<%=fieldCodes[iLen]%>").attr("disSub")=='Y')
				{
					checkValues_flag = true;
				} 
			<%}%>
...
$(document).ready(function(){
	$('#F80003').change(function(){
		$('#F80004').val($('#F80003').val());
	
	});
	$("#F70002").val($("#unit_id").val());
  	$("#F70002").attr("readOnly","true");
});
</script>
...
<%
		...
		{
			//静态下拉列表
			//wangzn begin 2011/5/24 13:21:43
			System.out.println("@:***["+iSmCode+"]***");
			if("CT".equals(iSmCode)||"RH".equals(iSmCode)){
				sqlStr = "SELECT a.param_code,a.config_code,a.config_name from sbizparamconfig a where 1=:tmpVar order by a.param_code,a.config_order ";
				paraIn2[0] = sqlStr; 
				paraIn2[1]="tmpVar=1";
			}else{	
				//liujian 2013-1-28 15:33:22
				System.out.println("---liujian7983---levelFlag=" + levelFlag);
				if(!"1".equals(levelFlag)){
					sqlStr = "select a.field_code,param_code,param_name from sUserFieldParamCode a , dGrpUserMsgAdd b where b.field_code = '1010' and trim(a.user_type)= b.field_value and param_code!='99' and b.id_no=:id_no";
					paraIn2[0] = sqlStr;    
	            	paraIn2[1]="id_no="+id_no;
	            	System.out.println("---liujian7983---sqlStr=" + sqlStr + id_no);
				}else {
            ...
            System.out.println("@:***["+paraIn2[0] +"]***");
            System.out.println("@:***["+paraIn2[1] +"]***");
        	//wangzn end 2011/5/24 13:21:55
%>
...
<%
			...
			System.out.println("---liujian7983---sssss-" + retArr4.length);
...
function segmentAdd<%=jsi%>(){
   ...
   do{
	...
	//判断是否是boolean型
					...
					{
						out.print("<TD width=\"32%\" colspan="+colspan+"> <select id=\"F"+fieldCodes[ii]+"\" name=\"F"+fieldCodes[ii]+"\" class=\"F"+fieldCodes[ii]+"\" datatype="+dataTypes[ii]+"><option value=\"Y\" ");
					...
					{
						out.print("<TD width=\"32%\" colspan="+colspan+"> <select id=\"F"+fieldCodes[ii]+"\" name=\"F"+fieldCodes[ii]+"\"  class=\"F"+fieldCodes[ii]+"\" datatype="+dataTypes[ii]+" onchange=\"clearCalcFields(frm.F"+fieldCodes[ii]+");\">");
					...
					else if (dataTypes[ii].equals("60"))//域名申请日期带默认值动态获取
					{
						...
						out.print("<TD width=\"32%\" colspan="+colspan+"> <input id=\"F"+fieldCodes[ii]+"\" name=\"F"+fieldCodes[ii]+"\"  class=\"F"+fieldCodes[ii]+"\" class=\"button\"   type=\"text\" datatype="+dataTypes[ii]+" maxlength=\""+fieldLengths[ii]+"\" value=\""+fieldValues[ii]+"\">");
						if(fieldCtrlInfos[ii].equals("N"))
						{out.print("&nbsp;<font color=\"#FF0000\">*</font></TD>");}
					}
	%>';
	...
	moduleControl();//wangzn add 2011/6/9 10:07:42
}
...
int len=0;
        	...
        	while(len<fieldCount)
        	{
	        	...
	        	else
	        	{
		        	...
		        	out.println("	{");
		        	out.println("		if($(\"#F"+fieldCodes[len]+"\").attr(\"disSub\")!=\"Y\")  ");
		        	out.println("		{");
...
//wangzn add 2011/6/8 21:28:53
$(document).ready(function(){
	$("#F70039").change(function(){
	  	moduleControl();
	});
	$("#F70040").change(function(){
	  	moduleControl();
	});
	moduleControl();
	...
	//业务部门要求，只提供被叫一号通功能
	$("#F70039").val("0");
	$("#F70039").find("option:not(:selected)").remove();
	
	
}); 
// wangzn add 2011/6/10 9:54:12 融合一号通表单控件个性化控制
function moduleControl()
{
	var disable0_40 = ["F70043","F70044","F70045"];
	var disable1_40 = ["F70046","F70047","F70048"];
	
	var disable0_39 = ["F70041","F70042"];
	var disable1_39 = ["F70040","F70043","F70044","F70045","F70046","F70047","F70048"];
	var disable2_39 = ["F70040","F70043","F70044","F70045","F70046","F70047","F70048"];
	
	var ringingType = $("#F70040").val();
	var serviceType = $("#F70039").val();
	
	
	//初始化，全部显示 
	$.each(disable0_40,function(j,m){
		...
		if (resultList != null)
		{
			while(i<fieldCount)
			{
				...
				for (int colNum = 0; (colNum < 2) && (i < fieldCount)||!fieldGroupNo[i].equals("0"); colNum ++)
				{
                    ...
                    else if (dataTypes[i].equals("80"))//vp - 当/下 月套餐类型
					{
						System.out.println("zhangyan add iUnitId=["+iUnitId+"]");
						...
						inParams[1] = "limit_value="+iUnitId;
						for ( int w=0 ; w<inParams.length; w++ )
						{
							System.out.println("zhangyan add inParams["+w+"]=["+inParams[w]+"]");
						}
						%>
						<wtc:service name="TlsPubSelCrm" 
							routerKey="region" routerValue="<%=regionCode%>" 
							retcode="retCodeW" retmsg="retCodeW" outnum="1">			
							<wtc:param value="<%=inParams[0]%>"/>
							<wtc:param value="<%=inParams[1]%>"/>	
						</wtc:service>			
						<wtc:array id="resultW" scope="end"/>
						<%									
						out.print("<TD width='32%' colspan="+colspan+">");
                        out.print("<select id='F"+fieldCodes[i]+"' name='F"+fieldCodes[i]+"'>");
                        String[][] result73 = null;
                        //wangzn 091205 B
                        String dateStr1SqlStr73 = new java.text.SimpleDateFormat("yyyy-MM-dd-HH-mm").format(new java.util.Date());
						...
						try
                        {
                            ...
                            //wangzn 091205 B String sqlStr73 ="select trim(pkg_code),trim(pkg_code)||'-->'||trim(pkg_name) from svpmnpkgcode where region_code='"+regionCode+"' and stop_time>=sysdate and " + powerRight + " >= power_Right order by to_number(pkg_code)";
                             /* yuanqs add 2011/6/10 13:11:58 修改下拉框内容 */
							sqlBufSqlStr73.append( " select trim(a.offer_id),trim(a.offer_id)||'-->'||trim(a.offer_name)                                     ");
							sqlBufSqlStr73.append( " from product_offer a,region d,sregioncode e                                                             ");
							...
							sqlBufSqlStr73.append( " and ( exists (                                                                                          ");
							sqlBufSqlStr73.append( "			 select 1 from svpmnpkgcodeconfig b where a.offer_id = b.pkg_code                            ");
							sqlBufSqlStr73.append( "			 and b.region_code = e.region_code                                                           ");
							...
							sqlBufSqlStr73.append( "			 and ( to_number('"+strTime[4]+"')>=to_number(b.start_minute) or (b.start_minute is null) )  ");
							sqlBufSqlStr73.append( "			 and ( to_number('"+strTime[4]+"')<to_number(b.end_minute) or (b.end_minute is null) )       ");
							sqlBufSqlStr73.append( "			 )																							 ");	
							sqlBufSqlStr73.append( "	or not exists (                                                                                      ");
							sqlBufSqlStr73.append( "			select 1 from svpmnpkgcodeconfig f where a.offer_id = f.pkg_code                             ");
							sqlBufSqlStr73.append( "			and f.region_code = e.region_code                                                            ");
							sqlBufSqlStr73.append( "			 )																							 ");	
							sqlBufSqlStr73.append( " 	)                                                                                                    ");
							sqlBufSqlStr73.append( " order by a.offer_id                                                                                     ");
                            /*
                            sqlBufSqlStr73.append( "select trim(a.pkg_code),trim(a.pkg_code)||'-->'||trim(pkg_name) from svpmnpkgcode a,svpmnpkgcodeconfig b where a.region_code='"+regionCode+"' and stop_time>=sysdate and " + powerRight + " >= power_Right");
    	                      sqlBufSqlStr73.append(" and ( to_number('"+strTime[0]+"')>=to_number(b.start_year) or (b.start_year is null) )");
    	                      sqlBufSqlStr73.append(" and ( to_number('"+strTime[0]+"')<=to_number(b.end_year) or (b.end_year is null) )");
                            ...
                            sqlBufSqlStr73.append(" and ( to_number('"+strTime[3]+"')>=to_number(b.start_hours) or (b.start_hours is null) )");
    	                      ...
    	                      sqlBufSqlStr73.append(" and a.pkg_code = b.pkg_code(+) and a.region_code = b.region_code(+)");
    	                      sqlBufSqlStr73.append(" order by to_number(a.pkg_code)");
    	                     */
                            String sqlStr73 = sqlBufSqlStr73.toString();
                            System.out.println("sqlStr73= "+sqlStr73);
                            //wangzn 091205 E
                            %>
                    		...
                    		if("1".equals(arcFlag))
                    		{
	                            ...
	                            {   
									...
									{
										if(!"36485".equals(result73[the7ii][0]))
										{
											out.println("<option class='button' value='" + result73[the7ii][0] + "'>" + result73[the7ii][1] + "</option>");
										}
									}
	                        ...
	                        else
	                        {
								...
								{   
									if (!resultW[0][0].equals("0"))
									{ 
									    /*155替换为36415,226替换为36485*/
										if(!"36415".equals(result73[the7ii][0]))
										{
											out.println("<option class='button' value='" 
												+ result73[the7ii][0] + "'>" 
												+ result73[the7ii][1] + "</option>");
										}
									}
									else
									{
										if(!("36415".equals(result73[the7ii][0]) || "36485".equals(result73[the7ii][0])) )
										{
											out.println("<option class='button' value='" + result73[the7ii][0] + "'>" + result73[the7ii][1] + "</option>");
										}
									}
					...
					else if (dataTypes[i].equals("76"))
					{
						...
						try
						{
							String sql_76="SELECT t.param_sql FROM SUSERFIELDPARAMCODE T  WHERE T.FIELD_CODE = '"
								+fieldCodes[i]+"' AND T.PARAM_CODE = '98'  AND USER_TYPE = '"+iUserType+"' ";
							System.out.println("sql_76 = "+sql_76);
							%>
							<wtc:pubselect name="sPubSelect" outnum="1" retmsg="msg" retcode="code" routerKey="region" routerValue="<%=regionCode%>">
								<wtc:sql><%=sql_76%></wtc:sql>
							</wtc:pubselect>
							<wtc:array id="retArr76" scope="end"/>
							<%
							String sel_sql76=retArr76[0][0];
							System.out.println("sel_sql76="+sel_sql76);
							/* ningtn 关于优化集团客户SA酬金结算系统的函 */
							if("81009".equals(fieldCodes[i])){
								/* 走新的逻辑，使用绑定变量查询 */
								String[] inParamsStr = new String[2];
								String pubSAGroupId = (String)session.getAttribute("groupId");
								inParamsStr[0] = sel_sql76;
								inParamsStr[1] = "groupId="+pubSAGroupId;
%>
								<wtc:service name="TlsPubSelCrm" routerKey="region" routerValue="<%=regionCode%>"
									 retcode="code" retmsg="msg" outnum="1000">
...
<%
								...
								/* 否则，走原有逻辑 */
							%>
								<wtc:pubselect name="sPubSelect" outnum="1000" retmsg="msg"
									 retcode="code" routerKey="region" routerValue="<%=regionCode%>">
								<wtc:sql><%=sel_sql76%></wtc:sql>
								</wtc:pubselect>
							<%
							}
							%>
							<wtc:array id="retSel76"  scope="end"/>
							<%
							String[][] sel_result76 =retSel76;

							int sel_recordNum = sel_result76.length;
							System.out.println("sel_recordNum=="+sel_recordNum);
							for(int i_76=0;i_76<sel_recordNum;i_76++)
							{
								%>
								<option class='button' value = '<%=sel_result76[i_76][0]%>'><%=sel_result76[i_76][1]%></option>
								<%
							}
						}catch(Exception e){
							logger.error("Call sunView is Failed!");
					...
					/* 增加 77 的销售代理商处理 ningtn */
					else if (dataTypes[i].equals("77"))
					{
						out.print("<TD width='32%' colspan="+colspan+">");
						out.print("<select id='F"+fieldCodes[i]+"' name='F"+fieldCodes[i]+"'>");
                       	out.print("<option value=''>---请选择---</option>");
						...
						{
							/*
							String sql_77="SELECT t.param_sql FROM SUSERFIELDPARAMCODE T  WHERE T.FIELD_CODE = '"
								+fieldCodes[i]+"' AND T.PARAM_CODE = '98'  AND USER_TYPE = '"+iUserType+"' ";
								*/
							String sql_77="select config_sql from sbizparamconfig where param_code = '" +fieldCodes[i] + "' and config_code='98'";

							System.out.println("sql_77 = "+sql_77);
							%>
							<wtc:pubselect name="sPubSelect" outnum="1" retmsg="msg" retcode="code" routerKey="region" routerValue="<%=regionCode%>">
								<wtc:sql><%=sql_77%></wtc:sql>
							</wtc:pubselect>
							<wtc:array id="retArr77" scope="end"/>
							<%
							String sel_sql77=retArr77[0][0];
							System.out.println("sel_sql77="+sel_sql77);
							/* ningtn 关于优化集团客户SA酬金结算系统的函 */
							if("81009".equals(fieldCodes[i])){
								/* 走新的逻辑，使用绑定变量查询 */
								String[] inParamsStr = new String[2];
								String pubSAGroupId = (String)session.getAttribute("groupId");
								inParamsStr[0] = sel_sql77;
								inParamsStr[1] = "groupId="+pubSAGroupId;
%>
								<wtc:service name="TlsPubSelCrm" routerKey="region" routerValue="<%=regionCode%>"
									 retcode="code" retmsg="msg" outnum="1000">
...
<%
							...
							%>
								<wtc:pubselect name="sPubSelect" outnum="1000" retmsg="msg"
									 retcode="code" routerKey="region" routerValue="<%=regionCode%>">
								<wtc:sql><%=sel_sql77%></wtc:sql>
								</wtc:pubselect>
							<%
							}
							%>
							<wtc:array id="retSel77"  scope="end"/>
							<%
							String[][] sel_result77 =retSel77;

							int sel_recordNum = sel_result77.length;
							System.out.println("sel_recordNum=="+sel_recordNum);
							for(int i_77=0;i_77<sel_recordNum;i_77++)
							{
								%>
								<option class='button' value = '<%=sel_result77[i_77][0]%>'><%=sel_result77[i_77][1]%></option>
								<%
							}
						}catch(Exception e){
							logger.error("Call sunView is Failed!");
					...
					else if (dataTypes[i].equals("82"))//ap - 漫游类型
					{
						...
						if(fieldCtrlInfos[i].equals("N"))
						{out.print("&nbsp;<font class=\"orange\">*</font></TD>");}
					} else if (dataTypes[i].equals("50")){
					    out.print("<TD width='32%' colspan="+colspan+"> <input id='F"+fieldCodes[i]+"' name='F"+fieldCodes[i]+"' class='button' type='text' readonly datatype="+dataTypes[i]+" maxlength='"+fieldLengths[i]+"' value='"+fieldValues[i]+"'>");
						if(fieldCtrlInfos[i].equals("N")){
						    out.print("&nbsp;<font color=\"#FF0000\">*</font>");
						}
%>
						  <input type="button" class="b_text" onclick="call_rh_flags();" value="修改"/>
						</TD>
<%
					} else {
