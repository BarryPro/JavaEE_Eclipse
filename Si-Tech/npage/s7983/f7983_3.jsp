...
<%
    ...
    //wuxy alter 20140127 门户网站项目 替换分隔符
...
<script language="JavaScript">
    function print_xls(){
        window.open('f7983_printxls.jsp?phoneNo=<%=errPhoneList%>&returnMsg=<%=errMsgList%>&unitID=<%=iUnitId%>&loginAccept=<%=iLoginAccept%>&grpName=<%=iGrpName%>&op_Code=<%=opCode%>&orgCode=<%=orgCode%>');
    }
</script>
<BODY>
...
<TABLE cellspacing="0">
    <tr id="footer">
        <td>
            ...
            <input class="b_foot" name=back onClick="window.location='f7983_1.jsp'" style="cursor:hand" type=button value=返回>
        </td>
    </tr>
</TABLE>
