...
<%  //绘制界面表头  
     ...
     System.out.println("  zhoubyi iccid " + iccid);
...
%> 
            <wtc:service name="sGetGroupInit" routerKey="region" routerValue="<%=regionCode%>" 
            	retcode="retCode1" retmsg="retMsg1" outnum="26" >
            ...
            System.out.println("-----------------7983---------recordNum="+recordNum);
      		for(int i=0;i<recordNum;i++)
      		{
      		    ...
      		    for(int j=0;j<26;j++)
      		    {
                    ...
                    }else if(j == 1 || j == 3 || j == 9 || j == 12 || j == 13 || j == 14 || j == 15||j==16||j==17||j==18||j==19){
          		        inputStr = inputStr + "<TD style='display:none'>" + (result[i][j]).trim() + "<input type='hidden' " +
          		            " id='Rinput" + i + "0" + j + "' class='button' value='" + 
          		            (result[i][j]).trim() + "'readonly></TD>";
          		            System.out.println("------------除了---------result["+i+"]["+j+"]="+result[i][j]);
          		      }else if(j == 20 || j == 21|| j == 22|| j==23){ 
          		         if(result[i][j] != null){
          		        inputStr = inputStr + "<TD style='display:none'>" + (result[i][j]).trim() + "<input type='hidden' " +
          		            " id='Rinput" + i + "0" + j + "' class='button' value='" + 
          		            (result[i][j]).trim() + "'readonly></TD>";
          		          }else{
          		            inputStr = inputStr + "<TD style='display:none'>" + "d0" + "<input type='hidden' " +
          		            " id='Rinput" + i + "0" + j + "' class='button' value='" + 
          		            "" + "'readonly></TD>";
          		            System.out.println("------------vpmn---------result["+i+"]["+j+"]="+result[i][j]);
          		            }
          		        }else if(j==24) {
          		        	if(result[i][j] == null) {
                    			typeStr = typeStr + "<td style='display:none'><input type='hidden' class='busiflag' name='List' value='' id='uniqueStatus" + i + j + "' /></td>";
	                    	}else {
	                    		typeStr = typeStr + "<td style='display:none'><input type='hidden' class='busiflag' name='List' value='" + (result[i][j]).trim() + "' id='uniqueStatus" + i + j + "' /></td>";
	                    	}
          		        }else if (j==25){
          		            typeStr = typeStr + "<td style='display:none'><input type='hidden' class='phoneHeader' name='List' value='" + (result[i][j]).trim() + "' " + i + j + "' /></td>";
          		        }else{   
          		        if(result[i][j] != null){
