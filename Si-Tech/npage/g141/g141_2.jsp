...
<%
	...
	// xl add for phoneNo=0的时候 需要根据账号查出来
	String [] paraIns = new String[2];
	paraIns[0]="7";
	paraIns[1]="acount_id="+count;
	if(phoneNo=="0" ||phoneNo.equals("0"))
	{
		%>
			<wtc:service name="sBossDefSqlSel"  retcode="retCode" retmsg="retMsg" outnum="1" >
				<wtc:param value="<%=paraIns[0]%>"/>
				<wtc:param value="<%=paraIns[1]%>"/> 
			</wtc:service>
			<wtc:array id="result_1" scope="end"/>
		<%
			if(result_1.length>0)
			{
				phoneNo=result_1[0][0];
			}
	}
	// xl add end
	...
	inParas[2] = beginTime;
	inParas[3] = endTime ;
	inParas[4] = "1";//otherFlag; //0 号码 1 帐户
	inParas[5] = region_code;
	inParas[6] = userType;      //0 在网, 1 离网
	inParas[7] = workNo;
	inParas[8] = op_code;
System.out.println("AAAAAAAAAAAAAAAAAAAAAAAAAAAA inParas[0]==="+inParas[0]);
...
<%
	...
	String url = "/npage/contact/upCnttInfo_boss.jsp?opCode="+opCode+"&retCodeForCntt="+sConMoreQryCode+"&opName=集团缴费信息查询&workNo="+workNo+"&loginAccept="+cnttLoginAccept+"&pageActivePhone="+inParas[0]+"&retMsgForCntt="+sConMoreQryMsg+"&opBeginTime="+opBeginTime;
%>	
