...
<%
		...
		String sAuditSuggestion2 = WtcUtil.repNull(request.getParameter("sAuditSuggestion2"));
		...
		String sIsAuditPass = WtcUtil.repNull(request.getParameter("sIsAuditPass2"));
   ...
   inParas[5]  = "根据业务审批:"+opNote;
   inParas[6]  = ipAddr;
   inParas[7] = sAuditAccept;
   inParas[8] = sIsAuditPass;
   inParas[9] = sAuditSuggestion2;
		
	 /**打印服务参数,测试后请可删除这个**/	
	 for(int i=0;i<inParas.length;i++){
	 		System.out.println("##################f9614_fieldCfm.jsp->s9614Cfm1->inParas["+i+"]->"+inParas[i]);
	 }	
%>
	<wtc:service name="s9614Cfm2" routerKey="region" routerValue="<%=regionCode%>" retcode="retCode1" retmsg="retMsg1" outnum="1" >
