...
function addLoginNoRelation(){
	...
	if(kf_login_no.trim() == ""){
			...
			rdShowMessageDialog("请输入平台工号");
			document.all.kf_login_no.focus();
			return false;
	}
	...
	core.ajax.sendPacket(packet,doProcessAddRelation);
	packet=null;
}
...
function doProcessAddRelation(packet) {
	...
	if (retType == 'addRelation' && retCode == "000000") {
		rdShowMessageDialog("平台工号绑定成功！");		
	  window.opener.document.sitechform.search.click();	
		window.close();
	} else {
		rdShowMessageDialog("平台工号绑定失败！");
		window.close();
	}
}
