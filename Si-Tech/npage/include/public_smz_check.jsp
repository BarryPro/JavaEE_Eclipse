...
<%
    ...
    String dangqianDate = new java.text.SimpleDateFormat("yyyyMMdd").format(new java.util.Date());
%>
...
function checkCustNameFunc16New(obj,objType,ifConnect){
		...
		*/
		...
		if(idTypeText == "6"){
         ...
         for (var i = 0; i < objValue.length; i ++){
            ...
            var key = checkNameStr16New3(code); //校验	
            //alert(key);
            if(key == undefined){
              rdShowMessageDialog("只允许是英文、汉字、俄文、法文、日文、韩文或其与‘括号’组合其中一种语言！请重新输入！");
              obj.value = "";
	...
	if(objValue != ""){
		...
		*/
		...
		/*1、身份证及户口薄 军人身份证时，证件号码长度为18位*/		
		
		if(idTypeText == "0" || idTypeText == "2"|| idTypeText == "D"){
			if(objType == 0){
			...
			//alert((cm_month/1)<(120*12));
				//if(cm_month>(11*12)&&cm_month<(120*12)){
				if(!bt_11_120_year(cust_b_date,"<%=dangqianDate%>")){
					...
					if(objType == 1||objType == 2||objType == 3){
						rdShowMessageDialog("责任人、经办人、使用人如果是身份证和户口薄，年龄在11-120岁(精确到月)之外不允许办理!");
						nextFlag = false;
						return false;
					}
					if(objType==0){
...
/**
	* 比较日期与当前日期是否在11岁到120岁之间，判断年即可
	* be_date 入参1 身份证日期 YYYYMMdd
	* cu_date 入参2 当前日期   YYYYMMdd
	* 如果在11岁-120之间返回 true 否则返回 false
	*/
function bt_11_120_year(be_date,cu_date){
	var retB = true;
	try{
			
			var year_cudae = cu_date.substring(0,4);
			var moda_cudae = cu_date.substring(4,8);
			
			//11岁的年份
			var year_11  = (Number(year_cudae)-11);
			//120岁的年份
			var year_120 = (Number(year_cudae)-120);	
			
			//判断是否在11岁-120岁的年月日 范围内
			
			/*
			alert(
				"Number(year_120+moda_cudae)=["+Number(year_120+moda_cudae)+"]"+"\n"+
				"Number(be_date)=["+Number(be_date)+"]"+"\n"+
				"Number(year_11+moda_cudae)=["+Number(year_11+moda_cudae)+"]"+"\n"
			);
			*/
			
			if(Number(be_date)>Number(year_120+moda_cudae) && Number(be_date)<Number(year_11+moda_cudae)){
				retB = false;
			}
...
function checkNameStr16New3(code){
	/* gaopeng 2014/01/17 9:50:35 优先匹配括号 因为括号可能是中文也可能是英文，优先返回KH 保证逻辑不失误*/
		if(forEnKuoHao16New(code)) return "KH";//括号
    if(forA2sssz116New(code)) return "EH"; //英语
    var re2 =new RegExp("[\u0400-\u052f]");
