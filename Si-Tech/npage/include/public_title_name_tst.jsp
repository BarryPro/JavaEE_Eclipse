...
<%
	...
	if(!"2".equals((String)session.getAttribute("accountType")) 
		...
		/*
			间隔时间大于10秒了，调用服务查询数据库中sessionId与自身的比较
			如果sessionId不同，就将自身的Session信息都清空。
			如果相同，就更新session中服务调用的时间（也就是上一次检验时间）
		*/
		/*服务返回的sessionId*/
		String serviceReturnSessionId = "";
%>
		<wtc:service name="sLoginSession" outnum="1" 
			 routerKey="region" retcode="pubTimeretCode" retmsg="pubTimeretMsg" 
			  routerValue="<%=region_code_public%>">
		  	<wtc:param value=""/>
		  	<wtc:param value="01"/>
		  	<wtc:param value=""/>
		  	<wtc:param value="<%=external_workNo%>"/>
		  	<wtc:param value=""/>
		  	<wtc:param value=""/>
		  	<wtc:param value=""/>
		</wtc:service>
		<wtc:array id="pubTimeResult_public" scope="end"/>
<%
		if("000000".equals(pubTimeretCode)){
			if(pubTimeResult_public != null && pubTimeResult_public.length > 0){
				serviceReturnSessionId = pubTimeResult_public[0][0];
			}
		}
		System.out.println("ningtn service return sessionId " + serviceReturnSessionId);
		System.out.println("ningtn          publicSessionId " + publicSessionId);
		
		if(publicSessionId.equals(serviceReturnSessionId)){
			/*相同，更新session中服务调用的时间，更新成现在时间*/
			System.out.println("======== ningtn, is OK");
			session.setAttribute(external_workNo+"pubTime",publicNowTime);
		}else{
			/*不相同，清空session*/
			System.out.println("======== ningtn, is invalidate");
%>
			<script language="JavaScript">
			{
				rdShowMessageDialog("对不起，发现您的工号有其他人登录，您的工号存在风险请联系管理员！");
				//工号有效操作时间过期后退出业务界面到登陆初始化界面
				window.open('/npage/login/index.html','','width='+screen.availWidth+',height='+screen.availHeight+',left=0,top=0,resizable=yes,scrollbars=yes,status=yes,location=no,menubar=no');
				window.opener=null;
			  this.close();
