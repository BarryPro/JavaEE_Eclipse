...
<%
	...
	String realOpCode = WtcUtil.repNull(request.getParameter("realOpCode"));
 ...
 System.out.println("------------------complexpro 1219 --------------------------"+new Date());
%>
...
System.out.println("---------------------complexpro 1219 end-----------------------"+new Date());
	StringBuffer logBuffer = new StringBuffer(80);
	WtcUtil.recursivePrint(retVal,1,"2",logBuffer);
	System.out.println("wanghfa ==== logBuffer          " + logBuffer);
...
{ 
	...
	{                   
						...
						System.out.println("ningtn --- comp " + prodcutUtype.getSize());

System.out.println("ningtn ---- comp " + prodcutUtype.getValue(9) + " , " + prodcutUtype.getValue(10) + " , " );
						...
						System.out.println("产品实例ID====="+prodcutUtype.getValue(3));
						...
						sb.append("product"+prodcutUtype.getValue(1)+".setGroupTitle('"+prodcutUtype.getValue(9)+"');");
						sb.append("product"+prodcutUtype.getValue(1)+".setMinNum('"+prodcutUtype.getValue(10)+"');");
						sb.append("product"+prodcutUtype.getValue(1)+".setMaxNum('"+prodcutUtype.getValue(11)+"');");
...
<script>
	...
	{ 
		...
		this.setGroupTitle=function(groupTitle)
		{
...
function showInfo2(node)
{
  	...
  	{
  		if("<%=realOpCode%>" == "e904"){
  			if(node.item[i].getId() == "1027"
  				|| node.item[i].getId() == "1026" 
  				|| node.item[i].getId() == "1025" 
  				|| node.item[i].getId() == "2042" ){
...
function showProd2(node){
		...
		$("#complex_pro_"+offerId).append("<span id='p_"+node.getId()+"'><input type='checkbox' name='complex_produce"+node.getId()+"' id='"+node.getId()+"' class='cls_"+node.getParentId()+"'  onClick=chgDetailProd2('"+node.getId()+"') groupTitle='" + node.getGroupTitle() + "' minNum='" + node.getMinNum() + "' maxNum='" + node.getMaxNum() + "' "+styleStr+"><a style='cursor:hand' onclick='cheThis(this)'>"+node.getName()+"</a></span>"+spaceStr);
		trNum=trNum+1;
		if(trNum%3==0)
		{
...
function showDetailProd2(node,showStatu,flag,startFlag,effTime,expTime)
{
		...
		packet.data.add("com_pro_intId","<%=com_pro_intId%>");
		core.ajax.sendPacketHtml(packet,doShowDetailProd2);
		packet =null;
		
		var startDateStr = "startDate_" + node.getId();
		$("input[name='" + startDateStr + "']")[0].focus();
		$("input[name='" + startDateStr + "']")[0].blur();
}
...
function chgDetailProd2(id)
{
	  var showStatu;
		...
		var node=nodesHash[id];
	if(!clickListener($("#"+ node.getId() +""),'groupTitle',true)){
		$("#"+ node.getId() +"").attr("checked","");
		return false;
	}
		if($("#"+node.getId()).attr("checked")==true && initStatu=="已订购")
		{
			$("input[name='startDate_" + id + "']")[0].focus();
			$("input[name='startDate_" + id + "']")[0].blur();
			showStatu="正常";
			$("#proName_"+node.getId()).text("正常");
		...
		{ 
			if(checkRoma(id)){
				//漫游需求 判断是否订购多个预约生效的漫游
				//返回值是true：订购了多个预约生效的漫游，不允许。
				$("#" + id + "").attr("checked","");
				return false;
			}
			showStatu="新购";
			...
			if(id=="1027"){
	    		if(document.all.custPPv1.value!="全球通"){
	    			$("#startDate_"+node.getId()).addClass("InputGrey");
	    			$("#startDate_"+node.getId()).attr("readOnly",true);
	    		}
	    		$("#stopDate_"+node.getId()).addClass("InputGrey");
				$("#stopDate_"+node.getId()).attr("readOnly",true);
			}
...
function change1027Date(id){
	$("#stopDate_"+id).val($("#select_"+id).val());
	$("#stopDateSave_"+id).val($("#select_"+id).val()+"000000");
}
</script>
