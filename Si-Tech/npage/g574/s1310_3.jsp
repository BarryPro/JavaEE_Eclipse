...
<%
	...
	String phoneno=request.getParameter("tt_phone"); //1064815004678 物联网号码
...
if("<%=s_invoice_flag%>"=="0")
   ...
   var prtFlag=0;
		...
		prtFlag=rdShowConfirmDialog("物联网缴费成功!当前发票号码是"+"<%=check_seq%>"+",是否打印发票?");
		if (prtFlag==1){
 ...
 function doyz(packet)
 {
	...
	else
	{
		...
		{
			var prtFlag=0;
			prtFlag=rdShowConfirmDialog("物联网缴费冲正成功!当前发票号码是"+ocpy_begin_no+",发票代码是"+bill_code+",是否打印发票?");
			if (prtFlag==1)
			{
...
function ifprint()
{
	...
	else if(returnValue=="3")
	{
		...
		var s_hjse=0;
		var s_xmmc="物联网缴费冲正";//项目名称 crm可能为多条? 看下zg17多组怎么传的
		var s_ggxh="";
	...
	else
	{
		...
		document.frm_print_invoice.action="../s1300/PrintInvoice_qx.jsp?opCode="+opCode+"&paySeq="+paySeq+"&phoneno="+phoneno+"&kphjje="+kphjje+"&s_hjbhsje="+s_hjbhsje+"&hjse="+s_hjse+"&returnPage="+return_page+"&hsbz=1&xmdj="+kphjje+"&contractno="+contractno+"&id_no="+id_no+"&sm_code="+sm_code+"&chbz=0&s_xmmc="+s_xmmc+"&paynote=g574缴费冲正";
