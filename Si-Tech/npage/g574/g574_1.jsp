...
<%
    String opCode ="g574";// request.getParameter("opCode");
    String opName = "物联网缴费冲正";//request.getParameter("opName");
...
function docheck()
{
	if(document.form.phoneNo.value=="" && document.form.phone_new.value=="")
	{   
		rdShowMessageDialog("请输入正确的物联网号码！");
		document.form.phone_new.focus();
		return false;
	}
	...
	if(form.billdate.value.substring(0,4)<"1990" || form.billdate.value.substring(0,4)>"2050")
   {
	...
	var prop="dialogHeight:"+h+"px; dialogWidth:"+w+"px; dialogLeft:"+l+"px; dialogTop:"+t+"px;toolbar:no; menubar:no; scrollbars:yes; resizable:no;location:no;status:no";
   	var  returnValue ;  
    //xl add new 如果增加铁通固话的话 需要新增判断 phone_new
	if (form.phone_new.value!="")    
    {
		//xl add 10648开头的转义 其他的不转义 begin
		var hd = document.all.phone_new.value.substring(0,5);
		var hd1 = document.all.phone_new.value.substring(0,3);
	//	alert("hd is "+hd);
		returnValue=window.showModalDialog('getCount.jsp?phoneNo='+document.form.phone_new.value,"",prop);
        if(returnValue=="3")
        {
        	rdShowMessageDialog("没有找到对应的物联网号码！",0);
        	document.form.phone_new.focus();
        	return false;
        }
		if(returnValue==null)
...
<div class="title">
    ...
    <table cellspacing="0">
        ...
        <tr> 
            ...
            <td colspan=2> 
                <input type="text" name="phone_new" maxlength=13  >
            </td>
        ...
        <tr> 
			...
			<td class="blue">物联网帐号</td>
            <td colspan=2>
 
                <input type="text" name="phoneNo" maxlength="25" onKeyDown="if(event.keyCode==13)form.billdate.focus()"  readonly  >
            </td>
        </tr>
