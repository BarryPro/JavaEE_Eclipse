...
/********************
 version v2.0
 开发商 si-tech
 create huangrong 2010-10-29 17:31
********************/
%>
...
<%
  ...
  String feeIndexNameSql = "   select trim(offer_name),trim(offer_comments) from product_offer where offer_id = '"+feeIndex_code+"'";
  String region_name = "";
	...
	region_code = "0"+region_code;
	 ...
	 <%
		if(code.equals("000000")&&result_feeIndex_name.length>0) {   	
	   	 	...
	   	 	offerComments = result_feeIndex_name[0][1];
    ...
    if(end_hours!=""){
	  ...
	  if(start_year!=""&&end_year!=""&&(parseInt(start_year)>parseInt(end_year))){
	 	...
	 	if(start_minute==""&&end_minute!=""){ //diling add
	 		...
	 		var toURL = "fb809_add_submit.jsp?start_year="+start_year+"&end_year="+end_year+"&start_month="+start_month+"&end_month="+end_month+"&start_day="+start_day+"&end_day="+end_day+"&start_hours="+start_hours+"&end_hours="+end_hours+"&region_code="+region_code+"&feeIndex_code="+feeIndex_code+"&start_minute="+start_minute+"&end_minute="+end_minute; //diling add 增加开始分，结束分参数
		...
		<div id="title_zi">添加智能网VPMN主套餐详细配置</div>
				...
				<table cellspacing="0" >
					<tr  height="22">				
	  					...
	  					<TD width="34%" ><%=feeIndex_name%>&nbsp;</TD>
	  		 		</tr>
	  		 		<tr  height="22">
	 					<TD width="15%" class="blue">&nbsp;&nbsp;&nbsp;资费描述</TD>
	 					<TD width="85%" class="blue" colspan='3'>&nbsp;&nbsp;&nbsp;<%=offerComments%></td>
	 				</tr>		
