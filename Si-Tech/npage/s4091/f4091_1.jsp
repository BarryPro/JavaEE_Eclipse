...
<%
    ...
    if("link".equals(openLabel)){
        try{
        %>
            <wtc:service name="s4091QryBatch" routerKey="region" routerValue="<%=regionCode%>" retcode="s4091QryBatchCode" retmsg="s4091QryBatchMsg" outnum="7" >
...
<script type=text/javascript>
    ...
    function doSub(obj){
        ...
        url = "s3690/f3690_1.jsp?in_ChanceId=<%=in_ChanceId%>&wa_no1=<%=wa_no1%>"+url1;
    ...
    <%
        if("link".equals(openLabel)){
            for(int i=0;i<orderList.length;i++){
                ...
                out.print("<td class="+tdClass+"><input type='button' class='b_text' value='接收' name='receive"+i+"' id='receive"+i+"' chanceName='"+orderList[i][0]+"' orderNo='"+orderList[i][1]+"' grpName='"+orderList[i][2]+"' smName='"+orderList[i][3]+"' offerName='"+orderList[i][4]+"' oldIdNo='"+orderList[i][5]+"' in_URL='"+orderList[i][6]+"' onclick='doSub(this)' /></td>");
                out.print("</tr>");
            }
        }
    %>
