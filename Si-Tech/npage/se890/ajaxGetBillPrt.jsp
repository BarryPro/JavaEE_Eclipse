...
String prepayFee = StrToPrice(retVal.getValue("2.4.2"));
		...
		String note = custid + " "+ mobile + "普通开户" + "现金款:" + WtcUtil.formatNumber(cash,2) + "支票款:" + WtcUtil.formatNumber(cheque,2) + "POS机款:" + WtcUtil.formatNumber(posPay,2);
...
//工号|流水|业务名称|年|月|日|用户名称|卡号|移动台号|协议号码|支票号码|合计金额(大写)|金额(小写)|业务项目|操作员|收款员|IMEINo|是否参与赠送礼品活动|支付方式|POS缴费条目
		retInfo = thework_no + "|";            //工号
		retInfo = retInfo + openrandom + "|";            //流水
		retInfo = retInfo + "全球通普通开户冲正费用明细" + "|";         //全球通开户交费明细
		retInfo += new java.text.SimpleDateFormat("yyyy",Locale.getDefault()).format(new java.util.Date())+"|";
		retInfo += new java.text.SimpleDateFormat("MM",Locale.getDefault()).format(new java.util.Date())+"|";
		retInfo += new java.text.SimpleDateFormat("dd",Locale.getDefault()).format(new java.util.Date())+"|";
		
		System.out.println("====wanghfa====ajaxGetBillPrt.jsp==== custName = " + custName);
		retInfo += custName + "|";          	   //用户名
		retInfo += " |";
		retInfo += mobile + "|";               //手机号
		retInfo += userid + "|"; 		       //协议号码
		retInfo += " |";
		...
		retInfo = retInfo + currentPay  + "|";     									 //小写		
		retInfo = retInfo + currentPay  + "|";     									 //小写		
		retInfo = retInfo + "入网费：  " + WtcUtil.formatNumber(innetFee,2) + "~";        	//入网费
		retInfo = retInfo + "手续费：    " + WtcUtil.formatNumber(handFee,2) + "~";        	//手续费
		retInfo = retInfo + "选号费：  " + WtcUtil.formatNumber(choiceFee,2) + "~";         	//选号费
		retInfo = retInfo + "机器费：    " + WtcUtil.formatNumber(machineFee,2) + "~";       	//机器费
		retInfo = retInfo + "SIM卡费： " + WtcUtil.formatNumber(simFee,2)+ "~";          		//SIM卡费
		retInfo = retInfo + "预存费：    " + WtcUtil.formatNumber(prepayFee,2) + "~";       	//预存费
		retInfo = retInfo + "现金款：  " + WtcUtil.formatNumber(cash,2) + "~";         		//现金款
		retInfo = retInfo + "支票款：    " + WtcUtil.formatNumber(cheque,2) + "~";       		//支票款
		retInfo = retInfo + "POS机款： " + WtcUtil.formatNumber(posPay,2) + "~";		/**冲正金额 tianyang add for pos**/
		retInfo = retInfo + "~";
		retInfo = retInfo + "备注：    " + note + "|";										//备注
		retInfo = retInfo + " |";
		retInfo = retInfo + " |";
		retInfo = retInfo + " |";
		retInfo = retInfo + " |";
		retInfo = retInfo + " |";
		System.out.println("====wanghfa====ajaxGetBillPrt.jsp==== retInfo = " + retInfo);
}
%>
