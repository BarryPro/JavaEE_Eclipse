...
<%
	...
	String old_region_code = request.getParameter("old_region_code");
	String old_level = request.getParameter("old_level");
	String old_begin_ymdh = request.getParameter("old_begin_ymdh");
	String old_end_ymdh = request.getParameter("old_end_ymdh");
	...
	String begin_ymdh = begin_year+begin_month+begin_day+begin_hour;
	String end_ymdh = end_year+end_month+end_day+end_hour;
	...
	inParas[6]  = op_note;
	inParas[7]  = old_region_code;
	inParas[8]  = old_level;
	inParas[9]  = old_begin_ymdh;	
	inParas[10]  = old_end_ymdh;
%>
	<wtc:service name="bs_g6363Cfm" routerKey="region" routerValue="<%=regionCode%>" outnum="2" retcode="retCode" retmsg="retMsg">
...
%>
		<SCRIPT LANGUAGE="JavaScript">
			rdShowMessageDialog("节假日修改失败!<br>错误代码：'<%=return_code%>'，错误信息：'<%=ret_msg%>'。");
			window.location.href="<%=return_page%>";
		</SCRIPT>
<%}
%>
