...
%>
	 <script language="JavaScript" >
	  ...
	  rdShowMessageDialog("此用户已在促销品统一付奖中进行<%=awardName%>领奖，请进行促销品统一付奖冲正，并确保奖品完好");
		location='/npage/bill/f8044_login.jsp?opCode=8044&opName=欢乐新农村神州行手机营销';
		</script>
<%
		}
  }
...
%>
	<wtc:service name="sG069Init" routerKey="phone" routerValue="<%=iPhoneNo%>" retcode="retCode1" retmsg="retMsg1" outnum="46">
...
<body>
	<form name="frm" method="post" action="/npage/bill/f1270_3.jsp?activePhone=<%=iPhoneNo%>" onKeyUp="chgFocus(frm)">
	...
	<tr>
	    ...
	    <input type="hidden" name="iOpCode" value="g069">
		...
		<input type="hidden" name="do_note" value="<%=iPhoneNo%>"+"TD固话自备机入网冲正">
		...
		<input type="hidden" name="beforeOpCode" value="g068">
		<input type="hidden" name="backaccept" value="<%=iLoginNoAccept%>">

		<input type="hidden" name="return_page" value="/npage/sg068/fG068Main.jsp?opName="TD固话自备机入网"&opCode=g068&activePhone=<%=iPhoneNo%>">	
