...
<wtc:service name="s9104Qry" outnum="7" routerKey="region" routerValue="<%=regionCode%>">
	<wtc:param value="<%=productId%>"/>
    <wtc:param value="1"/>
</wtc:service>
	...
	onload = function() {
	 ...
	 //	alert(tMemberProperty);
		...
		for(var i=0;i<document.getElementsByName("characterId").length;i++){
			...
			if (characterIdObj.value == "1015") {
				characterValueObj.value = "+86<%=memberNo%>_s@ims.hl.chinamobile.com";
				characterValueObj.readOnly = true;
			} else if (characterIdObj.value == "1017") {
		  ...
		  for(var i=0;i<document.getElementsByName("characterId").length;i++){
    		...
    		if(characterCheckObj.checked==true){
				...
				//2011/11/10 wanghfa添加 end
					...
					retValue += characterIdObj.value+"^";
					retValue += characterNameObj.value+"^";
					retValue += characterValueObj.value+"~";
					retFlag = true;
    	  }else{
					characterValueObj.v_must = "";
    	  }
...
<%@ include file="/npage/include/header_pop.jsp" %>  
	<form name="form_sublist" method="POST">
     ...
     <table cellspacing="0" border="0" cellpadding="0" id="memberTab" >
		 ...
		 %>
		  <tr>
		  <td><input type="hidden" name="characterId"  value="<%=result[i][0]%>" ><input type="checkbox" name="characterCheck" value="<%=result[i][2]%>" <%if("1".equals(result[i][2])){%>checked<%}%>><%=result[i][0]%></td>
		  <td><input type="hidden" name="characterName"  value="<%=result[i][1]%>" ><%=result[i][1]%></td>
		  <td>
			  <%
		  	if (result[i][4].equals("0")) {
		  		%>
				  <input type="text" name="characterValue" size="40">
		  		<%
				  ...
				  <select name="characterValue">
						...
						for (int j = 0; j < options.length; j ++) {
							%>
					  	<option value="<%=options[j][0]%>"><%=options[j][1]%></option>
							<%
						}
						%>
				  </select>
