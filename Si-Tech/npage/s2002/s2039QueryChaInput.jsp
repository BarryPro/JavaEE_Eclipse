		...
		var retType = packet.data.findValueByName("retType");
	   	...
	   	if(retType!="3")
	   	{
	   			...
	   			else
	   			{
	   				...
	   				if(document.getElementById('status'+v_id.trim()+'_'+code_th.trim()).value=="是")
	   				{
	   				  document.getElementById('status'+v_id.trim()+'_'+code_th.trim()).value="否";
	   		  	}
	   				 else
	   				{
	   				 	 document.getElementById('status'+v_id.trim()+'_'+code_th.trim()).value="是";
	   				}
	   			}
	   }
	   else
...
<%
	String sqlStr = " select a.pospec_number,a.productspec_number,c.productspec_name, "
	              + " a.productcharacter_num,a.productcharacter_name,b.field_code,b.character_name,decode(substr(a.character_attr_code,1,1),'0','否','1','是') ,decode(substr(a.character_attr_code,2,1),'0','否','1','是'),decode(substr(a.character_attr_code,3,1),'0','否','1','是'),decode(substr(a.character_attr_code,4,1),'0','否','1','是')"
...
<form name="form1" method="post" action="">	
  ...
  <table cellspacing=0>		
			...
			<tr>		
			  ...
			  <th nowrap align='center' colspan=3><b>属性设置</b></th>
			</tr>
...
%>		
		<tr bgcolor="#F5F5F5">			
				...
				<td align="center">
					...
					<input name="operator<%=i+1%>" style="cursor:hand"  type="button" value="修改" class="b_text" onclick="queryMod('<%=rateCodeStr[i][0].trim()%>','<%=rateCodeStr[i][1].trim()%>','<%=rateCodeStr[i][3].trim()%>','<%=i+1%>','1')"  />
				</td>	
			...
			<tr>
				...
				<td align="center">
					...
					<input name="operator<%=i+1%>" style="cursor:hand"  type="button" value="修改" class="b_text" onclick="queryMod('<%=rateCodeStr[i][0].trim()%>','<%=rateCodeStr[i][1].trim()%>','<%=rateCodeStr[i][3].trim()%>','<%=i+1%>','2')"  />
				</td>		
			</tr>
			<tr>
				<td nowrap>唯一</td>
				...
				<td align="center">
					...
					<input name="operator<%=i+1%>" style="cursor:hand"  type="button" value="修改" class="b_text" onclick="queryMod('<%=rateCodeStr[i][0].trim()%>','<%=rateCodeStr[i][1].trim()%>','<%=rateCodeStr[i][3].trim()%>','<%=i+1%>','3')"  />
				</td>		
			</tr>
			<tr>
				<td nowrap>必填必空</td>
				...
				<td align="center">
					...
					<input name="operator<%=i+1%>" style="cursor:hand"  type="button" value="修改" class="b_text" onclick="queryMod('<%=rateCodeStr[i][0].trim()%>','<%=rateCodeStr[i][1].trim()%>','<%=rateCodeStr[i][3].trim()%>','<%=i+1%>','4')"  />
				</td>		
			</tr>
<%
    }
