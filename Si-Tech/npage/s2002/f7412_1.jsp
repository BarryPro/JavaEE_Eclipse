...
<script  type="text/javascript">
    ...
    function checkCust(){
        ...
        core.ajax.sendPacket(packet,doCheckCust,true);
        packet = null;
    }
    ...
    function doCheckCust(packet){
        ...
        if(errCode == "000000"){
            if(cnt > 0){
                rdShowMessageDialog("该客户已经订购过动力100业务或者有待处理的动力100订单，不能重复订购！",0);
        ...
        if (retArr !=null) {
            for(var i=1;i<retArr.length;i++){
                if(retArr[i]!=null){
                    ...
                    vBbossFlagList = vBbossFlagList + retArr[i][2] + '~';
                    if(retArr[i][2] == 'N'){
    ...
    function doSetAttribute(packet){
        ...
        if(errCode != "000000"){
            ...
            var strTemp1 = "<tr>"
                        ...
                        + "</td>"
                        + "<td>"
                        + "<input type='button' id='taskBtn0' name='taskBtn0' class='b_text' value='接收' onClick='toReceive(\"N\",\"taskBtn0\",\""+$("#product_type").val()+"\",\"\",\""+vLoginAcceptArr[0]+"\",\""+vChildAcceptArr[0]+"\",\"&sm_code="+$("#product_type").val()+"&in_productCode="+$("#motive_code").val()+"&in_productName="+$("#motive_name").val()+"&in_bizCode=&in_bizName=&in_loginAccept="+vLoginAcceptArr[0]+"&in_childAccept="+vChildAcceptArr[0]+"&in_count="+vSmCodeArr.length+"&catalog_item_id="+$("#catalog_item_id").val()+"\")'/>"
                        + "</td>"
                        + "</tr>" ;
    ...
    function doToReceive(packet){
        ...
        if(retCode == "000000"){
            if(vBbossFlag == 'N'){
                if(vCustCnt == "0"){ /* 集团下没有该品牌的客户信息 */
                    var taskUrl = "../s3690/f3690_1.jsp?openFlag=DL100&inIccid="+$("#iccid").val()+"&inCustId="+$("#cust_id").val()+"&inUnitId="+$("#unit_id").val()+"&inCustName="+$("#cust_name").val()+"&inBelongCode="+$("#belong_code").val()+"&btn_id="+vBtnId+vUrl;
                    ...
                    selectCust(vSmCode,vBizCode,vBtnId,vLoginAccept,vChildAccept);
                }
            }else{
    ...
    /* 选择客户 */
    function selectCust(pSmCode,pBizCode,pBtnId,pLoginAccept,pChildAccept){
        var iCustId = $("#cust_id").val();
        
        var targetUrl = "custSelect.jsp?custId="+iCustId+"&smCode="+pSmCode+"&bizCode="+pBizCode+"&btnId="+pBtnId+"&loginAccept="+pLoginAccept+"&childAccept="+pChildAccept;
    ...
    function query_spbiz()
    {
        ...
        */
        ...
        fieldName = "产品代码|产品名称|";
        retQuence = "3|3|4|17|";
        retToField = "motive_code|motive_name|catalog_item_id|";
    ...
    <tr>
        ...
        <td>
            <select name="bizTypeL" id="bizTypeL"> 		  			
                <wtc:qoption name="sPubSelect" outnum="2">
                    <wtc:sql>select distinct main_type,main_type||'->'||main_name from sbiztypecode where sm_code='DL' and to_number(external_code)<=800</wtc:sql>
                </wtc:qoption>		  			
            </select>
            <font class="orange">*</font>
        </td>
