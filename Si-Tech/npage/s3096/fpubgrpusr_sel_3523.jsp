...
*/
    ...
    String user_no = WtcUtil.repNull(request.getParameter("user_no"));
	  ...
	  String op_code= WtcUtil.repNull(request.getParameter("op_code"));//luxc20070122 add
...
function saveTo()
{
      ...
      {     return false;   }
       //返回单条记录
        ...
        var cardNum = pobj.find('input[id^="cardNum"][id$="17"]').val();
        var cardStatus = pobj.find('input[id^="cardNum"][id$="18"]').val();
        var obj = {};
        obj.opCode = '3523';//status 设置成opCode
        obj.cardNum = cardNum;
...
<BODy>
<FORM method=post name="fPubSimpSel" id="numTable">
...
<%  //绘制界面表头
     ...
     out.print("");
    // fieldNum = String.valueOf(tempNum+1); //liujian 注释
     fieldNum = String.valueOf(tempNum+6);//liujian 2013-1-15 9:47:51 添加两个字段
%>
...
<%
      System.out.println(code2);
			...
			int num = 13;//liujian 2013-1-15 9:12:47 关于优化集团客户业务相关系统功能的函
			System.out.println("---liujian3096---result.length=" + result.length);
            for(int i=0;i<result.length;i++)
            {
                ...
                for(int j=0;j<num;j++)
                {
                    ...
                    else if(j==11)
                    {
                    	...
                    	//liujian 2013-1-15 9:12:47 关于优化集团客户业务相关系统功能的函 begin
                    }else if( j == 17 || j == 18){
                    	inputStr = inputStr + "<TD style='display:none'>" + result[i][j] + "<input type='hidden' " +
                        " id='cardNum" + i +"-"+ j + "'  value='" +
                        result[i][j] + "' readonly></TD>";
                        //liujian 2013-1-15 9:12:47 关于优化集团客户业务相关系统功能的函 end
                    }else
                    {
