...
<%
  ...
  String fieldNum 	= "";
  String fieldName 	= "sp业务代码|sp业务名称|业务描述|";    
  String sqlFilter 	= "";
  String retToField = request.getParameter("retToField") == null?"":request.getParameter("retToField");
	...
	String spcode = "";
	if(!busytype.equals("27")){
	 spcode = request.getParameter("spcode");
	}
	String sqlStr="";
	System.out.println("busytype================="+busytype);
	if(busytype.equals("27")){
	
		sqlStr="select bizcode,bizname,usagedesc,rownum id from oneboss.rinp_bizinfomsg";
	}else{

	 sqlStr="select b.bizcode,b.servname,b.BIZDESC,rownum id from ddsmpspinfo a,ddsmpspbizinfo b  where a.spid = b.spid  and b.spid='"+spcode+"' and b.BIZSTATUS='A' "; 
	if(oprcode.equals("99")||oprcode.equals("07")||oprcode.equals("02")){
		sqlStr="select b.bizcode,b.servname,b.BIZDESC,rownum id from ddsmpspinfo a,ddsmpspbizinfo b  where a.spid = b.spid  and b.spid='"+spcode+"' and b.bizcode in (select distinct c.bizcode from ddsmpordermsg c where c.phone_no='"+phoneNo+"' and c.serv_code='"+busytype+"' and c.spid='"+spcode+"' and c.opr_code<>'02' and c.opr_code<>'07' and c.opr_code<>'17')";
	}
	else if(oprcode.equals("04")){
		sqlStr="select b.bizcode,b.servname,b.BIZDESC,rownum id from ddsmpspinfo a,ddsmpspbizinfo b  where a.spid = b.spid  and b.spid='"+spcode+"' and b.bizcode in (select distinct c.bizcode from ddsmpordermsg c where c.phone_no='"+phoneNo+"' and c.serv_code='"+busytype+"' and c.spid='"+spcode+"' and (c.opr_code='06' or c.opr_code='05' or c.opr_code='01'))";
	}
	else if(oprcode.equals("05")){
		sqlStr="select b.bizcode,b.servname,b.BIZDESC,rownum id from ddsmpspinfo a,ddsmpspbizinfo b  where a.spid = b.spid  and b.spid='"+spcode+"' and b.bizcode in (select distinct c.bizcode from ddsmpordermsg c where c.phone_no='"+phoneNo+"' and c.serv_code='"+busytype+"' and c.spid='"+spcode+"' and (c.opr_code='04'))";
	}
 }
