...
<%
    ...
    String feeSql = "select function_code, hand_fee ,region_code,favour_code from sNewFunctionFee where region_code = '" + region_codeT + "' and function_code in ('b540','b541','b542','b543','b544')";
%>
<!--手续费的查询-->
...
function doProcess(packet) {
    ...
    if (cfmFlag == 1) {
        ...
        if (errCodeInt == 0) {
            ...
            if ((document.frm.opCode.value == "b540" || document.frm.opCode.value == "b541") 
            ...
            if(document.frm.opCode.value == "b542"&& parseFloat(document.frm.backPrepay.value) > 0){
            	rdShowMessageDialog("打印发票");
              showBroadBill("Bill","确实要进行发票打印吗？","Yes");
            }
    ...
    if (cfmFlag == 9) {
        ...
        rdShowMessageDialog(errCode + " : " + errMsg);
				if(errCode != "000000"){
					window.location='b540_1.jsp?activePhone=<%=activePhone%>';
				}
        var ii = 0 ;
    ...
    if (cfmFlag == 0) {
        ...
        /*2015/01/19 15:03:46 gaopeng b542 预销时,kh的展示可退金额*/
				/*获取品牌sm_code*/
				getPubSmCode($("#cfmLogin").val());
				var smCode = $("#pubSmCode").val();
				
				if(document.frm.opCode.value == "b542"){
					$("#noBackDiv").show();
					$("#showMyMsg").show();
        ...
        $("#bindId").val(bindVal);
				if(document.all.radioValue.value == 'b542' && bindVal != "63"){
       			//getOldResByCfmLogin();
        }
     }
}
...
function submitt() {
    ...
    /*获取宽带用户信息*/
  	  ...
  	  rdShowMessageDialog("查询宽带用户信息失败！",0);
  	  return false;
  	}
}
...
{	
	...
	/*zhangyan*/
	if(document.all.opCode.value.trim() == "b542")
	{
		/*指定Ajax调用页*/
    ...
    if(document.frm.opCode.value == "b542" && pubSmCode == "kh"){
            	rdShowMessageDialog("打印发票");
              showBroadBill("Bill","确实要进行发票打印吗？","Yes");
    }*/
    if(!printCommit()){
     ...
     if (printFlag != 1 && printFlag != 2) {
			...
			var myPacket = new AJAXPacket("b540Cfm.jsp", "正在提交，请稍候......");
				myPacket.data.add("opCode", document.frm.opCode.value);
...
/****根据转存号码查询并确定账号****/
function b540GetByPhone(formField) {
...
function clickRadio(){
	...
	if(opCode == "b540"){
		...
		document.frm.opCode.value='b540';
		...
		noBackDiv.style.display='none';
		backDiv.style.display='none';
		document.frm.opCode.value='b541';
		document.frm.handFee.value='0';
		document.frm.handFeeT.value='0';
		document.frm.factPay.value='0';
		...
		document.frm.opCode.value='b542';
		document.frm.handFee.value='0';
		document.frm.handFeeT.value='0';
		document.frm.factPay.value='0';
...
<td width="25%">
    <input type="radio" name="radiobutton" index="1" value="b540"
           onclick="clickRadio();" >暂停
</td>
<td width="25%">
    <input type="radio" name="radiobutton" index="2" value="b541"
           onclick="clickRadio();">暂停恢复
</td>
<td width="25%">
    <input type="radio" name="radiobutton" index="3" value="b542"
		...
		<table cellspacing="0">
			...
			<tr id=backDiv style="display:none">
			    ...
			    <td>
			        ...
			        <input class="b_text" type="button" name="qCustIdByPhone" value="查询" onClick="b540GetByPhone(vPhoneNo)">
			    </td>
			 ...
			 <tr  id="tr_otreson_1" style="display:none"  >
				  ...
				  <td colspan="5">
								<select id="sel_reasonDescription" style="width:300px;" name="sel_reasonDescription" onchange="set_otreson()">
								    ...
								    <option value="6">转用联通、电信、转网更换宽带帐号</option>
			...
			<tr id="showMyMsg" style="display:none">
				<td colspan="6" ><font color="red">注：所有品牌的宽带在宽带销户时，须给用户退费。</font></td>
			</tr>   
		</table>
		    ...
		    <tbody>
		        <tr>
		            <td id="footer">
		                ...
		                &nbsp;&nbsp;
		                <input class="b_foot" name=back type=button value="清除" onclick="window.location='b540_1.jsp?activePhone=<%=activePhone%>'">
		                &nbsp;&nbsp;
...
<input type=hidden name=sysRemark value="宽带综合变更">
...
function printInfo(printType)
{
    ...
    if (printType == "Detail"){
					...
					if (radio1 == "b540"){
					   ...
					   */
						if(pubSmCode == "kf" || pubSmCode == "kg" || pubSmCode == "kh" || pubSmCode == "ki"){
							...
							note_info2 += "如申请办理销户后，要求重新恢复此业务，将按照新用户申请宽带业务流程办理。" + "|";
						}
			...
			/*2014/04/09 9:39:49 gaopeng 修改免填单 移动家庭客户宽带业务运营支撑系统需求*/
			note_info1 += ""+"|";
			note_info1 += "备注："+"|";
			/* 
		   * 关于协助开发省广电合作宽带话费营销案和融合套餐需求的函（单品部分）@2014/7/24 
		   ...
		   */
			if(pubSmCode == "kf" || pubSmCode == "kg" || pubSmCode == "kh" || pubSmCode == "ki"){
				if (radio1 == "b540"){
					...
					note_info1 += "4、宽带业务到期时间按暂停期整月数量顺延，例暂停2个月10天、则到期时间顺延2个月，以此类推；"+"|";
					note_info1 += "5、一年内允许2次报暂停；"+"|";
					note_info1 += "6、与手机营销案捆绑的宽带不允许报暂停延长有效期。"+"|";
...
function printBill() {
        ...
        location.href = "chkPrint.jsp?retInfo=" + infoStr + "&dirtPage=b540_1.jsp";
}
function showBroadBill(printType,DlgMessage,submitCfm){
	...
	if(document.frm.opCode.value == "b540"){
		...
		printStr = "复机手续费：";
		opName = "宽带业务暂停恢复";
	}else if(document.frm.opCode.value == "b542" ){
		printStr = "退费金额：";
		opName = "宽带销户";
...
function printTwoBill() {
		 ...
		 infoStr += document.frm.handFee.value + "|";
		 infoStr += document.frm.sysRemark.value + "。*手续费：" + document.frm.handFee.value + "*流水号：" + document.frm.backLoginAccept.value + "|";
		 location.href = "chkTwoPrint.jsp?retInfo=" + infoStr + "&dirtPage=b540_1.jsp&activePhone=<%=activePhone%>";
}
function printTwoBillZero() {
    ...
    infoStr += document.frm.handFee.value + "|";
    infoStr += document.frm.sysRemark.value + "。*手续费：" + document.frm.handFee.value + "*流水号：" + document.frm.backLoginAccept.value + "|";
    location.href = "chkTwoPrintZero.jsp?retInfo=" + infoStr + "&dirtPage=b540_1.jsp&activePhone=<%=activePhone%>";
}
function printThreeBill() {
    ...
    location.href = "chkThreePrint.jsp?backDate=" + document.frm.backDate.value + "&retInfo=" + infoStr + "&dirtPage=b540_1.jsp&activePhone=<%=activePhone%>";
}
function printThreeBillZero() {
    ...
    //infoStr+=document.frm.backDate.value+"|";
    //rdShowMessageDialog(document.frm.backDate.value);
    location.href = "chkThreePrintZero.jsp?backDate=" + document.frm.backDate.value + "&retInfo=" + infoStr + "&dirtPage=b540_1.jsp&activePhone=<%=activePhone%>";
}
</script>
