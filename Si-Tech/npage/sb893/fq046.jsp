...
<%
     ...
     String pathFlag = request.getParameter("pathFlag");	
		 ...
		 String simPay_fee = request.getParameter("simPay_fee");		 
		 	...
		 	System.out.println("=====================simPay_fee======================================"+simPay_fee);		 
		 String phoneNo = request.getParameter("phone_no");		 //huangrong add 普通开户用户手机号码  2011-8-11 
		 				System.out.println("=====================phoneNo======================================"+phoneNo);
%>
<%
    //begin huangrong add 查询对账是否成功 2011-8-11 
		String checkCountSql = " select count(*)   from dwebopenphonemsghis  where status='3' and checkld_flag='1' and phone_no='"+phoneNo+"'";
%>
		<wtc:pubselect name="sPubSelect" outnum="1" retcode="retCode_checkCount" retmsg="retMsg_checkCount">
			<wtc:sql><%=checkCountSql%></wtc:sql>
		</wtc:pubselect>
		<wtc:array id="checkCountResult" scope="end"/>
<%
if("000000".equals(retCode_checkCount)||"0".equals(retCode_checkCount))
{
		if(checkCountResult.length > 0){
				System.out.println("==========================================================="+checkCountResult[0][0]);
			if ("1".equals(checkCountResult[0][0])) {
				prePay_Fee="0";
				simPay_fee="0";
			} else {
				
				System.out.println("对账失败缴纳预存款和sim卡费");
			}
		}else{
			 System.out.println("获得对账信息失败");	
		}
}else{
%>	
    <script language="javaScript">
    	rdShowMessageDialog("获得对账信息失败,错误代码：<retCode_checkCount>，错误信息：<%=retMsg_checkCount%>",1)
    </script>
<%    
}

    //end huangrong add 查询对账是否成功 2011-8-11 
%>		 
<%		 

		 System.out.println("====================");
		 System.out.println(gCustId);
		 System.out.println(prePay_Fee);
		 System.out.println(prePay_Fee);
        ...
        System.out.println("sb=============="+sb.toString()); //后来添加了过滤器,导致缴费失败,这个注释掉的程序可以用来检查获取参数*/ 
		 beginTime1 =  System.currentTimeMillis();
			...
			/** tianyang add for pos end **/
						/* wanghyd 根据用户优惠权限信息，设置可修改金额 */
			String[][] favInfo = (String[][])session.getAttribute("favInfo");
			String[] feeFav = new String[2];
			feeFav[0] = "";
			feeFav[1] = "";
			...
			for(int feefavi = 0; feefavi< favInfo.length; feefavi++){
				...
				if(tempStr.compareTo("a362") == 0){
				/*网站开户开户入网费优惠 可以修改开户预存、开户预存款、开户预存费*/
					feeFav[0] = favInfo[feefavi][0];
				}else if(tempStr.compareTo("a363") == 0){
					/*网站开户开户SIM卡费优惠 可以修改SIM卡费用*/
					feeFav[1] = favInfo[feefavi][0];
				}
			}
			for(int feei = 0; feei < feeFav.length; feei++){
				System.out.println("======= fav feefavsssss ===== " + feeFav[feei]);
			}
			
			/**
			System.out.println("zhouby --sb893- sq046.jsp ---opCode" + opCode);
			System.out.println("zhouby --sb893- sq046.jsp ---work_no" + work_no);
			System.out.println("zhouby --sb893- sq046.jsp ---password" + password);
			System.out.println("zhouby --sb893- sq046.jsp ---gCustId" + gCustId);
      */
      String sss = "根据cust_id[" + gCustId + "]进行查询";
%>
 <!-- 2013/11/12 16:06:23 gaopeng 修改zhouby之前的第二批客户敏感信息改造，更换服务sUserCustInfo 为  sQBasicInfo-->
	...
	if(errCodeGetCust.equals("0") || errCodeGetCust.equals("000000")){
		System.out.println("调用服务sQBasicInfo in fq046(sb893).jsp 成功@@@@@@@@@@@@@@@@@@@@@@@@@@");
		gCustName = retBd0002.getValue("2.0");
		System.out.println("调用服务sQBasicInfo in fq046(sb893).jsp custName : "+gCustName);
	}
	else{	 
		System.out.println("调用服务sQBasicInfo in fq046(sb893).jsp 失败@@@@@@@@@@@@@@@@@@@@@@@@@@");
	%>
		<script language="JavaScript">
...
<%     			   	
                 ...
                 <%String regionCode_sShowNumInfo = (String)session.getAttribute("regCode");%>
     			    ...
     			    for(int m = 0 ; m < retVal.getSize("2."+i+".1."+j+".1");m++){
     			    	...
     			    	out.println("totalFee1 = Number(totalFee1) + Number("+prePay_Fee+") + Number("+simPay_fee+");");	       	//应收费用	累加   
     			    	out.println("totalFee2 = Number(totalFee2) + Number("+feesUtype.getValue(2)+");");          				//优惠费用	累加
     			    	out.println("totalFee3 = Number(totalFee3) + Number("+prePay_Fee+") + Number("+simPay_fee+");");    
     			    		...
     			    		for (int k = 0 ; k< uuu.getSize()-8;k++){
     			    		  ...
     			    		  if(k == 0){
     			    		  	String  tmp2 = uuu.getValue(k+1);	//取费用代码
     			    		  	  /* wanghyd sim卡费用和入网预存款增加优惠权限，有优惠权限的才可以去修改这两个值 */
     			    		  		textStyle = "readonly";
     			    		  		classStyle = "forMoney required";
     			    		  		 /* 入网预存款 入网预存 入网预存费 */
     			    		  			if(WtcUtil.haveStr(feeFav,"a362")){
     												textStyle = "";
     												classStyle = "forMoney required";
     											}else{
     												textStyle = "readonly";
     											...
     											/* SIM卡费用 */
     			    		  			if(WtcUtil.haveStr(feeFav,"a363")){
     												textStyle = "";
     												classStyle = "forMoney required";
     											}else{
     												textStyle = "readonly";
     			    		  ...
     			    		  %>
     			    			...
     			    			if(k == 4){
     			    				...
     			    				//if sim 卡费 + simPay_fee else if 入网预存款 + prePay_Fee
     			    				if("SIM卡费用".equals(feeName)){
     			    			%>
     			    					feeOrder.push("<input type=text size=10  maxLength='15' name=realPayFee<%=n%>_<%=k%> class='<%=classStyle%>' vflag=1  onchange = changeRealPayFee() value='<%=simPay_fee%>'  <%=textStyle%>><input type=hidden name=realfee<%=n%>_<%=k%>  value='<%=simPay_fee%>'><input type='text' style='display:none' id='opchangeFee<%=n%>_<%=k%>' value='<%=opFee046%>' v_oldvalue='0'>");
     			    			<%
     			    				}else if("入网预存款".equals(feeName)){
     			    			%>
     			    					feeOrder.push("<input type=text size=10  maxLength='15' name=realPayFee<%=n%>_<%=k%> class='<%=classStyle%>' vflag=1  onchange = changeRealPayFee() value='<%=prePay_Fee%>'  <%=textStyle%>><input type=hidden name=realfee<%=n%>_<%=k%>  value='<%=prePay_Fee%>'><input type='text' style='display:none' id='opchangeFee<%=n%>_<%=k%>' value='<%=opFee046%>' v_oldvalue='0'>");
     			    			<%	
     			    				}else{
     			    			%>
     			    					feeOrder.push("<input type=text size=10  maxLength='15' name=realPayFee<%=n%>_<%=k%> class='forMoney required' vflag=1  onchange = changeRealPayFee() value='<%=tmp%>'><input type=hidden name=realfee<%=n%>_<%=k%>  value='<%=tmp%>'><input type='text' style='display:none' id='opchangeFee<%=n%>_<%=k%>' value='<%=opFee046%>' v_oldvalue='0'>");
     			    			<%
     			    				}
     			    				continue;
     			    			}
     			    			...
     			    			if(k == 2){
     			    				//if sim 卡费 + simPay_fee else if 入网预存款 + prePay_Fee
     			    				if("SIM卡费用".equals(feeName)){
     			    					out.println("feeOrder.push('" + simPay_fee + "');");
     			    				}else if("入网预存款".equals(feeName)){
     			    					out.println("feeOrder.push('" + prePay_Fee + "');");
     			    				}else{
     			    					out.println("feeOrder.push('" + tmp + "');");
     			    				}
     			    				continue;
     			    			}
     			    			if(k == 3){//加入优惠
       ...
       */ 	
			...
			function changeRealPayFee(){
				...
				/*if(opValueTmp < 0){	根据山西要求,去掉金额限制
					...
					opFee.value = opValueTmp;	//再新也要变老,孙子迟早要当爷爷的
 ...
 /**
	   ...
	   var val=$('input:radio[name="checkRadio"]:checked').val();
		if(val!=null){
 		...
 		$(billArgsObj).attr("10006","网站开户"); //业务类别
...
%>
			...
			//2010-7-5 8:46 wanghfa修改 铁通开户和普通开户最后提示信息不同 end
   			...
   			if(is_release == "Y"){
     			parent.L('1','b893','网站开户','sb893/fb893_index.jsp','000');
	     	 ...
	     	 var fee_sumPayss = "<%=prePay_Fee%>";
	     	 var simFeess="<%=simPay_fee%>";

				for(i=1; i < tabobj.rows.length; i++){
						var feename = tabobj.rows(i).cells(1).children[1].value;
						if(feename == "SIM卡费用"){
							simFeess  =parseInt(tabobj.rows(i).cells(4).children[0].value);
						}else if(feename.indexOf("入网预存款")!=-1){
							fee_sumPayss =parseInt(tabobj.rows(i).cells(4).children[0].value);
						}
				}
			//liujian 2012-7-20 10:38:50 在此处开户
				
			var getdataPacket = new AJAXPacket("fq046_NewOpr.jsp","正在进行开户操作，请稍候......");
			getdataPacket.data.add("phoneNo","<%=phoneNo%>");
			getdataPacket.data.add("phoneStatus","<%=phoneStatus%>");
			getdataPacket.data.add("gCustId","<%=gCustId%>");
			getdataPacket.data.add("fee_sumPay",fee_sumPayss);
			getdataPacket.data.add("simFee",simFeess);
			core.ajax.sendPacket(getdataPacket,doNewOPRS);
			getdataPacket = null;
   			...
   			return false;
   				...
   				function doNewOPRS(packet){
			var retCode = packet.data.findValueByName("retcode");
			var retMsg = packet.data.findValueByName("retmsg");
			if(retCode == "000000"){
        //rdShowMessageDialog("开户成功！",2);        
			}else{
				rdShowMessageDialog("开户失败！错误代码：" + retCode + "，错误信息：" + retMsg,0);
