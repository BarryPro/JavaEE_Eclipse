...
String tempStrf = "";
    ...
    //weigp===================
    String prtFlag = "Y"; //request.getParameter("prtFlag");//默认为合打Y     分打为N
    //weigp===================
    ...
    String flag_code1="";//小区资费代码   huangrong add 2011-8-29 
    String flagCodeString="select FLAG_CODE from dwebopenphonemsg where phone_no='"+phone_no+"'";
    
    /*gaopeng 2014/05/06 10:24:36 关于我省“网上选号”和“18元套卡省内版”增加SIM卡类型选择功能的需求 */
...
String opNote = "根据cust_id:"+gCustId+"查询用户姓名,密码等信息.";
	...
	//7个标准化入参
		...
		try{
			...
			<%
				...
				if(!"000000".equals(retCode)){//调用服务失败
					%>
					<script language="javascript" >
...
function doGetProdCompInfo(packet)
{
	...
	if(error_code == "0"){
		if(typeof(prodCompInfo) != "undefined"){
			...
			//weigp删除已定购产品信息 左端显示的class属性 class='item-row2 col-2'
			//$("#tab_addprod tr").append("<td><div id='items'><div ><div id='compProdInfoDiv'></div></div><div ><div class='title'><div id='title_zi'>已定购产品信息</div></div><div id='pro_component'></div></div></div></td>");
			/*liujian add 显示附加产品 begin*/
			...
			for(var i=0;i<prodCompInfo.length;i++){
				if(typeof(compRoleCdHash[prodCompInfo[i][3]]) == "undefined"){	//生成角色栏
					//weigp去掉展示附加销售品的复选框
					//$("#compProdInfoDiv").append("<div  name='compProdrole' id='"+prodCompInfo[i][3]+"' style='cursor:hand;display:none'><div class='title'><div id='title_zi'>附加产品</div></div></div><table cellspacing='0' style='display:none'><tr><td><div id='div_"+prodCompInfo[i][3]+"' style='font-family:\"宋体\";'></div></td></tr></table>");
					/*liujian add 显示附加产品 begin*/
          ...
          /*2014/12/15 8:47:12 gaopeng R_CMI_HLJ_xueyz_2014_1933493@关于申请制作万号段号源类型数字转TD数据的请示
          增加184号段
          */     
          else if(selNum3 == "184")
					{
...
function doGetOfferAttr(data)
{
  ...
  //huangrong add 如果小区资费有则设置为已选择的值
...
function mySub()
{
	var submitFlag = $("#submitFlag").val();
	 <%--if(submitFlag=="1"){//修改回去
		 rdShowMessageDialog("人像校验没通过不允许提交!");
		 return false;
	 }--%>
	
    getAfterPrompt();
    //huangrong add 提交前判断开户手机号是否与网上开户手机号一致   2011-8-19 
  	if(document.all.selNum.value.trim() != document.all.phone_no.value.trim())
		{
			rdShowMessageDialog("输入的开户手机号必须与网上开户的手机号一致，请重新输入！");
			return false;
		}	 
    // ningtn 漫游改造
...
function printInfo(printType)
{
    ...
    cust_info+="客户地址：	"+"<%=custAddr%>"+"|";
		...
		{
			opr_info+= opr_info+="业务受理时间："+cTime+"  "+"用户品牌:"+"动感地带"+"|";
...
function checksim(){
	/*gaopeng 2014/05/06 11:08:59 关于我省“网上选号”和“18元套卡省内版”增加SIM卡类型选择功能的需求 */
	if(("<%=outSimType%>" != "10060" && "<%=outSimType%>" != "10061") && "<%=simResBrandCode%>" == "K06"){
		rdShowMessageDialog("只允许写SIM类型为10060-LTE USIM卡(3FF)和10061-LTE USIM卡(4FF)的卡！",0);
    return false;   
	}
	<%--if("<%=is_check_readcard_result%>"=="1"){
	}else {
		if("<%=simResBrandCode%>" == "K06" && document.all.simTypeCfm.value != "<%=outSimType%>" ){
			rdShowMessageDialog("网站提供的sim卡类型与当前sim卡类型不匹配,不允许开户！",0);
	    return false;   
		}
	}--%>
...
function doGetHlrCode(packet)
{
	...
	if(result=="")
	{
		rdShowMessageDialog("输入的手机号码不对，请重新输入！");
...
<table cellSpacing=0 id="serviceNoInfo">	
	...
	<!-- gaopeng 2014/05/07 10:00:55 关于我省“网上选号”和“18元套卡省内版”增加SIM卡类型选择功能的需求  -->
	<tr>
		<td colspan="7"><font color = "red"> 注：只允许写SIM类型为10060-LTE USIM卡(3FF)和10061-LTE USIM卡(4FF)的卡</font></td>
	</tr>
	<tr  id="tr_serviceNo1">
...
<!-- weigp  webopen隐藏sys_note op_note  /npage/common/qcommon/bd_0007.jsp-->
<!-- liujian  显示sys_note op_note 
...
<jsp:include page="/npage/public/hwReadCustCardNew.jsp">
	...
	<table cellSpacing=0  align="center">
		...
		<INPUT class=b_foot onclick="ignoreThis()" type=button value=忽略 style="display:none"> 
