...
<%
	...
	System.out.println("===========wanghfa=sB608Valid==5====== 家长号码 = " + fatherPhone3);
...
<%
	...
	System.out.println("===========wanghfa=sB608Qry==5====== 家长号码 = " + fatherPhone3);
...
<script language=javascript>
	...
	function checkFatherPwd() {
		...
		checkPwd_Packet.data.add("phoneNo", "<%=fatherPhone3%>");		//移动号码,客户id,帐户id
		checkPwd_Packet.data.add("custPaswd", "<%=fatherPwd3%>");		//用户/客户/帐户密码
	...
	function printInfo(printType)
	{
		...
		cust_info+="手机号码：" + document.getElementById("fatherPhone3").value + "|";
		...
		opr_info += "解散的家庭代码：" + document.getElementById("familyGroupId").value + "|";
		opr_info += "家庭套餐：" + document.getElementById("productName").value + "|";
		
		document.getElementById("opNote").value = "工号<%=workNo%>为<%=fatherPhone3%>用户办理<%=opName%>。";
		note_info1 += "备注：工号<%=workNo%>为<%=fatherPhone3%>用户办理<%=opName%>。|";
