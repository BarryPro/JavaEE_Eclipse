...
<%
	...
	String memberPwd2 = WtcUtil.repNull(request.getParameter("memberPwd2"));
...
<%
	...
	System.out.println("===========wanghfa=sB608Valid==5====== 家长号码 = ");
	System.out.println("===========wanghfa=sB608Valid==6====== 家长密码 = ");
	System.out.println("===========wanghfa=sB608Valid==7====== 成员号码 = " + memberPhone2);
	System.out.println("===========wanghfa=sB608Valid==8====== 成员密码 = " + memberPwd2);
%>
<wtc:service name="sB608Valid" routerKey="phone" routerValue="<%=memberPhone2%>" retcode="retCode1" retmsg="retMsg1" outnum="13">
	<wtc:param value=""/>
	<wtc:param value="01"/>
...
<%
	...
	System.out.println("===========wanghfa=sB608Qry==5====== 家长号码 = ");
	System.out.println("===========wanghfa=sB608Qry==6====== 家长密码 = ");
	System.out.println("===========wanghfa=sB608Qry==7====== 成员号码 = " + memberPhone2);
	System.out.println("===========wanghfa=sB608Qry==8====== 成员密码 = " + memberPwd2);
	System.out.println("===========wanghfa=sB608Qry==9====== 查询类型 = 0");
%>
<wtc:service name="sB608Qry" routerKey="phone" routerValue="<%=memberPhone2%>" retcode="retCode2" retmsg="retMsg2" outnum="3">
	<wtc:param value=""/>
	<wtc:param value="01"/>
	<wtc:param value="<%=opCode%>"/>
	<wtc:param value="<%=workNo%>"/>
...
<script language=javascript>
	...
	function checkMemberPwd() {
		...
		checkPwd_Packet.data.add("phoneNo", "<%=memberPhone2%>");		//移动号码,客户id,帐户id
		checkPwd_Packet.data.add("custPaswd", "<%=memberPwd2%>");		//用户/客户/帐户密码
	...
	{  //显示打印对话框
		...
		var area_code = null;
		var opCode = "<%=opCode%>";
		var phoneNo = document.getElementById("memberPhone2").value;
	...
	function printInfo(printType)
	{
		...
		cust_info+="手机号码：" + document.getElementById("memberPhone2").value + "|";
		...
		opr_info += "退出的家庭代码：" + document.getElementById("familyGroupId").value + "|";
		opr_info += "家庭套餐：" + document.getElementById("productName").value + "|";
		
		document.getElementById("opNote").value = "工号<%=workNo%>为<%=memberPhone2%>用户办理<%=opName%>。";
		note_info1 += "备注：工号<%=workNo%>为<%=memberPhone2%>用户办理<%=opName%>。|";
...
<form name="frm" action="fb610_cfm.jsp" method="POST">
<input type="hidden" name="opCode" id="opCode" value="<%=opCode%>">
<input type="hidden" name="opName" id="opName" value="<%=opName%>">
<input type="hidden" name="printAccept" id="printAccept" value="<%=printAccept%>">
<input type="hidden" name="memberPhone2" id="memberPhone2" value="<%=memberPhone2%>">
...
<table cellspacing="0">
	<tr>
		...
		<td width="30%">
			<input type="text" name="memberPhone2" id="memberPhone2" value="<%=memberPhone2%>" v_must="1" class="InputGrey" readonly>
		</td>
	</tr>
	<tr>
		<td class="blue">
