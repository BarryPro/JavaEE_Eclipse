...
<%
	...
	System.out.println("=========wanghfa========= fb608_main.jsp " + opCode + ", " + opName + ", " + pwrf);
	
	String fatherPhone0 = WtcUtil.repNull(request.getParameter("fatherPhone0"));
	String fatherPwd0 = WtcUtil.repNull(request.getParameter("fatherPwd0"));
...
<%
	...
	System.out.println("===========wanghfa=sB608Valid==5====== 家长号码 = " + fatherPhone0);
	System.out.println("===========wanghfa=sB608Valid==6====== 家长密码 = " + fatherPwd0);
	System.out.println("===========wanghfa=sB608Valid==7====== 成员号码 = ");
	System.out.println("===========wanghfa=sB608Valid==8====== 成员密码 = ");
%>
<wtc:service name="sB608Valid" routerKey="phone" routerValue="<%=fatherPhone0%>" retcode="retCode1" retmsg="retMsg1" outnum="11">
	<wtc:param value=""/>
	<wtc:param value="01"/>
	<wtc:param value="<%=opCode%>"/>
	<wtc:param value="<%=workNo%>"/>
	<wtc:param value="<%=noPass%>"/>
	<wtc:param value="<%=fatherPhone0%>"/>
	<wtc:param value="<%=fatherPwd0%>"/>
	<wtc:param value=""/>
	<wtc:param value=""/>
</wtc:service>
...
<%
	...
	System.out.println("===========wanghfa=sB608Qry==5====== 家长号码 = " + fatherPhone0);
	System.out.println("===========wanghfa=sB608Qry==6====== 家长密码 = " + fatherPwd0);
...
%>
<wtc:service name="sB608Qry" routerKey="phone" routerValue="<%=fatherPhone0%>" retcode="retCode2" retmsg="retMsg2" outnum="3">
	<wtc:param value=""/>
	<wtc:param value="01"/>
	<wtc:param value="<%=opCode%>"/>
	<wtc:param value="<%=workNo%>"/>
...
<script language=javascript>
	...
	function checkFatherPwd() {
		...
		checkPwd_Packet.data.add("phoneNo", "<%=fatherPhone0%>");		//移动号码,客户id,帐户id
		checkPwd_Packet.data.add("custPaswd", "<%=fatherPwd0%>");		//用户/客户/帐户密码
	...
	{  //显示打印对话框
		...
		var area_code = null;
		var opCode = "<%=opCode%>";
		var phoneNo = document.getElementById("fatherPhone0").value;
	...
	function printInfo(printType)
	{
		...
		cust_info+="手机号码：" + document.getElementById("fatherPhone0").value + "|";
		...
		opr_info += "办理的家庭套餐：" + document.getElementById("productId")[document.getElementById("productId").selectedIndex].text + "|";
		
		document.getElementById("opNote").value = "工号<%=workNo%>为<%=fatherPhone0%>用户办理<%=opName%>。";
		note_info1 += "备注：工号<%=workNo%>为<%=fatherPhone0%>用户办理<%=opName%>。|";
...
<input type="hidden" name="fatherPhone0" id="fatherPhone0" value="<%=fatherPhone0%>">
<input type="hidden" name="opNote" id="opNote" value="">

<%@ include file="/npage/include/header.jsp" %>
<div class="title">
	<div name="title_zi" id="title_zi">家长信息</div>
</div>
<table cellspacing="0">
	<tr>
		...
		<td width="30%">
			<input type="text" name="fatherPhone0" id="fatherPhone0" value="<%=fatherPhone0%>" v_must="1" class="InputGrey" readonly>
		</td>
	</tr>
	<tr>
		<td class="blue">
...
<div class="title">
	<div name="title_zi" id="title_zi">选择家庭套餐</div>
</div>
<table>
	<tr>
		...
		<td width="70%">
			<select id="productId" name="productId" style="width:400px">
<%
			for (int i = 0; i < result2.length; i ++) {
%>
