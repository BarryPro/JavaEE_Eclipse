...
<%
	...
	Logger logger = Logger.getLogger("f2760_1.jsp");
...
//得到页面参数
	...
	if (action!=null&&action.equals("select")){
			...
			acceptList = callView.callFXService("sQueryShowMode", paramsIn, "25");
			...
			if(errCode == 139785)
			{
			    ...
			    <script language=javascript>
			    	...
			    	{
			    		window.showModalDialog("f2760addmode.jsp?show_mode=<%=show_mode%>&modetype=<%=modetype%>&contract_no=<%=contract_no%>","","dialogHeight:160px; dialogWidth:300px; status:no; help:no; scroll:no");
			    	 	document.location.href ="f2760_1.jsp?action=select&show_mode=<%=show_mode%>&modetype=<%=modetype%>&contract_no=<%=contract_no%>";
			   	 	}
		      </script>
			...
			if(errCode == 0)
			{
				...
				//查询字段名称列表dDefineCol
				sqlStr= "select col_name,trim(col_attr)||'->'||trim(col_note) from dDefineCol";
 ...
 function doQuery()
	{
		...
		var con=<%=contract_no%>;
		//alert(con);
		document.form.action = "f2760_1.jsp?action=select&contract_no="+con;
...
<%
	...
	function doProcess(myPacket)
	{
		...
		{  
			...
			{			
				rdShowMessageDialog("操作成功！");
				document.location.href ="f2760_1.jsp?action=select&show_mode=<%=show_mode%>&modetype=<%=modetype%>";
			}
			else if(retFlag=="del")
			{			
				rdShowMessageDialog("操作成功！");
...
*/	 
	...
	function previouStep(){
		document.location.href="<%=request.getContextPath()%>/page/s2760/f2760_1.jsp";
	}
	function nextStep(){
		var con=<%=contract_no%>;
		//alert(con);
		document.location.href="chosseUsers.jsp?contract_no="+con+"&show_mode=<%=show_mode%>";
	}
<%}%>
</script>
      ...
      <table width="98%" border="0" cellspacing="0" cellpadding="0" align="center" bgcolor="#FFFFFF">
            ...
            <table width="535" border="0" cellspacing="0" cellpadding="0">
              <tr>
                ...
                <td valign="bottom" width="493">
                  <table width="100%" border="0" cellspacing="0" cellpadding="0">
                    <tr>
                    <td background="<%=request.getContextPath()%>/images/jl_background_4.gif"><font color="FFCC00"><b>定制模板</b></font></td>
		...
		<TABLE  width=98%  align="center"  bgcolor="#FFFFFF" cellspacing="1" border="0" >
         ...
         <td align="left" colspan="20"> 
             <select name="modetype" <%if(nextFlag==2){out.print("disabled");}%>>
             	<option value="0">打印模版</option>
		       ...
		       <TABLE width="100%" border=0 align=center cellpadding="4" cellSpacing=1>
		          <TBODY>
		            <TR bgcolor="E8E8E8">
		             ...
		             <input class="button" name="previous" style="cursor:hand" type=button value="下一步" onclick="nextStep()">&nbsp;&nbsp;
