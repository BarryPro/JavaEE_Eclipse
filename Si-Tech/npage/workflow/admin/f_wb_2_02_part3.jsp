...
%>
		...
		<div align="right" id='mode3' style='display:none'>
  ...
  <div id="Operation_Table">
  	<div id='query3div' style='display:'>
	...
	<form name='form31' method="post" action='f_wb_2_02_part3.jsp'>
		...
		<table cellspacing="0">	
			...
			<select name="wo_status">
				...
				<td>
					<input type='text' value='<%=begin_time%>' readonly='true' name='begin_time' id='start3' >
					<img src='./js/cal/datePicker.gif' onclick="return showCalendar('start3', '%Y-%m-%d %H:%M', '24', true);">
			...
			<tr>
				...
				<td>
					...
					<input type='text' value='<%=end_time%>' readonly='true' name='end_time' id='end3'>
					<img src='./js/cal/datePicker.gif' onclick="return showCalendar('end3', '%Y-%m-%d %H:%M', '24', true);">
				...
				<td>
					<input class="b_text" type='button' onclick='condquery3()' value='查询'>
						&nbsp;&nbsp;&nbsp;
					
					<input class="b_text" type='button' onclick='clearquery3();' value='重置'>
				</td>
...
<%
							...
							%>
							 <TR align="center" id="rowc<%=ret[j][wonoIndex]%>"  type="<%=ret[j][statusIndex]%>" wano="<%=ret[j][wanoIndex]%>" onClick="trfunc3(this)"> 
							 <%
  ...
  <wtc:array id="ret1"  start="9" length="2"> 
	...
	%>
    	...
    	<div style="position:relative;font-size:12px" align="center">
  		...
  		[<a  href='javascript:void(0)' onclick='return gotopage3("first",<%=totalRec%>,<%=totalPage%>,<%=currPage%>,false);' >首页</a>]&nbsp;&nbsp;
  		[<a  href='javascript:void(0)' onclick='return gotopage3("before",<%=totalRec%>,<%=totalPage%>,<%=currPage%>,false);' >上页</a>]&nbsp;&nbsp;  		
  		[<a  href='javascript:void(0)' onclick='return gotopage3("next",<%=totalRec%>,<%=totalPage%>,<%=currPage%>,false);' >下页</a>]&nbsp;&nbsp;
  		[<a  href='javascript:void(0)' onclick='return gotopage3("end",<%=totalRec%>,<%=totalPage%>,<%=currPage%>,false);' >尾页</a>]&nbsp;&nbsp;
  	</div>
	...
	function gotopage3(obj,totalrec,totalpage,currpage,isfresh)
{
  ...
  if(!isfresh)
	{
  if(result==-1) return;
  	document.form31.currPage.value=result;
  }
	else
	{
