...
<%	
	...
	String tf_begin = request.getParameter("tf_begin");
	String tf_end = request.getParameter("tf_end");
	...
	paraAray[8] = "1";        //退费类型 全是单倍
	paraAray[9] = "1";           //退费一级原因code gprs退费一级原因=1
	String reason_a = request.getParameter("reason");
	paraAray[10] = reason_a;        //退费二级原因code 对应退费业务字段 tfyw
	
	String reason_b = request.getParameter("otherReason");
	paraAray[11] = request.getParameter("tfyw");;           //二级原因 对应reason_a 退费三级原因code 对应退预存款原因 
	paraAray[12] = "";      //Sp企业代码
	paraAray[13] = "";       //业务代码		
	paraAray[14] = strBackMoney;	    //退费金额 
	paraAray[15] = strBackMoney;	    //赔偿金额 
	paraAray[16] = op_note;             //备注
	paraAray[17] = regionCode;  //地市代码
	paraAray[18] = org_Code;   //归属代码
	paraAray[19] = "2";   //xl add for zlc优化后 2代表的gprs退费   //退费种类 2=退预存款
	/*new begin*/
	paraAray[20] = hjlx; 
	paraAray[21] = jflx; 
	paraAray[22] = ywsysj; 
	...
	paraAray[25] =dj;
	paraAray[26] =sl;
	//新增4个参数 reason_a reason_b begin_dt end_dt
...
<wtc:service name="s4141Cfm" routerKey="phone" routerValue="<%=phoneno%>" retcode="s4141CfmCode" retmsg="s4141CfmMsg" outnum="2" >
	...
	<wtc:param value="<%=reason_a%>"/><!--xl add new-->
	<wtc:param value="<%=reason_b%>"/>
	<wtc:param value="<%=tf_begin%>"/>
	<wtc:param value="<%=tf_end%>"/>
</wtc:service>
...
<script language="JavaScript">
	...
	window.location="zg68_1.jsp?opCode=4141&opName=投诉退费";
	</script>
<%
	}else{
%>   
