...
<%
    ...
    String url = "/npage/contact/upCnttInfo.jsp?opCode="+"2296"+"&retCodeForCntt="+errCode+"&opName="+"无线音乐俱乐部包年(季)会员"+"&workNo="+work_no+"&loginAccept="+paraAray[0]+"&pageActivePhone="+themob+"&retMsgForCntt="+errMsg+"&opBeginTime="+opBeginTime;
%>
...
<script language="JavaScript">
   ...
   window.location="f2296_login.jsp?activePhone=<%=themob%>&opCode=2296&opName=无线音乐俱乐部包年(季)会员";
   /*****刘春梅20090323修改不交现金，不打发票
   var infoStr="";
   ...
   location="/npage/public/hljBillPrint.jsp?retInfo="+infoStr+"&dirtPage=/npage/s2292/f2296_login.jsp?activePhone=<%=themob%>%26opCode=2296%26opName=无线音乐俱乐部包年(季)会员";
	*****/
</script>
<%
	}else{
%>   
<script language="JavaScript">
	rdShowMessageDialog("无线音乐包年(季)用户申请失败!(<%=errMsg%>",0);
	window.location="f2296_login.jsp?activePhone=<%=themob%>&opCode=2296&opName=无线音乐俱乐部包年(季)会员";
</script>
<%}%>
