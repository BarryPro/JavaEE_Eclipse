...
<%	
    ...
    paraAray[5] = request.getParameter("cur_level");
    paraAray[6] = request.getParameter("old_level");
    paraAray[7] = request.getParameter("opNote");
    paraAray[8] = request.getParameter("ip_Addr");

%>

<wtc:service name="s6908Cfm" routerKey="region" routerValue="<%=org_code.substring(0,2)%>" retcode="errCode" retmsg="errMsg" outnum="2">
...
<wtc:array id="s6908CfmArr" scope="end" />

<%
	System.out.println("in f6908_2.jsp - s6908Cfm :  "+errCode+" : "+errMsg);
    System.out.println("%%%%%%%%调用统一接触开始%%%%%%%%");
    String url = "/npage/contact/upCnttInfo.jsp?opCode="+"6908"+"&retCodeForCntt="+errCode+"&opName="+"号簿管家包月申请冲正"+"&workNo="+work_no+"&loginAccept="+paraAray[0]+"&pageActivePhone="+themob+"&retMsgForCntt="+errMsg+"&opBeginTime="+opBeginTime;
%>
...
<script language="JavaScript">
   ...
   window.location="f6907_login.jsp?activePhone=<%=themob%>&opCode=6908&opName=号簿管家包月申请冲正";
...
%>   
<script language="JavaScript">
	rdShowMessageDialog("号簿管家包月申请冲正失败!(<%=errMsg%>");
	window.location="f6907_login.jsp?activePhone=<%=themob%>&opCode=6908&opName=号簿管家包月申请冲正";
</script>
<%}%>
