...
%>
	<wtc:service name="sB552Cancel" routerKey="phone" routerValue="<%=activePhone%>" retcode="retCode1" retmsg="retMsg1" outnum="1">
...
<%
	...
	System.out.println("%%%%b484%%%调用统一接触开始%%%%%%%%");
...
<%	
	System.out.println("%%%%b484%%%调用统一接触结束%%%%%%%%");
//为统一接触  end
	
	if ("000000".equals(retCode1)) {
%>
		rdShowMessageDialog("<%=activePhone%>和<%=loverPhone%>二人世界业务取消成功！", 2);
<%
	} else {
%>
...
%>
		window.location = "fb551.jsp?opCode=<%=opCode%>&opName=<%=opName%>&activePhone=<%=activePhone%>";
	</script>
