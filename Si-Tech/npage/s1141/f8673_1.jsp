...
<%
  ...
  //rdShowMessageDialog("此用户为已中奖用户，中奖奖品为："<%=awardName%>", 请用户完好无损返回奖品，再继续办理冲正业务！");
   	if(rdShowConfirmDialog("此用户为已中奖用户，中奖奖品为：<%=awardName%> 请用户完好无损返回奖品，再继续办理冲正业务！")!=1)
		{
			location='f8672_login.jsp?activePhone=<%=phoneNo%>';
		}
	</script>
...
<wtc:service  name="s8673Qry" routerKey="phone" routerValue="<%=phoneNo%>" outnum="27"  retcode="errCode" retmsg="errMsg">
 ...
 */
  ...
  if(retList == null)
  {
	if(!retFlag.equals("1"))
	{
	   ...
	   retFlag = "1";
	   retMsg = "s8673Qry查询号码基本信息为空!<br>errCode: " + errCode + "<br>errMsg+" + errMsg;
    }
...
function printInfo(printType)
{
	...
	opr_info+="业务类型：预存赠G3上网卡冲正"+"|";
	
	opr_info+="业务流水："+document.all.login_accept.value+"|";
	opr_info+="上网卡型号: "+document.all.type_name.value+"|";
	opr_info+="IMEI码： "+"<%=IMEINo%>"+"|";

	var jkinfo="";
	jkinfo="捆绑语音卡号码："+document.all.second_phone.value+"退款合计："+document.all.sum_money.value+"元，含数据卡上网费"+document.all.net_fee.value+"元，语音卡话费"+document.all.pay_money.value+"元|";
...
<body>
<form name="frm" method="post" action="f8673_2.jsp" onKeyUp="chgFocus(frm)">
<%@ include file="/npage/include/header.jsp" %>
	...
	<tr>
		<td class="blue">购上网卡款</td>
            <td>
			  		<input name="phone_fee" type="text" id="phone_fee"  v_name="购上网卡款" value="<%=phone_fee%>" readonly>
			  		<font class="orange">*</font>
			</td> 
             <td class="blue">赠上网流量费</td>
             <td>
					<input name="net_fee" type="text"  id="net_fee" v_type="money" v_must=1 v_name="上网流量费" value="<%=net_fee%>" readonly>
					<font class="orange">*</font>
			</td>
			</tr>
	<tr>
			<td class="blue">上网流量费消费期限</td>
             <td>
					<input name="active_term" type="text"  id="active_term"  v_must=1 v_name="上网流量费消费期限" value="<%=active_term%>" readonly>
			</td>

			<td class="blue">语音卡号码</td>
            <td>
			  		<input name="second_phone" type="text" id="second_phone"  v_name="副卡号码" value="<%=second_phone%>" readonly>
			</td>
			</tr>
	<tr>
            <td class="blue">赠送话费</td>
            <td>
					<input name="pay_money" type="text"   id="pay_money" v_type="0_9" v_must=1   v_name="赠送话费" value="<%=pay_money%>" readonly>
					<font class="orange">*</font>
			</td>
			<td class="blue">话费消费期限</td>
             <td>
					<input name="base_term" type="text"  id="base_term"  v_must=1 v_name="话费消费期限" value="<%=base_term%>" readonly>
			</td>
	</tr>
	<tr>
            <td class="blue">应付金额</td>
            <td>
					<input name="sum_money" type="text"  id="sum_money" value="<%=sum_money%>" readonly>
					<font class="orange">*</font>
			</td>
			<td> &nbsp;</td> 
			<td> &nbsp;</td> 
	...
	<tr>
            <td class="blue" >备注</td>
            <td colspan="3">
			     <input name="opNote" type="text"  id="opNote" size="60" maxlength="60" value="预存赠G3上网卡冲正" >
            </td>
	</tr>
	<tr>
