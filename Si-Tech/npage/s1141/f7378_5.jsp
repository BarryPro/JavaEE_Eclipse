...
<%
	String opCode = "7378";		
	String opName = "接收信息名单查询";	//header.jsp需要的参数   
  ...
  else
  {
	...
	%>
		<script language="JavaScript">
			rdShowConfirmDialog("错误代码：<%=return_code%>，错误信息：<%=error_msg%>",0);
			history.go(-1);
		</script>
<%	
	}
...
<script language="JavaScript">
	...
	<!--
	  ...
	  function frmCfm(){
		...
		rdShowConfirmDialog(document.all.optypestr.value);
		document.all.opcodestr.value=opcodelist;
		rdShowConfirmDialog(document.all.opcodestr.value);
		document.all.opCount.value=opCount;
		rdShowConfirmDialog(document.all.opCount.value);
		if("<%=offon%>"=="0"){
  ...
  return true;
	...
	{			    	
		...
		var opr_info=""; //操作信息
			...
			note_info1+="备注：欢迎您参加“积分换机”活动，你的话费预存款在未消费完毕前不能退款"+"|";
...
<body>
	...
	<div class="title">
	  ...
	  <tr>
		...
		<font class="orange">*</font>
            ...
            <td class="blue">&nbsp;</td>
            <td>
		<input name="cust_addr" value="<%=bp_add%>" type="hidden"  v_must=1 readonly id="cust_addr" maxlength="20" >
            </td>
      	  </tr>           
         <tr>
            <td colspan="4"> 
			...
			<input type="radio" name="opFlag" value="two" onclick="opchange()" disabled checked>关
		<%}else{%>
			 <input type="radio" name="opFlag" value="one" onclick="opchange()"  disabled checked >开&nbsp;&nbsp;
			<input type="radio" name="opFlag" value="two" onclick="opchange()" disabled>关
		<%}%>
		</div>
            </td>
        </tr>
	...
	<table cellspacing="0">	
 	 ...
 	 <input name="cardId_no" value="<%=cardId_no%>" type="hidden"  v_must=1 readonly id="cardId_no" maxlength="20" >
