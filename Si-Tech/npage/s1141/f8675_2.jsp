...
%>
<wtc:service  name="s8675Cfm" routerKey="region" routerValue="<%=org_code%>" outnum="2"  retcode="errCode" retmsg="errMsg">
...
<script language="JavaScript">
   ...
   infoStr+='<%=paraAray[1]%>'+"|";
   infoStr+=" "+"|";//协议号码
   infoStr+="手机品牌型号："+'<%=type_name%>'+"|";//支票号码
   //infoStr+="IMEI码："+"<%=IMEINo%>"+"|";
   infoStr+="<%=chinaFee%>"+"|";//合计金额(大写)
   infoStr+="<%=WtcUtil.formatNumber(sum_money,2)%>"+"|";//小写
   var jkinfo="";
   jkinfo+="IMEI码<%=IMEINo%>"+",退款合计<%=sum_money%>"+"元,含赠送话费<%=pay_money%>"+"元,上网流量费<%=net_fee%>"+"元";
...
%>
<script language="JavaScript">
	rdShowMessageDialog("购G3手机，赠通信费冲正失败!(<%=errMsg%>");
