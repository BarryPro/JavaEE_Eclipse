...
<%	
	...
	///////<!-- ningtn add for pos end @ 20100722 -->
%>	
	<wtc:service name="s8679Cfm" routerKey="region" routerValue="<%=org_code.substring(0,2)%>" retcode="retCode" retmsg="retMsg" outnum="2" >
...
<script language="JavaScript">
   ...
   var infoStr="";
   infoStr+="<%=work_no%>  <%=paraAray[0]%>"+"       积分兑换G3上网本冲正"+"|";//工号  
   ...
   var jkinfo="";
   	...
   	jkinfo+="IMEI码<%=IMEINo%>"+"，退款合计：<%=sum_money%>"+"元，其中：话费：<%=pay_money%>"+"元，";
	jkinfo+="上网费：<%=net_money%>"+"元，退积分："+"<%=used_point%>"+"分|";
	infoStr+=jkinfo+"|";
  ...
  // location="/page/s1210/chkPrint.jsp?retInfo="+infoStr+"&dirtPage=/page/s1775/f1775_1.jsp";
   location="/npage/public/hljBillPrint.jsp?retInfo="+infoStr+"&dirtPage=/npage/s1141/f8678_login.jsp?activePhone=<%=request.getParameter("phone_no")%>%26opCode=8679%26opName=积分换G3上网本冲正";
</script>
<%
	}else{
%>   
<script language="JavaScript">
	rdShowMessageDialog("积分换G3上网本冲正失败!(<%=errMsg%>",0);
	window.location="f8678_login.jsp?activePhone=<%=request.getParameter("phone_no")%>&opCode=8679&opName=积分换G3上网本冲正";
</script>
<%}%>
