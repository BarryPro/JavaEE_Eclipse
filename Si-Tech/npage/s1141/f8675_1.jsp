...
<%
  ...
  //rdShowMessageDialog("此用户为已中奖用户，中奖奖品为："<%=awardName%>", 请用户完好无损返回奖品，再继续办理冲正业务！");
   	if(rdShowConfirmDialog("此用户为已中奖用户，中奖奖品为：<%=awardName%> 请用户完好无损返回奖品，再继续办理冲正业务！")!=1)
		{
			location='f8674_login.jsp?activePhone=<%=phoneNo%>';
		}
	</script>
...
<wtc:service  name="s8675Qry" routerKey="phone" routerValue="<%=phoneNo%>" outnum="24"  retcode="errCode" retmsg="errMsg">
 ...
 */
  ...
  if(retList == null)
  {
	if(!retFlag.equals("1"))
	{
	   ...
	   retFlag = "1";
	   retMsg = "s8675Qry查询号码基本信息为空!<br>errCode: " + errCode + "<br>errMsg+" + errMsg;
    }
...
<!--
  ...
  var arrPhoneType = new Array();//G3手机型号代码
  var arrPhoneName = new Array();//G3手机型号名称
...
function printInfo(printType)
{
	...
	opr_info+="业务类型：购G3手机，赠通信费冲正"+"|";
	
	opr_info+="业务流水："+document.all.login_accept.value+"|";
	opr_info+="手机品牌型号: "+document.all.type_name.value+"|";
	opr_info+="IMEI码： "+"<%=IMEINo%>"+"|";

	var jkinfo="";
	jkinfo="退款合计："+document.all.sum_money.value+"元，含赠送话费"+document.all.pay_money.value+"元，上网流量费"+document.all.net_fee.value+"元";
...
<body>
<form name="frm" method="post" action="f8675_2.jsp" onKeyUp="chgFocus(frm)">
<%@ include file="/npage/include/header.jsp" %>
	...
	<tr>
		<td class="blue">购手机款</td>
            <td>
			  		<input name="phone_fee" type="text" id="phone_fee"  v_name="购G3手机款" value="<%=phone_fee%>" readonly>
			  		<font class="orange">*</font>
			</td> 
	...
	<tr>
            <td class="blue" >备注</td>
            <td colspan="3">
			     <input name="opNote" type="text"  id="opNote" size="60" maxlength="60" value="购G3手机，赠通信费冲正" >
            </td>
	</tr>
	<tr>
