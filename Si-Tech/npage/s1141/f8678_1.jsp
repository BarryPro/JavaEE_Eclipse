...
<!--
  ...
  var arrPhoneType = new Array();//上网本型号代码
  var arrPhoneName = new Array();//上网本型号名称
...
%>
 ...
 function printCommit()
 {
  ...
  with(document.frm){
	...
	if(agent_code.value==""){
	  rdShowMessageDialog("请输入上网本品牌!");
      agent_code.focus();
	  return false;
	}
	if(phone_type.value==""){
	  rdShowMessageDialog("请输入上网本型号!");
      phone_type.focus();
	  return false;
	}
	if(sale_code.value==""){
	...
	opNote.value=phone_no.value+"办理积分换G3上网本业务";
...
function printInfo(printType)
{
	...
	opr_info+="业务类型：积分换G3上网本"+"|";
    opr_info+="业务流水："+document.all.login_accept.value+"|";
    opr_info+="上网本品牌型号："+document.all.agent_code.options[document.all.agent_code.selectedIndex].text+document.all.phone_type.options[document.all.phone_type.selectedIndex].text+"，IMEI码："+document.frm.IMEINo.value+"|";

	var jkinfo="";
	jkinfo+="缴款合计："+document.all.sum_money.value+"元 其中：话费 "+document.all.pay_money.value+"元，分"+document.all.Consume_Term.value+"个月赠送，";
	jkinfo+=document.all.Consume_Term.value+"个月内消费完毕"+"，上网费"+document.all.net_money.value+"元，兑换积分"+document.all.used_point.value+"分|";
...
function salechage(){
    ...
    document.all.opNote.value = "用户"+document.all.phone_no.value+"办理积分换G3上网本";
}
 ...
 function pointchange(){
 	 with(document.frm){
 		...
 		if(parseFloat(point_money.value)>parseFloat(price.value)){
 			rdShowConfirmDialog("积分消费冲抵金额("+point_money.value+")不能大于上网本款("+price.value+")");
 			used_point.value="";
 			return false;
 		}
...
<form name="frm" method="post" action="f8678_2.jsp" onKeyUp="chgFocus(frm)">
	...
	<%@ include file="/npage/include/header.jsp" %>   
		...
		<div class="title">
			<div id="title_zi">积分换G3上网本</div>
		</div>
		
        <table cellspacing="0">
		  <tr>
            <td class="blue">操作类型</td>
            <td colspan="3">积分换G3上网本申请</td>           
          </tr>
          <tr>
       ...
       <tr>
          ...
          <tr>
           <td class="blue">话费消费期限</td>
            <td>
				<input Class="InputGrey" name="Consume_Term" type="text"   id="Consume_Term" v_type="0_9" v_must=1   v_name="话费消费期限" readonly>
						</td>
            <td class="blue">赠上网费</td>
            <td>
				<input Class="InputGrey" name="net_money" type="text"  id="net_money" v_type="money" v_must=1   readonly v_name="赠上网流量费" >
			</td>
          </tr>
          <tr>
			<td class="blue">上网费消费期限</td>
            <td>
				<input Class="InputGrey" name="Active_Term" type="text"   id="Active_Term" v_type="0_9" v_must=1   v_name="上网费消费期限" readonly>
			</td>
         ...
         <tr> 
		  ...
		  <tr style="display:none">
            ...
            <td colspan="3">
             <input name="opNote" type="text" id="opNote" size="60" maxlength="60" value="积分换G3上网本" >
            </td>
          </tr>
          <tr>
