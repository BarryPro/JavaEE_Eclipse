...
<%
	...
	try{
%>
        <wtc:service name="sRepairBoss" routerKey="region" routerValue="<%=regionCode%>" retcode="retCode" retmsg="retMsg" outnum="2">
            <!--0-->
...
<%
        ...
        %>
            <script language='jscript'>
                ...
                frm.action="f3216_repair3.jsp";
            	frm.method="post";
            	frm.submit();
            	
            	//用location批量时有可能URL超长。
                //window.location="f3216_repair3.jsp?errPhoneList=<%=errPhoneList%>&errMsgList=<%=errMsgList%>&unitId=<%=iUnitId%>&loginAccept=<%=iLoginAccept%>&grpName=<%=iGrpName%>";
            </script>
        <%
        }else{
            %>
        ...
        %>
        <script type=text/javascript>
            rdShowMessageDialog("调用服务sRepairBoss失败！",0);
            window.close();
        </script>
        <%
        System.out.println("# return from f3216_repair2.jsp -> Call Service sRepairBoss Failed!");
        e.printStackTrace();
    }
