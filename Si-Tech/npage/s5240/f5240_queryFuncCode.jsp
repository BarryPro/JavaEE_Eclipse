...
String regionCode = (String)session.getAttribute("regCode");	
	...
	sqlStr1 = "select FUNCTION_CODE, FUNCTION_NAME from sFuncCode where FUNCTION_CODE like '12%' order by FUNCTION_CODE";
...
<script>
		...
		<%for(int i=0;i < retListString1.length;i++){ %>
			var str='<input type="hidden" name="op_code" value="<%=retListString1[i][0]%>">';
			str+='<input type="hidden" name="op_name" value="<%=retListString1[i][1]%>">';
		...
		function doCommit()
		{
			if("<%=retListString1.length%>"=="0"){
				rdShowMessageDialog("您没有选择任何业务代码！",0);
				return false;
			}
			else if("<%=retListString1.length%>"=="1"){//值为一条时不需要用数组
				if(document.all.num.checked){
					window.opener.form1.op_code.value=document.all.op_code.value;
					window.opener.form1.op_code_name.value=document.all.op_name.value;
					//window.close();
				}
				else{
					rdShowMessageDialog("您没有选择任何业务代码！",0);
				...
				if(a!=-1){
					window.opener.form1.op_code.value=document.all.op_code[a].value;
					window.opener.form1.op_code_name.value=document.all.op_name[a].value;
					//window.close();
				}
				else{
					rdShowMessageDialog("您没有选择任何业务代码！",0);
