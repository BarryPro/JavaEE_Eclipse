...
<%
    ...
    paraIn[0] = "select to_char(account_id),user_name,to_char(user_no),to_char(id_no) from dgrpusermsg where cust_id =(select cust_id from dgrpcustmsg where unit_id=:unit_id ) and run_code<'a'";    
    paraIn[1]="unit_id="+unit_id;
%>
    <wtc:service name="TlsPubSelBoss" routerKey="phone" routerValue="<%=unit_id%>" retcode="retCode" retmsg="retMsg" outnum="4" >
	...
	if(typeof(document.frm.account)!="undefined"){
	   for(i=0;i<document.frm.account.length;i++) {    
		  if (document.frm.account[i].checked==true) {
			...
			temp[3] = document.frm.idno.value;
			//alert(temp);
			window.returnValue = temp;
	        break;
		  }
    ...
    <% for(int i=0; i < result.length  ; i++)
	{
		%>
			<tr> 
			  <td > 
				<div align="center"> 
				  <input type="radio" name="account" value="<%=result[i][0].trim()%>" onClick="JavaScript:document.frm.userType.value='0',document.frm.userno.value='<%=result[i][2]%>',document.frm.idno.value='<%=result[i][3]%>'">
				</div>
			  </td>
			  <td > 
	...
	<% for(int i=0; i < resultdead.length  ; i++)
	{
		%>
			<tr> 
			  <td > 
				<div align="center"> 
				  <input type="radio" name="account" value="<%=resultdead[i][0].trim()%>" onClick="JavaScript:document.frm.userType.value='1',document.frm.userno.value='<%=resultdead[i][2]%>',document.frm.idno.value='<%=resultdead[i][3]%>'">
				</div>
			  </td>
			  <td> 
