	...
	if(_this.checked == false){
			if(typeof(comProdDepHash[clickId]) !="undefined" && comProdDepHash[clickId].length >0){
				$.each(comProdDepHash[clickId],function(i,n){   
					if($("#"+n)[0].checked == true){
						rdShowMessageDialog($("#"+n).attr("name").substring(9)+"依赖于"+$("#"+clickId).attr("name").substring(9)+",请先取消"+$("#"+n).attr("name").substring(9)+"!",0);
	...
	if(typeof(comPordHash[clickId]) != "undefined"){
		...
		if(_this.checked == true){
			$.each(items[0],function(i,n){
		     ...
		     {
		     	 rdShowMessageDialog($("#"+clickId).attr("name").substring(9)+"依赖于"+$("#"+n).attr("name").substring(9)+",请先选择"+$("#"+n).attr("name").substring(9)+"!",0);
