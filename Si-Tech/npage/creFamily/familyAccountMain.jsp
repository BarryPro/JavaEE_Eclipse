...
<%
	...
	%>
		<wtc:service name="sG630Init"  routerKey="region" routerValue="<%=regionCode%>"  retcode="error_code" retmsg="error_msg" outnum="16" >
...
function showPrtDlg(printType,DlgMessage,submitCfm){
		...
		document.all.t_sys_remark.value="用户"+"<%=cust_name%>"+"付费计划变更";
		if(document.all.t_op_remark.value.trim().length==0){
			document.all.t_op_remark.value="操作员<%=work_no%>"+"对用户"+"<%=cust_name%>"+"进行用户付费计划变更"
		}
		if(document.all.assuNote.value.trim().length==0){
			document.all.assuNote.value="操作员<%=work_no%>"+"对用户"+"<%=cust_name%>"+"进行用户付费计划变更"
		}
...
function printInfo(printType){
  ...
  var tempOprInfo1 = "变更成员号码：";
  var tempOprInfo2 = "默认付费账户：";
  var tempOprInfo3 = "付费方式    ：";
			...
			if(tdSize==9){//含有手机号的 冲销顺序 费用代码 明细代码 费用比率
				var memPhone   = ""; //手机号数组
		 		...
		 		var feeLimit   = ""; //限额
						...
						defContNo  = $(bt).find("td:eq(1)").text().trim();
						feeLimit   = $(bt).find("td:eq(2)").find("input").val().trim();
						tempOprInfo1 += memPhone+"     ";
						tempOprInfo2 += defContNo+"     ";
						tempOprInfo3 += feeLimit+"               ";
						
			}
 		});  
 		
 	opr_info+=tempOprInfo1+"|"+tempOprInfo2+"|"+tempOprInfo3+"|";
