...
<%
	...
	String idnostrs = request.getParameter("idnostrs");
	System.out.println("--------------------f1170Cfm.jsp---------idnostrs--------------"+idnostrs);
  ...
  String servIdArrStr              = request.getParameter("servIdArrStr");
  String servIdArr[] = servIdArrStr.split(",");


	System.out.println("servIdArrStr=====----------------=="+servIdArrStr);
  ...
  System.out.println("IssCode======================="+IssCode);
  System.out.println("AcqCode======================="+AcqCode);
  ...
  System.out.println("CardNoPingBi=================="+CardNoPingBi);
  System.out.println("ExpDate======================="+ExpDate);
  System.out.println("Remak========================="+Remak);
  ...
  */
  ...
  System.out.println("array1["+i+"]===================="+array1[i]);
  		...
  		for(int j=servIdArr.length-1;j>=0;j--){
  		System.out.println("服务订单["+j+"]===================="+servIdArr[j]);
  			UType servOrderInfo  = new UType(); //创建服务订单节点
  			servOrderInfo.setUe("STRING", servIdArr[j].trim());//存入服务订单号
  			servOrderInfoList.setUe(servOrderInfo);//把服务订单节点存入所属客户订单节点中
  		}
  		
  		
  		UType idnoInfoUtype = new UType();
  		String idnostrsArr[] = idnostrs.split(",");
  		for(int k = 0; k<idnostrsArr.length; k++){
  			idnoInfoUtype.setUe("LONG",idnostrsArr[k]);
  		}
  		
  		custOrderInfo.setUe(servOrderInfoList);//把服务订单节点存入服务订单列表中
  		custOrderInfo.setUe(idnoInfoUtype);
  		custOrderInfoList.setUe(custOrderInfo);//把服务订单列表节点存入客户订单子项列表中
 	}


System.out.println("-----------------custOrderInfoList--------------------------");
 	StringBuffer logBuffer = new StringBuffer(80);
	WtcUtil.recursivePrint(custOrderInfoList,1,"2",logBuffer);
