...
function printBill(){
	...
	var d_fee = "";
	if(Number(document.frm.remain.value)>0){
		d_fee = "-"+document.frm.remain.value;
	}else{
		d_fee = ""+document.frm.remain.value;
	}
	...
	$(billArgsObj).attr("10015", d_fee);   //本次发票金额
	$(billArgsObj).attr("10016", d_fee);   //大写金额合计	
	$(billArgsObj).attr("10017","*"); //本次缴费现金
	$(billArgsObj).attr("10021",d_fee); //手续费
