...
String tempStrf = "";
    ...
    String oCustName = WtcUtil.repNull(request.getParameter("oCustName"));
    ...
    String brandID = "";
		...
		String oCustPwd = WtcUtil.repNull(request.getParameter("oCustPwd"));
		oCustPwd = getFromBase64Pub(oCustPwd);
...
<%
	...
	System.out.println("gaopengSee1104===prtFlag==="+prtFlag);
	
	System.out.println("gaopengSee1104===closeId==="+closeId);
...
$(document).ready(function () {
		...
		$("input[name='userpwd']").val("<%=oCustPwd%>");
		$("input[name='userpwdcfm']").val("<%=oCustPwd%>");
...

{
    if(v_hiddenFlag=="Y"){ //当为Y时，进入新版小区代码展示页面
      ...
      var packet1 = new AJAXPacket("/npage/s1104/getOfferAttrNewKx.jsp","请稍后...");
      packet1.data.add("v_code" ,v_code);
      packet1.data.add("v_text" ,v_text);
      packet1.data.add("myActivePhone" ,"<%=myActivePhone%>");
...
{
 		var packet = new AJAXPacket("/npage/s1104/offerDetailQry_new.jsp","正在加载附加销售品构成明细，请稍后...");
...
{
	var packet2 = new AJAXPacket("/npage/s1104/getOfferRel.jsp","正在加载附加销售品构成依赖关系,请稍后...");
...
{
	var packet1 = new AJAXPacket("/npage/s1104/getMasterServType.jsp","请稍后...");
...
{
	var packet2 = new AJAXPacket("/npage/s1104/getProdAttr.jsp","请稍后...");
 ...
 var kehuids = document.all.contractNo.value;
	 ...
	 if(khtyspes=="1") {
        ...
        return ;
				...
				var packet = new AJAXPacket("/npage/s1104/checkLastIDNO.jsp","请稍后...");
...
function setGdNumBindOfferIds(goodType){
	var packet = new AJAXPacket("/npage/s1104/getGdNumBindOfferIds.jsp","请稍后...");
...
{
    //得到帐户ID
  var getAccountId_Packet = new AJAXPacket("/npage/s1104/f1100_getId.jsp","正在获得帐户ID，请稍候......");
...
function dogetContractNo(packet){
      ...
      var retMessage = packet.data.findValueByName("retMessage");	
     	...
     	if(retCode=="000000")
    	{
    	    var getAccountId_Packet = new AJAXPacket("/npage/s1104/ajaxContractNo.jsp","正在获得帐户ID，请稍候......");
...
function getMemberId(){
	var packet = new AJAXPacket("/npage/s1104/getMemberId.jsp","请稍后...");
...
function cfmOfferf(){
	...
	$("#div_offerComponent :checkbox").each(
	 ...
	 var myPacket = new AJAXPacket("/npage/s1104/f1104Ajax.jsp", "正在提交，请稍候......");
...
function ajaxOrderCmtChk(){
 ...
 //   	giveliyana="206"+giveliyana.substring(3,giveliyana.length);
  //  }
   ...
   var myPacketsd = new AJAXPacket("/npage/s1104/ajaxWebOpen.jsp","正在验证是否是网上开户，请稍候......");
 ...
 var  orderCmtChk_Packet = new AJAXPacket("/npage/s1104/ajaxOrderCmtChk.jsp","正在获得帐户ID，请稍候......");
	...
	if(nuCardType=="1"){
		...
		//selectSql = "select count(*) from srescode where res_code = :iSimType and brand_code = 'K06'";
		selectSql = "46";
		params =  "iSimType="+simType;
	}else{
		//实卡开户
		var simCode	= $("#simCode").val().trim();
		//selectSql = "select count(*) from dsimres a,srescode b where a.sim_no = :vSimNo and a.sim_type = b.res_code and b.brand_code = 'K06'";
		selectSql = "47";
		params =  "vSimNo="+simCode;
	}
	
	//alert("params = ["+params+"]"+"\n"+"selectSql = ["+selectSql+"]");
	
	var getServiceType_Packet = new AJAXPacket("/npage/public/pubSelectBySql_new.jsp","正在校验请稍候......");
...
function mySub()
{
	...
	var submitFlag = $("#submitFlag").val();
	 if(submitFlag=="1"){
		 rdShowMessageDialog("人像校验没通过不允许提交!");
		 return false;
	 }
	/*实际使用人姓名*/
	...
	if(document.all.Good_PhoneDate_GSM.style.display == "")
	{
		...
		if("<%=brandID%>"=="101"){
			$("#tab_addprod :checked").each(function(){
				var prodId = this.id;
				if(prodId=="3010"||prodId=="3011"){//判断产品
					var phoneNo = $("input[name='selNum']").val().trim();
					if(phoneNo.substring(0,5) != "20665"){
						//不包含，找到一个就可以退出了
		...
		if(brandID101ProdFlag==2){
			rdShowMessageDialog("38972销售品对物联网用户只允许14765号段订购！");
		 	return false;	
		}
		if(brandID101ProdFlag==1){
			rdShowMessageDialog("3010或3011产品对物联网用户只允许14765号段订购！");
		 	return false;	
		}
...
function checkPwdEasy(pwd) {
	var checkPwd_Packet = new AJAXPacket("/npage/public/pubCheckPwdEasy.jsp","正在验证密码是否过于简单，请稍候......");
...
/*2015/9/7 14:00:05 gaopeng 关于开发新入网客户赠送彩铃、铃音盒及和阅读精品畅读包免费体验的函 
	打印添加标识
*/
	...
	if (retResult == "1") {
		...
		if($("input[name='op_note']").val() == ""){
			if("<%=opCode%>" == "g784"){
				...
				$("input[name='op_note']").val("操作员"+"<%=workNo%>"+"对客户"+"<%=gCustId%>"+"进行校园迎新入网开户!");	
			}
...
function conf(path){
		...
		//document.prodCfm.action="/npage/sm275/fm275ConfirmNew.jsp";
		document.prodCfm.action="/npage/s1104/confm_new.jsp";
...
function printInfo(printType)
{
    ...
    cust_info+="客户地址：	"+"<%=custAddr%>"+"|";
		...
		if("<%=opCode%>" == "g784" || "<%=opCode%>" == "g785" || "<%=opCode%>" == "m028" || "<%=opCode%>" == "m094"){
			...
			if((document.all.modedxpay.value).trim().length!=0){
				opr_info+="办理业务：校园迎新入网开户"+"  "+"操作流水："+document.all.sysAcceptl.value+"   底线消费金额"+document.all.modedxpay.value+"元"+"|";
			}else{
				opr_info+="办理业务：校园迎新入网开户"+"  "+"操作流水："+document.all.sysAcceptl.value+"|";
			}
		}
...
function qrySimType()
{
  var simCode	= $("#simCode").val().trim();
   //得到sim卡类型
  var getAccountId_Packet = new AJAXPacket("/npage/s1104/pubGetSimType.jsp","正在获得sim卡类型，请稍候......");
	getAccountId_Packet.data.add("region_code","<%=regionCode%>");
...
function checkBatchSim(){
		...
		var packet = new AJAXPacket("/npage/s1104/checkBatchSIM.jsp","请稍后...");
...
function showGroup1(offIdv,offerNamev,groupIdv){
	...
	var groupTypeId = groupIdv;
	  ...
	  var packet1 = new AJAXPacket("/npage/s1104/showGroup.jsp","请稍后...");
...
function ignoreThis(){
	...
	if(rdShowConfirmDialog("请确认是否取消该业务？ 确认后，业务将被删除")==1){
		var packet1 = new AJAXPacket("/npage/s1104/ignoreThis.jsp","请稍后...");
				packet1.data.add("sOrderArrayId","<%=orderArrayId%>");
...
function getHlr()
{
 ...
 //   	giveliyana="206"+giveliyana.substring(3,giveliyana.length);
  //  }
	...
	//alert(giveliyana);
	var myPacket = new AJAXPacket("/npage/s1104/getHlrCode.jsp","正在提交，请稍候......");
  ...
  <tr>
    <td class="blue" width=17%>品牌</td>
  	<td width=33%>
  		...
  		<input class=b_text id="orderInfoDiv"  type=button value=查看已选择附加销售品信息 />
  	</td>
...
<div id="userBaseInfo" style="display:none">
 	<%@ include file="/npage/s1104/include/fUserBaseInfo.jsp" %>
</div>
</div>
</div>
...
<table cellSpacing=0 id="pt_PhoneDate_GSM"   style="display:none">
	<TR >
								...
								<TD nowrap >
									<input id="ptPhoneDate"  name="ptPhoneDate" maxlength="8"  v_format="yyyyMMdd" v_type="date"  value="<%=dateStr2%>">
									<font class="orange">*(格式YYYYMMDD)</font>&nbsp;&nbsp;
								</TD>
							...
							<tr>
								...
								<td >
									...
									<input type="hidden" name="isFeeTypes" id="isFeeTypes" value="0" >
								</td>
							</tr>	
<% 
...
<SCRIPT language=JavaScript>
	...
	function isGoodPhoneF(selNumValue){
		var isGoddNo_Packet = new AJAXPacket("/npage/s1104/isGoodPhoneres.jsp","正在获得绑定附加资费，请稍候......");
...
<table cellSpacing=0>	
	<TR>
	  ...
	  <input type="hidden" class="b_text" value="获得" id="btn_getConNo" onClick="getContractNo()"> <font class="orange">*</font>  
	  <td></td> <td></td>
	  </TD>
...
<div id="people_info" style="display:none"><!--担保人,经办人等的信息-->
<DIV class=input >
	<table cellSpacing=0>
		<tr>
			...
			<td>
				<select id='khType' name='khType' style='width:90%'  onChange="kaihfss()">
					...
					<option value="0" selected>0-->正常开户</option>
...
<jsp:include page="/npage/public/hwReadCustCardNew.jsp">
	...
	<jsp:param name="idIccid" value="<%=idIccid%>"/>
	<jsp:param name="custName" value="<%=oCustName%>"/>
</jsp:include>
<DIV class=input>
	<table   cellSpacing=0 align="center">
		<tr align="center">
		<td align="center" id="footer"> 
		<input class="b_foot" id="cfmBtn" onClick="mySub()" type=button value=确定&打印 />
		<input class="b_foot" id="cfmOffer" onClick="cfmOfferf()" type=button value=销售品选择确认  style="display:none" />
...
<input type="hidden" id="oCustName" name="oCustName" value="<%=oCustName%>"/>
...
<input type="text" name = "submitFlag" id="submitFlag" value="<%=submitFlag%>"/>
