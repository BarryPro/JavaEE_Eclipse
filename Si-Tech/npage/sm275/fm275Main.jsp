  ...
  if("<%=opCode%>" == "m275"){
  	var checkVal = $("select[name='isJSX']").find("option:selected").val();//个人开户分类 普通客户：0
		...
		if(idTypeSelect.indexOf("|") != -1){
			...
			if(v_idTypeSelect == "0"||v_idTypeSelect == "D"){ //身份证 军官身份证
...
/*下一步按钮，打开fm275To1104.jsp 创建订单，返回跳转页面的信息*/
function nextStepOpen(){
	...
	/*2015/7/8 16:30:35 gaopeng R_CMI_HLJ_guanjg_2015_2310398@关于哈尔滨申请储备开发校园迎新系统的请示
        	调用新增服务sXYYXInnetChk  判断是否可以开户
        */
	  var subFlag = false;
		var myPacket = new AJAXPacket("/npage/sm275/fm275InnetChk.jsp","正在查询信息，请稍候......");
	  ...
	  core.ajax.sendPacket(myPacket,function(packet){
		...
		myPacket = null;	
	if(subFlag){
		//if(rdShowConfirmDialog("确认选择该主资费信息么？")==1) {
...
function check_HidPwd(Pwd1,Pwd1Type,Pwd2,Pwd2Type)
{
  ...
  var checkPwd_Packet = new AJAXPacket("/npage/sq100/pubCheckPwd.jsp","正在进行密码校验，请稍候......");
...
function getInfo_withId()
{
  ...
  else
  {
    ...
    var getIdPacket = new AJAXPacket("/npage/sq100/f1100_rpc.jsp","正在获得上级客户信息，请稍候......");
...
function custInfoQuery(pageTitle,fieldName,sqlStr,selType,retQuence,retToField)
{
    ...
    */
    ...
    var path = "/npage/sq100/pubGetCustInfo.jsp";   //密码为*
...
function checkName(){
  ...
  var checkPwd_Packet = new AJAXPacket("/npage/sq100/f1100_checkName.jsp?custName="+custName,"正在进行客户名称校验，请稍候......");
		...
		*/
		...
		if(idTypeText == "B"){
			if(objValueLength < 12){
					rdShowMessageDialog(objName+"必须大于等于12位字符！");
  	...
  	if($("#isDirectManageCust").val() == "0"){ //否 不是直管客户 
  		if(obj.value != "" && obj.value != null){ //组织机构代码 有值
		  	...
		  	if(isDirectManageCust == "0"){ //否
		  		var packet = new AJAXPacket("/npage/sq100/f1993_ajax_qryDirectManageCustInfo2.jsp","正在获得数据，请稍候......");
...
/*2015/6/30 16:55:51 gaopeng 以下为查询 主资费 信息方法 start*/
function productOfferQryByAttr()
{
	...
	var custGroupId="";
	var channelSegment="";
	var group_id=document.all.receiveRegion.value;//区域编号;
...
function doProductOfferQryByAttr(packet)
{
	...
	if(retResult==null||retResult==""){
			rdShowMessageDialog("获取主资费信息失败！错误代码："+qryRetCode+",错误信息："+qryRetMsg);
			return false;
		}else{
				if(qryRetCode==0){
...
function getServBusId(offer_id){
			var packet = new AJAXPacket("/npage/portal/shoppingCar/getServBusId.jsp","请稍后...");
...
function clickTr()
{
	...
	getServBusId(offerid);
	   ...
	   // var offer_comments=trCur.getElementsByTagName("td")[3].innerHTML.split("|")[1];//销售品描述
	    ...
	    //var band_id = document.all.smCode.value;//品牌标识
	    var band_id = "";//品牌标识
		...
		var serv_bus_id =document.all.servBusId.value;//取得服务类型ID，servBusId
...
function ajaxGetLimitAdd(offerId){
	var packet = new AJAXPacket("/npage/portal/shoppingCar/ajax_LimitAdd.jsp","请稍后...");
...
/*2015/6/30 16:55:51 gaopeng 以下为查询 主资费 信息方法 end*/
</SCRIPT>
      ...
      %>
          ...
          <!--wangzn add 091201 潜在集团升签约 e-->
                  ...
                  <TD width="34%" class="blue" > 
                    ...
                    <font class=orange>*</font> 
                    <input name=custQuery type=hidden class="b_text" onmouseup="getId();" onkeyup="if(event.keyCode==13)getId();" style="cursor:hand" id="accountIdQuery" value=获得 index="7">
                  </TD>
                </TR>
  ...
  <TBODY>
    <TR> 
      <td align=center id="footer"> 
        <input class="b_foot_long" name="print"  onclick="printCommit()" onkeyup="if(event.keyCode==13)printCommit()" disabled="disabled" type=button value="客户开户"  index="39" >
      	<input class="b_foot" name=reset1 type=button  onclick=" window.location.reload();" value=重置 index="40">
    	</td>
     </TR>
  </TBODY>
</TABLE>
	...
	<div id="showAndHideUserOpen" style="display:none">
	  ...
	  <div class="title">
			<div id="title_zi">用户开户主资费信息</div>
		</div>
		<div class="title">
			<div id="title_zi">主资费列表</div>
		</div>
	   ...
	   <tr>
			<td align=center colspan="4" id="footer">
				<input class="b_foot" id="configBtn" name="configBtn"  type="button" value="下一步"   onclick="nextStepOpen();">&nbsp;&nbsp;
			</td>
		</tr>
