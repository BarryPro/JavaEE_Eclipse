...
<%
	...
	String token4a= request.getParameter("token4a");
  ...
  session.setAttribute("token4a",token4a);
	...
	String addressFlag = request.getParameter("addressFlag");
	
 	String strForCheck = null;
 	if (targeturl != null && !"".equals(targeturl)) {
 		strForCheck = user + cDate + targeturl + random;
 	} else {
 		strForCheck = user + cDate + random;
 	}
 	String check2 = getMD5ofStr(strForCheck); 
	
	String uri = request.getParameter("uri");
	if(uri!=null&&!"".equals(uri)&&targeturl!=null&&!"".equals(targeturl)){
		//uri = new String(new sun.misc.BASE64Decoder().decodeBuffer(uri.trim()));
		//uri = uri.replaceAll("%","&");
...
<%
if(str1[0][0].equals("000000")||str1[0][0].equals("900099")||retCode.equals("000000")||retCode.equals("900099"))
{
...
<%
	...
	//增加旧版客服系统需要的session值
	String powerCodekf = request.getParameter("powerCodekf");
	String kfno = request.getParameter("kfno");
	session.setAttribute("powerCodekf", powerCodekf);
	session.setAttribute("kf_login_no", kfno);
	session.setAttribute("kf_no", kfno);
	session.setAttribute("kfWorkNo", kfno);
	
	if (targeturl != null && !"".equals(targeturl)) {
		if(targeturl.indexOf("?")!=-1){
			targeturl = targeturl+"&user="+user+"&date="+cDate+"&url="+url+"&random="+random+"&check="+check+"&activePhone="+activePhone+"&opName="+opName+"&opCode="+opCode;
		}else{
			targeturl = targeturl+"?user="+user+"&date="+cDate+"&url="+url+"&random="+random+"&check="+check+"&activePhone="+activePhone+"&opName="+opName+"&opCode="+opCode;
		}
