...
function productOfferQryByAttr(){
	...
	var packet = new AJAXPacket("/npage/portal/shoppingCar/ajax_productOfferQryByAttr.jsp","请稍后...");
		...
		packet.data.add("opCode" ,"1104");// 临时写死
	...
	if(str.length != 0){
		...
		return str;
			...
			function valiRealUserIdTypes(idtypeVal){
				if(idtypeVal == "0"||idtypeVal == ""){//空值默认身份证
...
function go_getInterThin_Info(){
	if($("#ipt_InterThin_No").val().trim()==""){
		rdShowMessageDialog("请输入物联网号码");
		return;
	}
	
	  var packet = new AJAXPacket("fi067_getInterThin_Info.jsp","请稍后...");
        packet.data.add("opCode","<%=opCode%>");//
      ...
      rdShowMessageDialog(error_code+":"+error_msg);
	    return;
     	...
     	if(retArray.length>0){
	    	$("#custName").val(retArray[0][0]);//客户名称                               /* oCustName          客户名称                */
				
				//如果证件类型没有，证件号码不写
				$("#idTypeSelect").find("option").each(function (){
					if($(this).val().indexOf(retArray[0][1])!=-1){
							$(this).attr("selected","selected");//证件类型                           /* oIdType          	证件类型                */
		    			$("#idIccid").val(retArray[0][2]);//证件号码                                /* oIdIccId          	证件号码                */
					}
				});
				
				
	    	$("#idAddr").val(retArray[0][3]);//证件地址                                 /* oIdAddress         证件地址                */
	    	$("input[name='custAddr']").val(retArray[0][4]); //客户地址                 /* oCustAddress       客户地址                */
	    	$("#idValidDate").val(retArray[0][5]);//证件有效期                          /* oIdValidate        证件有效期	            */

	    	$("input[name='contactPerson']").val(retArray[0][6]);//联系人姓名           /* oContactName       联系人姓名              */
	    	$("input[name='contactPhone']").val(retArray[0][7]);//联系人电话            /* oContactPhone      联系人电话              */
	    	$("input[name='contactAddr']").val(retArray[0][8]);//联系人地址             /* oContactAddress    联系人地址	            */
	    	...
	    	$("input[name='gestoresName']").val(retArray[0][9]);//经办人姓名            /* oOprName          	经办人姓名              */
	    	$("input[name='gestoresIccId']").val(retArray[0][11]);//经办人证件号码      /* oOprIdIccId        经办人证件号码	        */
	    	$("input[name='gestoresAddr']").val(retArray[0][12]);//经办人联系地址       /* oOprAddress        经办人证件地址          */

				if(retArray[0][14]=="0"||retArray[0][14]==""){
					validateresponIdTypes("身份证");
				}else{
	    	...
	    	$("input[name='responsibleName']").val(retArray[0][13]);//责任人姓名         /* oDutyName          责任人姓名              */
	    	$("input[name='responsibleIccId']").val(retArray[0][15]);//责任人证件号码    /* oDutyIdIccId       责任人证件号码	        */
	    	$("input[name='responsibleAddr']").val(retArray[0][16]);//责任人联系地址     /* oDutyAddress       责任人证件地址          */

				//valiRealUserIdTypes(retArray[0][18]);
				
	    	//$("select[name='realUserIdType']").val(retArray[0][18]);//实际使用人证件类型 /* oActualIdType      实际使用人证件类型      */
	    	//$("input[name='realUserName']").val(retArray[0][17]);//实际使用人姓名        /* oActualName        实际使用人姓名          */
	    	//$("input[name='realUserIccId']").val(retArray[0][19]);//实际使用人证件号码   /* oActualIdIccId     实际使用人证件号码	    */
	    	//$("input[name='realUserAddr']").val(retArray[0][20]);//经办人联系地址        /* oActualAddress     实际使用人证件地址      */
    	}
...
%>
                ...
                <TR > 
                  <TD class="blue" > 
                    <div align="left">物联网号码</div>
                  </TD>
                  <TD colspan="3"> 
                    <input type="text" id="ipt_InterThin_No" name="ipt_InterThin_No"      />
                    <input type="button" class="b_text" value="查询" id="btn_InterThin_No" onclick="go_getInterThin_Info()" />
                    </TD>
                </TR>                
...
%>
                ...
                <TR> 
                  ...
                  <TD> 
                    <input type="hidden" name=contactPost class="button" v_type="zip" v_name="联系人邮编" maxlength="6"  index="22" size="20">
                  </TD>
                </TR>
                ...
                <tr>
                	...
                	<td>
                			<select name="smCode" id="smCode" onchange="setOfferType()">
                				<option value="101" selected>101-->物联卡</option>
                			</select>
                	</td>
...
<TABLE cellSpacing="0">
  <TBODY>
    <TR> 
          ...
          <input class="b_foot" name=reset1 type=button  onclick=" window.location.href='/npage/si064/fi067_main.jsp?opCode=<%=opCode%>&opName=<%=opName%>';" value=清除 index="40">
