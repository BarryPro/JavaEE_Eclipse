    ...
    $("#old_offer_table").find("input[type='hidden']").each(function(){
    		var t_v_CodeName = $(this).attr("v_CodeName");//拼节点的名称
				...
				if(t_v_CodeName=="ADD_OFFER_LIST"){
					...
					var t_temp_json={
	                "PHONE_NO": t_PHONE_NO,
	                "EFF_DATE": "<%=n_month_Date%>".substring(0,8),
	                "EXP_DATE": "20500101",
	                "OFFER_ID": $(this).attr("v_BPopedomCode")
					};
		...
		if("<%=j_offer_flag%>"=="Y"){
		    ...
		    "OPERATE_TYPE": "U",//I新增专款 U取消专款 D冲正专款
...
function do_cfm(packet){
   ...
   function showPrtDlg(printType,DlgMessage,submitCfm){  //显示打印对话框 
    ...
    function printInfo(printType){
      ...
      opr_info += "办理业务名称：家庭解散    操作流水: "+"<%=sysAcceptl%>" +"|";
      
      
      if("<%=j_ProdCode%>"=="JTRH"){
      	note_info1 += "备注：|";
				note_info1 += "1、户主号码办理家庭解散后，自次月起不再享受家庭优惠服务，并自动解除与所有家庭成员号码的关联关系。|";
				note_info1 += "2、户主号码办理家庭解散后，如订购了魔百和业务，魔百和将于办理解散当月最后一天的20点自动退订。|";
    	}else if("<%=j_ProdCode%>"=="JTDX"){
				note_info1 += "备注：|";
				note_info1 += "1、户主号码办理家庭解散后，自次月起不再享受家庭优惠服务，并自动解除与所有家庭成员号码的关联关系。|";    		
				note_info1 += "2、户主号码办理家庭解散后，赠送的魔百和互联网电视业务，魔百和将于办理解散当月最后一天的20点自动退订。|";    		
    	}else if("<%=j_ProdCode%>"=="JTHX"){
				note_info1 += "备注：|";
				note_info1 += "1、户主号码办理家庭解散后，自次月起不再享受家庭优惠服务，并自动解除与所有家庭成员号码的关联关系。|";    		
				note_info1 += "2、户主号码办理家庭解散后，赠送的魔百和互联网电视业务，魔百和将立即退订。|";   
			}else if("<%=j_ProdCode%>"=="JTCN"){
				note_info1 += "备注：|";
				note_info1 += "户主号码办理家庭解散后，自次月起不再享受家庭优惠服务，并自动解除与所有家庭成员号码的关联关系。|";    
			}else if("<%=j_ProdCode%>"=="JTHB"){
					note_info1 += "1、户主号码办理家庭解散后，自次月起不再享受家庭优惠（共享主卡流量、家庭成员间本地互打免费）服务，并自动解除与所有家庭成员号码的关联关系。|";    		
					
					if("<%=sp_type%>"=="Y"){
						note_info1 += "2、户主号码办理家庭解散后，如订购了魔百和互联网电视业务，魔百和将于办理解散当月最后一天的20点自动退订。|";
					}
					else{
						note_info1 += "2、户主号码办理家庭解散后，如订购了魔百和互联网电视业务，魔百和将立即退订。|";
					}
    	}else if("<%=j_ProdCode%>"=="JTFX"){
				note_info1 += "备注：|";
				if("D022"=="<%=j_GearCode%>"){ 			
					note_info1 += "1、伴侣卡主卡办理家庭解散后，主套餐转为标准资费，次月1日生效。|"; 
					note_info1 += "2、伴侣卡主卡办理家庭解散后，如订购了魔百和互联网电视业务，魔百和将立即退订。 |"; 
				}else{
					note_info1 += "1、户主号码办理家庭解散后，自次月起不再享受家庭优惠（共享主卡流量、家庭成员间本地互打免费）服务，并自动解除与所有家庭成员号码的关联关系。|";    		
