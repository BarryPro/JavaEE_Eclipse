...
<%!
 ...
 {
 	String fixStrings[]=new String[13+1];
 	fixStrings[0]="黑龙江移动通讯公司集团成员统付关系查询结果";
 	fixStrings[1]="区域：	";
 	fixStrings[2]="客户经理：	";
 	fixStrings[3]="详细信息			";
 	fixStrings[4]="操作日期：		";
 	...
 	try{ 			
		...
		for(int i=0;i<14;i++)
		{
            int fn=10;	//字号
            	...
            	if(i==0){fn=15;column=1;row=1;} //黑龙江移动通讯公司集团成员管理BBOSS处理结果查询————
         		...
         		if(i==5){column=1;row=7;}  	//付费帐号
         		if(i==6){column=3;row=7;}  	//被付费帐号		
         		if(i==7){column=5;row=7;}  	//手机号码 
         		if(i==8){column=7;row=7;}  //开始日期
         		if(i==9){column=9;row=7;}  //开始时间
         		if(i==10){column=11;row=7;}  //结束日期
         		if(i==11){column=13;row=7;}  //结束时间
         		if(i==12){column=15;row=7;}  //全额标志
         		if(i==13){column=17;row=7;}  //金额
       ...
       for(int i=0;i<((String [][])acceptlist.get(0)).length;i++)
       {
            ...
            for( int j=0;j<9;j++)
            {
            		if(j!=7){
	            			if(j==8){
	            				String payMoney=((String [][])acceptlist.get(7))[i][0].trim().equals("1")==true?"--":((String [][])acceptlist.get(j))[i][0];
	            				labelCF = new Label(temp, DataRow, payMoney, wcfF);
		            		  ws.addCell(labelCF);
	            			}else{
            			...
            			String pay=((String [][])acceptlist.get(j))[i][0].trim().equals("1")==true?"全额支付":"定额支付";
            			labelCF = new Label(temp, DataRow, pay, wcfF);
            		  ws.addCell(labelCF);
            		}
            		temp+=2;
						}
...
//读取集团的付费帐号
  ...
  currDate=s.format(c)+"_";
	...
	paraArr[2]=nopass;
	paraArr[3]=org_code;
	paraArr[4]="操作员"+workNo+"对付费帐号为"+QryValues+"集团按统付关系查询导出";
	paraArr[5]=ip_Addr;
	...
	if(errCode1!=0){
		System.out.println("OPCODE:5079导出操作日志记录失败");
