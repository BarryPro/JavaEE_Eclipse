 	...
 	if(!return_code.equals("000000"))
	{
		%>
			<script language="javascript">
				rdShowMessageDialog("冲正查询报错!错误代码"+"<%=return_code%>，错误原因："+"<%=return_msg%>");
				//history.go(-1);
		...
		<div class="title">
			<div id="title_zi">查询结果</div>
			  ...
			  <table cellspacing="0" id = "PrintA">
						...
						<%
							...
							for(int i =0;i<ret_val1.length;i++)
							{
								...
								if ((i% 2) == 1)
								{
									%>
										<tr align="center">
											<td class="<%=tbclass%>"><input type="radio" name="radiobutton" value="<%=ret_val1[i][0]%>" <%if(i==ret_val1.length-1){%>checked<%}%> ></td>
											<td class="<%=tbclass%>"><%=ret_val1[i][0]%></td>
											<td class="<%=tbclass%>"><%=ret_val1[i][1]%></td>
											<td class="<%=tbclass%>"><%=ret_val1[i][2]%></td>
										</tr>
								...
								else
								{
									%>
										<tr align="center">
											<div align="center">
												<td class="<%=tbclass%>"><input type="radio" name="radiobutton" value="<%=ret_val1[i][0]%>" <%if(i==ret_val1.length-1){%>checked<%}%>>
						</td>
												<td class="<%=tbclass%>"><%=ret_val1[i][0]%></td>
												<td class="<%=tbclass%>"><%=ret_val1[i][1]%></td>
												<td class="<%=tbclass%>"><%=ret_val1[i][2]%></td>
				  ...
				  <tr id="footer"> 
					<td colspan="9">
					  <input class="b_foot" name=query onClick="doQry()" type=button value=冲正>
					 
					  <input class="b_foot" name=back onClick="window.location.href='zg07_1.jsp?activePhone=<%=phone_no%>'" type=button value=返回>
					</td>
				  </tr>
...
<script language="javascript">
			function doQry()
			{
				...
				if(totaldate!=s_now)
				{
					rdShowMessageDialog("非当日交易不可以冲正!");
					return false;
				}
				else
				{
					document.frm.action="zg07_3.jsp?login_accept="+login_accept+"&totaldate="+totaldate+"&phone_no="+"<%=phone_no%>"+"&s_sys="+s_sys;
					document.frm.submit();
				}
				/*
				document.frm.action="zg07_3.jsp?login_accept="+login_accept+"&totaldate="+totaldate+"&phone_no="+"<%=phone_no%>"+"&s_sys="+s_sys;
