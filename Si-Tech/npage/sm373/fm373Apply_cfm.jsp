...
<%
			/* 拼接文件名 */
				...
				String sysAccept = "";
				sysAccept = seq;
			    System.out.println("# fm373Apply.jsp  @@@@@@@@@@     - 流水："+sysAccept);
...
%>
					...
					window.location='fm373Apply.jsp?opCode=<%=opCode%>&opName=<%=opName%>';
					</script>
<%
				}
				try{ 
...
<%
System.out.println("====liangyl====fm373Apply_cfm.jsp====sm373Cfm==== filename = " + filename);
System.out.println("====liangyl====fm373Apply_cfm.jsp====sm373Cfm==== ip = " + ip);
				} catch (Exception e){
					%>
										<SCRIPT language=javascript>
										rdShowMessageDialog("只允许上传.txt类型文本文件",0);
...
<%
	...
	%>
	<script language="javascript">
		rdShowMessageDialog("fm373Apply：<%=retcode%>,<%=retmsg%>.");
		//window.history.go(-1);
		window.location.href="fm373Apply.jsp?opCode=<%=opCode%>&opName=<%=opName%>";
	</script>
	<%
}
%>