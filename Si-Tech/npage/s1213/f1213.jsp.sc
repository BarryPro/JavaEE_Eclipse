...
<%
    ...
    String display2 = "style = \"display:none\"";//“报停(1213)”，此业务下线@2014/1/15 
    System.out.println("display="+display);
    String accountTypeTrim = accountType.trim();/*去空格*/
%>
...
<%
	...
	***/
	 /***begin  根据开机时间，获取停机保号(新)到期月份的下一个月的第一天：此种报停的时间由现在的“报停6个自然月”调整为“报停7个自然月”。@2013/1/17 ***/
	  String addMonthsStr = "";
	  String addMonthsY = "";
	  String addMonthsM = "";
...
<%
    ...
    /***end  根据开机时间，获取停机保号到期时间@2013/1/17 ***/
    ...
    ***/
    ...
    if (cfmFlag == 0) {
        ...
        if(document.frm.opCode.value=="m394"){
        	var myPacket = new AJAXPacket("getTx.jsp", "正在提交，请稍候......");
        	myPacket.data.add("iLoginAccept", document.frm.loginAccept.value);							//流水              
...
function submitCfm() 
{
	...
	/***begin diling add for 停机保号(新)(g073)时，进行提示@2014/1/15*/
	if(op_code=="g073"){
		rdShowMessageDialog("为避免用户预存款不足导致报停期间因欠费被拆机，请提醒用户至少预交大于本月底线、6个月停机保号费、未出帐话费总额的预存款。<%=("2".equals(accountType)?"客户报停后如可能操作预销，请提醒客户报停前先取消增值业务，避免办理报停业务后因增值业务未取消而无法办理预销。":"")%>");		
		if("<%=accountTypeTrim%>" == "2"){
			var promptPhoneNo = $.trim($("#promptPhoneNo").val());
			if(promptPhoneNo.length == 0){
				rdShowMessageDialog("请填写欠费提醒号码！");
				return false;
			}
	...
	if(op_code=="m394"){
		rdShowMessageDialog("为避免用户预存款不足导致报停期间因欠费被拆机，请提醒用户预交6个月的停机保号费用。");		
		if("<%=accountTypeTrim%>" == "2"){
			var promptPhoneNo = $.trim($("#promptPhoneNo").val());
			if(promptPhoneNo.length == 0){
				rdShowMessageDialog("请填写欠费提醒号码！");
	...
	/***begin diling add for 报停时，校验欠费手机信息格式@2012/9/6*/
    ...
    if (flag == 1) {
     ...
     if (printFlag != 1 && printFlag != 2) {
	    ...
	    if((pubSmCode=="PA"||pubSmCode=="PB") && op_code != "1214" && op_code != "1215")
			{
			    ...
			    myPacket.data.add("iTransConId", document.frm.vConID.value);                    //转存帐户          
			    myPacket.data.add("iTransMoney", document.frm.backPrepay.value);                //转存金额          
			    myPacket.data.add("iTransPhoneNo", document.frm.vPhoneNo.value);                //转存手机号        
			    myPacket.data.add("iTurnPhoneNo", "");                      										//转赠资源号码      
			    myPacket.data.add("iTurnType", "");                                      				//转赠资源号码类型  
			    myPacket.data.add("iRemindPhone", $("#promptPhoneNo").val());                  	//欠费提醒号码    
	    ...
	    //	alert("aaaa");
	    	var myPacket = new AJAXPacket("../sm394/fm394Cfm.jsp", "正在提交，请稍候......");
	    	myPacket.data.add("iLoginAccept", document.frm.loginAccept.value);							//流水              
		    myPacket.data.add("iChnSource", document.frm.iChnSource.value);									//渠道标识          
		    myPacket.data.add("iOpCode", document.frm.opCode.value);												//操作代码          
		    myPacket.data.add("iLoginNo", document.frm.workNo.value);												//操作工号	        
	    ...
	    myPacket.data.add("promptPhoneNo", $("#promptPhoneNo").val());/*diling add for 欠费提醒号码@2012/9/6 */
	    core.ajax.sendPacket(myPacket);
...
function clickRadio(){
  /*** begin diling add for 报停(1213)，增加一个欠费提醒号码的输入框，不是必须输入@2012/9/6 ***/
  var radiobuttonValue =$("input[@name=radiobutton][@checked]").val(); 
  if((radiobuttonValue=="1213")||(radiobuttonValue=="g073"&&"<%=accountTypeTrim%>"!="2")){/* 客服工号，办理停机保号时，不展示号码 */
...
function printInfo(printType)
{
  ...
  {       
      ...
      for (var i = 0; i < document.frm.radiobutton.length; i ++) {
        if (document.all.radiobutton[i].checked)
        {
          ...
          {	
              ...
              opr_info += "用户品牌：" + document.frm.smName.value + "*" + "办理业务：停机保号(新)" + "|";
              opr_info += "操作流水：" + document.frm.loginAccept.value + "|";
              opr_info += "用户资费：" + document.frm.productName.value + "|";
              /*** 关于哈分公司申请优化报停业务工单的请示 @2014/6/20 ***/
              if($("#promptPhoneNo").val()!=""){
			    opr_info += "欠费提醒号码：" + $("#promptPhoneNo").val() + "|";
			  }else{
			  	opr_info += "客户未设置欠费提醒号码" + "|";
			  }
			/*** 关于哈分公司申请优化报停业务工单的请示 @2014/6/20 ***/
          }else if (radio1 == "m394"){
              opr_info += "办理业务：连续报停" + "|";
      ...
      if(printOpCode=="g073"){/*停机保号的免填单*/
        note_info1+="备注："+"温馨提示：报停当月固定费用正常收取，报停费每月5元，报停有效期到<%=addMonthsY%>年<%=addMonthsM%>月1日，有效期之后手机号码将自动恢复为正常状态，每月收取相应的月租等费用。"+"|";
      }
      else if(printOpCode=="m394"){
    	  note_info1+="备注："+"温馨提示：报停费每月5元，报停有效期到<%=addMonthsY%>年<%=addMonthsM%>月1日，有效期后如号码无欠费将自动恢复为正常状态，每月收取相应的月租等费用。"+"|";
      }else{
        note_info1+="备注："+"|";
      }
...
<%/*** begin diling add for 增加停机保号(功能) @2012/9/7 ***/%>
<td >
    ...
    int tmd4 = 0;
     ...
     for(int i = 0 ; i < fee.length ; i ++){
        ...
        if(fee[i][0].equals("g073")){
          tmd4 = 1;
        System.out.println("g073数据库中有值"+fee[i][2]+fee[i][3]);
	...
	document.frm.handFee.disabled=false;
            <%}}%>
            <%if(tmd44==0){%>document.frm.handFee.disabled=true;<%}%> noBackDiv.style.display='none';backDiv.style.display='none';document.frm.sysRemark.value='停机保号(新)变更';document.frm.opCode.value=<%=fee[i][0]%>;document.frm.handFee.value=<%=fee[i][2]%>;document.frm.handFeeT.value=<%=fee[i][2]%>;"
    <%
    ...
    %>
           onclick="noBackDiv.style.display='none';backDiv.style.display='none';document.frm.handFee.disabled=true;document.frm.opCode.value='g073';document.frm.handFee.value='0';document.frm.handFeeT.value='0';document.frm.sysRemark.value='停机保号(新)变更';beforePrompt();clickRadio();"
...
<%/*** begin liangyl add for 连续报停 @2016/8/20 ***/%>
<td >
    ...
    int tmd5 = 0;
     ...
     for(int i = 0 ; i < fee.length ; i ++){
        ...
        if(fee[i][0].equals("m394")){
          tmd5 = 1;
        System.out.println("m394数据库中有值"+fee[i][2]+fee[i][3]);
    %>
           onclick="beforePrompt(); 
            <%
		for(int j = 0 ;  j< favInfo.length ; j ++){
	...
	document.frm.handFee.disabled=false;
            <%}}%>
            <%if(tmd55==0){%>document.frm.handFee.disabled=true;<%}%> noBackDiv.style.display='none';backDiv.style.display='none';document.frm.sysRemark.value='连续报停';document.frm.opCode.value=<%=fee[i][0]%>;document.frm.handFee.value=<%=fee[i][2]%>;document.frm.handFeeT.value=<%=fee[i][2]%>;"
    <%
    }}}if(tmd5==0){
    //System.out.println("不执行");
    %>
     onclick="noBackDiv.style.display='none';backDiv.style.display='none';document.frm.handFee.disabled=true;document.frm.opCode.value='m394';document.frm.handFee.value='0';document.frm.handFeeT.value='0';document.frm.sysRemark.value='连续报停';beforePrompt();clickRadio();"
    <%
    }
    %>
      >连续报停
</td>
<%/*** end liangyl add for 连续报停 @2016/8/20 ***/%>
<%} else {%>
...
<td>
    <input type="radio" name="radiobutton" index="0" value="g073"
           onclick="noBackDiv.style.display='none';backDiv.style.display='none';document.frm.opCode.value='g073';document.frm.handFee.value='0';document.frm.handFeeT.value='0';document.frm.sysRemark.value='停机保号(新)变更';clickRadio();">停机保号(新)
</td>
<%/*** end diling add@2012/9/7 ***/%>
<%/*** begin liangyl add for 连续报停 @2016/8/20 ***/%>
<td>
    <input type="radio" name="radiobutton" index="0" value="m394"
           onclick="noBackDiv.style.display='none';backDiv.style.display='none';document.frm.opCode.value='m394';document.frm.handFee.value='0';document.frm.handFeeT.value='0';document.frm.sysRemark.value='连续报停';clickRadio();">连续报停
</td>
<%/*** end liangyl add for 连续报停 @2016/8/20 ***/%>
<!--
<td>
...
<tr style="display:none">
    ...
    <tr id = "rowPromptPhone"  style="display:none">
      <td class="blue">欠费提醒号码</td>
      <td colspan= "5">
        <input id="promptPhoneNo" type=text size=17  name="promptPhoneNo" v_type="mobphone" v_name="欠费提醒号码"  maxlength="11" />
      </td>
    </tr>
    <tr>
        ...
        <TR>
            <TD id="footer">
                <input class="b_foot" name=submit type=button value="确认" index="10"
                       ...
                       disabled>
                &nbsp;&nbsp;
                <input class="b_foot" name=back type=button value="清除" onclick="window.location='f1213.jsp?activePhone=<%=activePhone%>&opCode=<%=opCode%>&opName=<%=opName%>'">
                &nbsp;&nbsp;
