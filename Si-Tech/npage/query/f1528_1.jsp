...
function doCheck2()
{
	...
	else//不需要密码
	{
		...
		var str = window.showModalDialog('../query/f1528_getCount.jsp?phoneNo='+document.frm1527.phoneNo.value,"",prop);
...
function doCheckPwd(packet) 
{
		...
		else 
		{
			...
			var l=screen.availWidth/2-w/2;
			var prop="dialogHeight "+300+"px; dialogWidth "+500+"px; dialogLeft "+l+"px; dialogTop "+t+"px;toolbar no; menubar no; scrollbars yes; resizable no;location no;status no";
			var str = window.showModalDialog('../query/getCount.jsp?phoneNo='+document.frm1527.phoneNo.value,"",prop);
    ...
    <TR> 
        ...
        <TD>
            <select size="1" name="otherFlag" onchange="changsearchtype()">
                ...
                <!--
				<option class=button value=1>帐户号码</option>
				-->
            </select>
        </TD>
...
%>
    ...
    <TR> 
        ...
        <TD>
            <input type="text" name="phoneNo" size="20" maxlength="11" onKeyDown="if(event.keyCode==13){ return query() }"   onKeyPress="return isKeyNumberdot(0)"  >
            <input type="button" class="b_text" name="Button1" value="查询帐号" onclick="doCheck2()">
        </TD>
        <TD class=blue>帐户号码 <input type="text" name="contractNo" readOnly size="20" maxlength="20"></TD>  
    </TR>
    <TR> 
