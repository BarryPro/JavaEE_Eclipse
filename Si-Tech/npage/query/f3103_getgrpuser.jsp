...
if(user_no == null){
    ...
    String sqlStr1 = "select * from (SELECT a.id_iccid, a.cust_id, TRIM (b.unit_name), c.id_no,decode(c.run_code,'A','正常','I','预销','a','销户',c.run_code),TRIM (c.user_name), c.product_code, d.product_name, b.unit_id, c.account_id, g.sm_name,c.user_no,h.apn_no,rownum id FROM dCustDoc a, dbvipadm.dGrpCustMsg b, dGrpUserMsg c, sProductCode d,sSmProduct f,sSmCode g,dGrpApnMsg h WHERE c.product_code = d.product_code AND a.cust_id = b.cust_id and b.cust_id = c.cust_id AND d.product_level = 1 AND d.product_status = 'Y' and f.product_code=d.product_code and g.sm_code=f.sm_code and g.region_code=c.region_code AND c.id_no=h.id_no(+) and c.REGION_CODE='"+regionCode+"'  " + sqlFilter + " ) where id <"+iEndPos+" and id>="+iStartPos;
