...
function doCheck()
{
	...
	if(document.frm1527.searchType.selectedIndex == 0)//选择的是按时间范围
	{
		...
		*/
		if(monthCheck(inbegintime.substr(0,6),nowDate.substr(0,6),8)==false)
		{
			rdShowMessageDialog("只允许查询最近6个月的详单!");
			return false;
		}
	...
	//alert("test "+$('#searchTime').val());
	if((parseInt(bt.substring(0,6)) >= <%=new_date%> && document.frm1527.searchType.selectedIndex == 0) || (document.frm1527.searchType.selectedIndex == 1 && parseInt($('#searchTime').val()) >= <%=new_date%> )) {
		document.frm1527.action="fDetQryX.jsp?qryType="+document.frm1527.detTypeNew.value+"&qryName="
			+document.frm1527.detTypeNew.options[document.frm1527.detTypeNew.options.selectedIndex].text;
	}else {
		document.frm1527.action="fDetQryZ.jsp?qryType="+document.frm1527.detType.value+"&qryName="
    ...
    // liujian 2012-3-22 10:29:24 设置详单类型显示内容
					...
					if(document.frm1527.searchType.selectedIndex == 0) {
						...
						if(bt.length == 8) {
							if((parseInt(bt.substring(0,6)) >= <%=new_date%> && document.frm1527.searchType.selectedIndex == 0) || (document.frm1527.searchType.selectedIndex == 1 && parseInt($('#searchTime').val()) >= <%=new_date%> )) {
								...
								$('#orderTypeNew').css('display','block');
							}else {
								$('#orderType').css('display','block');
								$('#orderTypeNew').css('display','none');
							}
						}else {
						 ...
						 try {
							 if(parseInt($('#searchTime').val()) >= <%=new_date%>) {
								 	$('#orderType').css('display','none');
									$('#orderTypeNew').css('display','block');
							 	}else {
							 		$('#orderType').css('display','block');
...
<%
	...
	String begin_ym="";
	String end_ym ="";
	String fs_SQL="SELECT to_char(add_months(SYSDATE, -1), 'yyyymm') ||"+
		"        trim(to_char(cycl_beg_day, '09')),"+
		"       to_char(SYSDATE, 'yyyymm') || trim(to_char(cycl_beg_day, '09'))"+
		"  FROM Dcustacctcycle"+
		" WHERE phone_no = :phoneNo"+
		"   AND to_number(to_CHAR(SYSDATE, 'yyyymmdd')) >= begin_cycle "+
		"   AND to_number(to_CHAR(SYSDATE, 'yyyymmdd')) <  end_cycle" ;
...
<wtc:service name="TlsPubSelCrm" routerKey="phone" routerValue="<%=phoneNo%>" retcode="fs_retCode" retmsg="fs_retMsg" outnum="2" >
	<wtc:param value="<%=paraIn[0]%>"/>
	<wtc:param value="<%=paraIn[1]%>"/> 
</wtc:service>
<wtc:array id="fs_result" scope="end"/>
	
<%
	if( fs_result.length == 0 )
	{
%>
	<script language="JavaScript">
		rdShowMessageDialog("<%=fs_retCode%><br>非分散账期用户，请确认",0);
		removeCurrentTab();
	</script>
<%
	}
	...
	begin_ym=fs_result[0][0];
	end_ym=fs_result[0][1];
%>
...
<FORM method=post name="frm1527">
    ...
    <TABLE cellSpacing="0">
        ...
        <tr>
            ...
            <TD colspan="3">
                <select name="searchType" onchange="seltimechange()">
                    <option value="0" selected>分散账期</option>
                </select>
            </TD>
        </TR>
        ...
        <TR style="display:''" id="IList1">
            ...
            <TD width=34%>
                <input type="text" name="beginTime" id="beginTime" size="20" maxlength="8" value=<%=begin_ym%>>
            </TD>
            <TD class="blue" width=16%>结束日期</TD>
            <TD width=34%>
                <input type="text" name="endTime" id="endTime" size="20" maxlength="8" value=<%=end_ym%>>
            </TD>
        </TR>
        ...
        <tr>
            ...
            <TD>
                ...
                <!-- liujian 2012-3-22 9:48:32 添加详单新类型 begin-->
                <select align="left" name="detTypeNew" width=50 index="4" id="orderTypeNew" style="display:none">
                	  <%
                    if("e155".equals(opCode)){
                    %>
                    <option value="701">铁通宽带</option>
                    ...
                    %>
                    <option value="0">全部</option>
                    <option value="71">套餐及固定费详单</option>
                    <option value="72">语音详单</option>
                    <option value="73">可视电话详单</option>
                    <option value="74">短/彩信详单</option>
                    <option value="75">上网详单(包括GPRS和WLAN)</option>
                    <option value="76">自有增值业务详单</option>  
                    <option value="77">集团业务扣费详单</option>
                    <option value="78">SP代收费详单</option>
                    <option value="79">其他扣费详单</option>
                    <option value="80">减免费用</option>
                    <%}%>                   
                </select>
                <!-- liujian 2012-3-22 9:48:32 添加详单新类型 end-->
            </TD>
