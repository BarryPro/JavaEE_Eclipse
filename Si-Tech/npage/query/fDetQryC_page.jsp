	  ...
	  <script type="text/javascript" src="/npage/public/printExcel.js"></script>	
		...
		<script language="javascript">
    	 /*  liujian  2012-3-14 9:06:36  添加详单js方法 begin */
					  ...
					  // 首页传递first，下一页传递next，上一页传递prev，尾页传递last
						$('#firstPage').click(function() {
							 ...
							 if(parseInt(currPage) > 1) {
							 		location ="fDetQryC_page.jsp?toPage=1&pageCount=<%=pageCount%>&lines_one_page=<%=lines_one_page%>&outFile=<%=outFile%>&phoneNo=<%=phoneNo%>&opCode=<%=opCode%>&opName=<%=opName%>";
								}
						});
				...
				function gohome()
				{
					location="f1863_2.jsp?activePhone=<%=phoneNo%>&opCode=<%=opCode%>&opName=<%=opName%>";
				}
