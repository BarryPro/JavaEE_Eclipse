...
request.getParameter("islowcust");
  //yuanqs add 2010/8/31 18:33:04 详单水印需求
          ...
          <tr> 
           <font class="orange">
		    <td> 共有 <%=(Integer.parseInt(icount))/pageSize + ((Integer.parseInt(icount)) % pageSize == 0 ? 0 : 1)%> 页，当前页为每 <%=beginLine/pageSize + 1%> 页 </td>
              <%if (beginLine != 0) {%>
                  <td><a href="fDetQry4.jsp?islowcust=<%=islowcust%>&qryName=<%=qryName%>&icount=<%=icount%>&outFile=<%=outFile%>&beginLine=0&phoneNo=<%=phoneNo%>&opCodeFlag=<%=opCode%>&opNameFlag=<%=opName%>" >【第一页】 </a> </td>
                  <td><a href="fDetQry4.jsp?islowcust=<%=islowcust%>&qryName=<%=qryName%>&icount=<%=icount%>&outFile=<%=outFile%>&beginLine=<%=beginLine - pageSize%>&phoneNo=<%=phoneNo%>&opCodeFlag=<%=opCode%>&opNameFlag=<%=opName%>" >【上一页】 </a> </td>
              <%}%>
                            
              <%if (beginLine + pageSize < Integer.parseInt(icount)) {%>
                  <td><a href="fDetQry4.jsp?islowcust=<%=islowcust%>&qryName=<%=qryName%>&icount=<%=icount%>&outFile=<%=outFile%>&beginLine=<%=beginLine + pageSize%>&phoneNo=<%=phoneNo%>&opCodeFlag=<%=opCode%>&opNameFlag=<%=opName%>" >【下一页】 </a> </td>
				  
				  <td><a href="fDetQry4.jsp?islowcust=<%=islowcust%>&qryName=<%=qryName%>&icount=<%=icount%>&outFile=<%=outFile%>&beginLine=<%=(((Integer.parseInt(icount))/pageSize + ((Integer.parseInt(icount)) % pageSize == 0 ? 0 : 1)-1)*pageSize)%>&phoneNo=<%=phoneNo%>&opCodeFlag=<%=opCode%>&opNameFlag=<%=opName%>" >【最后一页】 </a> </td>
