...
System.out.println("--liujian---qryType=" +  qryType + "---billTime=" + billTime);
    	...
    	<script language="JavaScript">
    		function doCheckPwd(packet) {
				...
				if (retResult != "000000") {
					...
					window.location="e796_1.jsp?activePhone=<%=phoneNo%>&broadPhone=<%=broadPhone%>&opCode=<%=opCode%>&opName=<%=opName%>";
				}
			}
...
<%
    ...
    String tline = null;
    File temp = null;
    System.out.println("---liujian---billCanView=" + billCanView);
    if (billCanView) {
        ...
        try {
            /* ningtn 铁通宽带 */
	            ...
	            paramterString = "0" + " " + "01" + " " + opCode + " " +
	            								...
	            								dateStr  + " " + "26 ";
	            System.out.println("---liujian--1-paramterString=" + paramterString);
	            ...
	            System.out.println("---liujian---2--paramterString=" + paramterString);
	            outFile = phoneNo + qryType + dateStr1 + ".txt";
            ...
            Runtime.getRuntime().exec(exePath);
        			...
        			System.out.println("---liujian---3--paramterString=");
            ioe.printStackTrace();
%>
			<script language="javascript">
        ...
        <tbody>
            <tr>
                <font class="orange">
                    ...
                    %>
                    <td><a href="e796_3.jsp?islowcust=<%=slowcust%>&qryName=<%=qryName%>&icount=<%=icount-1%>&outFile=<%=outFile%>&beginLine=<%=beginLine + pageSize%>&phoneNo=<%=phoneNo%>&broadPhone=<%=broadPhone%>&opCode=<%=opCode%>&opName=<%=opName%>">【下一页】 </a>
                    </td>

                    <td><a href="e796_3.jsp?islowcust=<%=slowcust%>&qryName=<%=qryName%>&icount=<%=icount-1%>&outFile=<%=outFile%>&beginLine=<%=((icount-1)/pageSize + ((icount-1) % pageSize == 0 ? 0 : 1)-1)*pageSize%>&phoneNo=<%=phoneNo%>&broadPhone=<%=broadPhone%>&opCode=<%=opCode%>&opName=<%=opName%>">【最后一页】 </a>
                    </td>
                    <%}%>
                    <td>
                    	跳转到
...
<script language="JavaScript">
    ...
    function setPage(goPage){
			if (goPage == "-1") {
				...
				var dirtPate = "e796_3.jsp?islowcust=<%=slowcust%>&qryName=<%=qryName%>&icount=<%=icount-1%>&outFile=<%=outFile%>&beginLine=" + beginLineVal + "&phoneNo=<%=phoneNo%>&broadPhone=<%=broadPhone%>&opCode=<%=opCode%>&opName=<%=opName%>";
				location = dirtPate;
			}
		}
</script>
