	...
	function doRetRegion(packet){
		...
		function nextStep(){
			...
			*/
			...
			getdataPacket.data.add("ioprtype","1008");
			getdataPacket.data.add("vOldParam","");
			getdataPacket.data.add("vNewParam","");
	  			...
	  			<%
	  				if(result_mail.length > 0 && "000000".equals(retCode_mail)){
	  					for(int i=0;i<result_mail.length;i++){
	  						...
	  						%>
	  							<input type="checkbox" name="tef" disabled value="<%=result_mail[i][0]%>"/><%=result_mail[i][1]%> &nbsp;
	  						<%
	  					}
	  				}
	  			%>
