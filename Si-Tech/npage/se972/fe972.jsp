	  ...
	  if(checkVal == 0){
		...
		function change_idType(){
			...
			/*2016/2/29 11:04:27 gaopeng 
				...
				6）系统需要对“宽带过户(e972)”界面的所有证件信息，联系人信息、客户信息等做合法性校验，即与“GSM过户(1238)”一致。
	...
	if(
      ...
      cust_info+="证件号码：   "+document.all.ic_no.value+"|";
	     ...
	     */
		  ...
		  opr_info+="宽带帐号"+"<%=broadPhone%>"+"由用户"+"<%=(String)custDoc.get(5)%>"+"过户到用户"+document.all.custName.value+"|";
		...
		function continueCfm(){
			document.all.t_sys_remark.value="用户"+jtrim(document.all.cust_name.value)+"宽带过户到"+document.all.custName.value;
			
			if(jtrim(document.all.t_op_remark.value).length==0){
				document.all.t_op_remark.value="操作员<%=workNo%>"+"对用户手机"+jtrim(document.all.srv_no.value)+"进行宽带过户"
			}
			if(jtrim(document.all.assuNote.value).length==0){
				document.all.assuNote.value="操作员<%=workNo%>"+"对用户手机"+jtrim(document.all.srv_no.value)+"进行宽带过户"
			}
			...
			if(typeof(ret)!="undefined"){
				...
				if(ret=="continueSub"){
					if(rdShowConfirmDialog('确认要提交宽带过户信息吗？')==1){
						conf();
					}
				}
			}else{
...
<div id="Operation_Table">
	...
	<table>
      ...
      <TD class="blue" colspan="3">
		...
		<!--2016/2/29 13:20:46 gaopeng 关于协助新增宽带实名制标识及开发经分报表需求的函（BOSS侧）
			增加经办人信息
		 -->
