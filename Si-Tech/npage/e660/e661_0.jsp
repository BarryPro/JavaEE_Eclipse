...
<%
		...
		inParas1[0] = "SELECT to_char(document_accept), region_code, document_name, document_no, "+
			"       to_char(action_flag), to_char(action_time,'yyyymmdd'), return_msg "+
			"  FROM pBackPay_check "+
			" WHERE region_code LIKE :region_code "+
			"   AND to_char(in_time, 'yyyymmdd') BETWEEN :begin_ymd AND :end_ymd AND action_flag = 3"; 
...
{
	document.frm.action="e661_2.jsp?document_accept="+document_accept;
     ...
     <tr> 
		...
		<%
			for( int i=0;i<result.length;i++ )
			{
				...
				v_action_flag			= result[i][4] ;
				v_action_time			= result[i][5] ;
				v_return_msg			= result[i][6] ;
