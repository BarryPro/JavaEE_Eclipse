...
%>
		<script language="javascript">
	 	...
	 	window.location="f6920_1.jsp?opCode=6920&opName=世博会门票付款确认&opr_numb=<%=oprNumb%>&tkt_type=<%=tktType%>&tkt_sum=<%=tktSum%>&tkt_date=<%=tktDate%>&price=<%=payFee%>&commit_type=1";
		</script>
<%		
		return;				 			
	}
...
function printInfo(printType)
{
	...
	if(0 == <%=getType%>)
	{
		...
		opr_info+="收票人详细地址：<%=custAddress%>|";
		opr_info+="邮政编码：<%=postCode%>|";		
		opr_info+="   |";
		<%
		...
		note_info1+="备注："+"|";
		note_info2+="a、本单为取票凭证之一。 "   +"|";                           
		note_info3+="b、指定日世博票仅限指定日期使用。"+"|";
		note_info4+="c、本票不记名、不挂失，请您妥善保管，丢失不补。您可通过受理营业厅或10086查询订单信息。|";
	}
	else
	{
		...
		note_info1+="备注：a、本单为取票凭证之一，需携带收票人有效身份证件及有效取票凭证（二维码或订购单）。 "+"|";
		note_info2+="b、指定日世博票仅限指定日期使用。 "+"|"; 
		note_info3+="c、本单不记名、不挂失，请您妥善保管，丢失不补；如因各种原因导致二维码丢失，客户可凭客户密码通过10086、受理营业厅请求重发二维码。您可通过受理营业厅或10086查询订单信息。"+"|";
		note_info4+="d、营业厅地址：上海市浦东新区耀华路12号，世博园区正门沿浦东南路向耀华路方向步行约10分钟，位于上南路与耀华路的交界处。营业厅联系电话：13916610086|";
	}
