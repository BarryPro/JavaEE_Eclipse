...
%>
  <wtc:service name="sm358Qry" outnum="21" retmsg="msg" retcode="code" routerKey="region" routerValue="<%=regionCode%>">
...
function cen_radio_all(bt){
	...
	if(("<%=j_ProdCode%>"!="JTHX"&&"<%=j_ProdCode%>"!="JTFX")&&"Y"==$(bt).parent().parent().find("input[type='radio'][v_ClssName='魔百和']").attr("v_IsOrder")){
		...
		/*
			 ...
			 不让取消
			 	...
			 	已经添加成员不允许取消此套餐
		*/
 			var v_BPopedomCode_JTHB = $(bt).parent().parent().find("input[type='radio']").attr("v_BPopedomCode");
 	
			$("#old_mem_table tr:gt(0)").each(function(){
							var e_role_code  = $(this).find("td:eq(1)").text().trim();
							var e_mem_number = $(this).find("td:eq(7)").text().trim();
							if("21096"==e_role_code&&"0"!=e_mem_number){
								//找到有普通成员
								if(v_BPopedomCode_JTHB=="57572"){
									rdShowMessageDialog("已经添加成员不允许取消此套餐");
									$(bt).parent().parent().find("input[type='radio']").attr("checked","checked");
									return false;
								}
							}
			});
...
function go_cfm(){
	...
	if("D001"=="<%=j_GearCode%>"){
		j_FX_FEE = $("#span_OfferSum").text().trim();
	}	
			...
			if($(this).is(":checked")){//所有选中的单选框
				...
				if(t_v_IsOrder=="Y"){//默认选中还是选中的，不变化
					...
					if(t_v_CodeName=="ADD_OFFER_LIST"){
						...
						if("52837"==offer_id_t){
							t_temp_json.EFF_DATE="<%=currentDate%>";
						}
						...
						//关于宽带和家系列融合资费升级的资费配置和开发需求（BOSS开发）
						...
						var t_temp_json={"OPERATE_TYPE": "I",//I代表新增 U代表退订
     	 ...
     	 opr_info += "办理业务名称：和家飞享套餐    操作流水: "+"<%=sysAcceptl%>" +"|";
			...
			$("#offer_table").find("input[type='radio']").each(function(){
				if($(this).is(":checked")){//所有选中的单选框
					...
					if("53341"==offer_id_temp&&v_IsOrder_tem!="Y"){
					...
					if("52839"==offer_id_temp&&v_IsOrder_tem!="Y"){
						temp_note_info_52839 = "您已成功办理10元1G省内流量包，套餐24小时内生效，当月可最多叠加办理10次。|";
					}

					if("52837"==offer_id_temp&&v_IsOrder_tem!="Y"){
			...
			if(returnValue=="Y"){
				note_info2 += "1、如单独退订魔百和业务，魔百和退订功能立即生效。|";
			}
			else{
				note_info2 += "1、如单独退订魔百和业务，退订将于当月最后一天的20点生效。|";
			}
...
for(int i=0;i<result_t4.length;i++){
	...
	//流量可选，加下拉框
	if(
			  "JT033".equals(result_t4[i][4])
			||"JT052".equals(result_t4[i][4])
			||"JT061".equals(result_t4[i][4])
			||"JT071".equals(result_t4[i][4])
			||"JT081".equals(result_t4[i][4])
			||"JT149".equals(result_t4[i][4])
		){
		JT033_select = "已订购次数："+result_t4[i][20];
	}else{
		JT033_select = "";
	}
...
%>
	...
	<input  type="radio" name="offer_radio_<%=result_t4[i][4]%>"
														...
														v_EfxfDate='<%=result_t4[i][17]%>'
														v_EfxfxDate='<%=result_t4[i][15]%>'
...
<table cellSpacing="0">
	<tr>
		<td><font class="orange">如取消家庭共享功能，请先在成员关系管理退订成员后才可办理。</font> </td>
	</tr>
</table>
<table cellSpacing="0">
	 <tr>
