...
<%
	...
	System.out.println("f飞飞飞飞飞飞飞飞飞飞飞飞飞飞飞飞飞飞飞飞飞飞飞飞飞飞飞凤飞飞 s_sm_code is "+s_sm_code);
	if(loginAccept==null){
	 %>
	...
	if(bill_opy!=null&&bill_opy.length>0)
	{
		...
		if(return_flag.equals("000000"))
		{
			 ...
			 %>
				...
				//	alert("发票预占成功:"+"<%=ocpy_begin_no%>"+" and bill_code is "+"<%=bill_code%>");
					//history.go(-1);
				</script>
			<%
		}
...
{ 
	...
	printctrl.Print(72, 13, 9, 0, "业务类别:补打<%=result[0][2]%>");
...
function ifprint()
{
  			if("<%=s_invoice_flag%>"=="0")
			{
				...
				{
					document.form.action='../s1300/s1352_printInvoice.jsp?pageOpCode=<%=cnttOpCode%>&pageOpName=<%=cttOpName%>&contractno=<%=contractno%>' + '&payAccept=<%=payAccept%>' + '&total_date=<%=total_date%>' + '&workno=<%=workno%>' + '&returnPage=../zg42/zg42_1.jsp'+'&check_seq=<%=ocpy_begin_no%>'+'&s_flag=N&pay_money=<%=pay_money%>&ocpy_begin_no=<%=ocpy_begin_no%>'+'&phoneno=<%=pageActivePhone%>'+'&bill_code=<%=bill_code%>'; 
...
//	alert("s_flag is "+s_flag);
	if(s_flag=="1")
	{
		rdShowMessageDialog("删除原未打印的预占发票失败!");
		history.go(-1);
	}
	else
	{
		if(s_flag=="0")
		{
			rdShowMessageDialog("删除原未打印的预占发票完成!",2);
			history.go(-1);
		}
		else
		{
			rdShowMessageDialog("删除原未打印的预占发票失败1!");
			history.go(-1);
		}
	}
 }
