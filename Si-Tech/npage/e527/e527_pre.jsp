...
<%
	...
	sql_2=" select * from (select a.phone_no,b.region_name,'网上交费三重礼', to_char(a.PREPAY_FEE),a.total_date ,rownum rn from PCUSTPREGIFT_ERROR a,sregioncode b,dcustmsg c where a.phone_no=c.phone_no and substr(c.belong_code,1,2)=b.region_code and  rownum  <="+(curPage)*page_record ;
	...
	sql_2+=")where rn > "+(curPage-1)*page_record;
	//System.out.println("!!!!!!!!2222222222222!!!!!!!!!!!!!!pre一页的sql "+sql_2);
%> 
...
<%
	...
	{
		%>
		<tr><td align=center colspan=5>  上一页 &nbsp;&nbsp;&nbsp;<a href="e527_next.jsp?phoneNo=<%=phoneNo%>&beginYm=<%=beginYm%>&endYm=<%=endYm%>&cityId=<%=cityId%>&curPage=<%=curPage%>&count=<%=i_count%>">下一页</a>&nbsp;&nbsp;&nbsp;共<%=i_count%>页，当前第<%=curPage%>页  </td></tr>
	<tr>
	...
	{
		%>
		<tr><td align=center colspan=5>  <a  href="e527_pre.jsp?phoneNo=<%=phoneNo%>&beginYm=<%=beginYm%>&endYm=<%=endYm%>&cityId=<%=cityId%>&curPage=<%=curPage-1%>&count=<%=i_count%>">上一页</a>&nbsp;&nbsp;&nbsp;<a href="e527_next.jsp?phoneNo=<%=phoneNo%>&beginYm=<%=beginYm%>&endYm=<%=endYm%>&cityId=<%=cityId%>&curPage=<%=curPage%>&count=<%=i_count%>">下一页</a>&nbsp;&nbsp;&nbsp;共<%=i_count%>页，当前第<%=curPage%>页  </td></tr>
	<tr>
		<td align="center" id="footer" colspan="8">
			<input class="b_foot" name="sure" type="button" value="导出" onClick="printTable(tabList);">
		&nbsp; <input class="b_foot" name=back onClick="window.location='e527_1.jsp'" type="button" value="返回">
