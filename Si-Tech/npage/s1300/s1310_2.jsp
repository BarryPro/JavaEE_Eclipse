...
<%
	...
	//xl add for sunqy关于优化缴费冲正操作功能的函 判断是否是特殊工号 如果是特殊工号不判断 否则每人每月只能冲正一次
	int i_cz_login=0;
	String inParas_cz_login[] = new String[2];
//	workno="aaa1zh";
	inParas_cz_login[0]="select to_char(count(0)) from scz_login_no where cz_login=:s_login_no and to_date(to_char(sysdate,'YYYYMMDD'),'YYYYMMDD') between eff_begin and eff_end";
	inParas_cz_login[1]="s_login_no="+workno;
	int i_phone_cz=0;//号码该月冲正几次
	String inParas_cz_phone[] = new String[2];
	inParas_cz_phone[0]="select to_char(count(0)) from  wpay"+totaldate+" where contract_no=:s_contract_no and op_code='1310'";
	inParas_cz_phone[1]="s_contract_no="+contractno;
	%>
		<wtc:service name="TlsPubSelBoss" routerKey="region" routerValue="<%=orgcode.substring(0,2)%>" retcode="retCodelczPhone" outnum="1">
			<wtc:param value="<%=inParas_cz_phone[0]%>"/>
			<wtc:param value="<%=inParas_cz_phone[1]%>"/>
	   </wtc:service>
		<wtc:array id="result_czPhone" scope="end" />
	<%
		if(result_czPhone.length>0)
		{
			i_phone_cz=Integer.parseInt(result_czPhone[0][0]);
		}
	%>
		<wtc:service name="TlsPubSelBoss" routerKey="region" routerValue="<%=orgcode.substring(0,2)%>" retcode="retCodelcz" outnum="1">
			<wtc:param value="<%=inParas_cz_login[0]%>"/>
			<wtc:param value="<%=inParas_cz_login[1]%>"/>
	   </wtc:service>
		<wtc:array id="result_czlogin" scope="end" />
	<%
	if(result_czlogin.length>0)
	{
		i_cz_login=Integer.parseInt(result_czlogin[0][0]);
		if(i_cz_login==0 && i_phone_cz>0)/*非特殊工号 并且已经冲正过一次 则报错*/
		{
			%>
			<script language="javascript">
				rdShowMessageDialog("此用户当月已办理过一次缴费冲正，若再次办理需使用缴费冲正超级工号操作。");
				history.go(-1);
			</script>
			<%
		}
	...
	int i_ls=1;
	inParas_ls[0]="select to_char(count(0)) from dinvoiceprint"+totaldate +" where login_accept=:s_ls and invoice_flag='1' ";
	inParas_ls[1]="s_ls="+loginaccept;
	%>
	<wtc:service name="TlsPubSelBoss" routerKey="region" routerValue="<%=orgcode.substring(0,2)%>" retcode="retCodels" outnum="1">
    	<wtc:param value="<%=inParas_ls[0]%>"/>
        <wtc:param value="<%=inParas_ls[1]%>"/>
   </wtc:service>
    <wtc:array id="result_ls" scope="end" />
	<%
	System.out.println("");
	if(result_ls.length>0)
	{
		i_ls = Integer.parseInt(result_ls[0][0]);
		i_ls=1; 
		System.out.println("fffffffffffffffffffssssssssssssssssssssssssssss test i_ls is "+i_ls);
	}
	if(i_ls<0)
	{
		%>
			<script language="javascript">
				rdShowMessageDialog("此业务已开具发票，请到zg18作废发票后才可以冲正。");
				history.go(-1);
			</script>
		<%
	}
	...
	else
	{
		%>
		<script language='JavaScript'>
			rdShowMessageDialog("判断冲正工号权限报错!",0);
			history.go(-1);
		 </script>
		<%
	}
