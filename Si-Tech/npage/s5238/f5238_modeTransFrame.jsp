...
<%
	...
	String defSendFlag = request.getParameter("defSendFlag")==null?"":request.getParameter("defSendFlag");
	String errCode="";
    String errMsg="";
	//获取所有的优惠信息
				...
				paramsIn[12]=modeTypeCond+"|"+defSendFlag+"|";
				
				//retArray=callView.callFXService("s5238_ModeQry",paramsIn,"5");	
%>

    <wtc:service name="s5238_ModeQry" outnum="5" retmsg="msg" retcode="code" routerKey="region" routerValue="<%=regionCode%>">
...
function commit()
{
	...
	var power_rights = new Array();
	if(document.all.checkbox1==undefined)
	{
		rdShowMessageDialog("没有需要转换的产品代码！",0);
	...
	else if(document.all.checkbox1.length==undefined)
	{
		...
		{
			document.form1.old_mode_array.value="<%=mode_code%>";
			document.form1.new_mode_array.value=document.all.old_mode.value;
		  	document.form1.mode_flag_array.value=document.all.mode_flag.value;
			document.form1.power_right_array.value=document.all.power_right.value;
		}
		else
		{
			document.form1.old_mode_array.value="<%=mode_code%>";
	...
	else if(document.all.checkbox1.length>0)
	{
		...
		for(var i=0;i<document.all.checkbox1.length;i++)
		{
			...
			{	
				old_modes[j]="<%=mode_code%>";
				new_modes[j]=document.all.old_mode[i].value;
				mode_flags[j]=document.all.mode_flag[i].value;
				power_rights[j]=document.all.power_right[i].value;
				j++;
			}	
		}
		if(j==0)
		...
		document.form1.power_right_array.value=power_rights;
	}
	document.form1.action="f5238_modeTransCfm.jsp";
...
<input type="hidden" name="power_right_array" value="">
      		...
      		<table cellspacing="0">
				...
				%>
						<tr  height="20" >
							...
							<TD width="140"><input type="text" name="old_mode" size="10"  readonly Class="InputGrey"  value="<%=result_t[i][1]%>"></TD>
							<TD width="418"><input type="text" name="old_name" size="30"  readonly Class="InputGrey"  value="<%=result_t[i][2]%>"></TD>
							<TD width="53"><input type="text" name="mode_flag" size="6"   value="<%=result_t[i][3]%>"></TD>
							<TD width="50"><input type="text" name="power_right" size="6"   value="<%=result_t[i][4]%>"></TD>
						</tr>
				<%
						}
					}
