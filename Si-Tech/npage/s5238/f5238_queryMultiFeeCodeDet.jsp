...
<%
	...
	String str="";
	String first_favour_object   = request.getParameter("fee_code");
...
<script>
	...
	function doCommit()
	{
		...
		else if("<%=retListString1.length%>"=="1")   //值为一条时不需要用数组
		{
			...
			{
				window.opener.form1.detail_code.value=document.all.detail_code.value+",";
				//window.opener.form1.acc_detail_name.value=document.all.fee_name.value;
			}
			else
			{
				window.opener.form1.detail_code.value="";
			}
		}
		else   //值为多条时需要用数组
		{
			...
			if(j>0)
			{
				window.opener.form1.detail_code.value=detail_codes+",";
