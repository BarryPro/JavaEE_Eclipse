...
<%
	...
	sqlStr1 ="select mode_code,mode_name, 0 ,region_code,sm_code,to_char(start_time,'yyyymmdd'),to_char(stop_time,'yyyymmdd') from sBillModeCode where rownum < 51 ";
...
<script>
		...
		<%for(int i=0;i < retListString1.length;i++){ %>
			...
			str+='<input type="hidden" name="login_accept" value="<%=retListString1[i][2]%>">';
			str+='<input type="hidden" name="region_code" value="<%=retListString1[i][3]%>">';
			str+='<input type="hidden" name="sm_code" value="<%=retListString1[i][4]%>">';
			str+='<input type="hidden" name="mode_begin_time" value="<%=retListString1[i][5]%>">';
			str+='<input type="hidden" name="mode_end_time" value="<%=retListString1[i][6]%>">';
		...
		function doCommit(){
			if("<%=retListString1.length%>"=="0"){
				...
				if(document.all.num.checked){
					...
					window.opener.form1.mode_begin_time.value=document.all.mode_begin_time.value;
					window.opener.form1.mode_end_time.value=document.all.mode_end_time.value;
					window.close();
				}
				else{
					rdShowMessageDialog("您没有选择产品代码！");
				...
				if(a!=-1){
					...
					window.opener.form1.mode_begin_time.value=document.all.mode_begin_time[a].value;
					window.opener.form1.mode_end_time.value=document.all.mode_end_time[a].value;
					window.close();
				}
				else{
					rdShowMessageDialog("您没有选择产品代码！");
...
function doQuery(){
	...
	this.location="f5238_queryModeCode2.jsp?queryFlag=Y&region_code=<%=region_code%>"+"&smCodeCond="+smCodeCond+"&modeCodeCond="+modeCodeCond+"&modeNameCond="+modeNameCond;
}
