...
<%
	  ...
	  inParas2[4]=s_dis_code;
 
%>
<wtc:service name="bi092Qry" routerKey="region" routerValue="<%=regionCode%>"  retcode="retCode2" retmsg="retMsg2" outnum="13">
...
<%
		...
		if(retCode2.equals("0")||retCode2.equals("000000"))
		{
			...
			allPage = (Integer.parseInt(mainInfo2[0][0])- 1) / 2000 + 1 ;//页数的 这里也要改
			%>
		...
		else if(retCode2.equals("i09292"))
		{
			%>
				<script language="javascript">
					rdShowMessageDialog("工号和查询号码的归属不同，不可以办理!");
					window.location.href="i092_2.jsp";
				</script>
			<%
		}
		else if(retCode2.equals("i09299"))
		{
			%>
				<script language="javascript">
					rdShowMessageDialog("存储过程执行未成功，不可以进行强制预拆操作!");
					window.location.href="i092_2.jsp";
				</script>
			<%
		}
...
<script language="javascript">
	...
	function deletes_all(i_count)
	{
		...
		if (prtFlag==1)
		{
			...
			var url="i092_delete_all.jsp?s_count="+i_count+"&regionCode="+"<%=s_region_code%>"+"&s_dis_code="+"<%=s_dis_code%>";
	...
	function gotos(page,total_1)
	{
		...
		window.location.href="i092_2_qry.jsp?pageNumber="+page+"&s_region_code="+"<%=s_region_code%>"+"&s_dis_code="+"<%=s_dis_code%>"; 
	}
