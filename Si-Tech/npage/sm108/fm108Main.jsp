...
function getFeeTypes(){
  ...
  var getdataPacket = new AJAXPacket("/npage/sq100/fq100GetIdTypes.jsp","正在获得数据，请稍候......");
			...
			getdataPacket.data.add("opName","<%=opName%>");
			getdataPacket.data.add("workChnFlag","<%=workChnFlag%>");
			core.ajax.sendPacketHtml(getdataPacket,resFeeTypes);
			getdataPacket = null;
	
}

function resFeeTypes(data){
	//alert(data);
//找到添加的select
	var markDiv=$("#feeTypes"); 
	//清空原有表格
	markDiv.empty();
	markDiv.append(data);
}
...
{   
     var Str = document.all.idType.value;
      ...
      checkCustNameFunc(document.all.custName,0,1); //校验客户名称是否符合
...
{    
	...
	/*重新校验*/
    		/*客户名称*/
    		if(!checkCustNameFunc(document.all.custName,0,1)){
    			return false;
    		}
    		/*联系人姓名*/
    		if(!checkCustNameFunc(document.all.contactPerson,1,1)){
				...
				/*证件号码*/
				if(!checkIccIdFunc(document.all.idIccid,0,1)){
					return false;
				}
				/*gaopeng 20131216 2013/12/16 19:50:11 关于在BOSS入网界面增加单位客户经办人信息的函 加入经办人信息确认服务前校验 start*/
					/*经办人姓名*/
					if(!checkCustNameFunc(document.all.gestoresName,2,1)){
						return false;
					}
					/*经办人联系地址*/
					...
					/*经办人证件号码*/
					if(!checkIccIdFunc(document.all.gestoresIccId,1,1)){
						return false;
					}
				/*gaopeng 20131216 2013/12/16 19:50:11 关于在BOSS入网界面增加单位客户经办人信息的函 加入经办人信息确认服务前校验 start*/
				/*经办人姓名*/
	...
	if(!checkCustNameFunc(document.all.responsibleName,3,1)){
		return false;
	}
	...
	if(!checkIccIdFunc(document.all.responsibleIccId,2,1)){
						return false;
	}	
...
var jsp_action="fm108UpCfm_new.jsp?opCode=<%=opCode%>&opName=<%=opName%>"+
...
"&endStr="+""+
"&isType="+$("#isType option:selected").val()+
"&isFeeTypes="+$("#isFeeTypes option:selected").val();
...
function changeType(opCode){
	...
	if(opCode == "m109"){
		window.location.href='/npage/sm108/fm108Main.jsp?opCode=m109&opName=批量渠道预开户';
...
function doProductOfferQryByAttr(packet)
{
	...
	if(retCode==0){
				...
				for(var i=0;i<retArray.length;i++){
					var appendStr = "<tr>";
					appendStr += "<td width='8%'>"+"<input type='radio' name='selOrder' value='"+retArray[i][0]+"' />"+"</td>"
              ...
              <TABLE cellSpacing="0" >
                ...
                <tr>
			        		...
			        		<td width=84% colspan="3">
			        			<input type="radio" name="opType"	value="m108" <%if("m108".equals(opCode)){out.print("checked");}%>  onclick="changeType(this.value);"/>m108-批量校园预开户 &nbsp;&nbsp;
			        			<input type="radio" name="opType"	value="m109" <%if("m109".equals(opCode)){out.print("checked");}%>  onclick="changeType(this.value);"/>m109-批量渠道预开户 &nbsp;&nbsp;
			        		</td>
        				</tr>
...
%>
                  ...
                  <TD> 
                    <input name=custName id="custName" value=""  v_must=1 v_maxlength=60 v_type="string"   maxlength="60" size=20 index="9"  onblur="if(checkElement(this)){checkCustNameFunc(this,0,0)}">
                ...
                <tr> 
                  ...
                  <td width=34%> 
                    <input name="idIccid"  id="idIccid"   value=""  v_minlength=4 v_maxlength=20 v_type="string" onChange="change_idType()" maxlength="20"   index="11" value="" onBlur="checkIccIdFunc(this,0,0)">
                 ...
                 <tr>
					...
					<td id="feeTypes">
						<select id="isFeeTypes" name="isFeeTypes" onchange="">
						    <option value="1" selected = "selected">返费规则1</option>
						    <option value="2">返费规则2</option>
						</select>
						<input type="hidden" name="isFeeTypes" id="isFeeTypes" value="1" >
					</td>
				</tr>
                <tr>
					        ...
					        <td colspan="3"> 
					            上传文件文本格式为“手机号码|SIM卡号|密码<%if("m109".equals(opCode)){out.print("|预存");}%>”，示例如下：<br>
					            <font class='orange'>
					            	&nbsp;&nbsp; 13836141518|898600452154261|123321<%if("m109".equals(opCode)){out.print("|10");}%><br/>
					            	&nbsp;&nbsp; 13836141519|898600452154262|123321<%if("m109".equals(opCode)){out.print("|20");}%><br/>
					            	&nbsp;&nbsp; 13836141510|898600452154263|123321<%if("m109".equals(opCode)){out.print("|30");}%><br/>
					            	&nbsp;&nbsp; 13836141511|898600452154264|123321<%if("m109".equals(opCode)){out.print("|40");}%><br/>
					            	&nbsp;&nbsp; 13836141512|898600452154265|123321<%if("m109".equals(opCode)){out.print("|50");}%><br/>
					            	&nbsp;&nbsp; 13836141513|898600452154266|123321<%if("m109".equals(opCode)){out.print("|60");}%>
					            </font>
					            <b>
...
<TABLE cellSpacing="0">
  <TBODY>
    <TR> 
          ...
          <input class="b_foot" name=reset1 type=button  onclick=" window.location.href='/npage/sm108/fm108Main.jsp?opCode=<%=opCode%>&opName=<%=opName%>';" value=清除 index="40">
