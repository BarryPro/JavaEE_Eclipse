...
function getSelNumFamily(){
var myPacket = new AJAXPacket("/npage/creFamily/getSelNumFamily.jsp","正在提交，请稍候......");
		myPacket.data.add("newSmCode",$("#newSmCode").val());
		core.ajax.sendPacket(myPacket,doGetSelNumFamily);
		myPacket=null;	
}
function doGetSelNumFamily(packet){
	var retCode = packet.data.findValueByName("code");
	var retMsg  = packet.data.findValueByName("msg");
	var selNum  = packet.data.findValueByName("selNum");
	if(retCode=="000000"){
		$("#selNum").val(selNum);
		g("b_hlr").disabled = true;
	}else{
		rdShowMessageDialog(retCode+"："+retMsg);
	}
}
</script>
<input name="selNum"  id="selNum" readOnly class="InputGrey" value="" type="text" v_fp="" v_lp=""  id="selNum"  size="16"  maxLength="13" />
<input type="button" name="b_hlr" id="b_hlr" value="获取" class="b_text" onClick="getSelNumFamily()" /> <font class="orange">*</font>
