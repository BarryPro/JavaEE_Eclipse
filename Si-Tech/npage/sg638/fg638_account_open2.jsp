...
<%@ include file="/npage/creFamily/ignoreIn.jsp" %>
...
String tempStrf = "";
    ...
    String brandID = "";
		...
		System.out.println("hejwa---------------orderArrayId---------------------"+orderArrayId);     
    System.out.println("hejwa---------------gCustId---------------------"+gCustId);
    System.out.println("hejwa---------------servOrderId---------------------"+servOrderId);     
    System.out.println("hejwa---------------closeId---------------------"+closeId);
    System.out.println("hejwa---------------prtFlag---------------------"+prtFlag);     
		System.out.println("hejwa---------------offerId---------------------"+offerId);     
		System.out.println("hejwa---------------offerName-------------------"+offerName);     
...
<%	 	
		for(int i=0;i<retOrdAryVal.getUtype("2").getSize();i++){
		 	...
		 	System.out.println("hejwa==============retOrdAryVal.getUtype(2.+"+i+").getValue("+j+")================"+retOrdAryVal.getUtype("2."+i).getValue(j));
%>
			<SCRIPT language=JavaScript>
...
%>
  ...
  <wtc:service name="sUserCustInfo" outnum="30" retmsg="msg" retcode="code" routerKey="region" routerValue="<%=regionCode%>">
		<wtc:param value="0" />
		<wtc:param value="01" />	
		<wtc:param value="<%=opCode%>" />		
		<wtc:param value="<%=workNo%>" />			
		...
		<wtc:param value="g629家庭客户开户获取身份证和地址" />				
...
$(document).ready(function () {
		...
		getSelNumFamily();
		getContractNo();
		$("input[type=password][name=userpwd]").val("123123");
		$("input[type=password][name=userpwdcfm]").val("123123");
		mySub();
});
...
{
	 	var packet1 = new AJAXPacket("/npage/creFamily/getMajorProd.jsp","请稍后...");
...

{
    if(v_hiddenFlag=="Y"){ //当为Y时，进入新版小区代码展示页面
      var packet1 = new AJAXPacket("/npage/creFamily/getOfferAttrNew.jsp","请稍后...");
      packet1.data.add("v_code" ,v_code);
      packet1.data.add("v_text" ,v_text);
    }else{
      var packet1 = new AJAXPacket("/npage/creFamily/getOfferAttr.jsp","请稍后...");
    }
		packet1.data.add("OfferId" ,offerId);
...
function jdugeAreaHidden(offer_id){
  var packet = new AJAXPacket("/npage/creFamily/ajax_jdugeAreaHidden.jsp","请稍后...");
...
{
 		var packet = new AJAXPacket("/npage/creFamily/offerDetailQry_new.jsp","正在加载附加销售品构成明细，请稍后...");
...
{
	var packet2 = new AJAXPacket("/npage/creFamily/getOfferRel.jsp","正在加载附加销售品构成依赖关系,请稍后...");
...
{
	var packet1 = new AJAXPacket("/npage/creFamily/getMasterServType.jsp","请稍后...");
...
{
	var packet2 = new AJAXPacket("/npage/creFamily/getProdAttr.jsp","请稍后...");
...
{
	var packet2 = new AJAXPacket("/npage/creFamily/getProdCompRel.jsp","正在加载附属产品的依赖关系,请稍后...");
	packet2.data.add("majorProductId" ,majorProductId);
 ...
 var kehuids = document.all.contractNo.value;
	 ...
	 if(khtyspes=="1") {
        ...
        return ;
				...
				var packet = new AJAXPacket("/npage/creFamily/checkLastIDNO.jsp","请稍后...");
...
function setGdNumBindOfferIds(goodType){
	var packet = new AJAXPacket("/npage/creFamily/getGdNumBindOfferIds.jsp","请稍后...");
...
function doGetMasterServType(packet)
{
	...
	//alert("mastServerType|"+mastServerType+"\nCDMATypeId|"+CDMATypeId);
	if(error_code == "0"){
...
function chkLimit1(id,iOprType)
{
	...
	var opCode="<%=opCode%>";
	var sendUrl = "/npage/creFamily/limitChk1.jsp";
	var senddata={};
...
function chkLimit(id,iOprType)
{
	...
	var phoneNo="<%=svcInstId%>";
	var opCode="<%=opCode%>";
	var sendUrl = "/npage/creFamily/limitChk.jsp";
	var senddata={};
	senddata["opCode"] = opCode;
	senddata["prodId"] = id;
	senddata["iOprType"] = iOprType;
...
function showDetailProd2(nodeId,nodeName,obj,etFlag){
  ...
  var packet = new AJAXPacket("/npage/creFamily/complexPro_ajax.jsp","请稍后...");//组合产品子产品展示
...
{
	var packet3 = new AJAXPacket("/npage/creFamily/getProdCompDet.jsp","请稍后...");
	packet3.data.add("goodNo" ,document.all.selNum.value); //caohq 20101206 需求 
	packet3.data.add("groupId" ,"<%=groupId%>");
	packet3.data.add("opCode","<%=opCode%>");//weigp add 需求 
...
{
    //得到帐户ID
  var getAccountId_Packet = new AJAXPacket("/npage/creFamily/f1100_getId.jsp","正在获得帐户ID，请稍候......");
...
function dogetContractNo(packet){
      ...
      var retMessage = packet.data.findValueByName("retMessage");	
     	...
     	if(retCode=="000000")
    	{
    	    var getAccountId_Packet = new AJAXPacket("/npage/creFamily/ajaxContractNo.jsp","正在获得帐户ID，请稍候......");
    	    ...
    	    retMessage = retMessage + "[errorCode1:" + retCode + "]";
    			...
    			document.all.btn_getConNo.disabled=false;
					return false;
    	}	
...
function ajaxOrderCmtChk(){
   ...
   var myPacketsd = new AJAXPacket("/npage/creFamily/ajaxWebOpen.jsp","正在验证是否是网上开户，请稍候......");
	myPacketsd.data.add("phoneNo",document.all.selNum.value);
	core.ajax.sendPacket(myPacketsd,checksimtypesd);
	myPacketsd=null;
	if(checkopenflag=="0") {
	   
 var  orderCmtChk_Packet = new AJAXPacket("/npage/creFamily/ajaxOrderCmtChk.jsp","正在获得帐户ID，请稍候......");
...
function mySub()
{
	...
	if(document.all.selNum.value==""){
		rdShowMessageDialog("请取得服务号码" , 0);
		return false;	
	}
	if(document.all.contractNo.value==""){
		rdShowMessageDialog("请取得账号ID" , 0);
		return false;	
	}	
        //getAfterPrompt();
...
function doCheckPwdEasy(packet) {
	...
	if (retResult == "1") {
		...
		if($("input[name='op_note']").val() == ""){
			$("input[name='op_note']").val("操作员"+"<%=workNo%>"+"对客户"+"<%=gCustId%>"+"进行家庭用户开户!");	
		}
		//压入基本销售品
...
function conf(path){
		...
		document.prodCfm.action="fg638_payment2.jsp";
		document.prodCfm.submit();
		
	//	document.prodCfm.action="/npage/creFamily/creFamilyConfm.jsp";
	//	document.prodCfm.submit();
	}
var retResultStr = "";
var retResultStr1 = "";
...
function printInfo(printType)
{
    ...
    cust_info+="家庭号码："+$("input[name='selNum']").val().trim()+"|";
		...
		}else if(document.all.newSmCode.value=="FM"){
			opr_info+="业务受理时间："+cTime+"  "+"用户品牌:"+"家庭用户"+"|";
		}
		
		 
		opr_info+="办理业务：家庭用户开户"+"  "+"操作流水："+document.all.sysAcceptl.value+"|";
...
function setLoginAccept(retVal){
	...
	document.prodCfm.action="/npage/creFamily/confm_new.jsp";
	document.prodCfm.submit();
}
...
function checksim(){
    ...
    */
    ...
    if($("#yz").val() == "Y"){
		...
		var checkResource_Packet = new AJAXPacket("/npage/creFamily/f1104_5.jsp","正在进行资源校验，请稍候......");
...
function qrySimType()
{
  var simCode	= $("#simCode").val().trim();
   //得到sim卡类型
  var getAccountId_Packet = new AJAXPacket("/npage/creFamily/pubGetSimType.jsp","正在获得sim卡类型，请稍候......");
	getAccountId_Packet.data.add("region_code","<%=regionCode%>");
...
function checkBatchSim(){
		...
		var packet = new AJAXPacket("/npage/creFamily/checkBatchSIM.jsp","请稍后...");
...
{ 
   ...
   var path = "/npage/creFamily/f1114_4gn.jsp";
...
function showGroup(offIdv,offerNamev,groupIdv){
	...
	var prop="dialogHeight:"+h+"px;dialogWidth:"+w+"px; dialogLeft:"+l+"px; dialogTop:"+t+"px;toolbar:no; menubar:no; scrollbars:yes; resizable:no;location:no;status:no;help:no";
	if(true){
	var ret=window.showModalDialog("/npage/creFamily/showGroup.jsp?opType=set&offerId="+offerId+"&offerName="+offerName+"&groupTypeId="+groupTypeId+"&brandID="+"<%=brandID%>"+"&curTime="+curDate+"&groutDesc="+document.all.groutDesc.value,"",prop);
		...
		var ret=window.showModalDialog("/npage/creFamily/showGroup.jsp?opType=look&offerId="+offerId+"&offerName="+offerName+"&groupInfo="+groupHash[offerId]+"&groupTypeId="+groupTypeId+"&groutDesc="+document.all.groutDesc.value,"",prop);
...
function showGroup1(offIdv,offerNamev,groupIdv){
	...
	var groupTypeId = groupIdv;
	  ...
	  var packet1 = new AJAXPacket("/npage/creFamily/showGroup.jsp","请稍后...");
...
function showAttribute(){
	...
	if($(this).attr("class") == "but_property"){
		var ret=window.showModalDialog("/npage/creFamily/showAttr.jsp?queryId="+queryId+"&queryType="+queryType,"",prop);
		if(typeof(ret) != "undefined"){
		...
		var ret=window.showModalDialog("/npage/creFamily/showAttr.jsp?queryId="+queryId+"&queryType="+queryType+"&attrInfo="+AttributeHash[queryId],"",prop);
		if(typeof(ret) != "undefined"){
...
function ignoreThis(){
	...
	if(rdShowConfirmDialog("请确认是否取消该业务？ 确认后，业务将被删除")==1){
		var packet1 = new AJAXPacket("/npage/creFamily/ignoreThis.jsp","请稍后...");
				packet1.data.add("sOrderArrayId","<%=orderArrayId%>");
...
function getHlr()
{
    ...
    */
	...
	//alert(giveliyana);
	var myPacket = new AJAXPacket("/npage/creFamily/getHlrCode.jsp","正在提交，请稍候......");
...
<div id="custInfo">
<%@ include file="/npage/creFamily/bd_0002.jsp" %>
</div>
...
<TABLE cellSpacing=0>
  <TR>
    <Td class="blue" width="15%">品牌</td>
  	<TD width=18%><%=brandName%>  
  		<input type="hidden" name="orderInfoV" value="0" />
  		<!--
  		<INPUT class=b_text id="orderInfoDiv"  type=button value=查看已选择附加销售品信息 />
  		-->
  		</TD>
    <Td class="blue"  width="15%">销售品名称</td>
     <TD  id="td_offerName"  colspan="3"><%=(offerId+"-->"+offerName)%> </TD>
	</TR>
	<tr>
    <Td class="blue"  width="15%">描述</Td>
    <TD   colspan="5"><%=offerComments%></TD>
   </TR>
  </table>  
...
<div id="userBaseInfo">
 	<%@ include file="/npage/creFamily/fUserBaseInfo2.jsp" %>
</div>
</div>
</div>
...
<table cellSpacing=0 id="serviceNoInfo">	
	<TR id="tr_serviceNo" >
	  ...
	  <TD  colspan="3"><%@ include file="bd_0022.jsp" %>  
	  </TD>
	   <input  type="hidden" name="hlrCode"  / >
	</tr>
		<TR>
		<td class="blue" >账号ID </td>
	  <TD colspan="3"><input type="text" name="contractNo" id="contractNo" class="InputGrey" size="16" maxlength="14" readonly>
	  <input type="button" class="b_text" value="获取" id="btn_getConNo" onClick="getContractNo()"> <font class="orange">*</font>  
	  </TD>
	</TR>
<input type='hidden' name='simCode' id='simCode' maxlength="20"  value="">
<input name="simTypeName" type="hidden"  readonly index="11" Class="InputGrey">
<!--
	<tr  id="tr_serviceNo1" style="display:none">
	...
	<tr style="display:none" >	
		...
		<td colspan="2" >
			<iframe name="frame_sub_1" id="frame_sub_1" src="/npage/creFamily/getFile.jsp" width="400" height="25" frameborder="0" marginwidth="0" marginheight="0" scrolling="no" >
			</iframe>
		</td>
		<td colspan="2">
...
<table cellSpacing=0 id="Good_PhoneDate_GSM"  style="display:none" >
 ...
 <input type="hidden"  name ="ptPhoneFlag"  value="Y" />
 <input type="hidden"  name="ptPhoneDate"  id="ptPhoneDate"   maxlength="8"   />
 
<DIV class="input" id="productInfo" >
	<br>
	 

<div id="majorProdRel" style="display:none" ></div> 

<div id="prodAttribute" style="display:none" ></div> <!--产品属性
...
-->
<div id="serviceOrderInfo" style="display:none">
...
<DIV class=input>
	<table cellSpacing=0>
		<tr>
			<td  class="blue"   width="15%">开户方式</td>
			<td   colspan="3">
				<select id='khType' name='khType'    onChange="kaihfss()">
					<option value="0" selected>0-->正常开户</option>
				</select>
				<font color='orange'>*</font>
			</td>
				<input type="hidden" id='fszqVal' name='fszqVal'   />
	</tr>	
</table>
</DIV>
<%@ include file="/npage/creFamily/bd_0007.jsp" %>	<!--sys_note op_note-->
<div id="vouch" style="display:none">
    <%@ include file="/npage/creFamily/bd_0008.jsp" %>
</div>
