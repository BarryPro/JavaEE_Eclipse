...
{	
	...
	else
	{
		...
		*/
		...
		$("#buzhou2",window.parent.document).css("display","");
		$("#buzhou2",window.parent.document).attr("src","fg638_shopping_car2.jsp?gCustId=<%=gCustId%>&loginType=<%=loginType%>&phone_no=<%=phone_no%>&activePhone=<%=phone_no%>&custOrderId=&broadPhone=&resultList="+resultList);
		
		//window.parent.window.getElementById("buzhou2").window.resultProcess(resultList);
		//window.close();
	}
}
...
function clickTr()
{
	...
	getServBusId(offerid);
	    //从查询列表中取得所需列的数据
		 var xiaoxpCode="39590";//销售品代码
	    var xiaoxpName="家庭用户0元资费";//x销售品名称
	    var bandName="全球通温馨家庭";	    //品牌名称
	  ...
	  //  var tdObj =trCur.getElementsByTagName("td")[3];
	    var offer_comments="家庭用户0元资费";
	    var spanH="39590|家庭用户0元资费|360|0|10||20004|01";
...
function pOfferQryByAttrLimit(xiaoxpCode)
{
		...
		var packet = new AJAXPacket("/npage/portal/shoppingCar/ajax_productOfferQryByAttr_m.jsp","请稍后...");
		packet.data.add("offerCode" ,offerCode);
		packet.data.add("offerName" ,offerName);
		packet.data.add("offerType" ,offerType);
		packet.data.add("offerAttrSeq" ,offerAttrSeq);
	...
	
{
    ...
    */
    var path = "/npage/portal/shoppingCar/ajax_qryModelByMacNo.jsp";   //密码为*
	 ...
	 if(productOfferQueryList.style.display = "none")
		  {
			...
			var packet = new AJAXPacket("/npage/portal/shoppingCar/ajax_qryProductOfferByRentInfo.jsp","请稍后...");
...
//				} 
    ...
    var goodNo = document.frmMain.goodNo.value.trim();
    var path="/npage/portal/shoppingCar/isGoodNoExist.jsp?goodNo="+goodNo;
    ...
    if(retInfo=="11"){//表示该特殊号码被占用
    	  ...
    	  return false;
    		...
    		var packet = new AJAXPacket("/npage/portal/shoppingCar/ajax_qryGoodTypeByGoodNo.jsp","请稍后...");
...
function delTr_m(offer_id)
{
	...
	if(okno){
	var packet = new AJAXPacket("/npage/portal/shoppingCar/ajax_DelMyFavorite.jsp","请稍后...");
		packet.data.add("offer_id" ,offer_id);
...
function saveOffer(offer_id){
		var packet = new AJAXPacket("/npage/portal/shoppingCar/ajax_saveProdBookMark.jsp","请稍后...");
...
<%@ include file="/npage/include/header_pop_3.jsp" %>
					...
					<SELECT name="offer_att_type" id="offer_att_type"> 
						...
						<tr id='footer'>
						  <td colspan="4" align="center">
						  	...
						  	<input class="b_foot" type="hidden" value="重置" onClick="resetInfo();"></td>
						</tr>	
						</table>
...
<div style="width:99%;display:none">
	<div style="">
		...
		<div class="title"><div id="title_zi">按属性查询结果列表</div></div>
			...
			<div class="product_chooseR" id="productOfferQueryList">
			<div class="list" style="height:100%; overflow-y:auto; overflow-x:hidden;">
...
<br>
<div class="title" style="height:0%;display: none">
	<div id="title_zi">已选销售品列表</div>
</div>
			<div class="list" style="height:0%; overflow-y:auto; overflow-x:hidden;display: none;">
			...
			<div id="footer" style="height:0%;display: none;">
