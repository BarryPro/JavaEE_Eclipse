...
<%
	...
	System.out.println("custName= " + custName);
	 ...
	 sqlStr = "select count(*)  "
            ...
            + " and unit_code='"+useUnit+"' "
            + " and department_code='"+useDepartment+"' "
            + " and center_code='"+useCenter+"' "
            + " and trim(cust_name)=trim('"+custName+"')";
	 //查询内容
     sqlStr1="select a.* from (select b.unit_name,c.department_name,d.center_name,a.cust_name,e.level_name,a.phone_no,decode(a.phone_type,'0','测试号','1','公务号') ,f.application_name , rownum rnum "
            + " from dTestPhoneMsg  a,sUseUnit b,Susedepartment c,susecenter d,suselevel e,suseapplication f   "
		    + " where a.type_code='"+typeCode+"' and b.type_code=a.type_code and b.unit_code = a.unit_code "
            + " and a.unit_code='"+useUnit+"' "
            + " and a.department_code='"+useDepartment+"' "
            + " and a.center_code='"+useCenter+"' "
            ...
            + " and trim(a.cust_name)=trim('"+custName+"') ) a where a.rnum <= "+iEndPos+" and  a.rnum > "+iStartPos;
...
<form name="form1" method="post" action="">
	<div id="Operation_Table">
		<table width="100%" id="tabList" border=0 align="center" cellspacing=0>
			<tr>
				...
				<td  align='center' class="blue"><div ><b>使用级别</b></div></td>
	...
	<%
		for(int i = iStartPos; i < result1.length; i++)
		{
	%>
			<tr>
