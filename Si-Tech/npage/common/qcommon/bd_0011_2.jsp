...
<%
	...
	String objectId_0011=(String)session.getAttribute("groupId");	//当前处理局
	...
	function doProcess_ba0011(packet)
{
	...
	if (verifyType=="chooseAddress"){
    	if( parseInt(error_code) != 0 ){	
		      ...
		      return false;
					...
					document.all.confirmFlag.value="true";
 //返回值格式:addrId, ADDR_SHORT，addrName,branchNo,typeNum,branchSer,lowNo,branchName,highNo,BUREAU_ID ,objectId,zipcode 
		...
		function chgAddrSel()
		{
			...
			$("input[name='selNum']").val(""); //selectDisabled(document.all.newbranchno); 
			//地址变换时 局站仍然可以选择变换 
            	    ...
            	    <td>
							...
							<tr>
								...
								<td>
									...
									<input  type="text"  name="newbranchno_name" />
									<input  type="hidden"  name="newbranchno"   />
									<input  type="button"  class="b_text"   value="选择"   onclick="getBranch(new String('<%=objectId_0011%>'));"/>
				        </td>
				         	<td class="blue"> 详细地址 </td>
									<td colspan=3>
										<input  type="text" id=addrTransIn name="addrTransIn"  value="" size="60" class="required" /> <font class="orange">*</font>  
										<input type="hidden" name="sAddrStand" value=""/>
										<input type="hidden" name="confirmFlag" value="false"/>
