...
<!DOCTYPE html PUBLIC "-//W3C//DTD XHTML 1.0 Transitional//EN" "http://www.w3.org/TR/xhtml1/DTD/xhtml1-transitional.dtd">
 <html xmlns="http://www.w3.org/1999/xhtml">
    ...
    if (WtcUtil.haveStr(favStr, "a971")) {
		...
		if(srv_no.equals(""))
		{
		response.sendRedirect("s1238Login.jsp?ReqPageName=s1238Main&retMsg=1");
		}
...
%>
      <script language=javascript>
        ...
        location='s1238Login.jsp?activePhone=<%=phone_no%>';
      </script>
<%
      return;
    }
      /*wanghyd一卡多号判断*/
    String  sqlflag [] = new String[2];
						sqlflag[0]="select count(*) from dmainsubbindmsg where phoneno = :iPhoneNo  and flag = 'A' ";
					  sqlflag[1]="iPhoneNo="+srv_no;		
 %>
  <wtc:service name="TlsPubSelCrm" routerKey="phone" routerValue="<%=srv_no%>" retcode="retCodflag" retmsg="retMsflag" outnum="1"> 
    <wtc:param value="<%=sqlflag[0]%>"/>
    <wtc:param value="<%=sqlflag[1]%>"/> 
  </wtc:service>  
  <wtc:array id="sqlflagarry"  scope="end"/>						 
<% 
System.out.println("是否是一卡多号中的主号===="+sqlflagarry.length);
   if(sqlflagarry.length>0) {
   		if(sqlflagarry[0][0].equals("0")) {
   		}else {
System.out.println("是否是一卡多号中的主号===="+sqlflagarry[0][0]);   			
%>   			
  <script type="text/javascript">
	  ...
	  var prop="dialogHeight:"+h+"px; dialogWidth:"+w+"px; dialogLeft:"+l+"px; dialogTop:"+t+"px;toolbar:no; menubar:no;	scrollbars:yes; resizable:no;location:no;status:no;help:no"
		  var path = "<%=request.getContextPath()%>/npage/s1210/cancelOneCardFlag.jsp?DlgMsg=" + "此号码是一卡多号主号，是否需要保留一卡多号副号码？";
  	  var ret=window.showModalDialog(path,"",prop);
		  if(typeof(ret)!="undefined")
		  {
		    ...
		    if(ret=="no")
		    {
        rdShowMessageDialog('请将此号码先做一卡多号的主号取消副号再做过户！');
        location='s1238Login.jsp?activePhone=<%=phone_no%>';
    		}
		  }
		  else
...
<%	  			
        ...
        /*wanghyd一卡多号判断*/
    String  sqlflags [] = new String[2];
						sqlflags[0]="select count(*) from dmainsubbindmsg where subphoneno = :subphonenos  and flag = 'A' ";
					  sqlflags[1]="subphonenos="+srv_no;		
 %>
  <wtc:service name="TlsPubSelCrm" routerKey="phone" routerValue="<%=srv_no%>" retcode="retCodflag" retmsg="retMsflag" outnum="1"> 
    <wtc:param value="<%=sqlflags[0]%>"/>
    <wtc:param value="<%=sqlflags[1]%>"/> 
  </wtc:service>  
  <wtc:array id="sqlflagarrys"  scope="end"/>						 
<% 
System.out.println("是否是一卡多号中的副号===="+sqlflagarrys.length);
   if(sqlflagarrys.length>0) {
   		if(sqlflagarrys[0][0].equals("0")) {
   		}else {
System.out.println("是否是一卡多号中的副号===="+sqlflagarrys[0][0]);   			
%>   			
  <script type="text/javascript">
			  var h=180;
		  var w=400;
		  ...
		  var l=screen.availWidth/2-w/2;
	  var prop="dialogHeight:"+h+"px; dialogWidth:"+w+"px; dialogLeft:"+l+"px; dialogTop:"+t+"px;toolbar:no; menubar:no;	scrollbars:yes; resizable:no;location:no;status:no;help:no"
		  var path = "<%=request.getContextPath()%>/npage/s1210/cancelOneCardFlag.jsp?DlgMsg=" + "此号码是一卡多号副号，是否需要保留一卡多号主号码？";
  	  var ret=window.showModalDialog(path,"",prop);
		  if(typeof(ret)!="undefined")
		  {
			  if((ret=="yes"))
		    ...
		    if(ret=="no")
		    {
        rdShowMessageDialog('请将此号码在平台侧先做实体副号取消对应主号再做过户！');
        location='s1238Login.jsp?activePhone=<%=phone_no%>';
    		}
		  }
		  else
...
System.out.println("-----1238---shehuiwork--"+isshehuiworkno);			 						
		
  /*** begin diling add for 如果用户有“NFC手机钱包业务”不允许办理过户业务 @2013/1/6  ***/
...
%>
        <script language=javascript>
          //rdShowMessageDialog("用户有“NFC手机钱包业务”，不允许办理过户业务!",1);
          //location='s1238Login.jsp?activePhone=<%=phone_no%>';
        </script>
<% 
      }
  ...
  /*** end diling add @2013/1/6 ***/
		...
		/* diling update for 申告：1238过户没有限制已经结束的亲情通用户时间 @2012/10/12 
...
<%
		if("000000".equals(retCode_))
		{
			...
			if(str_sql!=null && str_sql.length>0)
			{
				...
				{
					response.sendRedirect("s1238Login.jsp?ReqPageName=s1238Main&retMsg=15");
				}
			}
			*/
		}
...
%>
				<wtc:sequence name="sPubSelect" key="sMaxSysAccept" routerKey="phone"  routerValue="<%=srv_no%>" id="s"/>
<%
...
<%
    ...
    System.out.println("---1238-------retCode1="+retCode1);
    if(!"000000".equals(retCode1)){
%>
      <script language=javascript>
...
<%
      return;
		...
		if(sPubUsrBaseInfoArr!=null&&sPubUsrBaseInfoArr.length>0&&retCode1.equals("000000")){
		  ...
		  response.sendRedirect("s1238Login.jsp?ReqPageName=s1238Main&retMsg=2&activePhone="+phone_no);
		}
...
<%
		...
		if(custDoc.size()<34){ //如果长度不够34,就表明出错了
				response.sendRedirect("s1238Login.jsp?ReqPageName=s1238Main&retMsg=9");
		}
...
<%
		if(agentCodeStr ==null||agentCodeStr.length==0){
			...
			if(!agentCodeStr[0][0].equals("0")) {
				sql = "select count(*) from shighlogin where login_no='" +work_no+"' and op_code='1238'";
%>
				<wtc:pubselect name="sPubSelect" routerKey="phone" routerValue="<%=srv_no%>" outnum="1">
		    <wtc:sql><%=sql%></wtc:sql>
				</wtc:pubselect>
...
<%
    ...
    boolean oldFav_a972 = false;
    if (WtcUtil.haveStr(favStr, "a972")) {
    	 oldFav_a972 = true;
    } 		


System.out.println("-------hejwa1238------------workChnFlag------------->"+workChnFlag);  
System.out.println("-------hejwa1238------------good_flag--------------->"+good_flag);
System.out.println("-------hejwa1238------------oldFav_a972------------->"+oldFav_a972);
    
    
    boolean opr_goodPhone_flag = true; //默认普通号码大家都可以操作
    
    if(!"1".equals(workChnFlag)){
   		 //自有营业厅
    
    		if(!"0".equals(good_flag)){ 
				//特殊号码
				
						if(!oldFav_a972){
							//具有a972特殊号码过户权限
							opr_goodPhone_flag = false; //自有营业厅的a972权限可以操作特殊号码
						}
				}
		}		
System.out.println("-------hejwa1238------------opr_goodPhone_flag------------->"+opr_goodPhone_flag);

if(!opr_goodPhone_flag){
		//是特殊号码，但没找到有 自有营业厅 的 a972权限
%>

			<script language=javascript>
				rdShowMessageDialog("仅限有‘特殊号码过户’权限的自有营业厅才可以办理特殊号码过户",1);
				location='s1238Login.jsp?activePhone=<%=phone_no%>';
			</script>
<%		
}
...
<%
    ...
    **/
		...
		/*zhangyan 省内携号*/
		String sqlXhzw="select count (1)  from dloginmsg t , dcustres t1	"
			+"where T.LOGIN_NO='"+work_no+"' "
			+"and T1.PHONE_NO = '"+phone_no+"' 			"
			+"and substr(T.ORG_CODE , 1,2) = T1.REGION_CODE ";
		%>
		<wtc:pubselect name="sPubSelect" routerKey="regionCode" routerValue="<%=regionCode%>" 
			outnum="1">
		<wtc:sql><%=sqlXhzw%></wtc:sql>
		</wtc:pubselect>
		<wtc:array id="arrXhzw" scope="end"/>		
		<%
		System.out.println("@zhangyan~~~~arrXhzw[0][0]="+arrXhzw[0][0]);
		if(  !(arrXhzw[0][0].equals("1") ) )
		{
		  	/*判断用户是否做过携号专网  0是携转 , 1是没携转*/
		  	String sqlChkBeg="	select decode(substr(a.belong_code,0,2),b.region_code,'1','0') "
				+"	  from dcustmsg a, dcustres b	"
				+"	 where a.phone_no = b.phone_no	"
				+"	   and a.phone_no = '"+phone_no+"'"; 
			%>
			<wtc:pubselect name="sPubSelect" outnum="1"
				routerKey="regionCode" routerValue="<%=regionCode%>" >
			<wtc:sql><%=sqlChkBeg%></wtc:sql>
			</wtc:pubselect>
			<wtc:array id="rstChkBeg" scope="end"/>	
			<%
			System.out.println("e687~~~rstChkBeg[0][0]="+rstChkBeg[0][0]);
			if ( rstChkBeg[0][0].equals("0" ))
			{		
				response.sendRedirect("s1238Login.jsp?ReqPageName=s1238Main&retMsg=31");
			}
			else
			{
				response.sendRedirect("s1238Login.jsp?ReqPageName=s1238Main&retMsg=30");
			}
		}
		if(Double.parseDouble(((String)custDoc.get(19)))>Double.parseDouble(((String)custDoc.get(20))))
    {
        response.sendRedirect("s1238Login.jsp?ReqPageName=s1238Main&retMsg=100&oweAccount="+new String(((String)custDoc.get(21)).getBytes("GBK"),"ISO8859_1")+"&oweFee="+((String)custDoc.get(22)));
    }
      ...
      if(!WtcUtil.haveStr(favStr,((String)custDoc.get(31)).trim())){
		...
		if(Integer.parseInt(((String)custDoc.get(32)).trim())!=0)
		{
		  ...
		  {
          response.sendRedirect("s1238Login.jsp?ReqPageName=s1238Main&retMsg=101&errCode="+((String)custDoc.get(32)).trim()+"&errMsg="+new String(((String)custDoc.get(33)).trim().getBytes("GBK"),"ISO8859_1"));
		  }
		}
...
<!-- 20091201 fengry begin for 增加特殊号码过户限制 -->
<%
...
<%
	...
	if(GoodStr.length > 0 && GoodStr != null)
	{
		...
		{%>
			<script language=javascript>
				rdShowMessageDialog("号码<%=GoodStr[0][0]%>不允许办理过户!",1);
				location='s1238Login.jsp?activePhone=<%=phone_no%>';
			</script>
<%
		}else{
%>
			<script language=javascript>
				rdShowMessageDialog("号码 <%=GoodStr[0][0]%> 可过户时间为 <%=GoodStr[0][1]%> 日之后!",1);
				location='s1238Login.jsp?activePhone=<%=phone_no%>';
			</script>
<%
		}
	...
	if("000000".equals(retCode_realUser)){
		if(ret_realUser.length > 0){
		  ...
		  qRealUserAddr = ret_realUser[0][5];
				...
				/*2017.4.18关于申请取消物联卡及M2M客户开户界面使用人录入的函 3、修改“普通开户(1104)”和“批量普通开户(m349)”，单位证件类型时，如果订购的资费有此属性，则实际使用人信息不可见，即不允许录入实际使用人信息。*/
		String m2mFlag = "";
...
System.out.println("-------hejwa-------------1238------------>"+m2mFlag);
%>	
	 <head>
 	 <title>GSM过户</title>
	 <script language=javascript>
	...
	if(<%=listlen%>>0){rdShowMessageDialog("<%=liststr%>");}
	   ...
	   onload=function()
	   {
		  	$('#isTypesNo7').css('display','none');
       		$('#isTypesNo8').css('display','none');
       		$('#isTypesNo4').css('display','none');
       		$('#isTypesNo5').css('display','none'); 
       		$('#isTypesNo6').css('display','none'); 
       		alert(55555);
	   	 getIdTypes();
	 		 self.status="";
			 fillSelect();
			 chgCustType();
				...
				//20091201 begin
				/* if(<%=YesStrNum%>>0) */		//市场部要求对所有号码增加过户限制项,而不是只针对特殊号码
				{
					//特殊号码,过户限制项不隐藏,默认可选可不选
			...
			function valiRealUserIdTypes(idtypeVal){
				if(idtypeVal == "0"){ //身份证
					...
					$("#sendProjectList3").show();
		  			$("#qryListResultBut3").show();
					...
					$("#sendProjectList3").hide();
		  			$("#qryListResultBut3").hide();
...
function validateGesIdTypes(idtypeVal){
		...
		if(idtypeVal.indexOf("身份证") != -1){
  		...
  		$("#sendProjectPhones1").show();
			$("#sendProjectList1").show();
			$("#qryListResultBut1").show();
  		...
  		$("#sendProjectPhones1").hide();
			$("#sendProjectList1").hide();
			$("#qryListResultBut1").hide();
...
function validateresponIdTypes(idtypeVal){
		if(idtypeVal.indexOf("身份证") != -1){
  		document.all.responsibleIccId.v_type = "idcard";
  			...
  			$("#sendProjectList2").show();
  			$("#qryListResultBut2").show();
  		...
  		document.all.responsibleIccId.v_type = "string";
  			...
  			$("#sendProjectList2").hide();
  			$("#qryListResultBut2").hide();
	  ...
	  function doProcess(packet)
	  {
			...
			if(retType == "chkID")
			{
             ...
             rdShowMessageDialog("密码错误！");
                ...
                //rdShowMessageDialog("您无法继续进行GSM过户业务！");
						 		return false;
					   }
					}else{
...
function checkIccIdFuncNew(obj,objType,ifConnect){
	...
	var idTypeSelect_isC = $("#idType option:selected").val();//证件类型 
	//alert("1 idTypeSelect_isC=["+idTypeSelect_isC+"]");
	if(typeof(idTypeSelect_isC)=="undefined"){
		//切换证件类型后 selected 的丢失默认取第一个
	...
	if(
	   ...
	   idTypeSelect_isC.indexOf("C")!=-1){
			...
			if("<%=opCode%>" == "1238"){
				if(document.all.r_cus[0].checked){ //新建客户
					...
					if(idTypeSelect.indexOf("|") != -1){
						...
						if(checkVal == "0" && v_idTypeSelect == "0" && "<%=regionCodeFlag%>" == "Y"){ 	
		...
		var gongdanFlag=false;
		if(!$("#gestoresInfo1").is(":hidden")&&$("#gestoresIdType").val()=="0|身份证"){
			gongdanFlag=false;	
			if(($("#isQryListResultFlag1").val()=="N"||$("#isSendListFlag1").val()=="N")&&$("#gestoresIccId").val()==""){
				rdShowMessageDialog("请先进行经办人工单结果查询，再进行开户!");
				gongdanFlag=true;
			}
		}
		if(gongdanFlag){
		...
		if(!$("#responsibleInfo1").is(":hidden")&&$("#responsibleType").val()=="0|身份证"){
			gongdanFlag=false;	
			if(($("#isQryListResultFlag2").val()=="N"||$("#isSendListFlag2").val()=="N")&&$("#responsibleIccId").val()==""){
				rdShowMessageDialog("请先进行责任人工单结果查询，再进行开户!");
				gongdanFlag=true;
			}
		}
		if(gongdanFlag){
		...
		if(!$("#realUserInfo1").is(":hidden")&&$("#realUserIdType").val()=="0"){
			gongdanFlag=false;	
			if(($("#isQryListResultFlag3").val()=="N"||$("#isSendListFlag3").val()=="N")&&$("#realUserIccId").val()==""){
				rdShowMessageDialog("请先进行使用人工单结果查询，再进行开户!");
				gongdanFlag=true;
			}
		}
		if(gongdanFlag){
    		...
    		if(!checkCustNameFunc(document.all.contactPerson,1,1)){
			  ...
			  if(!checkCustNameFunc16New(document.all.realUserName,3,1)){
				...
				if(!checkIccIdFunc16New(document.all.realUserIccId,3,1)){
					...
					var GoodPhoneFlag = $.trim($("select[name='GoodPhoneFlag']").find("option:selected").val());
					if(GoodPhoneFlag == "0"){
						rdShowMessageDialog("请选择原服务号码过户限制！");
						return false;
					}
	...
	return false;
	 ...
	 function printInfo(printType)
	 {
      ...
      cust_info+="证件号码：   "+document.all.ic_no.value+"|";
 		  ...
 		  opr_info+="用户品牌："+"<%=sNewSmName%>"+"  *"+"办理业务："+"GSM过户"+"|";
		  ...
		  if(document.all.r_cus[0].checked)
		 {
		  ...
		  opr_info+="用户号码"+"<%=srv_no%>"+"由用户"+"<%=(String)custDoc.get(5)%>"+"过户到用户"+document.all.custName.value+"|";
		  opr_info+="新客户资料：客户名称："+document.all.custName.value+"*"+"证件号码："+document.all.idIccid.value+"|";
		  opr_info+="客户地址："+document.all.idAddr.value+"|";
		  }else{

		  opr_info+="用户号码"+"<%=srv_no%>"+"由用户"+"<%=(String)custDoc.get(5)%>"+"过户到用户"+document.all.h_custName.value+"|";
		  ...
		  if("<%=goodFlagNew%>"=="1"){
			...
			note_info1+=" "+"|";
			note_info1+="注意：该号码为特殊号码，底线消费金额为"+"<%=mode_name%>"+"元,过户后原底线消费标准保持不变，仍按原规定执行，即需在您选择的资费基础上设置底线消费（底线不包含信息费），如您当月消费的话费（不包括信息费）不足底线额度，将按底线标准收取。|";
		  }

		  if (document.all.HasPostBill.value=="Y" && document.all.HasEmailBill.value=="Y" ){
		  	note_info1+="由于该号码办理了过户业务，原机主的邮寄账单及电子账单将在过户后自动取消。"+"|";
		  }else if (document.all.HasPostBill.value=="Y"){
		  	note_info1+="由于该号码办理了过户业务，原机主的邮寄账单将在过户后自动取消。"+"|";
		  }else if (document.all.HasEmailBill.value=="Y" ){
		  	note_info1+="由于该号码办理了过户业务，原机主的电子账单将在过户后自动取消，如新机主需要使用此项业务则重新办理。"+"|";
		  }

			//20091201 begin
			if(document.all.GoodPhoneFlag.value == "N")
			{
				note_info1+="客户自愿承诺本号码仅用于甲方本人使用，不转售、不赠送、不申请过户、不使用此号码时主动到乙方营业厅申请销户。"+"|";
			}
			if(document.all.GoodPhoneFlag.value == "Y" && (document.all.GoodPhoneDate.value).trim().length == 8)
			{
				
				if(parseInt(document.all.GoodPhoneDate.value.substring(0,4))>=2050){
						note_info1+="客户自愿承诺：客户在网使用期间，本号码仅限于甲方本人使用，不转售、不赠送、不申请过户";
				}else{
					note_info1+="客户自愿承诺本号码仅用于甲方本人使用，"+document.all.GoodPhoneDate.value.substring(0,4)+"年"
					+document.all.GoodPhoneDate.value.substring(4,6)+"月"+document.all.GoodPhoneDate.value.substring(6,8)+"日"
					+"之前不转售、不赠送、不申请过户、不使用此号码时主动到乙方营业厅申请销户。"+"|";
				}
		  ...
		  note_info1+="备注："+document.all.assuNote.value+"|";
		   /* 20090402 zhangshuai begin */
		  if (("<%=attrcode%>" == '42')||("<%=attrcode%>" == '43'))
		  {
		  	note_info1 += "过户后，您的客户属性将变更为“一般客户”。"+"|";
		  }
		  if("<%=attrcode%>" == '41')
		  {
		  	note_info1 += "过户后，您的客户属性将变更为“一般客户”，您的“红名单”功能（不停机服务）将被取消。"+"|";
		  }
		  if("<%=attrcode%>" == '90')
		  {
		  	note_info1 += "过户后，您的客户属性将变更为“一般客户”，您的不停机服务将被取消。"+"|";
		  }
		  /* 20090402 zhangshuai end */
<%
			if(printFlag){
%>

					note_info1 += '您本次过户是通过模糊验证的方式进行办理，并未出具开户的业务工单及发票，|';
		  ...
		  if("<%=opCode%>" == "1238"){
			  ...
			  myPacket = null;	
				if(isExitCustFlag == "Y"){//说明客户已存在用户
					...
					if(userIdType == "0" || userIdType == "00"){ //身份证:如果是身份证开户的，就不允许进行1104普通开户
...
function forEnKuoHao(obj){
  ...
  function checkNameStr1(code){
	    ...
	    if(re6.test(code)||re7.test(code)||re8.test(code)) return "KR"; //韩国
	    if(forHanZi1(code)) return "CH"; //汉字
	    /*add 20170531身份证类型为外国公民护照校验客户姓名中间可以为空格*/
  ...
  function forKuoHao16New1(obj){ //允许输入括号·.． 这几种副号
	...
	function reSetCustName(){/*重置客户名称*/
		...
		if($("select[name='isJSX']").find("option:selected").val()=="1"){
	      	$('#isTypesNo4').css('display','block');
	      	$('#isTypesNo5').css('display','block');
	      	$('#isTypesNo6').css('display','block');
	      	$('#isTypesNo7').css('display','none');
	      	$('#isTypesNo8').css('display','none');
	      	alert(222222);
		}else{
  ...
  /*20131216 gaopeng 2013/12/16 15:11:05 当选择单位开户时，展示经办人信息并将其信息设计为必填选项 end*/
	  ...
	  if(checkVal == 0){
			...
			if(idTypeSelect.indexOf("|") != -1){
				var v_idTypeSelect = idTypeSelect.split("|")[0];
				<%-- if(v_idTypeSelect == "0" && "<%=workChnFlag%>" == "1" && "<%=opCode%>" == "1238" && (parseInt($("#sendListOpenFlag").val()) > 0) && "<%=regionCodeFlag%>" == "Y"){  --%>
				if(v_idTypeSelect == "0" && "<%=opCode%>" == "1238" && (parseInt($("#sendListOpenFlag").val()) > 0) && "<%=regionCodeFlag%>" == "Y"){ 	
...
function resIdTypes(data){
	...
	{
		if(document.all.idType.value=="8|营业执照"){
	    	$('#isTypesNo4').css('display','block');
	    	$('#isTypesNo5').css('display','block');
	    	$('#isTypesNo6').css('display','block');
	    	$('#isTypesNo7').css('display','none');
	    	$('#isTypesNo8').css('display','none');
	    	alert(222222);
	    }else if(document.all.idType.value=="A|组织机构代码"){
	    	$('#isTypesNo7').css('display','block');
	    	$('#isTypesNo8').css('display','block');
	    	$('#isTypesNo4').css('display','block');
	    	$('#isTypesNo5').css('display','none'); 
	    	$('#isTypesNo6').css('display','none'); 
	    	alert(33333);
	  ...
	  /* end diling update@2013/9/22 */
		...
		{   document.all.idIccid.v_type = "idcard";  
		    ...
		    /* begin add 如 1238+普通开户+身份证+社会渠道工号+开关+开展地市，则显示“下发工单”按钮 for 关于电话用户实名登记近期重点工作的通知 @2014/11/19 */
				var checkVal = document.all.isJSX.value;//个人开户分类 普通客户：0
				<%-- if(checkVal == "0" && "<%=workChnFlag%>" == "1" && "<%=opCode%>" == "1238" && (parseInt($("#sendListOpenFlag").val()) > 0) && "<%=regionCodeFlag%>" == "Y"){ --%>
				if(checkVal == "0" && "<%=opCode%>" == "1238" && (parseInt($("#sendListOpenFlag").val()) > 0) && "<%=regionCodeFlag%>" == "Y"){
					$("#sendProjectList").show();
					$("#qryListResultBut").show();
				}else{
					$("#sendProjectList").hide();
  		  ...
  		  if(document.all.r_cus[0].checked){//新建客户
  		  	var checkVal = $("select[name='isJSX']").find("option:selected").val();//个人开户分类 普通客户：0
					...
					if(idTypeSelect.indexOf("|") != -1){
						...
						if(checkVal == "0" && v_idTypeSelect == "0" && (parseInt($("#sendListOpenFlag").val()) > 0) && "<%=regionCodeFlag%>" == "Y"){
							$("#sendProjectList").show();
							$("#qryListResultBut").show();
						}
						/* begin update for 关于开发智能终端CRM模式APP的函 - 第二批@2015/3/10 */
...
function getInfo_IccId_JustSee()
{
   ...
   */ 
	...
	*/
    ...
    path = path + "?idIccid=" + document.frm.idIccid.value.trim()+"&idtype="+idTypesss+"&custnames="+custnamess+"&opcode=1238";
...
function continueCfm() {
			/*2015/7/9 9:37:22 gaopeng 关于讨论客户号码销户后重新放号提醒问题的会议纪要 
				增加提示语
			*/
			var useSmCode = "<%=sNewSmName%>";
			if(useSmCode == "dn" || useSmCode == "gn" || useSmCode == "zn"){
				rdShowMessageDialog("如原机主捆绑了银行或其它服务行业等第三方公司信息，或办理移动宽带时预留了本手机号码，应在申请办理本手机号码的过户或销户前主动变更所留存的联系电话号码或解除捆绑关系！");
			}
			if(document.all.r_cus[0].checked)
	  {
	      document.all.t_sys_remark.value="用户"+jtrim(document.all.cust_name.value)+"GSM过户到"+document.all.custName.value;
	     }else{
	     document.all.t_sys_remark.value="用户"+jtrim(document.all.cust_name.value)+"GSM过户到"+document.all.h_custName.value;
	     }

	      if(jtrim(document.all.t_op_remark.value).length==0)
          {
			  document.all.t_op_remark.value="操作员<%=work_no%>"+"对用户手机"+jtrim(document.all.srv_no.value)+"进行GSM过户"
	      }
		  if(jtrim(document.all.assuNote.value).length==0)
          {
			  document.all.assuNote.value="操作员<%=work_no%>"+"对用户手机"+jtrim(document.all.srv_no.value)+"进行GSM过户"
	      }
  	  ...
  	  var ret=window.showModalDialog(path,printStr,prop);
		  ...
		  if(typeof(ret)!="undefined")
		  {
		    ...
		    if(ret=="continueSub")
		    {
			  if(rdShowConfirmDialog('确认要提交GSM过户信息吗？')==1)
			  {
			   conf();
			  }
    		}
                  ...
                  <td nowrap style="DISPLAY: none">
                    ...
                    <table cellspacing="0">
                      <tr>
                        ...
                        <td colspan="1" class="blue" width="30%">
                          <div align="left">
                            <input type="radio" name="r_cus" value="radiobutton" onclick="chgCustType()" index="7" checked>
                            新建客户
                            <input type="radio" name="r_cus" value="radiobutton" onclick="chgCustType()" index="8" >
                            已有客户</div>
                        </td>
											  <td class="blue" width="13%">
		...
		showNumberDialog(document.all.custPwd);
                      ...
                      <tr>
                        ...
                        <td colspan="3">
		                 ...
		                 <%@ include file="/npage/sq100/gestoresInfo2.jsp" %>  
		                 <%@ include file="/npage/sq100/responsibleInfo2.jsp" %>
				...
				<table id='isTypesNo' style='display:block'>
                	<jsp:include page="/npage/public/hwReadCustCardNewCode.jsp">
							...
							<table cellspacing="0">
							<%@ include file="/npage/sq100/realUserInfo3.jsp" %>
<!-- 20091201 begin -->
							<TR style="display:none" id="Good_PhoneDate_GSM" >
								...
								<TD nowrap>
									<select style="width:220px" name ="GoodPhoneFlag" onchange="GoodPhoneDateChg();">
										<option class='button' value='0' selected>--请选择--</option>
										<option class='button' value='Y' >不限制过户和实名登记</option>
										<option class='button' value='N' >不能过户和实名登记(时间为2050年)</option>
										<option class='button' value='K' >24个月不能过户和实名登记</option>
...
<!-- 20091201 end -->
              ...
              <jsp:include page="/npage/public/hwReadCustCard.jsp">
								...
								<jsp:param name="sopcode" value="1238"  />
							</jsp:include>
            	<table>
                <tr>
                  <td nowrap colspan="6" id="footer">
                    <div align="center">
                      ...
                      <input class="b_foot" type="button" name="b_back" value="返回" onClick="location='s1238Login.jsp?activePhone=<%=phone_no%>' " index="45">
                    </div>
                  </td>
                </tr>
              </table>
