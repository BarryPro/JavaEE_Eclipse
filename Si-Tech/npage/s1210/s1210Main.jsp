...
function validateGesIdTypes(idtypeVal){
	...
	var idTypeSelect_isC = $("#s_idtype option:selected").val();//证件类型 
 ...
 //1210 个人 单位 证件类型 不能互换 关于调整一证5号系统限制要求的函 hejwa 
 		if("1210"=="<%=opCode%>"){
			...
			if(old_idtype=="8"||old_idtype=="A"||old_idtype=="B"||old_idtype=="C"){
				if($("#s_idtype").val()!="8"&&$("#s_idtype").val()!="A"&&$("#s_idtype").val()!="B"&&$("#s_idtype").val()!="C"){
						rdShowMessageDialog("原单位证件类型新证件类型只能选择单位证件类型");
						$("#s_idtype").val(old_idtype);
						return;
				}
			}else{
				if($("#s_idtype").val()=="8"||$("#s_idtype").val()=="A"||$("#s_idtype").val()=="B"||$("#s_idtype").val()=="C"){
						rdShowMessageDialog("原个人证件类型新证件类型只能选择个人证件类型");
						$("#s_idtype").val(old_idtype);
						return;
				}
			}   	
