...
<%
  ...
  String org_code = (String)session.getAttribute("orgCode");
	...
	paraStr[16]=WtcUtil.repSpac(request.getParameter("t_idno"));           //身份证号
	paraStr[17]=WtcUtil.repSpac(request.getParameter("t_id_address"));
	paraStr[18]=WtcUtil.repSpac(request.getParameter("t_id_valid"));       //证件有效期
...
*/
	...
	paraStr[27]=WtcUtil.repSpac(request.getParameter("t_birth"));         //出生日期
	...
	paraStr[43]=WtcUtil.repSpac(request.getParameter("isDirectManageCust"))+""+isceshijt_flag;  //是否直管客户
	paraStr[44]=WtcUtil.repSpac(request.getParameter("directManageCustNo"));//直管客户编码
	paraStr[45]=WtcUtil.repSpac(request.getParameter("groupNo"));//组织机构编码
...
%>
        <script>
	     rdShowMessageDialog("客户<%=paraStr[8]%>(<%=paraStr[4]%>)的资料已被成功修改！",2);
...
%>
        <script>
	     rdShowMessageDialog("客户<%=paraStr[8]%>(<%=paraStr[4]%>)的资料已被成功修改，下面将打印发票！",2);
 		 var infoStr="";
		 ...
		 infoStr+="现金"+"|";
		 infoStr+="<%=paraStr[32]%>"+"|";
	     infoStr+="<%=opName%>。*手续费："+"<%=paraStr[32]%>"+"*流水号："+"<%=s1210LoginAccept%>"+"|";
...
else
{
	if (retCode1.equals("000000"))
	{
		...
		%>
	     <script>
		   	 rdShowMessageDialog('物联网用户申请信息已经发送集团公司，审批通过后继续为您办理业务。',2);
	       location="s1210Login.jsp?activePhone=<%=userPhone%>&opCode=<%=opCode%>&opName=<%=opName%>";
		 </script>	
		<%
		}
	...
	else
	{
		...
		%>
	     <script>
		   rdShowMessageDialog('修改失败！<%=retCode1%>：'+'<%=retMsg1%>',0);
	       location="s1210Login.jsp?activePhone=<%=userPhone%>&opCode=<%=opCode%>&opName=<%=opName%>";
		 </script>	
		<%			
		}else{
		%>
	     <script>
		   rdShowMessageDialog('物联网用户申请信息发送失败！<%=retCode1%>：'+'<%=retMsg1%>，请重新操作！',0);
	       location="s1210Login.jsp?activePhone=<%=userPhone%>&opCode=<%=opCode%>&opName=<%=opName%>";
		 </script>	
		<%	
		}
