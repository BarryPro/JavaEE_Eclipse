...
//	String smCode=request.getParameter("smCode");
	...
	//lj. 绑定参数
	String sql_select1 = "select trim(sim_status),trim(status_name) from sSimStatus where sim_status='2'";
...
/*	   if(ret_code1.equals("0")||ret_code1.equals("000000")){
 ...
 <%
   ...
   /*2016/5/12 17:01:18 gaopeng 1220申告 历史遗留问题 */
   	String hlrcodeShow = "无";
		if("000000".equals(ret_code) && hlrcode.length > 0){
			hlrcodeShow = hlrcode[0][0];
		}
	 ...
	 if(document.all.cardtype[1].checked==true)  //空卡
  	{
        ...
        */
  		...
  		// path ="http://10.110.26.27:34000/writecard/writecard/ReadCardInfo.jsp";
...
function printCommit()
{
	...
	if((document.all.t_simFeef.value.trim().length==0)){
	 ...
	 <%if(istestNum&&opCode.equals("1220")){%>
...
// note_info4+="4、4G业务仅在4G网络所覆盖的范围内提供，中国移动将不断扩大4G覆盖区域、提高服务质量。"+"|";
 ...
 function chkSim(phoneNo,simOldNo,simNewNo)
 {
	...
	var myPacketsd = new AJAXPacket("checkWLYXLR.jsp","正在验证新SIM类型，请稍候......");
...
function doPubSmCodeBack(packet){
 ...
 function conf()
 {
	...
	/*2015/9/7 9:10:00 gaopeng 修改为获取品牌，如果是PA或者PB 则调用物联网服务*/
	    if(pubSmCode=="PA"||pubSmCode=="PB")
			{
			    ...
			    myPacket.data.add("iOpCode", document.all.op_code.value);												//操作代码          
        ...
        <table cellspacing="0" >
          ...
          <tr>
            ...
            <td nowrap width="18%">
              ...
              <div align="left"><%=hlrcodeShow %></div>
            </td>
          </tr>
          <tr>
            <td width="11%" nowrap  class="blue">
