...
function segmentAdd<%=jsi%>(){
	...
	if(segMentTab<%=jsi%>.rows.length< eval(eval(<%out.print(fieldMaxRows[ii]);%>)+eval(2))) {
		...
		tr1.bgcolor="E8E8E8";
		tr1.insertCell().innerHTML = '<td width="3%"><input class="b_text" type="button" name="del" value="删除" onclick="deleteIt<%=jsi%>(this)"></td>';
   ...
   do{
	...
	//判断是否是boolean型
					...
					else if (dataTypes[ii].equals("60"))//域名申请日期带默认值动态获取
					{
						...
						out.print("<TD width=\"32%\" colspan="+colspan+"> <input id=\"F"+fieldCodes[ii]+"\" name=\"F"+fieldCodes[ii]+"\" class=\"button\" type=\"text\" datatype="+dataTypes[ii]+" maxlength=\""+fieldLengths[ii]+"\" value=\"\">");
						if(fieldCtrlInfos[ii].equals("N"))
						{out.print("&nbsp;<font color=\"#FF0000\">*</font></TD>");}
					}	
	%>';
...
int len=0;
        	...
        	while(len<fieldCount)
        	{
	        	...
	        	else
	        	{
		        	...
		        	out.println("	{");
		        	out.println("		if($(\"#segMentTab"+fieldGroupNo[len]+"\").attr(\"class\")!=\"hiddenTdForSubmit\") { ");
...
System.out.println("--------wanghao--------"+len+"------");
%>
		...
		if (resultList != null)
		{
			while(i<fieldCount)
			{
				...
				if(trFlag||i==0)
				{
    				...
    				<TR>
    				<TD width="3%"><input type="button" name="addSegment<%=fieldGroupNo[i]%>" class="b_text"  value="增加" onclick="segmentAdd<%=fieldGroupNo[i]%>()"></TD>
    				<%
					}
					else
					{
    			...
    			if(!fieldGroupNo[k].equals("0"))
    			{
    				...
    				// liujian 2013-3-28 11:33:58
    				out.print("<TD width='3%'><input type='button' class='b_text' value='删除' name='delCurrRowBtn' /></TD>");
				...
				for (int colNum = 0; (colNum < 2) && (i < fieldCount)||!fieldGroupNo[i].equals("0"); colNum ++)
				{
					...
					else
					{
    					 ...
    					 {
	    					/* yuanqs add 2011/6/20 10:58:26 下月套餐资费生效日期 */
							if ("80006".equals(fieldCodes[i])) {
								out.print("<TD class=blue width='18%' style='display:none'>"+fieldNames[i]+"</TD>");
								out.print("<TD class=blue width='18%'></TD>");
							} else {
                    ...
                    else if (dataTypes[i].equals("80"))//vp - 当月套餐类型
					{
                        ...
                        System.out.println("gaopengSeeLog======7896=========fieldCodes[i]====="+fieldCodes[i]);
					...
					else if (dataTypes[i].equals("82"))//ap - 漫游类型
					{
						...
						out.print("<TD width='32%' colspan="+colspan+"><input readonly id='F"+fieldCodes[i]+"' name='F"+fieldCodes[i]+"' class='button' type='text' datatype="+dataTypes[i]+" maxlength='"+fieldLengths[i]+"' value='"+fieldValues[i]+"'>");
						
						if(fieldCtrlInfos[i].equals("N")){
						    out.print("&nbsp;<font color=\"#FF0000\">*</font>");
						}
						out.print("<input type='button' value='修改' class='b_text' onclick='call_rh_flags()'></TD>");
						
					}
					else {
						/* yuanqs add 2011/6/21 10:23:15 下月套餐资费生效日期 style='display:none' */

						if ("80006".equals(fieldCodes[i])) {
							out.print("<TD width='32%' colspan="+colspan+" style='display:none'><input id='F"+fieldCodes[i]+"' name='F"+fieldCodes[i]+"' class='button' type='text' datatype="+dataTypes[i]+" maxlength='"+fieldLengths[i]+"' value='"+fieldValues[i]+"'>");
							out.print("<TD width='32%' colspan="+colspan+"></td>");
						} else {
							//liujian
...
<script>
	$(function() {
		$('input[name="delCurrRowBtn"]').click(function() {
		...
		$('#F70040').change(function() {
			var val = $('#F70040').val();
			if(val == '0') {
				//此class只用来标识最后提交此div的数据
				$('#segMentTab2').css('display','none').addClass('hiddenTdForSubmit');	
				$('#segMentTab3').css('display','block').removeClass('hiddenTdForSubmit');
			}else if(val == '1') {
				$('#segMentTab3').css('display','none').addClass('hiddenTdForSubmit');	
				$('#segMentTab2').css('display','block').removeClass('hiddenTdForSubmit');	
			}
		});
