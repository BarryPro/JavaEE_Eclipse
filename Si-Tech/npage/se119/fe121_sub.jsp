...
<%
    ...
    System.out.println("=========================ip==================   "+ip);
		String radio = request.getParameter("flag");
		String powerCode = request.getParameter("powerCode");
		String workRulestarttime = request.getParameter("workRulestarttime");
		String workRuleendtime = request.getParameter("workRuleendtime");
		String workNos = "";
...
<%
System.out.println("====wanghfa====fe121_sub.jsp====se121Cfm==== radio = " + radio);
System.out.println("====wanghfa====fe121_sub.jsp====se121Cfm==== filename = " + filename);
System.out.println("====wanghfa====fe121_sub.jsp====se121Cfm==== powerCode = " + powerCode);
System.out.println("====wanghfa====fe121_sub.jsp====se121Cfm==== ip = " + ip);

%>
<wtc:service name="se121Cfm" routerKey="region" routerValue="<%=regionCode%>" 
	 retcode="Code" retmsg="Msg" outnum="4" >
        <wtc:param value="0"/>
        <wtc:param value="01"/>
        <wtc:param value="e121"/>
        <wtc:param value="<%=loginNo%>"/>
        <wtc:param value="<%=passWord%>"/>
        <wtc:param value=" "/>
        <wtc:param value=" "/>
        <wtc:param value="<%=radio%>"/>
        <wtc:param value="<%=filename%>"/>
        <wtc:param value="<%=powerCode%>"/>
        <wtc:param value="<%=ip%>"/>
        <wtc:param value="<%=workRulestarttime%>"/>	
        <wtc:param value="<%=workRuleendtime%>"/>
</wtc:service>
...
<script language="javascript">
	function goback(){
		window.location = "fe121.jsp?opCode=<%=opCode%>&opName=<%=opName%>";
	}
</script>
</head>
<body>
...
<%
	...
	%>
	<script language="javascript">
		rdShowMessageDialog("se121Cfm：<%=retcode%>,<%=retmsg%>.");
		//window.history.go(-1);
		window.location.href="fe121.jsp?opCode=<%=opCode%>&opName=<%=opName%>";
	</script>
	<%
}
%>