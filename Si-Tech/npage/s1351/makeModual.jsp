	...
	if (action!=null&&action.equals("select")){
			...
			if(errCode == 139785)
			{
			    ...
			    <script language=javascript>
			    	...
			    	{
			    		window.showModalDialog("addModual.jsp?show_mode=<%=show_mode%>&modetype=<%=modetype%>","","dialogHeight:160px; dialogWidth:300px; status:no; help:no; scroll:no");
			    	 	document.location.href ="makeModual.jsp?action=select&show_mode=<%=show_mode%>&modetype=<%=modetype%>";
			   	 	}
		      </script>
...
<script language=javascript>
 function choose()
 {
 	//alert("<%=phoneno%>");
 	//alert("<%=show_mode%>");
 	window.location.href="chooseCon.jsp?phoneno=<%=phoneno%>&show_mode=<%=show_mode%>";
 ...
 function doQuery()
	{
		...
		document.form.action = "makeModual.jsp?action=select";
...
*/	 
	...
	function previouStep(){
		document.location.href="<%=request.getContextPath()%>/npage/s1351/makeModual.jsp";
	}
<%}%>
</script>
		          ...
		          <TBODY>
		            <TR bgcolor="E8E8E8">
		             ...
		             <input class="button" name="previous" style="cursor:hand" type=button value="上一步" onclick="previouStep()">&nbsp;&nbsp;
						...
						-->
						     <input class="button" name="next" style="cursor:hand" type=button value="下一步" onclick="choose()">&nbsp;&nbsp;
