...
<%
  ...
  String region = request.getParameter("region");				//模版类型
  ...
  String action=request.getParameter("action");//提交到本页面
	...
	if (action!=null&&action.equals("select")){     
	  ...
	  String paraArr[] = new String[8];
		...
		paraArr[6]= modetype;
		paraArr[7]= region; 
		System.out.println("----------------zss------------------"+region);
		impl.callFXService("sAddDeModeNew",paraArr,"2");
		impl.printRetValue();
		...
		{%>
			<script language=javascript>
				rdShowMessageDialog("增加地市默认模板成功");
...
function doSubmit()
	{		
		document.form1.action = "/npage/s1351/addDeModual.jsp?action=select&show_mode=<%=show_mode%>&modetype=<%=modetype%>";
