...
<%
  ...
  System.out.println("liangyl-----activePhone---------------------"+activePhone);
  ...
  function check(){
	...
	if(isphonePay){
		if($("#phonePayCheck").val()=="0"){
			rdShowMessageDialog("请先校验支付手机状态后再进行提交！",0);
	     	return false;
		}
	}
		...
		/*
		alert("year_fee====="+Number(year_fee));
		alert("prepay_fee====="+Number(prepay_fee));
		alert("intePrice====="+Number(intePrice));
		var asdsada = Number(year_fee) - Number(prepay_fee) - Number(intePrice);
		*/
	var subFlag=false;
	if($("#phonePayCheck").val()=="1"){
	}
	else{
		if(Number(year_fee) - Number(prepay_fee) - Number(intePrice) > 0){
		  	var showMsg = "<%=sm_code%>" == "kh"?"包年金额大于当前可用预存"+appendStr+"，请先通过zg62，进行铁通宽带交费!":"包年金额大于当前可用预存"+appendStr+"，请先通过e006，进行宽带交费!";
...
function ifUseIntegralBtn(){
	...
	if(ifUseIntegral == true){
		var isphonePay = $("#isphonePay").attr("checked");
		if(isphonePay == true){
			if ("1"!=rdShowConfirmDialog("已使用手机支付如果使用积分支付请重新进行手机支付校验！")){
...
function getPhonePaygral(){
	...
	if(isphonePay){
		...
		var new_rate_code = $("#new_rate_code").val() 
		...
		if(new_rate_code==""){
			rdShowMessageDialog("请选择新套餐！",1);
			return false;
		}
		
		var getdataPacket = new AJAXPacket("fe974CheckPhonePay.jsp","正在获得数据，请稍候......");
		var iLoginAccept = "<%=loginAccept%>";
		var iChnSource = "01";
		var iOpCode = "<%=opCode%>";
		var iLoginNo = "<%=loginNo%>";
		...
		getdataPacket.data.add("iPhoneNo",iPhoneNo);
		getdataPacket.data.add("iUserPwd",iUserPwd);
		getdataPacket.data.add("iKDPhoneNo","<%=activePhone%>");
		getdataPacket.data.add("iOfferId",$("#new_rate_code").val());
		
		var ifUseIntegral = $("input[name='ifUseIntegral']").attr("checked");
		if(ifUseIntegral){
			getdataPacket.data.add("iScoreMoney",$("#intePrice").val());
		}
		else{
...
<input type='hidden' id='qry_ofr' name='qry_ofr' value=''>
	<div class="title">
      ...
      <%if("kf".equals(sm_code)){%>	
			...
			<tr>
				<td class="blue">是否使用手机代付</td>
				<td colspan="3">
					<input type="checkbox" name="isphonePay" id="isphonePay" value="" onclick="ifPhonePayBtn();"/>
				</td>	
			</tr>
			<tbody id="isphonePayTb" style="display:none">
			<tr>
				<td  class="blue">手机号码</td>
				<td ><input type="text" name="phonepayPhoneNo" id="phonepayPhoneNo" value=""/><font class="orange">*</font></td>	
				<td  class="blue">服务密码</td>
				<td >
					<input type="password" name="phonepayPassWord" id="phonepayPassWord" value=""/><font class="orange">*</font>
