...
<!--
  ...
  function setParaForNext(){
    var iOpCode = "<%=opCode%>";//iOpCode    
	//iAddStr格式 pay_type|year_fee|prepay_fee|iOldAccept|iContractNo|iOpType 
	var iAddStr = "<%=pay_type%>" + "|" + $("#prepay_year_fee").val() + "|" +  $("#prepay_fee").val() + "|" + "<%=old_accept%>" + "|" + "<%=contract_no%>" + "|" + "D";//iAddStr
...
<form name="frm" method="post">
	...
	<div class="title">
      ...
      <table  cellspacing="0" >
        ...
        <td <%if("NULL".equals(prepay_year_fee)){out.print("colspan='3'");}%>>
          <input name="prepay_fee" type="text" class="InputGrey" id="prepay_fee" value="<%=prepay_fee%>" readonly>
        </td>
        <%if(prepay_year_fee != null && !"".equals(prepay_year_fee) && !"NULL".equals(prepay_year_fee)){%>
        <td class="blue">下一档主套餐包年金额</td>
        <td>
