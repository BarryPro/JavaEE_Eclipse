...
<%
	...
	String opName = "销售品选择";
	String opCode = WtcUtil.repNull(request.getParameter("opCode"));// opcode 传过来 
	String startOpCode = WtcUtil.repNull(request.getParameter("startOpCode"));// opcode 传过来 
...
//				} 
    ...
    if(retInfo=="11"){//表示该特殊号码被占用
    	  ...
    	  return false;
    		...
    		/*2014/09/28 11:16:06 gaopeng 关于完善自助换卡功能和188号码开户界面的函
    			加入188号段的判断，如果188号段的号码不是特殊号码，不允许在特殊号码入网中开户
    		*/	
    		if(goodNo.substring(0,3)=="188"){
	    		var isGoddNo_Packet2 = new AJAXPacket("/npage/common/qcommon/isGoodNo.jsp","正在校验号码，请稍候......");
	...
	function doIsGoodNo2(packet){
			...
			if(countGoodNo==0){
					rdShowMessageDialog("该号码不是特殊号码，不允许进行特殊号码开户！",0);
					document.frmMain.goodNo.value="请输入特殊号码";
	    		document.frmMain.goodNo.focus();
...
<%@ include file="/npage/include/header_pop.jsp" %>
			...
			<table cellspacing=0>
				       ...
				       %>
							...
							%>
							<input type="radio" name="query" id="query11" value="11" >前台预开户
							<%
							}
							if("Y".equals(prodFuncArr[1])){
							%>
							<input type="radio" name="query" id="query12" value="12" >前台校园营销预开户
							<%
							}
							if("Y".equals(prodFuncArr[2])){
							%>
							<input type="radio" name="query" id="query13" value="13" >天猫开户
							<%
							}
							if("Y".equals(prodFuncArr[3])){
							%>
							<input type="radio" name="query" id="query14" value="14" >移动商城开户
							<%
							}
