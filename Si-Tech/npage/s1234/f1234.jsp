...
<script language=javascript>
	...
	onload=function()
	{
		if(<%=activePhone1%>==null||<%=activePhone1%>==""){
...
/* 2014/04/17 8:33:32 gaopeng 申告屏蔽
function validateGroupId11(){
	var packet = new AJAXPacket("<%=request.getContextPath()%>/npage/public/validateRegionCode.jsp","正在验证请稍等......");	
	packet.data.add("inPhone",document.s3216.cus_id.value);
	core.ajax.sendPacket(packet,doShowValidate11);
	packet = null;
}

function doShowValidate11(packet){
	var flag = packet.data.findValueByName("flag");//标志位 true 在同一地市  false 不再同一地市
	if(flag == "false"){
		rdShowMessageDialog("工号和客户手机归属地不一致，操作失败!");
		removeCurrentTab();
...
function changeType(ichange)
{
	...
	if (ichange=='1'){
		...
		s3216.action="f1234.jsp?changType=1&oprationType=change&activePhone1=<%=activePhone1%>&opCode=" + opCode + "&opName=" + opName + "&broadPhone=<%=broadPhone%>";
		s3216.submit();
	}else {
		document.all.cus_pass.disabled = false;
		document.all.cus_pass_button.disabled = false;
		...
		s3216.action="f1234.jsp?changType=0&activePhone1=<%=activePhone1%>&opCode=" + opCode + "&opName=" + opName + "&broadPhone=<%=broadPhone%>";
		s3216.submit();
	}
}
...
<tr>
    ...
    <td>
       ...
       <input type="hidden" name="cus_id" id="cus_id" value="<%=activePhone1%>" />
    </td>
	<%
		}else{
	%>
    ...
    <td>
       <input type="text" size="17" maxlength=11 name="cus_id" id="cus_id" value="<%=activePhone1%>" class="InputGrey" readonly>
    </td>
  <%
  	}
  %>
...
<tr id="identity_print" style="display:none">
    ...
    <td>
        <select name="identity_type" index="15" onChange="changeidtype()">
            ...
            <option value="02">02 --> 身份证核对</option>
            <option value="03">03 --> 非身份证核对</option>
