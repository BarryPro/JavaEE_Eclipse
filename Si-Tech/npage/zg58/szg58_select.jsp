...
%>
		<wtc:service name="szg58Init" routerKey="phone" routerValue="<%=phoneno%>" outnum="19" >
...
<HEAD><TITLE>黑龙江BOSS-补打收据</TITLE>
    ...
    <script language="JavaScript">
        ...
        function docheck()
        {
            ...
            /*
			       if((re_print=="1" ||totaldate!="<%=dateStr%>" )&& "<%=pwrf%>" =="0"&&login_no.substring(0,5)!="~~~~~"){
								rdShowMessageDialog("此收据已经补打过或者不是当天的收据,不允许重复补打!");
								document.form.sure.disabled = true ;
								return false;
							}
							if(re_print=="1" && "<%=pwrf%>" =="0"&&login_no.substr(0,5)=="~~~~~"){
								rdShowMessageDialog("此收据已经补打过,不允许重复补打!");
								document.form.sure.disabled = true ;
								return false;
							}
							if( "<%=pwrf%>" =="0" && login_no!="<%=workno%>"){
								rdShowMessageDialog("此工号不允许打印其他工号的收据!");
								document.form.sure.disabled = true ;
								return false;
							}
						*/
		    ...
		    if(returnValue=="1")//发票
			{
				 document.form.action = 'zg58_print.jsp?pageOpCode=<%=opCode%>&pageOpName=<%=opName%>&contractno=' + document.form.contractno.value + '&payAccept='+login_accept+'&total_date='+totaldate +'&workno=<%=workno%>'+'&check_seq=<%=check_seq%>'+'&pay_money='+pay_money+'&re_op_time='+re_op_time;
			...
			else if(returnValue=="3")
			{
				...
				var pay_note=s_kpxm;
				var returnPage ="../zg58/zg58_1.jsp?activePhone="+phone_no;
				var chbz="1";
...
<BODY>
<FORM action="zg58_print.jsp" method=post name=form>
