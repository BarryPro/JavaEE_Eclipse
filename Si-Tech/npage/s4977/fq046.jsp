...
<%
	...
	String isBaoNian = WtcUtil.repStr(request.getParameter("isBaoNian"),"");
	String anzhuangFangshi = WtcUtil.repStr(request.getParameter("anzhuangFangshi"),"02");
%>
<%
...
<%
     ...
     String pathFlag = request.getParameter("pathFlag");	
		 ...
		 /*2015/01/19 10:33:28 gaopeng 铁通宽带融合项目 取用户的开户主资费，包月费用sql */
...
%>
	...
	<!-- 2013/11/12 16:06:23 gaopeng 修改自己之前的第二批客户敏感信息改造，更换服务sUserCustInfo 为  sQBasicInfo-->
	...
	if(errCodeGetCust.equals("0") || errCodeGetCust.equals("000000")){
		System.out.println("调用服务sQBasicInfo in fq046(s4977).jsp 成功@@@@@@@@@@@@@@@@@@@@@@@@@@");
		gCustName = retBd0002.getValue("2.0");
		System.out.println("调用服务sQBasicInfo in fq046(s4977).jsp custName : "+gCustName);
	}
	else{	 
		System.out.println("调用服务sQBasicInfo in fq046(s4977).jsp 失败@@@@@@@@@@@@@@@@@@@@@@@@@@");
	%>
...
<%String regionCode_sOrderItemShow = (String)session.getAttribute("regCode");%>
 ...
 <%
				...
				out.println("var kdZdType = '';");		//宽带终端类型
				String kdZdType = "";
...
<%     			   	
     			  	for(int m = 0 ; m < servOrderUtype.getSize()-2;m++){ //和sq046不能保持一致！！
     			    	...
     			    	for(int n = 0 ; n< rowcount; n++){
     			    		...
     			    		for (int k = 0 ; k< uuu.getSize()-8;k++){
     			    		  ...
     			    		  %>
     			    		   	...
     			    		   	if(joinMarket == "on"){
     			    		   		/*参加营销案*/
     			    		   		/*2013/07/24 11:10:44 gaopeng 关于协助配置齐齐哈尔公司购终端打折购宽带营销方案的函 加入一个参数，joinType为1时代表是折扣进来的，0就是0元资费进来的*/
     			    		   		if("<%=joinType%>"=="1")
     			    		   		{
     			    		   		...
     			    		   		/*2015/01/19 9:03:30 gaopeng 这里是4977宽带开户 不参加营销案的不是初装费（9003）项*/
     			    		   		/*200为销售品预存的费用代码*/
     			    		   		if("<%=feeCode%>" == "200"){
                    ...
                    <wtc:array id="retFee"  scope="end"/>
     			    			...
     			    			if("000000".equals(retCodeFee)){
     			    			  if(retFee.length>0){
     			    			    ...
     			    			    out.print("feeOrder.push('<select style=width:100px onchange=changeKDvalues(this.value)>");	//压入费用JS数组中
     			    			    ...
     			    			    out.print("</select>');");	
     			    			    */
     			    			    out.print("feeOrder.push('<select name=chuzhuangfei style=width:100px onchange=changeKDvalues(this.value) onclick=getNowSelectVal(this.value)>");	//压入费用JS数组中
     			    			    for(int ii=0;ii<retFee.length;ii++){
                     ...
                     //out.println("feeOrder.push('<select style=width:100px onchange=changeKDvalues(this.value)><option value=100.00 "+chedk11+">100.00</option><option value=90.00 "+chedk10+">90.00</option><option value=80.00 "+chedk1+">80.00</option><option value=70.00 "+chedk2+">70.00</option><option value=60.00 "+chedk3+">60.00</option><option value=50.00 "+chedk4+">50.00</option><option value=40.00 "+chedk5+">40.00</option><option value=30.00 "+chedk6+">30.00</option><option value=20.00 "+chedk7+">20.00</option><option value=10.00 "+chedk8+">10.00</option><option value=0.00 "+chedk9+">0.00</option></select>');");	//压入费用JS数组中
...
<script type="text/JavaScript">
    ...
    $(document).ready(function(){/*diling add for 如获取不到初装费，则不能进行统一缴费@2012/9/19 */
...
%>
					...
					if("<%=opcodeadd%>" == "4977" && (joinMarket != "on" || feeCodeVal == "9003")){
						...
						if(feeCodeVal == "200" && "<%=v_smCode%>" == "kh" && "<%=monthFlag%>" == "true"){
							...
							if(Number(inputObj.value)%monthMoney != 0 && monthMoney!=0){
								rdShowMessageDialog("实收金额必须是"+monthMoney+"的整数倍!",0);	
								return false;
							}
	        ...
	        * 以前用ajax方式调用费用查询,后来根据免费单不走缴费页面的需求,不用这种方式了
	       	*/
		 /* function initData(){
		  		var myPacket = new AJAXPacket("/npage/sq046/fq046_ajax.jsp","正在获得费用信息，请稍候......");
       ...
       */ 	
			...
			function changeRealPayFee(){
				...
				if("<%=opcodeadd%>" == "4977"){
					...
					/*2015/01/19 14:02:24 gaopeng 铁通融合需求 销售品预存时，实收金额必须是sql语句查询出的金额的整数倍*/
					if(feeCodeVal == "200" && "<%=v_smCode%>" == "kh" && "<%=monthFlag%>" == "true"){
							var monthMoney = Number("<%=monthFee%>");
					...
					/*2015/01/19 14:05:41 gaopeng 重新获取一次应收金额*/
					if($(objs[2]).find("select").length > 0){
						ningtnVal = $(objs[2]).find("select").val();
					}else{
						ningtnVal = objs[2].innerHTML;
					...
					//2015/01/19 14:27:15 gaopeng 应收总额 = 实收总额+应收-实收
			//		alert(Number(g("totalFee3").value));
			//		alert(Number(feeValue));
			//		alert(Number(inputObjOld.value));
			//		alert(Number(ningtnVal));
			//		alert(Number(inputObj.value));
					if(shishouFee=="0"){
						shishouFee=g("totalFee3").value;
					}
					else{
						g("totalFee3").value=shishouFee;
					}
					g("totalFee1").value = Number(g("totalFee3").value) + Number(feeValue) - Number(inputObjOld.value) + Number(ningtnVal) - Number(inputObj.value)+Number(g("totalFee2").value);	
				  ...
				  g("totalFee3").value = Number(g("totalFee3").value)-Number(phonePayFee);
				  
				  inputObjOld.value = feeValue;
				  //alert(objs[4].childNodes[2].value);
			}
       
			function changeRealPayFee1(){
				var inputObj = $("select[name='chuzhuangfei']")[0];
				var feeValue = inputObj.value;
				var trObj = inputObj.parentNode.parentNode;
				var objs = trObj.childNodes;
				var inputObjOld =  objs[4].childNodes[1];	//实收文本框后第一个隐藏域,即当前实收副本,用于非法输入后的恢复
  	...
  	var tabobj = g(tableName);
		...
		getBroadMsg(phoneNo);
		  ...
		  $(billArgsObj).attr("10028","");   //参与的营销活动名称：
			...
			/* 2015/04/20 14:42:32 gaopeng 从ONT CPE <%=kdZdType%>改为统一的 宽带终端费用 */
...
function showBroadBill(printType,DlgMessage,submitCfm){
	...
	for (var m = 0 ; m < prtFlagSet.length; m ++){
	  ...
	  $("#servtbl :radio").each(function(i){
		...
		for(i=1; i < tabobj.rows.length; i++){
				...
				}else if(feename.indexOf("宽带设备终端款")!=-1){
					kdZdFee += parseInt(tabobj.rows(i).cells(4).children[0].value);
		...
		printInfo += cashPay + "|";
		printInfo += "宽带开户" + "|";
		printInfo += "初装费：" + installfee + "元" + "~";
		if(Number(prefee) > 0){
			printInfo += "宽带套餐预存款：" + prefee + "元" + "~";
		}
		//liujian 添加pos机缴费 
		if(document.all.payType.value=="BX" || document.all.payType.value=="BY"){	
			printInfo +=  "pos机款为：" + document.all.totalFee3.value + "~";						
		}
		if("<%=v_smCode%>" == "kf" || "<%=v_smCode%>" == "ki"){
				printInfo += "宽带终端费用："+kdZdFee+" 元"+ "|";
		}
		...
		//liujian 添加pos机缴费 参数传递
		printInfo +=  document.all.payType.value + "|";
		
		
		
		if(Number(Number(cashPay)-Number(kdZdFee))==0){
			return;
		}		
		...
		$(billArgsObj).attr("10001","<%=loginNo%>");       //工号
 		...
 		$(billArgsObj).attr("10006","宽带开户"); //业务类别
 		$(billArgsObj).attr("10008",phoneNo); //用户号码
 		$(billArgsObj).attr("10015", ""+Number(Number(cashPay)-Number(kdZdFee))); //本次发票金额(小写)￥
 		$(billArgsObj).attr("10016", ""+Number(Number(cashPay)-Number(kdZdFee))); //大写金额合计
   ...
   $(billArgsObj).attr("10071","8"); //模版
    /********tianyang add at 20090928 for POS缴费需求****end*******/
    var path ="";
  /*2014/11/21 11:00:25 gaopeng 这里使用了宽带账号中开头字母来判断打印什么发票 我建议使用品牌，但是问题来了
  	这个属于统一缴费，断点恢复的获取不到品牌smcode 那该怎么办呢？
  	好办，断点恢复宽带开户获取不到smcode 也就获取不到动态初装费了，它都不好使了咱也不管这个好不好使了。整吧。
  	kh修改为铁通自建 与原来的kd一致，所以这里发票不用改
	...
	return true;
     	  ...
     	  var iss2;
     		function submit_cfm(){
     	  	...
     	  	if("<%=opcodeadd%>" =="1104" ||"<%=opcodeadd%>" =="d535" ||"<%=opcodeadd%>" =="4977"){/* lijy add @20110517 d535,4977分支的添加，用户ims开户,宽带开户免填单打印*/
	     	  	var tableName = "tbl"+servOrderNoArray[0];
	     	  	var tabobj = g(tableName);
	     	  	var simcardfee = 0;
	     	  	var prefee = 0;
     	  	...
     	  	if("<%=errCode%>"!=0){
		   		...
		   		<%if(!"0".equals(isBaoNian)){%>
		   		var isphonePay = $("#isphonePay").attr("checked");
		   		if(isphonePay == true){
		   			if($("#phonePayCheck").val()=="0"){
		   				rdShowMessageDialog("手机支付校验没有通过不可以进行收费处理");
		     	  		return false;
		   			}
		   		}
		   		<%}%>
	       ...
	       */ 
						function conf(){
							...
							for ( var i =1; i < trObjs.length; i++)	{
								...
								//		alert("1-----"+Number(ningtnVal));
								//		alert("2-----"+(Number(tdObjs[3].innerText)+Number(tdObjs[4].childNodes[0].value)+Number(tdObjs[4].childNodes[2].value)));
								//		alert("3-----"+Number(tdObjs[3].innerText));
								//		alert("4-----"+Number(tdObjs[4].childNodes[0].value));
								//		alert("5-----"+Number(tdObjs[4].childNodes[2].value));
								...
								for(var j = 1; j < tdObjs.length-1; j++){
									...
									if(j == 1  || j == 4 || j == 5 || j == 6 ){
										...
										%>
											if(joinMarket == "on"){
												/*2013/07/26 5:36:48 gaopeng 如果是0元资费宽带*/
												if("<%=joinType%>"=="0"){
												...
												/*2013/07/26 5:36:48 gaopeng 如果是打折宽带 仍然正常走*/
												else if("<%=joinType%>"=="1")
													{
														feeStr += inputObj.value; 
													if(j == 4){
...
%>
	   			...
	   			if("<%=anzhuangFangshi%>"=="00"){
	   				rdShowMessageDialog("因为选择了固话同装请到m462进行固话开户!",0);
	   			}
<%
			}
%>
			...
			//2010-7-5 8:46 wanghfa修改 铁通开户和普通开户最后提示信息不同 end
		      ...
		      if((typeof parent.removeTab )=="function")
   			...
   			rdShowMessageDialog("缴费失败!"+err_code+retMessage,0);	
   			$("#fee_submit").attr("disabled",false);	
   			return false;
   		}
   	}
  	...
  	var kdZdFee = "";
		for(i=1; i < tabobj.rows.length; i++){
			...
			<%}else if(opcodeadd.equals("4977")){%>
				...
				showBroadBill("Bill","确实要进行统一缴费发票打印吗？","Yes");
			<%}%>
			...
			myPacket.data.add("jifenPayFee",g("totalFee2").value);
			<%if(!"0".equals(isBaoNian)){%>
				myPacket.data.add("phonepayPhoneNo",$.trim($("#phonepayPhoneNo").val()));
			<%}%>
			
			/** pos机新增参数 start **/
			/* ningtn 如果用代金券的话，缴费类型就用EF */
			if("<%=opcodeadd%>" =="4977" && joinMarket == "on" && "<%=joinType%>"=="0"){
				...
				if("<%=v_smCode%>" == "kf"){
					...
					/*铁通宽带*/
					payTypeVal = "ER";
				}else if("<%=v_smCode%>" == "kg" || "<%=v_smCode%>" == "kh"){ //关于协助开发省广电合作宽带话费营销案和融合套餐需求的函（单品部分）@2014/7/24 
					/*新增省广电kg，备用品牌kh*/
					payTypeVal = "WG";
				}
...
/*tianyang add POS缴费 end*/ 
  ...
  function changeKDvalues1(ss) {
	  
  $("input[name='"+savenames+"']").val(ss);
...
{   //支票支付方式
    ...
    if(document.all.checkRadio.checked == true)
    {
        ...
        <%if(!"0".equals(isBaoNian)){%>
	        document.all.isphonePay.checked = false;
	        ifPhonePayBtn2();
       	<%}%>
...
/*2017/05/20 10:40:56 liangyl 关于协助开发手机代付宽带费用功能的函*/
var phonePayFee=0;
function ifPhonePayBtn(){
	$("#phonepayPhoneNo").val("");
	$("#phonepayPassWord").val("");
	$("#phonepayPhoneNo").attr("class","");
	$("#phonepayPhoneNo").attr("readonly","");
	$("#phonepayPassWord").attr("class","");
	$("#phonepayPassWord").attr("readonly","");
	$("#phonePayCheck").val("0");
	 document.all.payType.value = "0";/*交易类型*/
	document.all.ccb.checked = false;
    document.all.icbc.checked = false;
    ...
    $("#check0").hide();
	$("#chPayNum0").hide();
	if(Number(phonePayFee)!=0){
		$("input[name='realPayFee"+savenames1+"']").val(phonePayFee);
		$("input[name='realfee"+savenames1+"']").val(phonePayFee);
		$("#opchangeFee"+savenames1).val((Number($("#opchangeFee"+savenames1).val())-Number(phonePayFee)));
		phonePayFee="0";
		getNowSelectVal($("input[name='chuzhuangfei']").val());
		changeKDvalues1($("input[name='chuzhuangfei']").val());
	}
	var isphonePay = $("#isphonePay").attr("checked");
	if(isphonePay == true){
		$("#isphonePayTb").show();
...
/*2017/05/20 10:40:56 liangyl 关于协助开发手机代付宽带费用功能的函*/
function getPhonePaygral(){
	
	var isphonePay = $("input[name='isphonePay']").attr("checked");
	
	if(isphonePay){
	
		var iPhoneNo = $.trim($("#phonepayPhoneNo").val());
		var iUserPwd = $.trim($("#phonepayPassWord").val());
		...
		var getdataPacket = new AJAXPacket("../se974/fe974CheckPhonePay.jsp","正在获得数据，请稍候......");
		var iLoginAccept = "0";
		var iChnSource = "01";
		var iOpCode = "<%=opCode%>";
		...
		getdataPacket.data.add("iKDPhoneNo","<%=servPhoneNo%>");
		getdataPacket.data.add("iOfferId","<%=offeridkd%>");
		getdataPacket.data.add("iCustOrderId","<%=custOrderId%>");
		getdataPacket.data.add("iScoreMoney","");
		
		core.ajax.sendPacket(getdataPacket,doRetPhoenPay);
		getdataPacket = null;
	}
	
}
function doRetPhoenPay(packet){
		var retCode = packet.data.findValueByName("retCode");
		var retMsg = packet.data.findValueByName("retMsg");
		if(retCode == "000000"){
			rdShowMessageDialog("校验成功！",2);
			$("#phonepayPhoneNo").attr("class","InputGrey");
			$("#phonepayPhoneNo").attr("readonly","readonly");
			$("#phonepayPassWord").attr("class","InputGrey");
			$("#phonepayPassWord").attr("readonly","readonly");
			$("#phonePayCheck").val("1");
			$.each($("input[type='text'][name^='realPayFee']"),function(){
				if($(this).attr("v_feeCode")=="201"){
					phonePayFee = $(this).val();
					$(this).val("0");
					$("input[name='realfee"+savenames1+"']").val("0");
					$("#opchangeFee"+savenames1).val((Number($("#opchangeFee"+savenames1).val())+Number(phonePayFee)));
					getNowSelectVal($("input[name='chuzhuangfei']").val());
					changeKDvalues1($("input[name='chuzhuangfei']").val());
				}
...
<div id="Operation_Table">
	...
	<div class="search">
		<table cellSpacing=0>
		  <tr>
		   ...
		   <TR  > 
                  ...
                  <TD nowrap  > 
                    <input name="checkRadio" type="radio" onclick="checkWay();" value="check"  index="48">支票交款 <br>	
                   ...
                   <!-- ningtn 20100201 tianyang add for pos --><br>
				    ...
				    <%if(!"0".equals(isBaoNian)){%>
				    <br>
				    &nbsp;<input name="isphonePay" id="isphonePay" on type="radio" onclick="ifPhonePayBtn()"  value="sjdf"/>手机代付
				    	
				    	<input type="hidden" id="phonePayCheck" name="phonePayCheck" value="0"/>
				    <%}%>
                  </TD>
                  <td>&nbsp;</td>
                  <td>&nbsp;</td>
        ...
        <%if(!"0".equals(isBaoNian)){%>
        <table id="isphonePayTb"  cellSpacing="0" style="display:none">
          <tbody> 
            <tr> 
                  <td  nowrap class=blue width="15%"> 
            ...
            <td  width="35%">
              	   <input type="text" name="phonepayPhoneNo" id="phonepayPhoneNo" value=""/>
                    <font color="red">*</font> </td> 
                  <td  class=blue width="15%"> 
                    <div align="left">服务密码:</div>
                  </td>
                  <td  width="35%"> 
                    <input type="password" name="phonepayPassWord" id="phonepayPassWord" value=""/>
                    <input type="button" class="b_text" name="phonepayValide" id="phonepayValide" value="校验" onclick="getPhonePaygral();"/>
                  </td>               
            </tr>            
          </tbody>
        </table>
