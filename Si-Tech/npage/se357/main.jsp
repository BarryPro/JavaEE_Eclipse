...
<%
    ...
    String iUserPwd = "";
		...
		System.out.println("gaopengSeeLog============rCus=="+rCus);
	  ...
	  String org_code = (String)session.getAttribute("orgCode");
		...
		System.out.println("gaopengSeeLog=====op_code="+op_code);
...
%>
 	<wtc:service name="s1234Init" routerKey="phone" routerValue="<%=activePhone%>"  outnum="48" >
...
<%
	System.out.println("gaopengSeeLoge358==========result1==="+result1.length);
	if(result1.length==0){
	%>
...
%>
		<script language=javascript>
			...
			checkPwd_Packet.data.add("phoneNo","<%=activePhone%>");	//移动号码,客户id,帐户id
			...
			function doCheckPwd(packet) {
				...
				if (retResult != "000000") {
					...
					window.location="f1234.jsp?activePhone=<%=activePhone%>&broadPhone=<%=broadPhone%>&opCode=<%=opCode%>&opName=<%=opName%>";
				}
			}
		</script>
<%
/*
		  if(0==Encrypt.checkpwd2((result1[0][5]),passFromPage))		   
				response.sendRedirect("f1234.jsp?ReqPageName=main&retMsg=3&activePhone="+activePhone);
*/
...
%>
		<wtc:service name="sRightQryRecord" routerKey="phone" routerValue="<%=activePhone%>" retCode="s1234PrtQryRetCode" retMsg="s1234PrtQryRetMsg" outnum="2" >
		<wtc:param value="<%=paraAray[8]%>"/>
		<wtc:param value="<%=paraAray[5]%>"/>
		<wtc:param value="<%=paraAray[3]%>"/>
		<wtc:param value="K338"/>
		</wtc:service>
		<wtc:array id="resultRightQry" scope="end"/>
<% 	  
	 		String errCode = s1234PrtQryRetCode;
			String errMsg = s1234PrtQryRetMsg;
			String orCode = "";
			if ("000000".equals(s1234PrtQryRetCode) && resultRightQry.length > 0){
				orCode = resultRightQry[0][2];
			}
			
			if ("123487".equals(errCode) && !"0".equals(orCode)){
				%>
				 <script language="javascript">
		    	rdShowMessageDialog("错误代码：" + "<%=errCode%>" + "，错误信息：" + "<%=errMsg%>" , 1);
	 	      window.location="f1234.jsp?ReqPageName=main&retMsg=8&activePhone=<%=activePhone%>&broadPhone=<%=broadPhone%>&opCode=<%=opCode%>&opName=<%=opName%>";
 	  		</script>
				<%
			}else	if (!"0".equals(errCode)  && !"0".equals(orCode)){
				%>
				<script language="javascript">
	    	
	    	rdShowMessageDialog("错误代码：" + "<%=errCode%>" + "，错误信息：" + "<%=errMsg%>" , 1);
 	      window.location="f1234.jsp?ReqPageName=main&retMsg=7&activePhone=<%=activePhone%>&broadPhone=<%=broadPhone%>&opCode=<%=opCode%>&opName=<%=opName%>";
 	  	</script>
				<%
...
function checkPwdEasy(pwd) {
	...
	checkPwd_Packet.data.add("phoneNo", "<%=activePhone%>");
...
<tr>
    <td nowrap colspan="6" id="footer">
        ...
        <input class="b_foot" type="button" name="b_back" value="返回" onClick="window.location.href='f1234.jsp?activePhone=<%=activePhone%>&broadPhone=<%=broadPhone%>&opCode=<%=opCode%>&opName=<%=opName%>'">
...
function showNext(ret,submitCfn){
...
function printInfo(printType)
{
	...
	}else if(document.all.opCode.value == "1235")
	{
		...
		if (document.frm.idenType.value == "01")
		{
			opr_info+="办理业务："+"<%=opName%>  办理方式: 详单核对"+"  "+"业务流水："+document.all.loginAccept.value+"|";
		}else if (document.frm.idenType.value == "02")
		{
			opr_info+="办理业务："+"<%=opName%>  办理方式：凭证件"+"  "+"业务流水："+document.all.loginAccept.value+"|";
		}else{
			opr_info+="办理业务："+"<%=opName%>"+"  "+"业务流水："+document.all.loginAccept.value+"|";	
		}
	}
   ...
   if(pubSmCode == "kf"){
   		...
   		note_info2 += "备注:"+"|";
   		note_info2 +="1、当联系电话变动时，请及时与移动公司联系，以便于有新活动或服务到期时及时收到通知。"+"|"
   		note_info2 +="2、如需帮助，请拨打服务热线：10086。"+"|"
  	}
...
function printBill(){
	   ...
	   infoStr+='<%=new java.text.SimpleDateFormat("yyyy", Locale.getDefault()).format(new java.util.Date())%>'+"|";
		 ...
		 location="chkPrint.jsp?retInfo="+infoStr+"&dirtPage=f1234.jsp&activePhone=<%=activePhone%>&broadPhone=<%=broadPhone%>&opCode=<%=opCode%>&opName=<%=opName%>";
}
</script>
</html>
