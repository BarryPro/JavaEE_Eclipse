...
function calcFieldValue_grp(calaField)//根据公式计算单个字段的值
{
<%
...
function clearCalcFields_grp(selectId)//当下拉框发生变化时,清空计算字段,并且重值各个编辑框的状态
{
<%
...
function calcAllFieldValues_grp()//计算所有计算字段的值
{
...
function checkDynaFieldValues_grp(haveCalc)//检查编辑框内数据是否合法，haveCalc:是否包含对计算字段的检查
{
...
function segmentAdd_grp<%=jsi%>(){
	...
	tr1.bgcolor="E8E8E8";
	tr1.insertCell().innerHTML = '<td><input class=button type="button" name="del" value="删除" onclick="deleteIt_grp<%=jsi%>(this)"></td>';
		...
		if (resultList != null)
		{
			while(i<fieldCount)
			{
				...
				if(trFlag||i==0)
				{
    				...
    				<TR bgcolor='E8E8E8'>
    				<TD width='20'><input type="button" name="addSegment<%=fieldGroupNo[i]%>" class="button"  value="增加" onclick="segmentAdd_grp<%=fieldGroupNo[i]%>()"></TD>
    				<%
					}
					else
					{
