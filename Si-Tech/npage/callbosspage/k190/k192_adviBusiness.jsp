...
<%
    ...
    String sqlStr="";
    String tableName = "";
    String strOrderSql=" order by t.begin_date desc ";
    if(beginTime!=null&&!"".equals(beginTime)){
       tableName = "dcallcall"+beginTime.substring(0,6);
    }
    String action = request.getParameter("myaction");
System.out.println("=========action======="+action);
    if ("doLoad".equals(action)) {
	      if((beginTime!=null && !beginTime.trim().equals("")) && (endTime!=null && !endTime.trim().equals(""))){
       	 	 sqlFilter += " where   to_char(t.begin_date,'yyyymmdd hh24:mi:ss') >= '"+beginTime+"'  and to_char(t.begin_date,'yyyymmdd hh24:mi:ss') <= '"+endTime+"' ";
       	}
        if(acceptNo!=null){
        if(!("").equals(acceptNo)){
        sqlFilter += " and t.caller_phone = '"+acceptNo.trim()+"'";
        }     	
        }
        if(operNo!=null){
        if(!("").equals(operNo)){
        sqlFilter += " and t.accept_login_no = '"+operNo.trim()+"'";
        } 
        
        }
        String sqlTemp = "select to_char(count(*)) count  from "+tableName+" t "+sqlFilter;
%>	
...
System.out.println("\n\n=====rowCount==========="+rowCount);
        ...
        sqlStr = "select t.contact_id,t.caller_phone,t.callcausedescs,to_char(t.begin_date,'yyyy-MM-dd hh24:mi:ss'),t.accept_login_no  from "+tableName+" t"+sqlFilter+strOrderSql;     
...
function doSubmit(flag){
   ...
   if(flag=='0'){
       ...
       window.sitechform.action="k192_adviBusiness.jsp?con_id="+vCon_id;
		}else{
			 window.sitechform.action="k192_adviBusiness.jsp";
		}
...
for(var i=0;i<e.length;i++){
 ...
 window.location="k192_adviBusiness.jsp";
}
      ...
      <table  cellSpacing="0" >
        <input type="hidden" name="chkBoxNum"  value="<%=dataRows.length%>">
          ...
          <tr >
            ...
            <th align="center" class="blue" width="25%" > 咨询项目 </th>
            <th align="center" class="blue" width="18%" > 时间 </th>
            <th align="center" class="blue" width="14%" > 受理工号 </th>
          </tr>
