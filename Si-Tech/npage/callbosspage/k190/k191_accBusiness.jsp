...
<%
    ...
    String buss_type  = request.getParameter("buss_type");          //业务类型 
    String bussType   = request.getParameter("bussType");          //业务类型id
    ...
    if(beginTime!=null&&!"".equals(beginTime)){
       tableName = "dbcalladm.wloginopr";//beginTime.substring(0,6);
    }
    if ("doLoad".equals(action)) {
	      if((beginTime!=null && !beginTime.trim().equals("")) && (endTime!=null && !endTime.trim().equals(""))){
       	  sqlFilter+= " and to_char(t.op_time,'yyyymmdd hh24:mi:ss') >= '"+beginTime+"'  and to_char(t.op_time,'yyyymmdd hh24:mi:ss') <= '"+endTime+"' ";
       	}
        if(acceptNo!=null){
        if(!"".equals(acceptNo))
           sqlFilter+=" and t.phone_no='"+acceptNo.trim()+"'";
        }
        if(buss_type!=null){
        if(!("").equals(buss_type)){
        sqlFilter+=" and t.op_code='"+buss_type+"'";
        }    
        }
        if(operNo!=null){
        if(!("").equals(operNo)){
        sqlFilter+=" and t.login_no='"+operNo.trim()+"'";
        }
        }
        sqlFilter+=" and t.contact_flag='Y' order by t.op_time desc";
...
<% 
        ...
        sqlStr = "select t.phone_no,decode(t1.op_name,'',t.op_code,t1.op_name),to_char(t.op_time,'yyyy-MM-dd hh24:mi:ss'),t.login_no,t.contact_id,'成功' flag  from "+tableName+" t,sfunccode t1 where 1=1 and trim(t.op_code)=trim(t1.function_code (+))";     
        sqlStr+=sqlFilter;
...
function doSubmit(flag){
	  ...
	  if(flag=='0'){
       ...
       window.sitechform.action="k191_accBusiness.jsp?con_id="+vCon_id;
		}else{
			 window.sitechform.action="k191_accBusiness.jsp";
		}
...
function doLoad(operateCode){
   ...
   window.sitechform.buss_type.vale="<%=buss_type%>";
   window.sitechform.bussType.value="<%=bussType%>"; 
   window.sitechform.operNo.value="<%=operNo%>"; 
...
for(var i=0;i<e.length;i++){
  ...
  window.location="k191_accBusiness.jsp";
}
...
<%@ include file="/npage/include/header.jsp"%>
    ...
    <!-- THE FIRST LINE OF THE CONTENT -->
      <tr >
       <td > 受理号码  </td>
	     <td >
			  <input name="acceptNo"  maxlength="15" type="text" value="<%=(acceptNo==null)?"":acceptNo%>" onkeyup="hiddenTip(this);value=value.replace(/[^\d]/g,'') "onbeforepaste="clipboardData.setData('text',clipboardData.getData('text').replace(/[^\d]/g,''))">
	     </td>
	     <td > 开始时间 </td>
	     <td >
     ...
     <!-- THE FIRST LINE OF THE CONTENT -->
      <tr >
       <td >业务类型</td>
	     <td >
			  <select name="buss_type" id="buss_type" size="1" onchange="bussType.value=this.options[this.selectedIndex].text">
				<%if(buss_type==null  || buss_type.equals("") || request.getParameter("bussType").equals("")|| request.getParameter("bussType")==null){%>
		  	<option value="" selected>--所有业务类型--</option>
				<wtc:qoption name="sPubSelect" outnum="2">
				<wtc:sql>select function_code ,function_code || '-->' || op_name from sfunccode where function_code like 'K%'</wtc:sql>
				</wtc:qoption>    		  	
		  	<%}else {%>
      	 	 	<option value="" >--所有业务类型--</option>		  	
      	 		<option value="<%=buss_type%>" selected >
      	 			<%=request.getParameter("bussType")%>
      	 		</option>
				<wtc:qoption name="sPubSelect" outnum="2">
				<wtc:sql>select function_code ,function_code || '-->' || op_name from sfunccode where function_code like 'K%' and function_code<>'<%=buss_type%>'</wtc:sql>
				</wtc:qoption>      	 	 	
      	 	<%}%>
        </select>
        <input name="bussType" type="hidden" value="<%=request.getParameter("bussType")%>">
        </select>
	     </td>
	     <td > 受理工号</td>
	     <td>
			  <input name ="operNo" type="text" id="operNo"  value="<%=(operNo==null)?"":operNo%>">
		  </td> 
	     <td colspan="2">&nbsp;</td>
	    </tr>
    ...
    <tr >
      ...
      <table  cellSpacing="0" >
        <input type="hidden" name="chkBoxNum"  value="<%=dataRows.length%>">
          ...
          <tr >
            <th align="center" class="blue" width="15%"  > 受理号码 </th>
            <th align="center" class="blue" width="20%" > 业务类型 </th>
            <th align="center" class="blue" width="15%" > 时间 </th>
            <th align="center" class="blue" width="12%" > 受理工号 </th>
            <th align="center" class="blue" width="20%" > 流水号 </th>
            <th align="center" class="blue" width="18%" > 状态  </th>
          </tr>
