...
function addNewNode(){
	...
	if(newNodeName == ""||newNodeId==""){
		rdShowMessageDialog("请输入新增类型的编号和名称！",1);
		return;
	}
	...
	//节点类型由用户决定，操作类的节点是叶子
	var isleaf = nodeType;
	var packet = new AJAXPacket(<%=request.getContextPath()%>"/npage/callbosspage/K196/k196_insertNewNode.jsp","\u6b63\u5728\u5904\u7406,\u8bf7\u7a0d\u540e...");
...
function doneUpdateProcess(packet) {
	...
	if (retCode == "000000") {
	  ...
	  //updated by tangsong 20100501 继续添加功能为实现，去掉此对话框
	  /*
		if(rdShowConfirmDialog("添加类型成功，是否继续添加？",2) != 1){
			window.close();
		}	
		*/
		rdShowMessageDialog("添加类型成功", 2);
		window.close();
	} else {
...
function defineSubNodeId(){
	var pdoc = window.dialogArguments;
	var selectItemId =pdoc.tree.getSelectedItemId();	
	var packet=new AJAXPacket(<%=request.getContextPath()%>"/npage/callbosspage/K196/k196_defineNewNodeId.jsp","\u6b63\u5728\u5904\u7406,\u8bf7\u7a0d\u540e...");	
...
function checkNodeId(el){
	...
	var packet=new AJAXPacket(<%=request.getContextPath()%>"/npage/callbosspage/K196/k196_checkNewNode.jsp","\u6b63\u5728\u5904\u7406,\u8bf7\u7a0d\u540e...");	
