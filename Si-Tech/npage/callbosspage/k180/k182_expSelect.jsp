...
String sqlMulKfCfm="";
    ...
    String stautsflag=request.getParameter("stautsflag");  // 保存用户选择状态新增
		String strCountSql="select to_char(count(*)) count  from dnoticecontent";
	  String strOrderSql=" order by send_time desc ";
	  String statusSql="select t.field_id from DCALLEXPSTATUS t where t.boss_login_no=:loginNo and  t.op_code='K182'";
	  myParamsExp="loginNo="+loginNo;
	  String[] strHead =null;
	  String head[]=null;
	  String excelName="通知发送查询";
	  ...
	  sqlStr+="select "+sql+" from dnoticecontent"+sqlFilter+strOrderSql;
%>	
		<%
   //第一次载入页面，获取上次选中记录
...
<%
        XLSExport.toExcel(strHead,queryList,intMaxRow,excelName,response);
			...
			String deleteStatus="delete from DCALLEXPSTATUS t where t.boss_login_no=:v1 and t.op_code='K182'&&"+loginNo;
			String insertStatus="insert into DCALLEXPSTATUS t(t.boss_login_no,t.op_code,t.field_id,t.create_time) values(:v1,'K182',:v2,sysdate)&&"+loginNo+"^"+seq;
...
<input name="area[]" type="checkbox" id="area[]" seq="0" sql="send_login_no" value="发送工号" onclick="judgeCheckAll('area[]')"/>发送工号<br>
<input name="area[]" type="checkbox" id="area[]" seq="1" sql="receive_login_no" value="接收工号" onclick="judgeCheckAll('area[]')"/>接收工号<br>
<input name="area[]" type="checkbox" id="area[]" seq="2" sql="content" value="通知内容" onclick="judgeCheckAll('area[]')"/>通知内容<br>
<input name="area[]" type="checkbox" id="area[]" seq="3" sql="to_char(send_time,'yyyyMMdd hh24:mi:ss')" value="发送时间" onclick="judgeCheckAll('area[]')"/>发送时间<br> 
<input name="area[]" type="checkbox" id="area[]" seq="4" sql="msg_type" value="消息类别" onclick="judgeCheckAll('area[]')"/>消息类别<br>
...
function s_exp(x){
  ...
  window.sitechform.stautsflag.value = stautsflag;
    if(head==''){
   	...
   	temSql="select "+sql+ "from dnoticecontent"+ window.sitechform.sqlFilter.value;
   submitExp(expFlag);
 }
 function submitExp(str){
 	 window.sitechform.action="k182_expSelect.jsp?exp="+str;
