...
<table width="98%" height="100%" border="0" align="center" cellpadding="1" cellspacing="1">
      ...
      <tr height="10%">
        ...
        <td nowrap> 
        <input name="flashTime" maxlength="15" index="27"  v_must=1 v_maxlength=15 v_type="integer" size="15" value="15" onchange="resetTimer(this)">秒<font class="orange">*</font>
        </td>
      ...
      <tr height="10%"> 
          <td align="right" colspan="10"> 
            ...
            <input class="b_foot" name="btn_internalcall" type="button" value="旁听" onclick="gocalll()">
...
var current_CurState;
	...
	if(current_CurState==0){
		window.opener.parent.similarMSNPop("坐席未签入");
	...
	if(current_CurState==5){
		...
		window.opener.parent.similarMSNPop("坐席处于工作态");
		setTimeout("statusRefreshAgent()", 300);
		return false;
	}
		if(current_CurState==7){
		window.opener.parent.similarMSNPop("坐席处于示忙态");
		setTimeout("statusRefreshAgent()", 300);
		return false;
	}
		if(current_CurState==8){
		window.opener.parent.similarMSNPop("坐席处于请假态");
		setTimeout("statusRefreshAgent()", 300);
		return false;
	}
	if(current_CurState==9){
		window.opener.parent.similarMSNPop("坐席处于学习态");
		setTimeout("statusRefreshAgent()", 300);
		return false;
	}
		if(current_CurState==10){
		window.opener.parent.similarMSNPop("坐席处于调整态");
		setTimeout("statusRefreshAgent()", 300);
		return false;
	}*/
...
if(ret==0||ret==104)
{
	...
	//parent.window.opener.top.chgStatus("13","开始监听");
	//window.opener.chgStatus("3","开始监听");
	
	//window.close();
	//显示监听时显示的内容 by libin 2009-05-08
	var mypacket = new AJAXPacket(<%=request.getContextPath()%>"/npage/callbosspage/K047/selectInfo.jsp","正在处理，请稍候......");
					
	mypacket.data.add("called_no_agent",called_no_agent);//被监听的工号
...
//显示监听时显示的内容的回调函数 by libin 2009-05-08
function doProcess(packet){
	...
	window.opener.document.getElementById('contactingMsg').innerHTML = "正在旁听,工号："+called_no_agent+"，流水号："+contact_id;
}
