...
function initBaseTree(){
	...
	tree.loadXML(<%=request.getContextPath()%>"/npage/callbosspage/checkWork/K310/K310bqcTreeXml.jsp");
...
function onCheckStateChange(id){	
	...
	var checkedGroupIds = pdoc.document.getElementById("left_select_object_id").value;
	var packet = new AJAXPacket(<%=request.getContextPath()%>"/npage/callbosspage/checkWork/K310/K310bqcCheckRelation.jsp?flag=qc","请等待");
	packet.data.add("checkGroupId",id);
	packet.data.add("checkedGroupIds",checkedGroupIds);
	core.ajax.sendPacket(packet,doProcessRelation,true);
	packet=null;
}
...
function doProcessRelation(packet){
	...
	if(retCode=="000000"){
		if(checkedNum.trim()>tmpNum.trim()){
			similarMSNPop("该质检组对已选中的被质检组没有完全操作权限！");
...
function getTreeListByNodeId(strSelectedNodeid){
	var packet = new AJAXPacket(<%=request.getContextPath()%>"/npage/callbosspage/checkWork/K310/K310bqcTreeList.jsp","\u6b63\u5728\u5904\u7406,\u8bf7\u7a0d\u540e...");
