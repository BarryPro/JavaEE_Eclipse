...
function checkLoginNo(){
	...
	if(typeRange=='1'){
		if(parseInt(startLoginNo)>parseInt(endLoginNo)){
			return;
			}
...
<input type="radio"  onclick="isRadioSelect();"  name="typeRange" id="typeRange" value="1">工号范围选择</input>
		...
		<table>
			<tr id="orientation" >
  				<td>定位：</td>
  				<td width="80%">
  				<!--modify by zhengjiang 20090927 正则表达式[^\d]改为[^kf\d]-->
       <input id="loginNo" name ="loginNo" size="15" type="text"  onkeyup="value=value.replace(/[^fk\d]/g,'') "onbeforepaste="clipboardData.setData('text',clipboardData.getData('text').replace(/[^fk\d]/g,''))"  value="">
	  			</td>
			</tr>
				</table>
		</div>
		<div id="div2" style="display: none" >
			...
			<tr id="range" >
  				<td >起始：</td>
  				<td width="80%">
      <input id="startLoginNo" name ="startLoginNo" size="15" type="text"  onkeyup="value=value.replace(/[^fk\d]/g,'') "onbeforepaste="clipboardData.setData('text',clipboardData.getData('text').replace(/[^fk\d]/g,''))" value="">
	  			</td>
			</tr>
			<tr id="range2" >
  				<td>终止：</td>
  				<td width="80%">
       <input id="endLoginNo" name ="endLoginNo" size="15" type="text"  onkeyup="value=value.replace(/[^fk\d]/g,'') "onbeforepaste="clipboardData.setData('text',clipboardData.getData('text').replace(/[^fk\d]/g,''))"   value="">
	  			</td>
			</tr>
				</table>
		</div>
