...
<%
	...
	sqlCount += sqlFilter + "AND t1.is_del='N'";  //查询符合条件的记录数sql
...
for(var i=0;i<e.length;i++){
 ...
 document.getElementById("groupflag").value="0";	//0为多人之间结果查询
}
...
<div id="Operation_Table" style="width: 100%;"><!-- guozw20090828 -->
	<div class="title"><div id="title_zi">个人质检结果查询</div></div>
	...
	<%}%>
        ...
        if(checkRole(K211B_RolesArr)==true){
      	...
      	if(checkRole(K211C_RolesArr)==true){
      		document.write('<img style="cursor:hand" onclick="checkRight(\'<%=dataRows[i][0]%>\',\'<%=dataRows[i][2]%>\',\'<%=dataRows[i][7]%>\',\'<%=dataRows[i][32]%>\',\'<%=dataRows[i][33]%>\',\'<%=dataRows[i][34]%>\');return false;" alt="修改质检结果" src="<%=request.getContextPath()%>/nresources/default/images/callimage/operImage/2.gif"  width="16" height="16" align="absmiddle">&nbsp;');
       	}
        if(checkRole(K211D_RolesArr)==true){
        	if('<%=isModifyed%>'=='true'){
        	  if('<%=dataRows[i][34]%>'=='2'&&'<%=dataRows[i][34]%>'!='3'){
            document.write('<img style="cursor:hand" onclick="getModDetail(\'<%=dataRows[i][0]%>\',\'<%=dataRows[i][21]%>\',\'<%=dataRows[i][22]%>\');return false;" alt="显示修改结果详细情况" src="<%=request.getContextPath()%>/nresources/default/images/callimage/operImage/4.gif"  width="16" height="16" align="absmiddle">&nbsp;');
           }else{
            document.write('&nbsp; &nbsp;');
           }
         }
        ...
        if(checkRole(K211E_RolesArr)==true){
        	if('<%=dataRows[i][35]%>'!='1'){
      		  document.write('<img style="cursor:hand" onclick="checkIsHavePlan(\'<%=dataRows[i][0]%>\',\'<%=dataRows[i][7]%>\',\'<%=dataRows[i][32]%>\',\'<%=dataRows[i][33]%>\',\'<%=dataRows[i][34]%>\',\'<%=dataRows[i][35]%>\')" alt="对该流水进行质检复核" src="<%=request.getContextPath()%>/nresources/default/images/callimage/operImage/5.gif"  width="16" height="16" align="absmiddle">&nbsp;');
      	  }else{
      		  document.write('&nbsp; &nbsp;&nbsp;');
      	  }
      	}
      	if(checkRole(K211F_RolesArr)==true){
      		document.write('<img style="cursor:hand" onclick="deleteQcInfo(\'<%=dataRows[i][0]%>\',\'<%=dataRows[i][34]%>\')" alt="删除该条记录" src="<%=request.getContextPath()%>/nresources/default/images/callimage/operImage/del.gif"  width="16" height="16" align="absmiddle">&nbsp;');
      	}
        if(tempBool=='true'){
      		document.write('</td>');
      	}
