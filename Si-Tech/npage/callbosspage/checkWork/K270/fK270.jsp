...
<%
  ...
  String sql = "";
%>
<wtc:service name="sK270Qry"  routerKey="region"  routerValue="<%=regionCode%>"  outnum="5">
</wtc:service>
...
<html>
	...
	<script type="text/javascript" src="<%=request.getContextPath()%>/njs/csp/checkWork_dialog.js"></script>
        ...
        var retValue = "";
        retValue = window.showModalDialog("fK270_update.jsp?op=add&parentnod=" + parentnode + "&etime=" + etime,"",cssv);
			...
			function mod_data(){
				if(prevclick.indexOf("tdata")==-1){
					similarMSNPop("请选择修改评定原因！");
				}else{
					var etime=new Date();
					var service_name = trim(document.all[""+prevclick].innerText);
					var note = trim(document.all[""+prevclick].nextSibling.innerText);
					var service_id = trim(document.all[""+prevclick].nextSibling.nextSibling.innerText);
					var parent_service_id = trim(document.all[""+prevclick].nextSibling.nextSibling.nextSibling.innerText);
					var valid_flag = trim(document.all[""+prevclick].nextSibling.nextSibling.nextSibling.nextSibling.innerText);
					var param = "&service_name=" + service_name +
											"&note=" + note +
	        ...
	        var cssv = "dialogHeight:"+h+"px; dialogWidth:"+w+"px; dialogLeft:"+l+"px; dialogTop:"+t+"px;toolbar:no; menubar:no; scrollbars:yes; resizable:yes;location:no;status:yes;help:no";
	        var retValue = "";
	        retValue = window.showModalDialog("fK270_update.jsp?op=mod" + param + "&etime=" + etime,"",cssv);
					if(typeof(retValue) != "undefined"){
						retValue = trim(retValue);
						if(retValue != ""){
							var arr = retValue.split("~");
			...
			function del_data(){
				if(prevclick.indexOf("tdata")==-1){
					...
					if(rdShowConfirmDialog("确认删除当前选中的评定原因么？") == 1){
						var myPacket = new AJAXPacket("fK270I_AddMod.jsp","正在提交，请稍候......");
						myPacket.data.add("retType","del");
						myPacket.data.add("login_no","<%=workNo%>");
						myPacket.data.add("service_id",tmpId);
	...
	<body>
		...
		-->	
			<div id="Operation_Table" style="width: 100%;"><!--zhengjiang20090930-->
				<div class="title"><div id="title_zi"><div style="float:left">评定原因定义</div>
