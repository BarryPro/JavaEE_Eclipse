...
function delete_qc_item(){
	...
	if(undefined==item_id||''==item_id||'0'==item_id){
		similarMSNPop("请选择要删除的考评项！");
		return false;
	}
	if(leafNum>0){
		similarMSNPop("请先删除子考评项！");
		return false;
		}
...
function doProcessDelQcItem(packet){
	...
	if(retType=="delQcItem"){
		if(retCode=="000000"){
			...
			similarMSNPop("成功删除测评项！");
			
		}else{
			similarMSNPop("删除测评项失败！");
			return false;
		}
	}
}
...
function manage_level(){
	...
	if("0"==userData1.trim()){
		similarMSNPop("请选择评分的考评项！");
		return false;
		}
	if(selectedItemId == '' || selectedItemId == undefined){
		similarMSNPop("请选择考评项！");
		return false;
	}
	var object_id = document.getElementById("object_id").value;
	var content_id = document.getElementById("content_id").value;
