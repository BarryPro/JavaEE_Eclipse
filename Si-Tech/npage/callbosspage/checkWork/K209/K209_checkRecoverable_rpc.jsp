...
<%
 ...
 String regionCode = org_code.substring(0,2);
	...
	String currTimesSql = "select to_char(plan_time - decode(current_times,null,0,current_times)) from dqcloginplan where plan_id =:plan_id and object_id =:objectid and content_id =:contentid and login_no = :staffno";
	myParams = "plan_id="+plan_id+",objectid="+objectid+",contentid="+contentid+",staffno="+staffno ;
	if(group_flag!=null && "0".equals(group_flag)){
			strSql = "select to_char(nvl(count(*),0)) from dqcinfo where RECORDERNUM = :recordernum and EVTERNO = :evterno  and plan_id = :plan_id and is_del = 'N' " + " and group_flag = :group_flag";
			myParams1 = "evterno="+evterno+",plan_id="+plan_id+",group_flag="+group_flag ;
	}else{
			strSql = "select to_char(nvl(count(*),0)) from dqcinfo where RECORDERNUM =:recordernum and EVTERNO = :evterno  and plan_id = :plan_id and is_del = 'N' " + " and (group_flag is null or group_flag = :group_flag )";
			myParams1 = "evterno="+evterno+",plan_id="+plan_id+",group_flag="+group_flag ;
	}
%>	
