...
<%
  ...
  String sql = "";
%>
<wtc:service name="sK250Qry"  routerKey="region"  routerValue="<%=regionCode%>"  outnum="5">
</wtc:service>
...
<html>
	...
	<script type="text/javascript" src="<%=request.getContextPath()%>/njs/csp/checkWork_dialog.js"></script>
        ...
        var retValue = "";
        retValue = window.showModalDialog("fK250_update.jsp?op=add&parentnod=" + parentnode + "&etime=" + etime,"",cssv);
			...
			function mod_data(){
				if(prevclick.indexOf("tdata")==-1){
					similarMSNPop("请选择修改评语范文！");
				}else{
					var etime=new Date();
					var error_level_name = trim(document.all[""+prevclick].innerText);
					var note = trim(document.all[""+prevclick].nextSibling.innerText);
					var error_level_id = trim(document.all[""+prevclick].nextSibling.nextSibling.innerText);
					var parent_error_level_id = trim(document.all[""+prevclick].nextSibling.nextSibling.nextSibling.innerText);
					var valid_flag = trim(document.all[""+prevclick].nextSibling.nextSibling.nextSibling.nextSibling.innerText);
					var param = "&error_level_name=" + error_level_name +
											"&note=" + note +
											"&error_level_id=" + error_level_id +
											"&parent_error_level_id=" + parent_error_level_id +
	        ...
	        var cssv = "dialogHeight:"+h+"px; dialogWidth:"+w+"px; dialogLeft:"+l+"px; dialogTop:"+t+"px;toolbar:no; menubar:no; scrollbars:yes; resizable:yes;location:no;status:yes;help:no";
	        var retValue = "";
	        retValue = window.showModalDialog("fK250_update.jsp?op=mod" + param + "&etime=" + etime,"",cssv);
					if(typeof(retValue) != "undefined"){
						retValue = trim(retValue);
						if(retValue != ""){
							var arr = retValue.split("~");
	...
	<body>
		...
		-->	
			<div id="Operation_Table" style="width: 100%;"><!--zhengjiang20090930-->
				<div class="title"><div id="title_zi"><div style="float:left">评语范文定义</div>
				...
				<table id="ErrTable" cellspacing="0">
					<tr>
						<th align="left" class="blue" width="20%" ><nobr> 名称 </th>
						<th align="left" class="blue" width="20%" > 描述 </th>
						<th align="left" class="blue" width="20%" ><nobr> 编号 </th>
						<th align="left" class="blue" width="20%" ><nobr> 上层节点 </th>
						<th align="left" class="blue" width="20%" ><nobr> 是否可用 </th>
					</tr>
<%
	int i = 0;
