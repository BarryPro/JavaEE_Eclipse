...
<%
  ...
  String sql = "";
%>
<wtc:service name="sK271Qry"  routerKey="region"  routerValue="<%=regionCode%>"  outnum="5">
</wtc:service>
<wtc:array id="result" scope="end"/>

<html>
	<head>
		<title>典型案例类型定义</title> 
	...
	<script type="text/javascript" src="<%=request.getContextPath()%>/njs/csp/checkWork_dialog.js"></script>
        ...
        var retValue = "";
        retValue = window.showModalDialog("fK271_update.jsp?op=add&parentnod=" + parentnode + "&etime=" + etime,"",cssv);
			...
			function mod_data(){
				if(prevclick.indexOf("tdata")==-1){
					similarMSNPop("请选择修改典型案例类型！");
				}else{
					var etime=new Date();
					var remark_class_name = trim(document.all[""+prevclick].innerText);
					var note = trim(document.all[""+prevclick].nextSibling.innerText);
					var remark_class_id = trim(document.all[""+prevclick].nextSibling.nextSibling.innerText);
					var parent_remark_class_id = trim(document.all[""+prevclick].nextSibling.nextSibling.nextSibling.innerText);
					var valid_flag = trim(document.all[""+prevclick].nextSibling.nextSibling.nextSibling.nextSibling.innerText);
					var param = "&remark_class_name=" + remark_class_name +
											"&note=" + note +
											"&remark_class_id=" + remark_class_id +
											"&parent_remark_class_id=" + parent_remark_class_id +
	        ...
	        var cssv = "dialogHeight:"+h+"px; dialogWidth:"+w+"px; dialogLeft:"+l+"px; dialogTop:"+t+"px;toolbar:no; menubar:no; scrollbars:yes; resizable:yes;location:no;status:yes;help:no";
	        var retValue = "";
	        retValue = window.showModalDialog("fK271_update.jsp?op=mod" + param + "&etime=" + etime,"",cssv);
					if(typeof(retValue) != "undefined"){
						retValue = trim(retValue);
						if(retValue != ""){
							var arr = retValue.split("~");
	...
	<body>
		...
		-->	
			<div id="Operation_Table" style="width: 100%;"><!--zhengjiang20090930-->
				<div class="title"><div id="title_zi"><div style="float:left">典型案例类型定义</div>
