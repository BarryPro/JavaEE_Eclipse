...
<HTML>    
    <HEAD> 
        ...
        <SCRIPT src="<%=request.getContextPath()%>/njs/csp/dhtmlxtree_js/dhtmlxtree.js" type=text/javascript></SCRIPT>
				<!-- added by tangsong 20100823 安徽样式导致树形连接线断开，修正-->
				<style type="text/css">
					#basetree td {
						padding:0;
					}
...
function onDbClickSelect(){
	var pdoc = window.dialogArguments;
	 if(tree.getUserData(tree.getSelectedItemId(),"isleaf")=='Y'){
		pdoc.document.getElementById("objectType").value=tree.getSelectedItemId();
	  	pdoc.document.getElementById("objectTypeName").value=tree.getSelectedItemText();
    	window.close();
  }
...
function initBaseTree(){	
          ...
          tree.setXMLAutoLoading(<%=request.getContextPath()%>"/npage/callbosspage/checkWork/K280/k280_createObjectIdTreeXml.jsp");      
          tree.setOnClickHandler(onNodeSelect);
          tree.setOnDblClickHandler(onDbClickSelect);
          tree.enableHighlighting(1);
          tree.loadXML(<%=request.getContextPath()%>"/npage/callbosspage/checkWork/K280/k280_createObjectIdTreeXml.jsp");   
...
function getTreeListByNodeId(strSelectedNodeid){
	var packet = new AJAXPacket(<%=request.getContextPath()%>"/npage/callbosspage/checkWork/K280/k280_getObjectIdChildTreeList.jsp","\u6b63\u5728\u5904\u7406,\u8bf7\u7a0d\u540e...");
