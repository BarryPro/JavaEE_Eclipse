...
function submitLoad(){
	...
	formbar.action="<%=request.getContextPath()%>/npage/callbosspage/checkWork/K280/K280_loadPage.jsp?iniFlag=1&selectedItemId="+'<%=selectedItemId%>';
	formbar.submit();
}
...
function saveDataOp(){
		...
		if(""==tmpData || tmpData=="undefined"){
				similarMSNPop("请选择有效工号的文件！");
				document.getElementById('submit2').disabled=false;
				return false;
		}
		if(opener.parent.rightTopFrame.document.getElementById("check").checked == true){
				opener.parent.rightTopFrame.document.getElementById("check").click();
		}
		var packet = new AJAXPacket("<%=request.getContextPath()%>/npage/callbosspage/checkWork/K280/k2801/k280_saveLoginNo.jsp","正在导入,请稍后...");
		packet.data.add("selectedItemId",selectedItemId);
		packet.data.add("unCheckValue",tmpData);
		packet.data.add("getDataType","import");
...
function doProcessSaveData(packet){
	...
	if(retCode="000000"){
			...
			if(notIsExitBuffer.length>0 && notIsExitBuffer!=undefined){
					...
					similarMSNPop("导入成功,以下为非法工号：" + tmpNotIsExitBuffer);
					rdShowMessageDialog("导入成功,以下为非法工号：" + tmpNotIsExitBuffer,1);
			}else{
					similarMSNPop("被检工号全部导入成功！");
			}
		*/
			setTimeout("window.close()",1000);
			//parent.frameleft.window.location.reload();
	} else {
			similarMSNPop("被检工号导入失败！");
			setTimeout("window.close()",1000);
	}
}
...
<table width="98%" border="0" align="center" cellpadding="0" cellspacing="0">
  <tr>
    ...
    <table width="100%" border="0" cellpadding="0" cellspacing="0">
    	<tr>
      	<td width=16% class="blue">请选择工号excel文件</td>
      </tr>
      <tr>
        <td width="34%"> 
        <input type="file" size="50"  name="uploadStaffno" id="uploadStaffno" />
        <div id="showAttachmentDiv" style="white-space:normal; word-break:break-all; position: static; width: 80%;" >&nbsp;</div>
