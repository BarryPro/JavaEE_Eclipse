...
<%
 ...
 String myParams="";
  ...
  String[][] result = new String[][] {};
	...
	String sqlStr = "SELECT id,to_char(minlength),to_char(maxlength),to_char(getcount),description FROM dserialNoTimeLength "
	+ " ORDER BY minlength";
%>
...
//统计接触时间设置的总条数
String getTotalCount = "select to_char(sum(getcount)) from dserialNoTimeLength where getcount >= 0 ";
...
<html>
	...
	<script type="text/javascript" src="<%=request.getContextPath()%>/njs/csp/checkWork_dialog.js"></script>
      ...
      function numCheck(opType){
					var mypacket = new AJAXPacket(<%=request.getContextPath()%>"/npage/callbosspage/checkWork/K314/K314_getCount.jsp","正在进行取值，请稍候......");
			...
			function add_data(getCountResult,getTCResult){
		        ...
		        var retValue = "";
		        retValue = window.showModalDialog("K314_update.jsp?op=add&getCountResult="+getCountResult+"&getTCResult="+getTCResult ,"",cssv);
			...
			function mod_data(){
				if(prevclick.indexOf("tdata")==-1){
					similarMSNPop("请选择修改接触时长！");
				}else{
					var id = trim(document.all[""+prevclick].id.substr(5));
					var minlength = trim(document.all[""+prevclick].name);
					var maxlength = trim(document.all[""+prevclick].nextSibling.name);
			        ...
			        var cssv = "dialogHeight:"+h+"px; dialogWidth:"+w+"px; dialogLeft:"+l+"px; dialogTop:"+t+"px;toolbar:no; menubar:no; scrollbars:yes; resizable:yes;location:no;status:yes;help:no";
			        var retValue = "";
			        retValue = window.showModalDialog("K314_update.jsp?op=update" + param ,"",cssv);
					if(typeof(retValue) != "undefined"){
						retValue = trim(retValue);
						if(retValue != ""){
							var arr = retValue.split("~");
			...
			function del_data(){
				if(prevclick.indexOf("tdata")==-1){
						similarMSNPop("请选择要删除的接触时长！");
				}else{
						if(rdShowConfirmDialog("确认删除当前选中的接触时长么？") == 1){
	...
	<body>
		<form name="form1" method="POST">
			<div id="Operation_Table" style="width: 100%;">
				<div class="title"><div id="title_zi"><div style="float:left">接触时长定义</div>
				...
				<table id="ErrTable" cellspacing="0">
					<tr>
						<th align="left" class="blue" width="5%" ><nobr> 最小时长 </th>
						<th align="left" class="blue" width="5%" ><nobr> 最大时长 </th>
