...
<%
    ...
    String remarkflag = request.getParameter("isremarkflag");
    ...
    HashMap pMap=new HashMap();
		...
		pMap.put("remarkflag", remarkflag);		
		pMap.put("flag", flaga);
...
<%
   ...
   if("cur".equalsIgnoreCase(pageFlag)||"all".equalsIgnoreCase(pageFlag)){
	...
	if("cur".equalsIgnoreCase(expFlag)){ //导出当前显示数据
     ...
     strPage = request.getParameter("page");
        ...
        start = (curPage - 1) * pageSize + 1;
		    ...
		    pMap.put("end", ""+end);
		    List queryList2 =(List)KFEjbClient.queryForList("query_K210_expcur",pMap);     
	...
	if("all".equalsIgnoreCase(expFlag)){//导出全部
			    List queryList4 =(List)KFEjbClient.queryForList("query_K210_expall",pMap);     
	...
	if("all".equalsIgnoreCase(expFlag)||"cur".equalsIgnoreCase(expFlag)){
		String deleteStatus="delete from DCALLEXPSTATUS t where t.boss_login_no=:v1 and t.op_code='K210'&&"+loginNo;
		String insertStatus="insert into DCALLEXPSTATUS t(t.boss_login_no,t.op_code,t.field_id,t.create_time) values(:v1,'K210',:v2,sysdate)&&"+loginNo+"^"+seq;
...
<input name="remarkflag"     type="hidden" value="">
...
function s_exp(x){
  ...
  document.all("checkreasondesc").value   = opener.document.all("checkreasondesc").value;
    document.all("remarkflag").value   = opener.document.all("remarkflag").value;
...
function submitExp(str){
	window.sitechform.action="k210_expSelect.jsp?exp="+str;
