...
<head>
	...
	{
		alert("Begin call getLocalCfg()....");
	    var chkPacket = new AJAXPacket("<%=request.getContextPath()%>/npage/callbosspage/K016/K016.jsp","正在获取本机配置信息,请稍后...");
	    chkPacket.data.add("retType", "sK016insert");
		chkPacket.data.add("login_no", "kfaa15");
		chkPacket.data.add("staffstatus","1");
		chkPacket.data.add("checkno","1");
		chkPacket.data.add("ccsworkno", "1");
		chkPacket.data.add("op_code", "1");
		chkPacket.data.add("kf_no", "1");
		chkPacket.data.add("kf_name", "1");
		chkPacket.data.add("class_id", "1");
		chkPacket.data.add("org_id", "1");
		chkPacket.data.add("duty", "1");		
	    core.ajax.sendPacket(chkPacket,doProcessK016,true);
		chkPacket =null;
		alert("End call getLocalCfg()....");
	}
	<%--------------------------------------------------------------------------%>
