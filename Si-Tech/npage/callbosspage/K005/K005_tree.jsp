...
<HTML>
	...
	<BODY >
		...
		<table width="100%" height="90%" >
			...
			<TR>
				<TD vAlign=top>
					<DIV style="height:500" id="baseTree"></DIV>
				</TD>
			</TR>
		</TABLE>
	</BODY>
...
function onClickNodeSelect(){
	...
	if(tree.getSelectedItemId()!=0){
     if(document.getElementById(tree.getSelectedItemId()+'input').checked==true){
		 $("#"+tree.getSelectedItemId()+"input").attr('checked','');
	  }else if(document.getElementById(tree.getSelectedItemId()+'input').checked==false){
	   $("#"+tree.getSelectedItemId()+"input").attr('checked','checked');
	  }
	}
  ...
  getTreeListByNodeId(tree.getSelectedItemId()); //用ajax动态查询数据
	...
	if(flag==0)
	{
	  //获取所有的配置好的技能队列ID（CCNO条件)
	  var  tmpSelItemId= tree.getSelectedItemId();
		var packet = new AJAXPacket(<%=request.getContextPath()%>"/npage/callbosspage/K005/K005_getSkillID.jsp","\u6b63\u5728\u5904\u7406,\u8bf7\u7a0d\u540e...");
		packet.data.add("tmpSelItemId",tmpSelItemId);
		packet.data.add("ccno",window.dialogArguments.document.getElementById("ccno").value);
		core.ajax.sendPacket(packet,getSkillDone,false);
		packet=null;
	}
	flag = 0;
}
function getSkillDone(packet){
		var tmpArr = packet.data.findValueByName("tmpArr");
	  //如果技能队列中含有此ID选中
	  if(document.getElementById(tree.getSelectedItemId()+'input').checked==true){
		parent.frameright.ischeckBoxSelect(tmpArr);
	  }else if(document.getElementById(tree.getSelectedItemId()+'input').checked==false){
	  parent.frameright.ischeckBoxNoSelect(tmpArr);
	  }
}
...
function initBaseTree(){	
    ...
    tree.loadXML(<%=request.getContextPath()%>"/npage/callbosspage/K777/k777_createXml.jsp");
...
function getTreeListByNodeId(){
	...
	var packet = new AJAXPacket(<%=request.getContextPath()%>"/npage/callbosspage/K777/k777_getChildNodeList.jsp","aa");
...
function insertChildNodeList(retData){
    ...
    //过滤当前节点下子节点与数据库是否相同
       if(!isStr(retData[i][0],str)){
        tree.insertNewItem(retData[i][1],retData[i][0],"<input type='checkbox'  id='"+retData[i][0]+"input'/>"+retData[i][2],0,0,0,0,'SELECT') ; 
    ...
    //如果当前节点下无子节点，则在其当前节点下新增子节点
     	...
     	for(var i=0;i<retData.length;i++){
        tree.insertNewItem(retData[i][1],retData[i][0],"<input type='checkbox' disabled='disabled'  id='"+retData[i][0]+"input'/>"+retData[i][2],0,0,0,0,'SELECT') ; 
        tree.setUserData(retData[i][0],"isleaf",retData[i][3]);
        tree.setUserData(retData[i][0],"note",retData[i][4]);		
         if(retData[i][3]=='N'){
        	  tree.setItemImage2(retData[i][0],'folderClosed.gif','folderOpen.gif','folderClosed.gif');
...
function showNewNodeWin(){
	...
	window.showModalDialog("k777_newNodeWin.jsp?time="+time,window, winParam);
}
...
function showModifyNodeWin(){
  ...
  if(nodeId=='0'){
	...
	var time     = new Date();
	var winParam = 'dialogWidth=580px;dialogHeight=400px';
	window.showModalDialog("./k2901/k777_modifyNodeWin.jsp?time="+time,window, winParam);
}
function deleNode(){
		var nodeId = tree.getSelectedItemId();
	  var nodeName =tree.getSelectedItemText(); 
...
function doneProcess(packet) {
	...
	if(retType=="deleteNode"){
			...
			similarMSNPop("删除分组成功！");
			return;
		} else {
			similarMSNPop("删除分组失败！");
			return false;
		}
		}
}
...
function deleteNode(login_group_id,par_id){
	var packet = new AJAXPacket(<%=request.getContextPath()%>"/npage/callbosspage/K777/k2901/k777_deleteNode.jsp","\u6b63\u5728\u5904\u7406,\u8bf7\u7a0d\u540e...");
