...
<html>
	...
	function doLoad(operateCode){
	   ...
	   else if(operateCode=="last")
	   {
	    ...
	    keepValue();
	    window.sitechform.action="k199_Main.jsp" ;
		...
		function jumpToPage(operateCode){
		   ...
		   else if(operateCode=="jumpToPage1")
		   {
			...
			function modifysceTrans(){
				...
				if($("input:checked").length !=1)
				{
				  ...
				  if($(":checkbox").length==1 )
				 {
			    		...
			    		openWinMid('k199_modifysceTrans.jsp?sceid='+document.all.sceid.value.trim(),'修改结点',650,1000);
			   }else
			   {
			   	   
			  	    openWinMid('k199_modifysceTrans.jsp?sceid='+document.all.sceid[$("input:checked")[0].value ].value.trim(),'修改结点',650,1000);
			   }
			...
			if(rdShowConfirmDialog("你确定删除此记录么？")=='1'){	 
				...
				var packet = new AJAXPacket("k199_delsceTrans_rpc.jsp","...");
...
function doProcessdelsceTrans(packet) {
	...
	if (retCode == "000000") {
		rdShowMessageDialog("删除数据成功！",1);	
		 document.location.replace("k199_Main.jsp");	
     window.close();
