...
function manage_level(){
	...
	function unexpert_qc_item(){
		var msg_mod_id = tree.getSelectedItemId();
		
		openWinMid("K503_unexpert.jsp?msg_mod_id="+msg_mod_id,"预定义短信内容导入",250,550);
	}
</script>
...
<table width="100%" border="0" cellpadding="0" cellspacing="0">
	<tr>
	  <td>
		...
		<input type="button" name="btn_unexpert" value="增加导入" class="b_text" onclick="unexpert_qc_item()"/>
	</td>
	</tr>
</table>
...
function insertChildNodeList(retData){
    ...
    //过滤当前节点下子节点与数据库是否相同
       if(!isStr(retData[i][0],str)){
        ...
        tree.setUserData_preconcertContent(retData[i][0],"isleaf",retData[i][3]);
        tree.setUserData_preconcertContent(retData[i][0],"isscored",retData[i][4]);
        tree.setUserData_preconcertContent(retData[i][0],"object_id",retData[i][5]);
       }
       //设置节点显示的图片
       if(retData[i][3]=='N'){
