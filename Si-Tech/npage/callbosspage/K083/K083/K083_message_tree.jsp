...
if(content_id == null || content_id.equals("")){
	sqlGetContent_id = "select to_char(min(contect_id)) from dqccheckcontect where object_id=:object_id and bak1='Y'";
...
function add_qc_item(){
	...
	var time     = new Date();
	var url      = 'K083_add.jsp?time='+time+'&current_node_id=' + current_node_id + '&object_id=' + object_id + 
...
function update_qc_item(){
	...
	var time     = new Date();
	var winParam = 'dialogWidth=800px;dialogHeight=320px';
	window.showModalDialog('K083_update.jsp?time='+time+'&item_id=' + item_id+'&parentItem_id='+parentItem_id, window, winParam);		
}
...
function delQcItem(){
	...
	var item_id = tree.getSelectedItemId();
	var parentItem_id = tree.getParentId(item_id);
	var chkPacket = new AJAXPacket("<%=request.getContextPath()%>/npage/callbosspage/K083/K083_message_delete.jsp?parentItem_id="+parentItem_id+"&object_id="+object_id+"&content_id="+content_id,"请稍后...");
...
<form name="sitechform" method="post"></form>
</body>
</html>
...
function initBaseTree(){
	...
	tree.setXMLAutoLoading("<%=request.getContextPath()%>/npage/callbosspage/K083/K083_message_tree_xml.jsp?id=<%=content_id%>&object_id=<%=object_id%>");
	//设置鼠标单击事件
	tree.setOnClickHandler(onNodeClick);
	tree.enableCheckBoxes(0);
	tree.loadXML("<%=request.getContextPath()%>/npage/callbosspage/K083/K083_message_tree_xml.jsp?id=<%=content_id%>&object_id=<%=object_id%>");
...
function getTreeListByNodeId(strSelectedNodeid){
	...
	var packet = new AJAXPacket("<%=request.getContextPath()%>/npage/callbosspage/K083/K083_message_tree_child.jsp?object_id=<%=object_id%>&content_id=<%=content_id%>", "...");
...
function insertChildNodeList(retData){
    ...
    //过滤当前节点下子节点与数据库是否相同
       if(!isStr(retData[i][0],str)){
        ...
        tree.setUserData_sendMsg_1(retData[i][0],"isleaf",retData[i][3]);
        tree.setUserData_sendMsg_1(retData[i][0],"isscored",retData[i][4]);
        tree.setUserData_sendMsg_1(retData[i][0],"object_id",retData[i][5]);
       }
       //设置节点显示的图片
       if(retData[i][3]=='N'){
