...
//导出Excel
    public void toExcel(String[][] queryList,String[] strHead,HttpServletResponse response){
        ...
        String headname = "呼出失败原因查询";//Excel文件名
        try {
...
<%
    ...
    String opName="综合查询-呼出失败原因查询";
  	...
  	String sqlStr = "select login_no,contact_id,to_char(oper_date,'yyyy-MM-dd hh24:mi:ss'),pre_call_cause_des  from DLOGCALLCAUSEHIS";
    ...
    String[] strHead= {"话务员工号","呼出号码","呼出时间","失败原因描述"};
...
function submitMe(){
   ...
   window.sitechform.action="k1713_callOutFailQry.jsp";
...
function expExcel(expFlag){
	  document.sitechform.action="<%=request.getContextPath()%>/npage/callbosspage/k170/k1713_callOutFailQry.jsp?exp="+expFlag;
...
window.open("k174_callCauseSelectTree.jsp?flag="+strflag,'外呼类型','scrollbars=yes,height=600, width=400');
}
...
function keepValue(){
   ...
   window.sitechform.call_cause.value="<%=request.getParameter("call_cause")%>";
   window.sitechform.last_call_cause_id.value="<%=request.getParameter("3_=_last_call_cause_id")%>";
...
//导出窗口
	function showExpWin(flag){
	  ...
	  window.sitechform.sqlWhere.value="<%=sqlFilter%>";
		...
		openWinMid('k1713_expSelect.jsp?flag='+flag,'选择导出列',340,320);
	 }
...
<%@ include file="/npage/include/header.jsp"%>
    ...
    <!-- THE FIRST LINE OF THE CONTENT -->
      ...
      <td > 开始时间 </td>
      <td >
				<input  id="start_date" name="start_date" type="text"  onclick="WdatePicker({dateFmt:'yyyyMMdd HH:mm:ss',position:{top:'under'}});hiddenTip(this);"  readOnly="true" value="<%=(start_date==null)?getCurrDateStr("00:00:00"):start_date%>">
 ...
 -->
    ...
    <tr >
      <td > 结束时间 </td>
      <td >
			  <input id="end_date" name ="end_date" type="text" readOnly="true"  value="<%=(end_date==null)?getCurrDateStr("23:59:59"):end_date%>" onclick="WdatePicker({dateFmt:'yyyyMMdd HH:mm:ss',position:{top:'under'}});hiddenTip(document.sitechform.end_date);">
		  ...
		  <td > 失败原因</td>
      <td colspan="3">
			 <select id="last_call_cause_id" name="3_=_last_call_cause_id" size="1" onchange="call_cause.value=this.options[this.selectedIndex].text">
			  	<%if(last_call_cause_id==null || last_call_cause_id.equals("") || request.getParameter("call_cause").equals("") || request.getParameter("call_cause")==null){%>
			  	 <option value="" selected>--所有客户级别--</option>
			  	<%}else {%>
			  	    <option value="<%=last_call_cause_id%>" selected >
      	 				<%=request.getParameter("call_cause")%>
      	 			</option>
      	 	 	<option value="" >--所有失败原因--</option>
      	 	<%}%>
					<wtc:qoption name="s151Select" outnum="2">
				<wtc:sql>select failure_code , failure_code|| '-->' ||failure_name from scalloutfailreson</wtc:sql>
				</wtc:qoption>
        </select>
				<input name="call_cause" type="hidden" value="<%=request.getParameter("call_cause")%>"> 
		  </td> 
 </tr>
    ...
    <tr >
      ...
      <table  cellSpacing="0" >
          ...
          <tr >
            ...
            <th align="center" class="blue" width="10%" > 呼出号码 </th>
            <th align="center" class="blue" width="10%" > 呼出时间</th>
            <th align="center" class="blue" width="10%"  > 失败原因描述 </th>
          </tr>
