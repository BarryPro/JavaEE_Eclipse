...
　 * 作者: fangyuan
　 * 版权: sitech
   * update:  yinzx 2009/07/17  客服功能调试  1.修改获取地市的服务名 使用sPubSelect 2 修改了回去地市的语句 去掉了条件 valid_flag = 'Y' 3.去点地市的事件  onchange="region.value=this.options[this.selectedIndex].tex" 不清楚为何要加这个  4.没发现nodetype
...
function updateNode(){
	...
	if(window.opener.checkNode(flag))
	{
	   window.opener.updateOldNode(parNode,nodeName,note,cityid,flag);
  }else
  {
  	   rdShowMessageDialog('对不起,不能单独显示叶子节点!',2);
  }
	window.close();
       ...
       <input id="nodeName" name ="nodeName" size="30" type="text" readOnly  Class="InputGrey" value="">
			...
			<tr>
  				...
  				<option value="" selected>--选择地市--</option>
				   <wtc:qoption name="TlsPubSelCrm" routerKey="region" routerValue="<%=regionCode%>" outnum="2">
	    			...
	    			<font color="orange">*</font>
	    			<input id="flag" type="checkbox" name="flag" onclick="getFlag()">是否显示
	  			</td>	
			</tr>
			...
			<tr >
  				<td colspan="2" align="center">
   					<input name="add" type="button" class="b_text" id="add" value="确定" onClick="updateNode()">
...
function initValue(){	
	...
	if(level=='1'){
      ...
      var nodeName = 	window.opener.tree.getSelectedItemText();
      var cityId = window.opener.tree.getUserData(selectItemId,"cityid");
      var flag = window.opener.tree.getUserData(selectItemId,"visible");
      var note = window.opener.tree.getUserData(selectItemId,"causedes");
		  document.getElementById("nodeName").value=parnertName;
	    document.getElementById("parNode").value=parnertId;
	    document.getElementById("groupName").value=nodeName;
	    document.getElementById("region_code").value=cityId;
