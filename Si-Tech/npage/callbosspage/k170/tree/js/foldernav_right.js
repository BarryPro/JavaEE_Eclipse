...
function iniRootNodes(){
  //alert("iniRootNodes");
  var par_node= {};
  par_node.par_id=gobal_start_node_id; 
  //alert("packet");
  var packet = new AJAXPacket("/npage/callbosspage/k170/k170_test_rpc.jsp","");
  //alert("iniRootNodes2");
  packet.data.add("nodeId",gobal_start_node_id);
...
function iniCallBack(packet){
  ...
  var is_one_leaf=rootNodes.length;
  if(is_one_leaf==0){
  	 document.getElementById('MainImg').src='/npage/callbosspage/k170/tree/images/icon-page.gif';
  	 var newcheck=document.createElement("input");
  	 newcheck.type='checkbox';
  	 newcheck.name='chk'+nodeId;
  	 newcheck.id='chk'+nodeId
  	 //newcheck.onclick="judgeChecked('"+newcheck.name+"')";
  	 document.getElementById('mParent').insertBefore(newcheck,document.getElementById('mainCheck'));
  	 //document.getElementById('chk'+nodeId).checked=true;
  	 judgeChecked(newcheck.name);
  	 document.getElementById('mainCheck').onclick=function(){
  	 	 judgeChecked(newcheck.name);
  	 }
  }
   ...
   for(var i=0;i<rootNodes.length;i++){
     ...
     if(rootNodes[i].is_Leaf==0){//不是叶子
        ...
        +rootNodes[i].node_id+'Folder\'>';
       	is_leaf_onclick='onclick="FolderExpand(\'m'
        +rootNodes[i].node_id+'\',\'\',\'1\',\''+rootNodes[i].node_id+'\');return false;"';
        ...
        +rootNodes[i].node_id+'Folder\'>';
     	  ...
     	  is_leaf_check_onclick='onclick="judgeChecked(\'chk'+rootNodes[i].node_id+'\');return false;"';
     } 
     ...
     if(rootNodes[i].is_Leaf==0){
      parnodeHTML=parnodeHTML+'<A style=\'cursor:hand\' onclick="FolderExpand(\'m'
	     ...
	     +'<DIV traceName=\''+curPardiv.traceName+rootNodes[i].node_name+'->\'  layer=\''+curlayer+'\' class=child id=\'m'+rootNodes[i].node_id+'Child\'>'+'</DIV>'
     	    ...
     	    var is_leaf_check_onclick='onclick="judgeChecked(\'chk'+rootNodes[i].node_id+'\');return false;"';
	        parnodeHTML=parnodeHTML+'<A style=\'cursor:hand\' node_id=\''+rootNodes[i].node_id+'\' traceName=\''+curPardiv.traceName+rootNodes[i].node_name+'\' href="#"  name=\'m'+rootNodes[i].node_id+'a\'>'
	          +layerHTML
	          +img_folder
	          +is_checkbox
	         +'<span  '+is_leaf_check_onclick+'>'+rootNodes[i].node_name+'['+rootNodes[i].node_id+']'+'</span>'
