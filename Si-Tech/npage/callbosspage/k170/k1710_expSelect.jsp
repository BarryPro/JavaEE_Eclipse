...
<%
    ...
    String strCountSql="select to_char(count(*)) count  from DAGENTOPRINFO t1,sforcetype t2,dloginmsgrelation t3,dloginmsgrelation t4";
	  ...
	  String statusSql="select t.field_id from DCALLEXPSTATUS t where t.boss_login_no=:loginNo and  t.op_code='K1710'";
	  myParamsExp="loginNo="+loginNo;
	  String[] strHead =null;
	  String head[]=null;
	  String excelName="坐席强制操作";
	  ...
	  sqlStr+="select "+sql+" from DAGENTOPRINFO t1,sforcetype t2,dloginmsgrelation t3,dloginmsgrelation t4 "+sqlFilter+strOrderSql;
%>	
		<%
   //第一次载入页面，获取上次选中记录
...
<%
        XLSExport.toExcel(strHead,queryList,intMaxRow,excelName,response);
			...
			String deleteStatus="delete from DCALLEXPSTATUS t where t.boss_login_no=:v1 and t.op_code='K1710'&&"+loginNo;
			String insertStatus="insert into DCALLEXPSTATUS t(t.boss_login_no,t.op_code,t.field_id,t.create_time) values(:v1,'K1710',:v2,sysdate)&&"+loginNo+"^"+seq;
...
<input name="area[]" type="checkbox" id="area[]" seq="0" sql="to_char(t1.operate_begin,'yyyy-MM-dd hh24:mi:ss')" value="操作时间" onclick="judgeCheckAll('area[]')"/>操作时间<br>
<input name="area[]" type="checkbox" id="area[]" seq="1" sql="t3.boss_login_no staffno" value="执行工号" onclick="judgeCheckAll('area[]')"/>执行工号<br>
<input name="area[]" type="checkbox" id="area[]" seq="2" sql="t4.boss_login_no operate_object" value="被执行工号" onclick="judgeCheckAll('area[]')"/>被执行工号<br>
<input name="area[]" type="checkbox" id="area[]" seq="3" sql="t2.FORCETYPE_NAME" value="操作类型" onclick="judgeCheckAll('area[]')"/>操作类型<br>
...
function s_exp(x){
 ...
 function submitExp(str){
 	 window.sitechform.action="k1710_expSelect.jsp?exp="+str;
