...
<%
    ...
    String stautsflag=request.getParameter("stautsflag");  // 保存用户选择状态新增
	  ...
	  String[] strHead =null;
	  String head[]=null;
	  String statusSql="select t.field_id from DCALLEXPSTATUS t where t.boss_login_no=:loginNo and  t.op_code='K173'";
	  myParamsExp="loginNo="+loginNo;
	  String excelName="来电转接";
	  ...
	  sqlStr+="select "+sql+" from dcalltransfer t1,sltranstype t2,dloginmsgrelation t3"+sqlFilter+strOrderSql;
%>	
...
<%
        XLSExport.toExcel(strHead,queryList,intMaxRow,excelName,response);
			...
			String deleteStatus="delete from DCALLEXPSTATUS t where t.boss_login_no=:v1 and t.op_code='K173'&&"+loginNo;
			String insertStatus="insert into DCALLEXPSTATUS t(t.boss_login_no,t.op_code,t.field_id,t.create_time) values(:v1,'K173',:v2,sysdate)&&"+loginNo+"^"+seq;
...
<input name="area[]" type="checkbox" id="area[]" seq="1" sql="transfer_login_no" value="转接/拦截工号" onclick="judgeCheckAll('area[]')"/>转接/拦截工号<br>
<input name="area[]" type="checkbox" id="area[]" seq="2" sql="accept_login_no" value="受理/被拦截工号" onclick="judgeCheckAll('area[]')"/>受理/被拦截工号<br>
<input name="area[]" type="checkbox" id="area[]" seq="3" sql="decode(t3.boss_login_no,null,t1.caller_phone,t3.boss_login_no)" value="主叫号码" onclick="judgeCheckAll('area[]')"/>主叫号码<br> 
<input name="area[]" type="checkbox" id="area[]" seq="4" sql="to_char(accept_date,'yyyy-MM-dd hh24:mi:ss')" value="受理时间" onclick="judgeCheckAll('area[]')"/>受理时间<br>
<input name="area[]" type="checkbox" id="area[]" seq="5" sql="sltranstype_name" value="操作类型" onclick="judgeCheckAll('area[]')"/>操作类型<br>
<input name="area[]" type="checkbox" id="area[]" seq="6" sql="skill_quence" value="技能队列" onclick="judgeCheckAll('area[]')"/>技能队列<br>
<input name="area[]" type="checkbox" id="area[]" seq="7" sql="decode(transfer_type,'0','呼叫转移','1','拦截','2','内部求助','3','内部呼叫','4','三方应答','5','夜班抢答','7','旁听','8','插入','9','拦截','10','强制示闲','11','强制示忙','12','强制签出')" value="转接类型" onclick="judgeCheckAll('area[]')"/>转接类型<br>
<input name="area[]" type="checkbox" id="area[]" seq="8" sql="decode(success_flag,'Y','成功','N','失败')" value="结果" onclick="judgeCheckAll('area[]')"/>结果<br>
...
function s_exp(x){
 ...
 function submitExp(str){
 	 window.sitechform.action="k173_expSelect.jsp?exp="+str;
