				...
				for(int i=0;i<r_return_detail.length;i++)
				{
					%>
						<tr>
							...
							<td>
								<select name="ppxx" id="ppxxid" disabled>
						...
						<%}%> 	
							...
							<td>
							<%=r_return_detail[i][9]%> </td>
							</td>
							<td>
							<%=r_return_detail[i][10]%> </td>
							</td>
							
							</td>
							<td>
								<select name="sxzt" id="sxztid" disabled>
									<%
										String s_zt="";
...
%>
          ...
          <tr id="footer"> 
      	    <td colspan="15">
    	      <input class="b_foot" name=back onClick="window.location = 'zg87_del.jsp' " type=button value=返回>
			  <input class="b_foot" name=back onClick="dels()" type=button value=删除>
...
<script language="javascript">
	...
	function dels()
	{
		var	prtFlag = rdShowConfirmDialog("是否确定删除该条记录？");
		if (prtFlag==1)
		{
			var actions = "zg87_del_cfm.jsp?dsxx="+"<%=dsxx%>"+"&ppxx="+"<%=ppxx%>"+"&jflx="+"<%=jflx%>"+"&jsys="+"<%=jsys%>"+"&jfjhmc="+"<%=jfjhmc%>"+"&wl_max="+"<%=wl_max%>"+"&wl_min="+"<%=wl_min%>"+"&jfbl="+"<%=jfbl%>"+"&jsysz="+"<%=jsysz%>";
