	...
	<table cellspacing="0" id="tabList">
			...
			<tr> 
			  <td class="blue">生效时间</td>
				<td class="blue">
				<input type="text" name="shengxiao_time" id="shengxiao_timeid" size="18" readonly="true"/>&nbsp;
			  ...
			  <td class="blue">失效时间</td>
				<td class="blue">
				<input type="text" name="shixiao_time" id="shixiao_timeid" size="18" readonly="true"/>&nbsp;
			...
			<tr style="display:block" id="jfbl_id"> 
			  <td class="blue">积分倍率</td>
				<td class="blue">	
					<input type="text" name="jfbl" onKeyPress="return isKeyNumberdot(1)">
			  </td>            
			  <td class="blue">积分计划名称</td>
				<td class="blue">
					<input type="text" name="jfjhmc" size="10" maxlength="10">
				</td>
		...
		<tr> 
			...
			<td class="blue">备注</td>
				<td class="blue" colspan=3>
					<input type="text" name="beizhu" maxlength="60" size="60">
				</td>
		</tr>
		<tr >
			<td align=center colspan=4><input type=button class="b_foot" name="check2" value="增加" id="cz" onclick="docfm()" >
			<!--
...
<script language="javascript">
			...
			*/
			...
			else if(jsys_new=="VIP_TYPE")
			{
				...
				*/
				...
				document.all.sxzt[document.all.sxzt.selectedIndex].value="0";
				//alert(document.all.sxzt[document.all.sxzt.selectedIndex].value);
	...
	function docfm()
	{
		...
		var jfbl = document.all.jfbl.value;
		var jfjhmc = document.all.jfjhmc.value;
		...
		/*xl add CON_TYPE-消费积分(计算要素值=1)
ON_LINE-网龄积分?才显示6.7最大值、最小值, 计算要素值是空
VIP_TYPE-VIP类型( 0-非VIP,1-是VIP)
BILL_TYPE-星级积分(计算要素值：0-5)
		*/
		
		else if(shengxiao_time=="" || shixiao_time=="")
		{
			rdShowMessageDialog("请输入生效时间和失效时间!");
			return false;
		}
		else
		{
			var	prtFlag = rdShowConfirmDialog("是否确定新增该条记录?");
		 
			if (prtFlag==1)
		    {
				var actions = "zg87_add_2.jsp";
