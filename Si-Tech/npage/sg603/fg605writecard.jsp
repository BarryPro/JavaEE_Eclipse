...
<script language="javascript">
 	function goBack(){
	  window.location.href="fg605Main.jsp?opCode=<%=opCode%>&opName=<%=opName%>";
	}
</script>
</HEAD>
<body>
  ...
  <div class="title">
    ...
    <tr>
          ...
          <font color="red">*</font>
           ...
           <input class="b_text" type="button" name="b_write" value="写卡" onmouseup="writechg(this)" onkeyup="if(event.keyCode==13)writechg()"  index="19" />
...
<script language="javascript">
  ...
  function writechg(obj){
	<%--if("<%=simstypes%>" != simtypess && "g842" == "<%=opCode%>"){
   ...
   // return false;   
	...
	function billdeal(phoneno,phonenames,liushui){
		...
		infoStr+=chineseNumber("<%=vPrepayFee%>")+"|";//合计金额(大写)
		infoStr+="<%=vPrepayFee%>"+"|";//小写
		infoStr+="SIM卡费：0元~入网预存费：<%=vPrepayFee%>元"+"|";
		...
		infoStr+=" "+"|";
		/*
		dirtPate = "/npage/sg603/fg605Main.jsp?opCode=<%=opCode%>&opName=<%=opName%>";
	  ...
	  */
		...
		$(billArgsObj).attr("10006","网上开户物流配送写卡");
		$(billArgsObj).attr("10008",phoneno);
		$(billArgsObj).attr("10015","<%=vPrepayFee%>");//小写
		$(billArgsObj).attr("10016","<%=vPrepayFee%>");//合计金额(大写) 传小写，公共页转换
		...
		$(billArgsObj).attr("10025","<%=vPrepayFee%>");//预存金额
		...
		location = "/npage/sg603/fg605Main.jsp?opCode=<%=opCode%>&opName=<%=opName%>";
