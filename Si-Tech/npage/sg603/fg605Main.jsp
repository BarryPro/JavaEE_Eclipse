...
%>
	...
	<script language="javascript">
			...
			{
				var myPacket = new AJAXPacket("fg603_holdNo.jsp","正在获得记录总数，请稍候...");
				myPacket.data.add("accept","<%=loginAccept%>");
				myPacket.data.add("opCode","<%=opCode%>");
				myPacket.data.add("phoneNo",tempPhone.trim());
				myPacket.data.add("isDispatch","9");//写卡预占传9，外呼预占传6 2013/08/09 11:27:13 gaopeng
					   ...
					   if(dcust2.length>0) {
						...
						%>
								<tr>
									...
									&nbsp;
										<input type="button" id="writeCard<%=i%>" name="writeCard<%=i%>"  value="写卡" class="b_text" onclick="javascript:window.location.href='<%=request.getContextPath()%>/npage/sg603/fg605writecard.jsp?opCode=<%=opCode%>&vOrderId=<%=dcust2[i][13]%>&vPrepayFee=<%=dcust2[i][16]%>&groupids=<%=dcust2[i][3]%>&opName=<%=opName%>&phoneNO=<%=dcust2[i][0]%>&kehuxingming=<%=dcust2[i][4]%>&zhengjianmingcheng=<%=dcust2[i][11]%>&zhengjianhaoma=<%=dcust2[i][5]%>&simming=<%=dcust2[i][9]%>&haomaguishu=<%=dcust2[i][10]%>&dingdanzhuangtai=<%=orderStatus%>&kehudizhi=<%=dcust2[i][12]%>&simtype=<%=dcust2[i][2]%>&kapianhaoma=<%=dcust2[i][6]%>&yonghubianma=<%=dcust2[i][7]%>&simnono=<%=dcust2[i][1]%>&offerids=<%=dcust2[i][17]%>&offernames=<%=dcust2[i][18]%>&offfercomments=<%=dcust2[i][15]%>'"/>
									<%}else{%>
										<input type="button" id="holdOnIt<%=i%>" name="holdOnIt<%=i%>"  value="预占" class="b_text" onclick="holdOnSe('<%=dcust2[i][0]%>','<%=i%>','<%=dcust2[i][13]%>')"/>
									&nbsp;
										<input type="button" id="writeCard<%=i%>" name="writeCard<%=i%>"  value="写卡" disabled="disabled" class="b_text" onclick="javascript:window.location.href='<%=request.getContextPath()%>/npage/sg603/fg605writecard.jsp?opCode=<%=opCode%>&vOrderId=<%=dcust2[i][13]%>&vPrepayFee=<%=dcust2[i][16]%>&groupids=<%=dcust2[i][3]%>&opName=<%=opName%>&phoneNO=<%=dcust2[i][0]%>&kehuxingming=<%=dcust2[i][4]%>&zhengjianmingcheng=<%=dcust2[i][11]%>&zhengjianhaoma=<%=dcust2[i][5]%>&simming=<%=dcust2[i][9]%>&haomaguishu=<%=dcust2[i][10]%>&dingdanzhuangtai=<%=orderStatus%>&kehudizhi=<%=dcust2[i][12]%>&simtype=<%=dcust2[i][2]%>&kapianhaoma=<%=dcust2[i][6]%>&yonghubianma=<%=dcust2[i][7]%>&simnono=<%=dcust2[i][1]%>&offerids=<%=dcust2[i][17]%>&offernames=<%=dcust2[i][18]%>&offfercomments=<%=dcust2[i][15]%>'"/>
									<%}%>
									</td> 
									<td width="8%"><%=dcust2[i][0]%></td>
									<td width="10%">
									<%
										System.out.println("---liujian--"+opCode+"--" + dcust2[i][8]);
...
<script language="javascript">					  	
	...
	function doreturn(){
		if("<%=opCode%>"=="g842"){
			window.location.href = "/npage/sg603/fg605Main.jsp?opCode=<%=opCode%>&opName=<%=opName%>";
		}else{
