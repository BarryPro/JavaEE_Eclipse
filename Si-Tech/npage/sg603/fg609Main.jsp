...
%>
  ...
  Page.prototype.drawOrder = function (button) {
  		...
  		var myPacket = new AJAXPacket("orderDraw.jsp","服务正在提交，请稍候...");
			...
			//g845 m002 都是5 其他的还是25
			if("<%=opCode%>"=="g845"||"<%=opCode%>"=="m002"){
				myPacket.data.add("phoneNo", records[button.attr('line')][5]);/*修改*/
			}else{
				myPacket.data.add("phoneNo", records[button.attr('line')][25]); 
			}
  ...
  $(function(){
	...
	function doreturn(){
		if("<%=opCode%>"=="g845"){
			window.location.href = "/npage/sg603/fg609Main.jsp?opCode=<%=opCode%>&opName=<%=opName%>";
		}else{
