...
%>
 	...
 	Page.prototype.setupArea = function () {
      ...
      for (var i = 0; i < records.length; i++){
	      ...
	      if("<%=selectedRegion%>" == "10014"){
    		...
    		t +=					'<input type="button" value="详细信息" class="b_text showMore"  line="' + i + '"/>';	
    		t +=				'</td>';
    	    t +=		 '</tr>';
    	}
  ...
  Page.prototype.requestUpdateAddr = function (button) {
  		var t = '',self = this;
  		var myPacket = new AJAXPacket("fg603_holdNo.jsp","正在执行服务，请稍候...");
			myPacket.data.add("accept",Constants.ACCEPT);
			myPacket.data.add("opCode","<%=opCode%>");
	...
	Page.prototype.submitRequest = function (button, flag) {
		if(rdShowConfirmDialog('确认提交吗？')==1){
		    ...
		    if(address == "" && flag != '0') {//成功情况下的配送地址不能为空
			...
			var i = button.attr('line');
  			var myPacket = new AJAXPacket("fg603_getResultCfm.jsp","正在获得记录总数，请稍候...");
			myPacket.data.add("accept", Constants.ACCEPT);
			myPacket.data.add("opCode","<%=opCode%>");
...
<form name="frm" method="post" action="">
  ...
  <table cellspacing="0">
  	 <tr>
      <td colspan="7">需向用户复核身份证号码、购买号码和原配送地址3项信息后才能为用户修改配送地址，如果外呼失败，那么外呼三次后才可以录入外呼结果</td>
    </tr>
  </table>
