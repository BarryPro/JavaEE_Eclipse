...
function change()
{
	...
	var dealType="";
	var sqlBuf="";
	var myPacket = new RPCPacket("getBalanceType_new.jsp","正在获得送费明细信息，请稍候......");
...
document.form.action="zgai_2.jsp";
form.submit();
}
}
...
function dynAddRow(){
			...
			var s_tmp;
			s_tmp="<td><div align=center><select name=fee_name class=button><option class=button value=0 selected></option>";
			...
			//String sqlStr ="select * from (select trim(fee_code) || '|' || trim(detail_code) as a,trim(fee_code) || '-' || trim(detail_code) || '--' ||trim(detail_name) from sFeeCodeDetail where fee_code != '*'   union all  select trim(fee_code) || '|' || trim(detail_code),trim(fee_code) || '-' || trim(detail_code) || '--' ||trim(detail_name) from sFeeCodeDetail where trim(fee_code)= 'XY' and trim(detail_code)='q001')  order by a";
			String[] inParas1 =new String[2];
			inParas1[0]="46";
			//sm_code="kd";
			inParas1[1]="s_code="+sm_code;
			//String sqlStr ="select * from (select trim(fee_code) || '|' || trim(detail_code) as a,trim(fee_code) || '-' || trim(detail_code) || '--' ||trim(detail_name) from sFeeCodeDetail where fee_code != '*' and decode(sm_flag,'1','kd','2','ke','3','kf','4','kg') ='"+sm_code+"' )  order by a";
			  //System.out.println("ZZZZZZZZZZZZZZZZZZZZZZZZZZAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA sql is "+sqlStr);
      		inParas1[0]="select * from (select trim(fee_code) || '|' || trim(detail_code) as a,trim(fee_code) || '-' || trim(detail_code) || '--' ||trim(detail_name) from sFeeCodeDetail where fee_code in ('XY','XZ') and detail_code in ('ls','fs','ft','fu','gx','hp') )  order by a";
		  %>
              ...
              <table width="100%" border=0 align=center cellpadding="4" cellspacing=1>
                ...
                <tr bgcolor="#EEEEEE">
                  <td align=center bgcolor="F5F5F5" width="100%">
                    ...
                    <input class="button" name=reset type=reset value=返回 onClick="window.history.go(-1)">
                    &nbsp; </td>
                </tr>
                </tbody>
              </table>
