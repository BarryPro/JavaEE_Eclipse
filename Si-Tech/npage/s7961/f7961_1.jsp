...
<%
	...
	String Pwd1=WtcUtil.repNull(contactInfo.getPasswdVal(2));
...
//			ArrayList retPwd1 = new ArrayList();
...
//			retPwd1 = co.sPubSelect("1",sql,"region",regionCode);
			...
			inParam[0] ="select user_passwd from dcustmsg  where phone_no=:phone_no"; 
			inParam[1] = "phone_no="+phone_no;
%>
	<wtc:service name="TlsPubSelCrm"  outnum="1">
		<wtc:param value="<%=inParam[0]%>"/>
		<wtc:param value="<%=inParam[1]%>"/> 
	</wtc:service>
	<wtc:array id="rt1" scope="end"/>
<%	
	System.out.println("rt1==="+rt1.length);
//			rt1 = (String[][])retPwd1.get(0);
...
//		acceptList = callView.callFXService("s7961Init", paramsIn, "13");
//		callView.printRetValue();
%>
	<wtc:service name="s7961Init" routerKey="region" routerValue="<%=regionCode%>" outnum="13" retcode="retCode1" retmsg="retMsg1">
...
*/			
			...
			sOutUsingCRProdCode =result  [0][8];           
...
<html xmlns="http://www.w3.org/1999/xhtml">
	<HEAD><TITLE>黑龙江BOSS-彩铃秀申请</TITLE>
...
{ 	
	getAfterPrompt();
	document.all.sysNote.value = "手机["+(document.all.phone_no.value).trim()+"]申请彩铃秀业务";
	
	if((document.all.opNote.value).trim().length==0)
	{
      document.all.opNote.value="<%=workno%>[<%=workname%>]"+"对手机["+(document.all.phone_no.value).trim()+"]进行彩铃秀业务申请";
	} 
    showPrtDlg("Detail","确实要打印电子免填单吗？","Yes");
	if (rdShowConfirmDialog("是否提交确认操作？")==1){
		document.form.action="f7961_2.jsp";
...
function doQuery()
	{	
		...
		document.form.action = "f7961_1.jsp?action=select&activePhone=<%=phone_no%>";
		document.form.submit();
...
{
    var path = "<%=request.getContextPath()%>/page/s7961/fpubprod_sel.jsp";
