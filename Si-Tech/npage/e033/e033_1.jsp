...
<%
	...
	String flag="";
	String flag1="";
	if((opCode.equals("e033")) || (opCode=="e033")){
		flag="checked";
	}else{
		flag1="checked";
	}
...
function sel1()
 {
	...
	document.all.busy_type.value = "2";
 	document.all.opCode.value ="e033";
...
function getPhoneKd()
{
	...
	var myPacket = new AJAXPacket("getKdNo_new.jsp","正在查询客户，请稍候......");
		...
		myPacket.data.add("return_page","e033_1.jsp");
...
function doProcess(packet)
{
	...
	if(s_sm_code=="kh")
	{
		rdShowMessageDialog("单品需在zg65进行宽带退费!");
		return false;
	}
	else
	{
...
function docheck()
{
    ...
    if (document.all.busy_type.value=="2") {
		...
		returnValue =window.showModalDialog('getCountdead.jsp?phoneNo_new='+document.getElementById("phoneNo").value+'&password='+document.all.cus_pass.value+'&reqPass=<%=pwrf%>'+'&cfm_lg='+document.getElementById("cfm_lg").value,"",prop);
		//alert("e033 end");
    } else {
		//alert("新写service");
	     returnValue=window.showModalDialog('getCount.jsp?phoneNo='+document.getElementById("phoneNo").value+'&password='+document.all.cus_pass.value+'&reqPass=<%=pwrf%>',"",prop);
	}
	...
	document.form.action="e033_select.jsp";
	form.submit(); 
}
