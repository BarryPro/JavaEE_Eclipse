...
<%
	String opCode = request.getParameter("opCode");
  	...
  	if(opCode.equals("3505")){
  		opName="PushEMail成员管理";
  		flag="selected";
  	}else if(opCode.equals("3506")){
  		opName="PushEMail成员用户销户";
  		flag1="selected";
  	}else{
  		flag2="selected";
  	}
...
%>
	...
	{   //得到成员终端类型和邮件地址
      ...
      {
          rdShowMessageDialog("获取成员信息失败，请重新获取！",0);
  				return false;
  		}
 	}
...
//    var fieldName = "证件号码|客户ID|集团用户ID|集团产品编码|集团ID|业务品牌|";
    ...
    {
        rdShowMessageDialog("请输入身份证号、客户ID、集团ID或集团用户编号进行查询！");
    ...
    {
    	frm.user_no.value = "";
        rdShowMessageDialog("集团产品编号不能为0！");
    	return false;
    }
...
function getCustCode(){
	var getAccountId_Packet = new AJAXPacket("../s3500/f3505_getCustcode.jsp","正在获得成员用户编码，请稍候......");
...
{       
		...
		if (document.frm.billTime.value==""){
			rdShowMessageDialog("计费时间必须填入!");
			return false;
		}
        if(checkFlag < 0){
            rdShowMessageDialog("计费时间:"+document.frm.billTime.value+",日期不合法!!");
...
function changeOp(obj){
	if(obj.value=="1"){
		...
		*/
		...
		document.all.op_name.value="PushEMail成员用户销户";
	...
	if(obj.value=="0"){
		...
		document.all.op_name.value="PushEMail成员管理";
	...
	if(obj.value=="2")
	{
		...
		document.all.op_name.value="pushemail成员资费变更";
...
function getDeteilInfo(){
	...
	var getDeteilInfo_Packet = new AJAXPacket("f3505_getDeteilInfo.jsp","正在获得相关信息，请稍候......");
...
function dDelete(){
	...
	if(document.all.opType.value=="2")
	{
		document.all.sysnote.value="pushEmail成员用户资费变更";
		document.all.tonote.value="pushEmail成员用户资费变更";
	}
	else
	{
		document.all.sysnote.value="pushEmail成员用户销户";
		document.all.tonote.value="pushEmail成员用户销户";
	}
	getSysAccept2();
...
{
	var getSysAccept2_Packet = new AJAXPacket("pubSysAccept.jsp","正在生成操作流水，请稍候......");
        ...
        <TABLE  cellSpacing="0">
          <TR>
            ...
            <TD>
                ...
                <input name=custQuery type=button id="custQuery" class="b_text" onClick="getInfo_Cust();" onClick="if(event.keyCode==13)getInfo_Cust();" style="cursor：hand" value=查询>
...
<%//add
				try
				{
					...
					**/
					sqlStr = "select substr(:org_code,1,2),substr(:org_code,1,7)||'|'||:GroupId,'工号所在地' from dual "
...
%>            
            ...
            <TD >
              ...
              <input name="ProdAttrQuery" type="button" id="ProdAttrQuery"  class="button" onClick="getInfo_ProdAttr();" onClick="if(event.keyCode==13)getInfo_ProdAttr();" value="选择">
			  <font class="orange">*</font>
            </TD>
          </TR>
          <TR id="productcode_div" style="display:'' ">
            ...
            <TD  id="productcode_div" style="display:''" colspan=3>
              ...
              <input name="prodQuery" type="button" id="ProdQuery"  class="button" onClick="getInfo_Prod();" onClick="if(event.keyCode==13)getInfo_Prod();" value="选择">
			  <font class="orange">*</font>
            </TD>
          </TR>
          <TR>
            ...
            <TD  colspan=3>
              ...
              <input name="ProdAppendQuery" type="button" id="ProdAppendQuery"  class="button" onClick="getInfo_ProdAppend();" onClick="if(event.keyCode==13)getInfo_ProdAppend();" value="选择">
            </TD>
          </TR>
<%
}
...
%>
          ...
          <TR>
          	...
          	<TD colspan=3>
          		<select name="opType" onchange="changeOp(this)">
          			...
          			<option value="2" <%=flag2%> >资费变更</option>
          		</select>
          	</TD>
          <%}%>    
          ...
          <TR id="productcode_div" style="display:'' ">
            ...
            <TD colspan=3>
            	<input name="mainProduct" class="button" type="text" v_must=1 v_maxlength=8 v_type="string" v_name="主产品" index="8" value="<%=mainProduct%>" readonly >
          ...
          <TR id="tr1">
		    ...
		    <TD  colspan="1">
              ...
              <input name="UserTypeQuery" type="button" id="UserTypeQuery"  class="b_text" onMouseUp="getInfo_UserType();" onKeyUp="if(event.keyCode==13)getInfo_UserType();" value="选择">
           <font class="orange">*</font>
            </TD>
		   ...
		   <input class="b_text" id="selectAdditive" onkeyup="if(event.keyCode==13)getAdditiveBill()" onmouseup="getAdditiveBill()" style="cursor:hand" type=button value=选择  index="20" >
		   <font class="orange">*</font>
            </TD>
...
%>				
		   ...
		   <TR>
				...
				<input name=cash_num type=button id="cash_num" class="button" onMouseUp="getCashNum();" onKeyUp="if(event.keyCode==13)getCashNum();" style="cursor：hand" value=查询>
				<font class="orange">*</font>
				</TD>
		   </TR>
		 <tr id="cashPay_div" style="display:''">
           ...
           <td class="blue">现金交款</td>
            <td>
				...
				<input name="checkPass" id="next5" type="button" onClick="check_cashPay()" class="button" value="交款校验">
				<font class="orange">*</font>
			</td>
         </tr>
		<tr id="checkNo_div" style="display:none">
            ...
            <td>
				...
				<input name="townCodeQuery" type="button" class="b_text" onClick="getBankCode()" value="查询">
			</td>
           <td class="blue">支票号码</td>
            <td>
				<input class="button" type="text" name="checkNo" maxlength="20" value="">
				<font class="orange">*</font>
				<input name="checkPass" id="next4" type="button" onClick="check_checkNo()" class="b_text" value="验证">    
			</td>
         </tr>
		 <tr id="checkPrePay_div" style="display:none">
            ...
            <td>
				...
				<input name="checkPass" id="next6" type="button" onClick="check_checkPrePay()" class="b_text" value="交款校验">
				<font class="orange">*</font>
			</td>
         </tr>
         	<%}else{%>
  ...
  <TR>
	...
	<TR>
               ...
               <input class="InputGrey" name="sysnote" size="60" 
                <%if(sm_code.equals("pe")) {%>
                value="手机邮箱用户开户 <%=cust_code%>" readonly
                <%}else{%>
                value="集团产品成员开户 <%=cust_code%>" readonly
                <%}%>
               >
               </TD>
           </TR>
          ...
          <TBODY>
            <TR>
			 ...
			 &nbsp;
			  ...
			  <input name="doDelete" class="b_foot"  type=button value="确认" onclick="dDelete()" disabled style="display:none">
			 <%
			 }else {
			 %>
			 <script>
			 ...
			 &nbsp;
			  ...
			  &nbsp;
              <input class="b_foot" name="sure"  type=button value="确  认" onclick="refMain()"  >
			  <input class="b_foot" name="sure2"  type=button value="打印免填单" onclick="refMain2()"  >
			 <%
			 }
			 %>
              &nbsp; 
               <input class="b_foot" name=back  type=button value="清 除" onclick='window.location="f3505_1.jsp?opCode=<%=opCode%>&opName=<%=opName%>"'>
			   &nbsp;
              <input class="b_foot" name="kkkk"  onClick="removeCurrentTab()" type=button value="关 闭">
              </TD>
            </TR>
			<!-------------隐藏域--------------->
