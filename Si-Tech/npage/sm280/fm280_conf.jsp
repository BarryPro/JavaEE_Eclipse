...
%>
 ...
 <%
  ...
  /**
	...
	System.out.println("gaopengSeelogm280~~~~~~~~~~~~~~smCode~~"+smCode);
	if("铁通e固话".equals(smCode)){
		System.out.println("gaopengSeelogm280~~~~~~~~~~~~~~simTypeOne~~"+WtcUtil.repNull(request.getParameter("simTypeOne")));
...
System.out.println("%%%%%%%调用统一接触结束%%%%%%%%"); 	
	   ...
	   if(retCode.equals("0")||retCode.equals("000000")){
    	...
    	String statisUrl1 = "/npage/public/pubSendNPS.jsp"
	  	...
	  	System.out.println("-------hejwa--------------statisUrl1----------->"+statisUrl1);		
   		if (statisOpCode.equals("m280"))
		{
		%>
		...
		<%	
								...
								%>
								        <script>
								         ...
								         location="fm280.jsp?op_code=<%=op_code%>&activePhone=<%=srv_no%>";
									    </script>
								<%
									  }
									  else
								...
								%>
									...
									var path = "/npage/public/pubBillPrintCfm_YGZ.jsp?dlgMsg=客户<%=cus_name%>(<%=cus_id%>)<%=op_name%>已成功，下面将打印发票！";
									...
									var path = path +"&loginAccept="+loginAccept+"&opCode=m280&submitCfm=Single";
									var ret = window.showModalDialog(path,billArgsObj,prop);
									location="<%=request.getContextPath()%>/npage/sm280/fm280.jsp?op_code=<%=op_code%>&activePhone=<%=srv_no%>";
									 </script>
								<%
									  }
