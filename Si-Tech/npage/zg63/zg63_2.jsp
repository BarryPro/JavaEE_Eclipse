...
<%
 	String pwd_flags = request.getParameter("pwd_flags");//0通过 1不通过
  ...
  String dbBand_Account = request.getParameter("dbBand_Account");
  ...
  if (request.getParameter("ispopmarket") != null) {
	...
	System.out.println("000---dbBand_Account=["+dbBand_Account+"]");
	...
	if(busy_type.equals("1") || busy_type.equals("2")) {
		...
		inParas2[0]="select to_char(count(*)) from dHgsalecode a where a.phone_no=:phone_no";
		inParas2[1]="phone_no="+phoneNo;
%>
	<wtc:service name="TlsPubSelBoss" routerKey="phone" routerValue="<%=phoneNo%>"  retcode="retCode1" retmsg="retMsg1" outnum="1">
...
<%
 ...
 /*******芦学琛20060301add,查询开户时间 *****  begin******/
		inParas1[0]="select to_char(a.open_time,'YYYY-MM-DD HH24:MI:SS') from dcustinnet a,dCustMsg b where a.id_no=b.id_no and substr(b.run_code,2,1)<'a' and b.phone_no=:phone_no";
...
<%
 ...
 /*****芦学琛20060301add,查询开户时间 ***** end******/
		...
		if (contractno != null && !contractno.equals("")) {
		  ...
		  inParas1[0] = "select to_char(count(*)) from dconusermsg where contract_no = :contract_no and serial_no = '0'";
	...
	<%
		...
		//xl  这个是原来 单独=1的部分 1 2 可通用
		System.out.println("222---contractno=["+contractno+"]");
		inParas1[0] = "select to_char(nvl(count(*),0)) from dConShort where contract_no = :contract_no";
		inParas1[1] ="contract_no="+contractno;
%>
...
<%
		...
		if(count_num.equals("0")) {
			inParas1[0] = "select to_char(nvl(count(*),0)) from dconusermsg where contract_no = :contract_no and serial_no='0'";
			inParas1[1]="contract_no="+contractno;
		%>
			<wtc:service name="TlsPubSelBoss" routerKey="phone" routerValue="<%=phoneNo%>"  retcode="retCode4" retmsg="retMsg4" outnum="1">
				<wtc:param value="<%=inParas1[0]%>"/>
...
%>
	<%
	inParas1[0] = "select account_type from dconmsg  where contract_no=(select contract_no from dcustmsg where phone_no = :phone_no)  ";	
	inParas1[1]="phone_no="+phoneNo;
...
<%
	...
	<wtc:service name="bs_zg63_Valid" routerKey="phone" routerValue="<%=phoneNo%>" retcode="retCode5" retmsg="retMsg5" outnum="28">
...
/* dujl add at 20090506 for 哈尔滨关于开展中低端预存赠礼的需求* start ********* */
	...
	if(busy_type.equals("1")||busy_type.equals("2"))
	{
		...
		inParas1[0]="select to_char(add_months(open_time,3),'yyyymm'),to_char(sysdate,'yyyymm') From dcustmsg where phone_no=:phone_no";
		inParas1[1]="phone_no="+phoneNo;
%>
...
<%
	String sqlStr1 = "";
	inParas1[0] = "select to_char(id_no),to_char(substr(belong_code,0,2)) from dcustmsg where phone_no=:phone_no ";
	inParas1[1]="phone_no="+phoneNo;
%>
<wtc:service name="TlsPubSelBoss" routerKey="region" routerValue="<%=regionCode%>"  retcode="retCode1" retmsg="retMsg1" outnum="2">
		<wtc:param value="<%=inParas1[0]%>"/>
...
<%
	...
	String sqlStr2 = "";
	inParas1[0] = " select to_char(count(*)) from wawarddata where phone_no= :phone_no and id_no = :id_no and award_code='01' and award_detail_code='5064'";
	inParas1[1]="phone_no="+phoneNo+",id_no="+id_no;
	String[] inPutArray2 = new String[2];
	inPutArray2[0]=phoneNo;
	inPutArray2[1]=id_no;
%>
<wtc:service name="TlsPubSelCrm" routerKey="phone" routerValue="<%=phoneNo%>" retcode="retCode6" retmsg="retMsg6" outnum="1">
		<wtc:param value="<%=inParas1[0]%>"/>
		<wtc:param value="<%=inParas1[1]%>"/>
</wtc:service>
...
<%
		...
		inParas1[0]= "select to_char(count(phone_no)) from dcustmsg where substr(belong_code,1,2) = '01' and substr(belong_code,3,2) in ('02','01') and phone_no = :phone_no";
		inParas1[1]="phone_no="+phoneNo;
		
%>
<wtc:service name="TlsPubSelBoss" routerKey="phone" routerValue="<%=phoneNo%>" retcode="retCode_card" retmsg="retCode_card" outnum="1">
		<wtc:param value="<%=inParas1[0]%>"/>
		<wtc:param value="<%=inParas1[1]%>"/>
...
//xl add 只可以办理一次 begin 5906
String sqlStr2_card = "";
	inParas1[0] = " select to_char(count(*)) from wawarddata where phone_no= :phone_no and id_no = :id_no and award_code='01' and award_detail_code='5906'";
	inParas1[1]="phone_no="+phoneNo+",id_no="+id_no;
	String[] inPutArray2_card = new String[2];
	inPutArray2_card[0]=phoneNo;
	inPutArray2_card[1]=id_no;
%>
<wtc:service name="TlsPubSelCrm" routerKey="phone" routerValue="<%=phoneNo%>" retcode="retCode6_card" retmsg="retMsg6_card" outnum="1">
		<wtc:param value="<%=inParas1[0]%>"/>
		<wtc:param value="<%=inParas1[1]%>"/>
</wtc:service>
		<wtc:array id="resultxl2_card" scope="end" />
<%
 	
String num11_card = " ";
	if(resultxl2_card!=null&&resultxl2_card.length>0){
		num11_card = (resultxl2_card[0][0]).trim();
...
<%
	...
	if( 0 != phoneNo.trim().length() )
	{
		...
		inParas1[0]="SELECT to_char(contract_no_pay) FROM dSharePayMember WHERE id_no = (SELECT id_no FROM dCustMsg WHERE phone_no = :phone_no) AND begin_ymd <= to_char(SYSDATE, 'yyyymmdd') AND end_ymd >= to_char(SYSDATE, 'yyyymmdd') AND end_ymd >= to_char(SYSDATE, 'yyyymmdd') AND op_code = 'd977' ";
		inParas1[1]="phone_no="+phoneNo;
	}
	else if( 0 != contractno.trim().length() )
	{
		...
		sqlStrSharePay+="   AND end_ymd >= to_char(SYSDATE, 'yyyymmdd')";
		sqlStrSharePay+="   AND op_code = 'd977'";*/
		inParas1[0]="SELECT to_char(contract_no_pay) FROM dSharePayMember WHERE contract_no = :contract_no  AND begin_ymd <= to_char(SYSDATE, 'yyyymmdd') AND end_ymd >= to_char(SYSDATE, 'yyyymmdd') AND op_code = 'd977'";
		inParas1[1]="contract_no="+contractno;
	}
...
<wtc:service name="TlsPubSelBoss" routerKey="phone" routerValue="<%=phoneNo%>" retcode="retCode_sharePay" retmsg="retMsg_sharePay" outnum="1">
		<wtc:param value="<%=inParas1[0]%>"/>
		<wtc:param value="<%=inParas1[1]%>"/>
</wtc:service>
...
<!--  20130625 宽带未峻工，不可交费 begin-->
<%
	inParas1[0]="";
	inParas1[1]="";
	
	inParas1[0]+="SELECT to_char(count(*)) " ;
	inParas1[0]+="  FROM dCustMsg a, dbroadbandmsg b " ;
	inParas1[0]+=" WHERE a.id_no = b.id_no " ;
	inParas1[0]+="   and b.cfm_login = :dbBand_Account " ;
	inParas1[0]+="   and substr(a.run_code, 2, 1) = 'P' " ;
	
	inParas1[1]="dbBand_Account="+dbBand_Account;
%>

	<wtc:service name="TlsPubSelBoss" routerKey="phone" routerValue="<%=phoneNo%>" retcode="retCode_band" retmsg="retMsg_band" outnum="1">
			<wtc:param value="<%=inParas1[0]%>"/>
			<wtc:param value="<%=inParas1[1]%>"/>
	</wtc:service>
	<wtc:array id="result_P" scope="end" />

<%
		if( 0 != result_P.length)
			if( !result_P[0][0].trim().equals("0") )
			{
%>
				<script language=javascript>
					rdShowMessageDialog("宽带未峻工，无法交费");
					window.location.href="<%=return_page%>";
				</script>
<%
			}
%>
<!--  20130625 宽带未峻工，不可交费 end-->

<!--  20120517 宽带营销案，是绑定用户且停机的宽带不能缴费 begin-->
<%
	inParas1[0]="";
	inParas1[1]="";
	//is_ok1 是否为参与宽带营销案
	//is_ok2 绑定手机号是否为 I J a b c 状态
	//is_ok3 绑定手机号是否在dCustMsg 中。
	inParas1[0]+="SELECT to_char(is_ok1||is_ok2||is_ok3) FROM " ;
	inParas1[0]+="(SELECT count(*) is_ok1 FROM mk_phonebinding_info a " ;
	inParas1[0]+="  WHERE a.bdband_account = :dbBand_Account " ;
	inParas1[0]+="  and sysdate between start_date and end_date) , " ;
	inParas1[0]+="(SELECT count(*) is_ok2 FROM dCustMsg a , mk_phonebinding_info b " ;
	inParas1[0]+="  WHERE a.id_no = b.id_no and b.bdband_account = :dbBand_Account " ;
	inParas1[0]+="  and sysdate between b.start_date and b.end_date and substr(a.run_code,2,1) = any('I','J','a','b','c') and rownum < 2 ) , " ;
	inParas1[0]+="(SELECT count(*) is_ok3 FROM dCustMsg a , mk_phonebinding_info b " ;
	inParas1[0]+="  WHERE a.id_no = b.id_no and b.bdband_account = :dbBand_Account " ;
	inParas1[0]+="  and sysdate between b.start_date and b.end_date and rownum < 2) " ;
	
	inParas1[1]="dbBand_Account="+dbBand_Account+",dbBand_Account="+dbBand_Account+",dbBand_Account="+dbBand_Account;
%>
	...
	<wtc:array id="result_band" scope="end" />

<%
		if( 0 != result_band.length)
			if( result_band[0][0].trim().equals("111") 
			|| result_band[0][0].trim().equals("100") )
			{
%>
				<script language=javascript>
					rdShowMessageDialog("客户您参与宽带营销案，手机号处于预销、预拆状态，不得为宽带缴费，请务必保证参与活动手机号状态正常");
					window.location.href="<%=return_page%>";
				</script>
<%
			}
%>
<!--  20120517 宽带营销案，是绑定用户且停机的宽带不能缴费 end-->
...
function searchUnbillDetail() {
	...
	var returnValue = window.showModalDialog('../e005/s1300UnbillDetail.jsp?phone_no=<%=phoneNo%>&contractno=<%=contract%>',"",prop);
}
...
<input type="hidden" name="pwd_flag" value="<%=pwd_flags%>">
...
<!--xl add for  发票显示-->
<input type="hidden" name="yckye"  value="">
<input type="hidden" name="wcz"  value="">
...
<%@ include file="/npage/include/header.jsp" %>
		...
		<div class="title">
          ...
          %>
				...
				%>
							...
							<table cellspacing="0">
                <tr nowrap>
                  <td class="blue" width="15%">选择账期类型</td>
                  <td>
                    <select name="zqlx">
     ...
     %>
              ...
              <table cellspacing="0">
                ...
                <tr>
                  <td class="blue" noWrap>用户当前资费</td>
				  <td class noWrap><%=s_zf%></td>
                ...
                <tr>
                  <td noWrap colspan="6" id="footer">
                    <div align="center">
                      ...
                      &nbsp;
                      <input type="button" name="return1" class="b_foot" value="返回" onClick="window.location.href='zg63_1.jsp'">
                      &nbsp;
     ...
     with(document.frm)
     {
			...
			if(!dataValid( 'b' , paymoney))
		   {
			...
			//xl add for 判断资费是包年or包月 比较金额 begin
			 ...
			 var s_zqlx_value = document.all.zqlx[document.all.zqlx.selectedIndex].value;
			 //alert("s_zqlx_value is "+s_zqlx_value);
			 if(s_zqlx_value=="0")//包月
		     {
				 i_count=paymoney%s_zf;
				 //alert(i_count);//判断是否可以整除 i_count=0代表整除
				 if(i_count=="0")
				 {
				 ...
				 {
					 rdShowMessageDialog("续费金额为"+payMoney.value+"元,用户资费为"+s_zf+"元,续费金额不是资费的整数倍,不允许续费!");
					 return false;
				 }
			 }
			 else if(s_zqlx_value=="1")//包年
			 {
				 if(paymoney>=s_zf*12)
				 {
					 i_count=paymoney%(s_zf*12);
					 //alert(i_count);
					 if(i_count=="0")
					 {
						payMoney.value = formatAsMoney(paymoney);
				 ...
				 {
					 rdShowMessageDialog("续费金额为"+payMoney.value+"元,包年金额为"+s_zf*12+"元,不足包年金额!");
					 return false;
				 }	
