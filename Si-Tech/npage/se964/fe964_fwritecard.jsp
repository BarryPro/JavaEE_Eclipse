...
<%  String path=request.getRealPath("");
    ...
    if(!"e964".equals(op_code)){
%>
		 	<script>
				alert("此功能只提供 e964·领取网上选号SIM卡 模块进行写卡操作！");
				window.close();
			</script>
<%
    }
    ...
    String selectRegionCode = regioncode == null ? "" : regioncode;
	 		//得到写卡数据
	 	String sqlStr="select trim(imsi_no),trim(sim_no)||','||trim(imsi_no)||','||trim(eki_no)||','||trim(smsc_no)||','||trim(pin1_no)||','||trim(pin2_no)||','||trim(puk1_no)||','||trim(puk2_no)  from dBlkCardData where  sim_status='q' and sim_no='"+sim_no+"' and  phoneno_head in (select phoneno_head from shlrcode "
 				+" where hlr_code = (select hlr_code from shlrcode where phoneno_head=substr('"+phone+"',1,7)";
 				if(selectRegionCode != null && selectRegionCode.length() > 0){
 					sqlStr += " and region_code='"+selectRegionCode+"'";
 				}
 				sqlStr += "))";
...
function savedata(){
	//var getjysim_Packet = new AJAXPacket("/npage/innet/fsavecard_1104.jsp","正在修改空卡状态，请稍候......");  sungq
...
function turnToDepend()
{
    ...
    var retInfo = retCode+"|"+returnsim+"|"+document.all.cardstatus.value+"|"+document.all.card_no.value +"|";   // add by sungq
    //alert(retInfo);
