...
<%
		...
		//开具信息查询DINVOICE_TAX_split begin
		...
		//end of 查询DINVOICE_TAX_split拆分信息表
		%>
 ...
 ******************************************************/
  ...
  {   
	...
	//	alert("5 err_code is "+err_code+" and err_msg is "+err_msg);// 0000 成功
		if(err_code=="0000"){ 
			...
			//	alert("0000判断成功！！！！！！！！！！！！！！！ and TaxCode is "+TaxCode);//TaxCode有值
			}
		}
		var sidType = "SID01";
		var strREQ ;
	...
	//	alert("SID01入口报文测试 strREQ is "+strREQ+" and strSIDMsg is "+strSIDMsg+" and sidmsgDom is "+sidmsgDom);
	//	alert1("SID01测试 strREQ is "+strREQ+" and strSIDMsg is "+strSIDMsg+" and sidmsgDom is "+sidmsgDom+" and showMesgInfoDetail01(sidmsgDom.toString(),sidType) is "+showMesgInfoDetail01(sidmsgDom.toString(),sidType));
		...
		if(showMesgInfoDetail01(sidmsgDom.toString(),sidType)=="SID01_true")
		{
			...
			{
				//alert("多税率的重新整!");//肯定有 11 和 6的 fpmx的比较难弄  makes_data_muti
				var sidType = "SID05";
				var str="";
	...
	//	alert("add test sidmsgDom is "+sidmsgDom+" and sidmsgDom.toString() is "+sidmsgDom.toString());
		...
		//alert("第一次 xmlInfoDoc is "+xmlInfoDoc +" and elementsErr is "+elementsErr+ "and elementsData is "+elementsData);
		if(sidType=="SID01")
		{
...
<input type="hidden" name="tax_contract_no" value="<%=tax_contract_no%>">
		...
		<input type="hidden" id="invoice_number">
		  ...
		  <table cellSpacing="0">
			...
			<input type="hidden" id="print_accept" value="<%=cfls%>"><!--传拆分的流水号码?-->
			<input type="hidden" id="print_ym" value="<%=begindate%>"><!--查询年月 是否可以?-->
			<tr> 
			  <td id="footer" colspan=8> 
			    
				  <input type="button" name="return1" class="b_foot" value="专票打印" onclick="doPrint(1,1)" >
				  &nbsp;
				  <!--
