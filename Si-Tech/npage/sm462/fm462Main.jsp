...
<%
	if(retCode.equals("000000")){
	  ...
	  System.out.println("liangyl---------ipAddType---------------"+ipAddType);
	 	System.out.println("liangyl---------user_type----------------"+user_type);
	 	System.out.println("liangyl---------bandWidth----------------"+bandWidth);
	 	System.out.println("liangyl---------prefix----------------"+prefix);
	 	System.out.println("liangyl---------domainName----------------"+domainName);
	 	System.out.println("liangyl---------userRegionName----------------"+userRegionName);
	 	System.out.println("liangyl---------offerName----------------"+offerName);
	 	System.out.println("liangyl---------offExpSet----------------"+offExpSet);
	 	System.out.println("liangyl---------offExpSetUnit----------------"+offExpSetUnit);
	 	System.out.println("liangyl---------offExpDate----------------"+offExpDate);
	}else{
%>
<script>
...
//	var suijiMima = Math.round(Math.random()*1000000);
//	$("#cfmPwd").val(suijiMima);
	
//	document.all.userpwd.value="123123";
//	document.all.userpwdcfm.value="123123";
//	document.all.cfmPwd.value=suijiMima;
//	document.all.cfmPwdConfirm.value=suijiMima;
	 $("#ifMBH").val(1);
	   	ifMBHShow();
	   	$("#yuankuandaiTd1").hide();
	   	$("#yuankuandaiTd2").hide();
...
function mySub()
{
    ...
    alert($("#noPort").val()+"--"+document.all.isYzResource.value+"--"+document.all.isDoNoResource.value);
    if(($("#noPort").val() != "1") && 
  ...
  //   alert(1292);
    ...
    if($("input[name='op_note']").val() == ""){
      $("input[name='op_note']").val("操作员"+"<%=workNo%>"+"对客户"+"<%=gCustId%>"+"进行IMS固话开户!"); 
    }
    //压入基本销售品
...
{ 
	 idleResInfo();
	 var anzhuangFangshi = $("#anzhuangFangshi").val();
	 if(anzhuangFangshi==""){
		 rdShowMessageDialog("请先选择安装方式后,再进行资源查询！");	
		 $("#anzhuangFangshi").focus();
		 return false;
	 }
	 $("#jiaoyanFlag").val("1")
	 
	 var jiaoyanFlag = $("#jiaoyanFlag").val();
	 if(jiaoyanFlag==""){
		 rdShowMessageDialog("请先选择安装方式后,再进行资源查询！");	
		 $("#jiaoyanFlag").focus();
		 return false;
   ...
   if(document.all.isYzResource.value!="1"){
	   ...
	   if(anzhuangFangshi=="02"){
		   path ="queryResource.jsp?opCode="+"<%=opCode%>"+"&smCode="+smcodess;
	   }
	   else{
		   path ="queryResource1.jsp?opCode="+"<%=opCode%>"+"&smCode="+smcodess+"&iObjectType="+anzhuangFangshi;
	   }
	   
	   window.open(path,'装机资源查询','width=840px,height=600px,left=100,top=50,resizable=yes,scrollbars=yes,status=yes,location=no,menubar=no');
    }else{	 
    		yzResource();//释放资源 	
    }
} 
...
function returnResBack(retInfo)
{
  ...
  document.all.factory.value=oneTok(resContent, "|", 11);
	...
	*/
	...
	document.all.kuangdaiZhanghao.value=oneTok(resContent, "|", 23);
	
	document.all.kuandaiQuxian.value=oneTok(resContent, "|", 24);
...
function sfResource(){
	var myPacket = new AJAXPacket("ajax_yzResource.jsp", "正在校验，请稍候......");
	myPacket.data.add("serviceOrder","<%=servBusiId%>"  );
	myPacket.data.add("customerCode",$("#phoneNo_h").val());//document.all.cfm_login.value);/*用户编号*/
...
function yzResource()
{
	  if(document.all.isGetAreaResource.value=="0"){
		...
		if(document.all.contactPhone.value==null || document.all.contactPhone.value=="" || document.all.contactPhone.value=="0"){
		 ...
		 if($("#phoneNo_h").val()==""||$("#phoneNo_h").val()==null){
			 rdShowMessageDialog("服务号码不可以为空，请输入");
			 $("#phoneNo_h").focus();
			 return false;
		 }
		 if(document.all.kuangdaiZhanghao.value==null || document.all.kuangdaiZhanghao.value==""){
				rdShowMessageDialog("宽带账号不可以为空，请输入");
				document.all.kuangdaiZhanghao.focus();
				return false;
			 }
...
/*预占接口 2013/2/26 星期二 16:27:36 gaopeng*/	
				   ...
				   myPacket.data.add("serviceOrder","<%=servBusiId%>"  );
						...
						myPacket.data.add("setType", "4");
						myPacket.data.add("relationProductName", document.all.kuangdaiZhanghao.value);
  ...
  var iType = packet.data.findValueByName("iType");
	       if(retCode=="000000"){	//表示调用服务成功
			...
			//				document.all.cfm_loginF.readOnly=true;
	       	 ...
	       	 rdShowMessageDialog(retMsg,0);
	       	$("#phoneNo_h").attr("readonly","");
			$("#chkPhone").removeAttr("disabled");
...
function chk_Phone(){
	var phoneNo = $("#phoneNo_h").val().trim();
	if(phoneNo == ""){
...
function doChkPhoneBack(packet){
	...
	if(errorCode != "000000"){
		...
		rdShowMessageDialog("校验成功",2);
		$("#phoneNo_h").attr("readonly","readonly");
...
function anzhuangChg(){
	var anzhuangFangshi = $("#anzhuangFangshi").val();
	$("#kuangdaiZhanghao").val("");
	if(anzhuangFangshi!="02"){
		
		$("#kuangdaizhanghaoFont").show();
	}
	else{
		$("#kuangdaiZhanghao").attr("readonly","readonly");
		$("#kuangdaiZhanghao").attr("class","");
		$("#kuangdaizhanghaoFont").hide();
	}
}
...
<%@ include file="/npage/common/qcommon/bd_0002.jsp" %>
		...
		<div class="dis" id="tbc_01">
				...
				<div class="title">
					<div id="title_zi">小区资源信息</div>
				</div>	
				<table cellSpacing=0 id="serviceNoInfo">	
				    <tr>
			        <td colspan=6 align="center">     
			          <input type="button" class="b_text" name="query_res" value="装机资源查询" onClick="queryResource()" >
			          ...
			          <span  style="color:red;">如果点击“资源预占”，不进行固话开户请点击“资源释放”，不要直接关闭界面。</span>
			          <%}%>
			        </td>     
			      </tr>
			      <tr>  
					...
					<td colspan="5"><input type="text" name="contactPhone" id="contactPhone" class="required andCellphone" value=""/> <font class="orange">*</font> </td>
			      </tr>
						<tr>
							<td class="blue" width="8%">安装方式</td>
							<td>
								<select id="anzhuangFangshi" name="anzhuangFangshi" onchange="anzhuangChg()">
									<option value="" selected="selected">-请选择-</option>
									<option value="00">同装</option>
									<option value="01">加装</option>
									<!-- <option value="02">单装</option> -->
								</select>
								<font class="orange">*</font>
							</td>
							<td class="blue" width="8%">宽带账号</td>
							<td>
								<input type="text" id="kuangdaiZhanghao" name="kuangdaiZhanghao" maxlength="20" class=""  value="" readonly="readonly"/> <font id="kuangdaizhanghaoFont" class="orange">*</font>
							</td>
							<td></td>
							<td></td>
						</tr>
			      ...
			      <tr>
			        <td class="blue">施工单派发时间</td>
				      <td colspan="5"><input type="text" name="appointvTime"  maxlength="8" id="appointvTime"  v_format="yyyyMMdd" class="required" value="<%=dateStr2%>"><font class="orange">(格式YYYYMMDD)</font></td>	
			      </tr>  
        ...
        <div class="title">
					<div id="title_zi">号码资源信息</div>
				</div>	
				<table cellSpacing=0 id="serviceNoInfo">	
					<tr id="tr_serviceNo" >
						<td class="blue">服务号码</td>
						<td >
							<input type="text" id="phoneNo_h" name="phoneNo_h" v_must="1" maxlength="11"/>
							<font class="orange">*</font> 
							<input type="button" name="chkPhone" id="chkPhone" value="校验" class="b_text"  onclick="chk_Phone()" />  
							<input type="hidden" id="jiaoyanFlag" name="jiaoyanFlag" value="0"/>
							<input name="cfmPwd" id="cfmPwd" type="hidden" value="123123"/>
							<input name="cfmPwdConfirm" id="cfmPwdConfirm" type="hidden" value="123123"/>
						</td>
...
<table cellSpacing=0>
	<tr id="footer">
				<td align="center"> 
						<INPUT class=b_foot_long id="cfmBtn" onClick="mySub()" type=button value=确定&打印  disabled="disabled"/>
...
<input type="text" name="offerEffectTime"/>
<input type="text" name="offerExpireTime"/>
<!--产品信息-->
<input type="hidden" name="sonParentArr"/>
<input type="hidden" name="productIdArr" />
<input type="text" name="prodEffectDate"/>
<input type="text" name="prodExpireDate"/>
...
<input type="hidden"  id="kuandaiQuxian" name="kuandaiQuxian"  value="" >
<!---- 是否需要预占    0不需要     1需要 --->
...
<input type="hidden" name="ipAddType"  value="0"/>
