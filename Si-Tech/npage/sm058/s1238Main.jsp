...
if("<%=m2mFlag%>"!="Y"){	
	...
	var printAddFlag = false;
	 ...
	 function printCommit()
	 {
	 	...
	 	var v_idTypeSelect_c = "0";
	 	if(typeof($("#idType").val())!="undefined"){
			var idTypeSelect_c = $("#idType option[@selected]").val();//证件类型：身份证
			v_idTypeSelect_c = idTypeSelect_c.split("|")[0];		
		}
				
    var packet = new AJAXPacket("ajax_sPlanIdChk.jsp","请稍后...");
        packet.data.add("opCode","<%=opCode%>");//
        packet.data.add("phoneNo","<%=phone_no%>");//
        
        packet.data.add("idType",v_idTypeSelect_c);//
        packet.data.add("idIccid",document.all.idIccid.value);//
        packet.data.add("custName",document.all.custName.value);//
    ...
    core.ajax.sendPacket(packet,function(packet){
    		    var error_code = packet.data.findValueByName("code");//返回代码
				    ...
				    if("23039"==error_code||"23042"==error_code){
