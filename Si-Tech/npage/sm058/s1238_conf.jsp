...
<%
  ...
  request.setCharacterEncoding("GBK");
	...
	/*g551 进来的传的是空 默认传K*/
	if(is_sPwdAuthChk_sel==null || "".equals(is_sPwdAuthChk_sel)){
		is_sPwdAuthChk_sel = "K";
	}
...
*/
  ...
  paraStr[33]=WtcUtil.repSpac(request.getParameter("t_op_remark"));
	...
	System.out.println("-------hejwa-------m2mFlag------------------>"+m2mFlag);
...
if("Y".equals(m2mFlag)){
   ...
   System.out.println("-------hejwam058-------paraStr["+i+"]------------------>"+paraStr[i]);
  }
...
%>
		<wtc:service name="sm058Cfm" routerKey="phone" routerValue="<%=srv_no%>" retcode="retCode1" retmsg="retMsg1" outnum="2" >
...
%>
      <script language="javascript">
	     	 rdShowMessageDialog("实名登记成功！",2);  
         window.location.href="/npage/sm058/s1238Login.jsp?activePhone=<%=srv_no%>";
	    </script>
<%
	  }else{
%>
	     <script language="javascript">
		     rdShowMessageDialog("实名登记成功，下面将打印发票！",2);
	 			 var infoStr="";
	       ...
	       infoStr+='<%=new java.text.SimpleDateFormat("yyyy", Locale.getDefault()).format(new java.util.Date())%>'+"|";
		     ...
		     infoStr+="<%=cust_addr%>"+"|";
	   		 infoStr+="现金"+"|";
			 	 infoStr+="<%=paraStr[31]%>"+"|";
		     infoStr+="实名登记。*手续费："+"<%=paraStr[31]%>"+"*流水号："+"<%=s1238LoginAccept%>"+"|";
