...
<!--	
   ...
   function doProcess(packet)
  {
	...
	var phone_old = packet.data.findValueByName("phone_old");
	document.all.phone_input.value=phone_old;//document.all.contractno.value;
	document.all.contractno.value=contract_new;
	document.getElementById("phoneNo").value= phone_new;
//	rdShowMessageDialog("test phone_input is "+document.all.phone_input.value);
	...
	{
		rdShowMessageDialog("非物联网品牌不允许进行物联网缴费操作!");
	}
	
  }
  function getpasswd(){
  	if(document.all.contractno.value.trim().len()==0){
  		rdShowMessageDialog("物联网帐号不能为空!");
  		return false;
  	}
  	
  	document.all.dbBand_Account.value=document.all.contractno.value;
  	
   	var myPacket = new AJAXPacket("getKdNo_new_zh.jsp","正在查询客户，请稍候......");
		...
		myPacket.data.add("return_page","g573_zh.jsp");
...
<input type="hidden" name="s_flag" value="2" >     
 <%@ include file="/npage/include/header.jsp" %>   
		...
		<div class="title">
			<div id="title_zi">请选择缴费方式</div>
              ...
              <table cellspacing="0" >
                <tr> 
                  <td  class="blue" width="15%">物联网帐号</td>
                  <td> 
