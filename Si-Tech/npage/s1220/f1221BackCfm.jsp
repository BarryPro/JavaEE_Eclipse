...
%>
        ...
        <script language='jscript'>
        	rdShowMessageDialog("换卡冲正成功！");
						...
						$(billArgsObj).attr("10006","换卡冲正"); //业务类别
						...
						$(billArgsObj).attr("10030","<%=login_accept%>"); //流水号--业务流水
						$(billArgsObj).attr("10036","1221"); //操作代码	
						...
						//发票项目修改为新路径
						$(billArgsObj).attr("11215","<%=login_accept%>"); //正业务流水
						$(billArgsObj).attr("11216","<%=paraAray[6].trim().substring(0,6)%>"); //原业务日期			
						$(billArgsObj).attr("11214","HID_PR");	 //隐藏收据按钮
						var path = "/npage/public/billPrtNew/Bill_ELE_Prt.jsp?dlgMsg=换卡冲正成功,下面将打印发票！";
							
						var loginAccept = "<%=login_accept%>";
						var path = path + "&loginAccept="+loginAccept+"&opCode=1221&submitCfm=Single";
						var ret=window.showModalDialog(path, billArgsObj, prop);
						location="s1221_1.jsp?activePhone=<%=phoneNo%>";
						window.close();
/*
            var h=180;
            var w=360;
...
%>   
<script language="JavaScript">
	rdShowMessageDialog("换卡冲正失败!(<%=errMsg%>",0);
	history.go(-1);
</script>
<%}
%>
