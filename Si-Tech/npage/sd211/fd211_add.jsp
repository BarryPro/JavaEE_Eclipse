...
<%
  ...
  String closeFeeNameSql = "  select trim(offer_name) from product_offer where offer_id ="+closeFee_code;
  String region_name = "";
	 ...
	 /*** diling add@2011/10/1 增加开始分，结束分的校验  end ***/
	  ...
	  if(start_year!=""&&end_year!=""&&(parseInt(start_year)>parseInt(end_year))){
	 	...
	 	if(start_minute==""&&end_minute!=""){ //diling add
	 		...
	 		var toURL = "fd211_add_submit.jsp?start_year="+start_year+"&end_year="+end_year+"&start_month="+start_month+"&end_month="+end_month+"&start_day="+start_day+"&end_day="+end_day+"&start_hours="+start_hours+"&end_hours="+end_hours+"&region_code="+region_code+"&closeFee_code="+closeFee_code+"&start_minute="+start_minute+"&end_minute="+end_minute; //diling add 增加开始分，结束分参数
...
<body>
  ...
  <form name="form1"  method="post" action="fd211_add_submit.jsp">
   ...
   <DIV id="Operation_Table">                     
	...
	<div class="title">
		<div id="title_zi">添加智能网vpmn闭合群资费配置</div>
	</div>
