...
<%
  ...
  String sqlRegion="select substr(a.org_code,1,2) from dloginmsg a where login_no between '"+ begin_login +"' and '"+ end_login +"'";
	System.out.println("sqlRegion====="+sqlRegion);
%>
...
<%
	...
	String prtSql=" select a.print_accept, a.phone_no, a.op_code, b.function_name, to_char(a.op_time, 'yyyy-mm-dd hh24:mi:ss') " +
		  " from wHandWritePrt a, sfunccodenew b, dloginmsg c "+
		 " where a.op_code = b.function_code "+
		   " and a.login_no = c.login_no "+
		   " and a.group_id = '"+groupId+"' "+
		   " and a.login_no between '"+begin_login+"' and '"+end_login+"' "+
		   " and a.op_time >= to_date('"+ begin_date.substring(0,8) +" 00:00:00', 'yyyymmdd hh24:mi:ss') "+
		   " and a.op_time <= to_date('"+ end_date.substring(0,8) +" 23:59:59', 'yyyymmdd hh24:mi:ss') "+
...
function updateOrderNew(regCode,prtDate,prtAcc){
	...
	window.open(form.action = "http://10.110.0.100:59000/bp006.go?method=crmModifyInit&sysAccept="+prtAcc+"&opTime="+prtDate+"&iccidInfo="+iccidInfo);
		...
		for(int i=0; i < prtOrdArr.length  ; i++)
	{
		...
		%>
			<tr align="left">
			  ...
			  <td class="<%=tbclass%>">					
					<input type="button" name="b2_<%=prtOrdArr[i][0]%>" class="b_text"   value="录身份证" onclick="loadICCID('<%=regCode%>','<%=prtOrdArr[i][4].substring(0,8)%>','<%=prtOrdArr[i][0]%>')">
					<input type="button" name="b3_<%=prtOrdArr[i][0]%>" class="b_text"   value="修改" onclick="updateOrderNew('<%=regCode%>','<%=prtOrdArr[i][4]%>','<%=prtOrdArr[i][0]%>');" >
			  </td>
			</tr>
    <%}%>
		</table>
