...
<%@ include file="/npage/include/public_title_name.jsp" %>
	...
	<%
		...
		String projectCode = request.getParameter("projectCode"); // 营销案代码
		String gradeCode   = request.getParameter("gradeCode");     // 等级代码
  	...
  	<tr align="center">
    	...
    	<td>
      	...
      	<input type="hidden" name="cardType">
				<input type="hidden" name="valuebox0<%=j%>" value="<%=s6842SelDetArr[j][0]%>"><!--礼包code-->
				<input type="hidden" name="valuebox1<%=j%>" value="<%=s6842SelDetArr[j][1]%>"><!--礼包name-->
				<input type="hidden" name="valuebox2<%=j%>" value="<%=s6842SelDetArr[j][2]%>"><!--促销品code-->
				<input type="hidden" name="valuebox3<%=j%>" value="<%=s6842SelDetArr[j][3]%>"><!--促销品name-->
				<input type="hidden" name="valuebox4<%=j%>" value="<%=s6842SelDetArr[j][4]%>"><!--数量-->
				<input type="hidden" name="valuebox5<%=j%>" value="<%=s6842SelDetArr[j][5]%>"><!--有价卡标识-->
			</td>
    	</tr>
		<%
			}
