...
%>
	...
	inParas1[0] = "select to_char(c.account_id)  from dcustmsgadd a,dcustmsg b,dgrpusermsg c  where b.id_no=a.id_no and a.field_code='1004' and  a.field_value=to_char(c.id_no) and c.sm_code='TG'  and c.run_code='A' and b.phone_no= :phone_no ";
	...
	<%
		...
		if(ret_vals==null || ret_vals.length==0){
		%>
		<script language ='javascript'>
		rdShowMessageDialog("查询错误!<br>错误代码：'135136'，错误信息：'查询TD商务固话集团账号出错'。");
...
<%
  ...
  if(busy_type.equals("1") || busy_type.equals("4")){
     //value  = viewBean.callService("2", phoneNo ,  "s1300_Valid", "28"  ,  lens , inParas,map) ;
%>
	<wtc:service name="s1300_Valid5" routerKey="phone" routerValue="<%=phoneNo%>" retcode="retCode5" retmsg="retMsg5" outnum="28">
...
<%
		...
		iretMsg = retMsg5;
		System.out.println("----------------------------------------------------------------------------------------");
	System.out.println("retcode= "+iretCode +" and retmsg is "+iretMsg) ;
  }
		...
		var returnValue=window.showModalDialog('getUserCount.jsp?accountstr=<%=accountstr%>&accounttypestr=<%=accounttypestr%>&paytypestr=<%=paytypestr%>&shoudpaystr=<%=shoudpaystr%>',"",prop);
		 ...
		 else
		 {
			  document.frm.contractno.value = returnValue;
				document.frm.action = "s1300Cfm_td.jsp?ispopmarket=<%=ispopmarket%>";
				document.frm.submit();
			}
...
<BODY>
<FORM action="PayCfm_td.jsp" method="post" name="frm">
<input type="hidden" name="tdcontractno"  value="<%=TDcontractno%>">
...
<input type="hidden" name="TDname"  value="111">
<!-- tianyang add at 20090928 for POS缴费需求*****end*******-->


<%@ include file="/npage/include/header_td.jsp" %>
                ...
                <tr>
                  <td noWrap colspan="6" id="footer">
                    <div align="center">
                      ...
                      &nbsp;
                      <input type="button" name="return1" class="b_foot" value="返回" onClick="window.location.href='b301.jsp'">
                      &nbsp;
...
<%} else { %>
	 <script language="JavaScript">
	if('<%=return_code%>'=='135136'){
	rdShowMessageDialog("查询错误!<br>错误代码：'<%=return_code%>'，错误信息：'查询TD商务固话集团账号出错'。");
		window.location.href="<%=return_page%>";
}
else if('<%=return_code%>'=='135137'){
rdShowMessageDialog("查询错误!<br>错误代码：'<%=return_code%>'，错误信息：'查询TD商务固话集团账号数量级'。");
		window.location.href="<%=return_page%>";
}
else if('<%=return_code%>'=='135138'){
rdShowMessageDialog("查询错误!<br>错误代码：'<%=return_code%>'，错误信息：'该号码成员存在于多个TD固话产品中，请客户经理取消后操作!'。");
		window.location.href="<%=return_page%>";
}
else{
