...
//	result2 = (String[][])arlist.get(1);
	...
	{%>
	<script language="JavaScript">
		rdShowMessageDialog("查询错误！无结果返回111111！",0);
		history.go(-1);
	</script>
	<%}
...
function docheck(sm_code)
{
   ...
   if(sm_code=="PB")
   {
		rdShowMessageDialog("物联网号码不允许办理!");
		return false;
   }
   else
   {
		...
		else if (form.contractno.value==form.contractno2.value)
	   {
		...
		if((ret=="confirm"))
		{
			...
			{
				  form.action="s1364_2.jsp?reason1=<%=reason1%>&reason2_txt=<%=reason2_txt%>";
				  form.submit();
			}
...
function printInfo(printType)
{
    ...
    opr_info+="用户品牌："+"<%=sm_name%>"+"  办理业务：帐户转帐"+"  操作流水："+"<%=printAccept%>"+"|";
...
<BODY>
<FORM action="s1364_2.jsp" method=post name=form>
