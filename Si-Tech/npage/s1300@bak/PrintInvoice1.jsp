...
<SCRIPT language="JavaScript">
    function printInvoice()
    {
        ...
        printctrl.PageStart();
        /*20100528 liuxmc 添加发票防伪码*/
        printctrl.Print(13, 3, 9, 0, "防伪码：<%=ran%>");
        /*******************************************/
        printctrl.Print(13, 5, 9, 0, "<%=workNo%>");
        printctrl.Print(20, 5, 9, 0, "<%=result[0][15]%>");

    <%if (printtype.equals("0")) {%>
        printctrl.Print(40, 5, 9, 0, "<%=result[0][2]%>");
    <%} else {%>
        printctrl.Print(40, 5, 9, 0, "补打<%=result[0][2]%>");
    <%}%>
    ...
    <%}%>
        ...
        printctrl.Print(30, 10, 10, 0, "<%=sm_name%>");
        ...
        printctrl.Print(20, 15, 9, 9, "<%=result[0][10]%>");
        printctrl.Print(20, 16, 9, 9, "<%=result[0][11]%>");
        
        printctrl.Print(20, 17, 9, 9, "<%=result[0][12]%>");
        printctrl.Print(20, 18, 9, 9, "<%=result[0][13]%>");
        
        printctrl.Print(20, 20, 9, 9, "<%=result[0][14]%>");
     ...
     /********tianyang add at 20090928 for POS缴费需求****end*******/
      ...
      if(length1>0){
       for(int i=0;i<=record;i++)
        {
        %>
        printctrl.Print(5, "<%=(i+28)%>", 9, 0, "<%=temp[i]%>");
        <%
        if(i==record){
        %>
