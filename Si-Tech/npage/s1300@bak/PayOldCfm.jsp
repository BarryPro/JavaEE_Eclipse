...
%>	
    <!-- chenhu add end-->
	<wtc:service name="s1354Cfm" routerKey="phone" routerValue="<%=phoneNo%>" retcode="retCode1" retmsg="retMsg1" outnum="3">
...
<%
	...
	System.out.println("retCode11354================="+retCode1);
...
<INPUT TYPE="hidden" name="returnPage" value="s1354.jsp?opCode=<%=opCode%>&opName=<%=opName%>&activePhone=<%=phoneNo%>">
<script language="javascript">
	 function doqx(packet)
	 {
		...
		if(s_flag=="1")
		{
			...
			window.location.href="s1354.jsp?opCode=1354";
		}
		else
		{
			if(s_flag=="0")
		...
		function ifprint()
		{
			...
			*/
			...
			else if(returnValue=="3")
			{
				...
				var s_hjse=0;
				var s_xmmc="陈账回收";//项目名称 crm可能为多条? 看下zg17多组怎么传的
				var s_ggxh="";
				...
				var pay_note=s_kpxm;
				var returnPage = "s1354.jsp?opCode=<%=opCode%>&opName=<%=opName%>&activePhone=<%=phoneNo%>";
				var chbz="1";
				var kphjje="<%=payMoney%>";
			...
			else
			{
				...
				var opCode="<%=op_code%>";
				var return_page = "s1354.jsp?opCode=<%=opCode%>&opName=<%=opName%>&activePhone=<%=phoneNo%>";
...
else
{
 %>
   <script language="JavaScript">
        ...
        document.location.replace("s1354.jsp?opCode=<%=opCode%>&opName=<%=opName%>&activePhone=<%=phoneNo%>");
   </script>
<%
}
%>
