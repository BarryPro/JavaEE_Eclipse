...
<%
	...
	String colNum = "5";
	String sqlStr = "select to_char(a.contract_no),a.bank_cust,to_char(b.open_time,'yyyy-mm-dd hh24:mi:ss'),b.user_passwd,to_char(b.id_no) from dConMsgDead a,dCustMsgDead b where a.contract_no=b.contract_no and b.phone_no='?'";
...
<%
		...
		else if ( result.size() == 1 )//仿照多个的 判断实名制
		{
%>
			window.returnValue="<%=result1[0][0].trim()%>,<%=result1[0][4].trim()%>";
			window.close(); 
<%
		}
	}
...
<table cellspacing="0">
	...
	<tr> 
		<td align="center" class="<%=tbclass%>"> 
			<input type="radio" name="radio1" value="<%=row[0].trim()%>,<%=row[4].trim()%>"  <%=checkflag%>>&nbsp;
		</td>
