...
<%
  ...
  String[] inParas = new String[2];
	...
	String contractCount = "-1";
		...
		inParas[0] = "select to_char(nvl(count(*),0)) from dConShort where contract_no = :contract_no ";
		inParas[1] ="contract_no = "+contractno;
%>
...
<%
		...
		if(count_num.equals("0")) {
			inParas[0] = "select to_char(nvl(count(*),0)) from dconusermsg where contract_no = :contract_no and serial_no='0'";
			inParas[1] ="contract_no="+contractno;
		%>
			<wtc:service name="TlsPubSelBoss" routerKey="phone" routerValue="<%=phoneNo%>"  retcode="retCode4" retmsg="retMsg4" outnum="1">
			<wtc:param value="<%=inParas[0]%>"/>
...
//	System.out.println("aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaab");
		busy_name="集团缴费成员明细查询";
		return_page="d341.jsp";
...
%>
	<wtc:service name="sd341Qry" routerKey="region" routerValue="<%=org_code.substring(0,2)%>" retcode="retCode5" retmsg="retMsg5" outnum="22">
	<wtc:param value="<%=inParas[1]%>"/>
...
/* dujl add at 20090506 for 哈尔滨关于开展中低端预存赠礼的需求* start ********* */
	...
	if(busy_type.equals("1"))
	{
		...
		inParas[0]="select to_char(add_months(open_time,3),'yyyymm'),to_char(sysdate,'yyyymm')  From dcustmsg where phone_no=:phone_no  ";
...
<%
	...
	inParas3[0]="select to_char(a.unit_id),to_char(b.cust_id),a.unit_name,nvl(a.owner_code,' '),nvl(e.name,' '),nvl(e.service_no,' ') from dgrpcustmsg a,dcustdoc b,dgrpusermsg d,dgrpmanagermsg e where a.cust_id=b.cust_id  and a.cust_id = d.cust_id and a.service_no=e.service_no(+) and d.account_id=:contract_no ";
	inParas3[1]="contract_no= "+contractno;
%> 
<wtc:service name="TlsPubSelCrm" retcode="sretCode" retmsg="sretMsg" outnum="6">
          ...
          %>
			...
			//	   System.out.println("dddddddddddddddddd");
				...
				<tr id="phoneId">
                  ...
                  <td>
                    <input type="text" readonly class="InputGrey"  value="<%=jtcpmc%>">
                  </td>
                ...
                <tr>
                  <td noWrap colspan="6" id="footer">
                    <div align="center">
                      ...
                      <input type="button" name="return1" class="b_foot" value="返回" onClick="window.location.href='d341.jsp'">
                      &nbsp;
