...
String s_unitId = request.getParameter("t_unidId");
String s_prodId = request.getParameter("prodId");
String s_unitOffer = request.getParameter("hd_uOfrid");
%>
...
<DIV id="Operation_Table">
	...
	<table	id="tb_prodIfo" style="display:none">
		...
		<tr>
			...
			<td colspan="3">
				<input type="text" ch_name="产品订购实例唯一标识" name="t_prodBsId" id="t_prodBsId"/>
				<input type="button" class="b_text" name='b_prodBsId' value="获取" onclick='fn_getProdOdr()'/>		
			</td>	
...
function fn_getProdOdr()
{
	...
	packet.data.add("ajaxType"			,"fn_doGetProdOdr");
	
	core.ajax.sendPacket(packet		,fn_doGetProdOdr,true);//异步		
}
function fn_doGetProdOdr(packet)
{
...
{
	var s_bsProd=window.showModalDialog("f_g680_pAdd.jsp"
...
function fn_qryBsProdOne(){
	var packet = new AJAXPacket("f_g680_ajax1.jsp","请稍后...");
	packet.data.add("ajaxType"		,"doGetProdAA");
	packet.data.add("s_loginacc"	,"<%=s_loginacc%>");
	packet.data.add("s_chnSrc"	,"<%=s_chnSrc%>");
	packet.data.add("s_opCode"	,"<%=s_opCode%>");
	...
	core.ajax.sendPacketHtml(packet		,fn_doShowAddAtt1);//异步
	packet =null;
}
/*add 20170526*/
function fn_doShowAddAtt1( data )
{
	$("#d_addAtt2One").empty();
...
function fn_next()
{
	...
	if ( ""== $("#t_bsOfrId").val() )
	{
		rdShowMessageDialog( "原子产品信息不能为空" , 0 );
		return false;
	}	
