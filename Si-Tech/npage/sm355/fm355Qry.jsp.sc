...
<body>
		...
		%>
			...
			if("1".equals(result2[y][12])){
				...
				%>
				<td class="<%=tbClass%>" width="3%"><img src='/nresources/default/images/task-item-close1.gif' style='cursor:hand' alt='返还押金' onclick="deleteoffer('<%=result2[y][5]%>','<%=result2[y][11]%>','<%=result2[y][0]%>','<%=result2[y][1]%>','<%=result2[y][9]%>','<%=result2[y][2]%>','<%=result2[y][10]%>','<%=querymsg2%>','<%=result2[y][13]%>','<%=result2[y][4]%>','<%=result2[y][14]%>')" /></td>
				<%
			}
