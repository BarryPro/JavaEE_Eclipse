...
<%
    ...
    String phoneNo = request.getParameter("phoneNo"); //KDZ443@JTKD
	 
	System.out.println("1111111111111111111test the phoneNo is "+phoneNo );
	...
	else
	{
		// xl add for 宽带号码
	String s_sm_code="";
	String sql_kd = "select a.phone_no,b.sm_code from dbroadbandmsg a,dcustmsg b where cfm_login= '?' and a.phone_no = b.phone_no";
	
	String phone_kd = "";
	String phone_kd_new = request.getParameter("phone_new");
	if(phone_kd_new!=null && !("".equals(phone_kd_new)))
	{
		phone_kd = phone_kd_new;
		System.out.println("fffffffffffffffffffffff test ffffffffff kong      ");
	}
	else
	{
		System.out.println("cccccccccccccccccccc should go here111111111111111111 ");
		%>
		<wtc:pubselect name="TlsPubSelBoss"   retcode="retCode12" retmsg="retMsg12" outnum="2">
	...
	<%
		...
		{
			phone_kd = result21[0][0];
			s_sm_code= result21[0][1];
		}
	}
	// end of 宽带号码查询
    // xl add for phoneNo like KDZ443@JTKD 为空
	if(((phoneNo == null) || ("".equals(phoneNo))) && phone_kd_new!=null && !"".equals(phone_kd_new) )
	{
		System.out.println("2222222222222222222222 查询phoneNo and phone_kd_new is "+phone_kd_new);
		String sql_phone = "select a.cfm_login,b.sm_code from dbroadbandmsg a,dcustmsg b where phone_no = '?' and a.phone_no = b.phone_no ";
		%>
			<wtc:pubselect name="TlsPubSelBoss"   retcode="retCode22" retmsg="retMsg22" outnum="2">
			<wtc:sql><%=sql_phone%></wtc:sql>
			<wtc:param value="<%=phone_kd_new%>"/>
			</wtc:pubselect>
			<wtc:array id="result22" scope="end" />
		<%	
			//List result12 = new ArrayList();
			if( (result22 == null) || (result22.length == 0) )
			{
		%>
				<SCRIPT LANGUAGE="JavaScript">
					//window.returnValue="3";
		...
		<%
			...
			{
				phoneNo = result22[0][0];
				s_sm_code= result22[0][1];
			}
...
<!-- tianyang add at 20090928 for POS缴费需求*****end*******-->
    ...
    <table border=0>
        ...
        <tr> 
            ...
            <td colspan=2>
                <input type="text" readonly name="tt_phone" value="<%=phone_kd%>">
            </td>
        </tr>
