...
function docheck()
{
	...
	{   
		rdShowMessageDialog("请输入正确的宽带号码或铁通固话号码！");
	...
	if(form.billdate.value.substring(0,4)<"1990" || form.billdate.value.substring(0,4)>"2050")
   {
    ...
    {
		rdShowMessageDialog("按号码查询");
        returnValue=window.showModalDialog('getCountByPhone.jsp?phoneNo='+document.form.phone_new.value,"",prop);
        if(returnValue=="3")
        {
        	rdShowMessageDialog("没有找到对应的宽带号码！",0);
        	document.form.phoneNo.focus();
        	return false;
        }
		if(returnValue==null)
	...
	if (form.contractno.value.length<5)    
    {
        ...
        if(returnValue=="3")
        {
        	rdShowMessageDialog("1没有找到对应的宽带号码！",0);
        	document.form.phoneNo.focus();
        	return false;
        }
		if(returnValue==null)
    ...
    <table cellspacing="0">
        ...
        <tr> 
            ...
            <td colspan=2>
                ...
                <input type="text" name="phoneNo" maxlength="25" onKeyDown="if(event.keyCode==13)form.billdate.focus()"   >
            </td>
