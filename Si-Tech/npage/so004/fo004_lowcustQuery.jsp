...
<div class="list">
   ...
   <tr>
		...
		<td><%=retList[i][8]%></td>	
		<td>
			<input class="butCha" type="button" title="修改" onClick="uptTaxPayerInfo('<%=retList[i][2]%>','<%=retList[i][6]%>','<%=retList[i][7]%>','<%=retList[i][3]%>','<%=retList[i][4]%>','<%=retList[i][5]%>')"/>
			<input class="butDel" type="button" title="删除" onClick="dodelTaxPayerInfo('<%=retList[i][2]%>')"/>
		</td>	
   </tr>
   <%
	...
	<div align="center">
				<table align="center">
					<tr>
						...
						每页行数：10
						<a href="javascript:doQueryTaxpayreluptList('1');">[第一页]</a>&nbsp;&nbsp;
						<a href="javascript:doQueryTaxpayreluptList('<%=pre_page+""%>');">[上一页]</a>&nbsp;&nbsp;
						<a href="javascript:doQueryTaxpayreluptList('<%=next_page+""%>');">[下一页]</a>&nbsp;&nbsp;
						<a href="javascript:doQueryTaxpayreluptList('<%=allPage%>');">[最后一页]</a>&nbsp;&nbsp;
						<span class="pages">转到第 
			<input type="text" class="W_30px" name="jump" value="" onkeyup="if(event.keyCode===13){doQueryTaxpayreluptList(this.value,'<%=allPage+""%>');}" /> 页</span>
