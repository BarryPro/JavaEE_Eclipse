...
<%
	//查询操作模板配置
  String keySql = "select conf_key_name,conf_key,conf_value from dng35_Template_conf where  work_no=:work_no and conf_funccode=:opCode";
  String sqlParam = "work_no="+workNo+",opCode="+opCode;
  
  System.out.println("---------keySql------------"+keySql);
%>
  <wtc:service name="TlsPubSelCrm" outnum="3" retmsg="msg" retcode="code" routerKey="region" routerValue="<%=reg_code%>">
		<wtc:param value="<%=keySql%>" />
		<wtc:param value="<%=sqlParam%>" />	
	</wtc:service>
	<wtc:array id="keyResult" scope="end" />
         ...
         <TR>
				...
				<TD width="16%"  class="blue">地区名称：</TD>
					...
					<select name="region_code" onchange = "chg_region()">
					  ...
					  String sqlStr1 = "";
					  int i=0;
					 if("10014".equals(groupId))//省级工号权限
							  {
		...
		<TABLE id=tb1  cellspacing="0"  >
			<TR >
				...
				<TR >
					...
					<TD COLSPAN="4" >
						<textarea COLSPAN="3" cols=70 rows=3 name="bullet_content1" id="bullet_content1"  maxlength=255 v_must=1 v_type=string  ></textarea>
					</TD>
					</TR>
					<TR >
					<TD width="16%"  class="blue">公告单位：</TD>
					<TD width="34%">
						<input class="button" name="boot_name1" id="boot_name1" maxlength=60 v_must=1 v_type=string > 
					</TD>
...
//------------ng3.5项目改造--------开始--------2012年12月5日14:50:09---------------

 
function   document.onkeydown(){ 
	//禁止 ctrl=17 shift=16 alt=18 5=53
	//alert("event.keyCode|"+event.keyCode);
	if(   event.keyCode==17|| event.keyCode==16){   
		rdShowMessageDialog("此页面禁止使用ctrl或shift键");    
     	return   ;       
  }       
...
<%
	for(int iii=0;iii<keyResult.length;iii++){
%>
	keyArray[<%=iii%>] = new Array();
<%	
		for(int jjj=0;jjj<keyResult[iii].length;jjj++){
		System.out.print("	"+keyResult[iii][jjj]);
%>
		keyArray[<%=iii%>][<%=jjj%>] = "<%=keyResult[iii][jjj].trim()%>";
<%
		}
			System.out.println();
	}
...
$(document).ready(function(){
	...
	$("table:visible").each(function(){
		$(this).find("td").each(function(){
			var keyText = $(this).text().trim();
				for(var i=0;i<keyArray.length;i++){
					if(keyText==keyArray[i][0]&&keyArray[i][0]!=""){
						//判断下一个td内的类型 input select textarea
						var nextObj = $(this).next();
						//alert("nextObj.html()|"+nextObj.html());
						if(nextObj.html()!=null){
							if(nextObj.html()!=""&&nextObj.html().indexOf("TEXTAREA")!=-1){
								nextObj.find("textarea:visible").text(keyArray[i][2]);
							}else{
								nextObj.find("input[type='text']:visible").val(keyArray[i][2]);
							}
						}
					}
				}
	...
	//通过ID或Name找
	//alert(keyArray[1][1]+"document.getElementById(keyArray["+1+"][1])|"+document.getElementById(""+keyArray[1][1]));
	for(var i=0;i<keyArray.length;i++){
		//alert("document.getElementById(keyArray["+i+"][1])|"+document.getElementById(""+keyArray[i][1])+"\n"+"document.getElementByName(keyArray["+i+"][1])|"+document.getElementByName(""+keyArray[i][1]));
		if(document.getElementById(""+keyArray[i][1])!=null){
			if(typeof(document.getElementById(""+keyArray[i][1]))!="undefined"){
				document.getElementById(""+keyArray[i][1]).value=keyArray[i][2];
			}else if(typeof(document.getElementByName(""+keyArray[i][1]))!="undefined"){
				document.getElementByName(""+keyArray[i][1]).value=keyArray[i][2];
			}
		}
	}
	
	$("#bullet_content1").focus();
});
//------------ng3.5项目改造--------结束--------2012年12月5日14:50:09---------------
