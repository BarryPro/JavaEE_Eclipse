...
function qryAppInfo(bt){
		...
		var iProductId = $(bt).attr("iProductId");
	 	...
	 	var packet = new AJAXPacket("fm474_AppInfo_Qry.jsp","请稍后...");
...
function go_Qry(){
			...
			return;
				...
				if("9991010000000010006"==prodId){
					if(id_array.length>3){
						rdShowMessageDialog("该资费只允许选择三个APP",1);
						return;
					}
				}
    ...
    var packet = new AJAXPacket("fm474_Qry.jsp","请稍后...");
...
function do_Query(packet){
    var code = packet.data.findValueByName("code"); //返回代码
		var msg = packet.data.findValueByName("msg"); //返回信息
	  var opr_type = packet.data.findValueByName("opr_type"); //返回信息
	  if(code=="000000"){//调用服务成功
...
function go_Cfm(opr_type){
    ...
    var packet = new AJAXPacket("fm474_Cfm.jsp","请稍后...");
        packet.data.add("opCode","<%=opCode%>");//
        packet.data.add("iOfferId",iOfferId);//
        packet.data.add("iProductId",iProductId);//
        packet.data.add("iProdOffInsId",iProdOffInsId);//
...
function printInfo(printType){
	  ...
	  opr_info +="业务类型：视频APP调整    操作流水: "+"<%=loginAccept%>" +"|";
	  opr_info += "业务受理时间：" + '<%=new java.text.SimpleDateFormat("yyyy-MM-dd HH:mm:ss", Locale.getDefault()).format(new java.util.Date())%>' + "   "  + "|";
	  opr_info += "本次申请可选套餐："+ $("#tab_offer").find("input[type='radio']:checked").parent().parent().find("td:eq(3)").text().trim()+"|";
...
<div class="title"><div id="title_zi">订购APP</div></div>
<table cellSpacing="0">
		<tr>
		  <td class="blue" width="15%">新APP应用</td>
...
<table cellSpacing="0">
	 <tr>
	 	<td id="footer">
	 		<input type="button" class="b_foot" value="确定" onclick="go_Qry()"            />
