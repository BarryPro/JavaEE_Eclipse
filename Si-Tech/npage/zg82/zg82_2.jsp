...
<%
	...
	String phoneNo = request.getParameter("phoneNo");//被充值号码
	...
	if(phoneNo==""||"".equals(phoneNo)){
		str="持卡人（身份证号码为："+user_no+",手机号码为："+user_phone_no+")，在"+new java.text.SimpleDateFormat("yyyy-MM-dd HH:mm:ss", Locale.getDefault()).format(new java.util.Date())+"通过工号"+workno+"，激活充值卡卡号"+card_no+"，激活成功后将卡拿走。";

	}else{
	 str="持卡人（身份证号码为："+user_no+",手机号码为："+user_phone_no+")在"+new java.text.SimpleDateFormat("yyyy-MM-dd HH:mm:ss", Locale.getDefault()).format(new java.util.Date())+"，通过工号"+workno+"，激活充值卡卡号"+card_no+"，激活成功后为号码"+phoneNo+"充值成功。";
	}
...
%>			
				<script language="javascript">
					rdShowMessageDialog("特殊再激活充值失败！错误代码"+"<%=result[0][0]%>"+",错误原因:"+"<%=result[0][1].replaceAll("[\\pP\\p{Punct}]", "")%>");
				//window.location="zg82_1.jsp";
				history.go(-1);
				</script>
<%		}
...
%>   
			<script language="javascript">
			rdShowMessageDialog("特殊再激活充值失败!,请于管理员联系！");
			//window.location="zg82_1.jsp";
			history.go(-1);
			</script>
<%
