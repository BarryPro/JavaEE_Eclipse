...
<%
  ...
  String return_page="";
	...
	if(busy_type.equals("2"))   return_page="../g659/g659_1.jsp?opCode="+opCode+"&opName="+opName;
...
<SCRIPT>
	 ...
	 function doyz(packet)
	 {
		...
		else
		{
			...
			{
				var prtFlag=0;
				prtFlag=rdShowConfirmDialog("家庭缴费成功!当前发票号码是"+ocpy_begin_no+",发票代码是"+bill_code+",是否打印发票?");
				if (prtFlag==1)
				{
					document.frm_print.action="PrintInvoice.jsp?check_seq="+ocpy_begin_no+"&bill_code="+bill_code;
...
function ifprint()
{
	...
	else if(returnValue=="3")
	{
		...
		var s_hjse=0;
		var s_xmmc="家庭缴费";//项目名称 crm可能为多条? 看下zg17多组怎么传的
		var s_ggxh="";
	...
	else
	{
		...
		document.frm_print.action="../s1300/PrintInvoice_qx.jsp?opCode="+opCode+"&paySeq="+paySeq+"&phoneno="+phoneno+"&kphjje="+kphjje+"&s_hjbhsje="+s_hjbhsje+"&hjse="+s_hjse+"&returnPage="+return_page+"&hsbz=1&xmdj="+kphjje+"&contractno="+contractno+"&id_no="+id_no+"&sm_code="+sm_code+"&chbz=1&s_xmmc="+s_xmmc+"&paynote=缴费";
