...
<%
	...
	//xl add forliusw需求 先手动置为0 上线时要去掉
	int i_count=0;
	//pwrf = "0";
...
<%
    ...
    System.out.println("dddddddddddddddpwrf=" + pwrf);
    String op_code = "1352";
...
%>
		<wtc:service name="s1352Init" routerKey="phone" routerValue="<%=phoneno%>" outnum="20" >
		...
		<wtc:array id="result2" start="8" length="12" scope="end"/>	
<%
    String return_code = "";
    String error_msg = "";
    if(result.length>0){
    ...
    <script language="JavaScript">
        ...
        function docheck()
        {
            ...
            if (lines == 1)
            {
				...
				re_op_time = document.form.totaldate.value; 
				...
				//xl add for liusw
				i_count_login =document.form.login_flag.value; 
            }
            else
            {
                ...
                for (var i = 0; i < lines; i++)
                {
                    if (document.form.radiobutton[i].checked == true)
                    {
						...
						re_op_time = document.form.totaldate[i].value;
						...
						//xl add for liusw
						i_count_login =document.form.login_flag[i].value; 
					}
                }
            }
            ...
            if (re_print == "1") {
                ...
                if ("<%=pwrf%>" == "0") {//没有高级权限的时候才判断工号是否是可以打印渠道的
                	 //alert("2? ");xl add 增加除了银行的渠道工号i_count_login
                    if (((login_no.substr(0, 5) != "~~~~~") && i_count_login==0) && (totaldate != "<%=dateStr%>" || login_no != "<%=workno%>")) 
                    {
			...
			//select_invoice_1352.jsp 
            /*
			document.form.action = 's1352_print.jsp?pageOpCode=<%=opCode%>&pageOpName=<%=opName%>&contractno=' + document.form.contractno.value + '&payAccept='+login_accept+'&total_date='+totaldate +'&workno=<%=workno%>'+'&check_seq=<%=check_seq%>'+'&pay_money='+pay_money+'&re_op_time='+re_op_time;
			...
			*/
			...
			var path="select_invoice_1352.jsp?login_accept="+login_accept+"&ym="+totaldate.substring(0,6);
			//alert("path is "+path);
			...
			else//qx
			{
				...
				var opCode="<%=opCode%>";
				var return_page = "s1352.jsp";
				document.form.action="PrintInvoice_qx.jsp?opCode="+opCode+"&paySeq="+paySeq+"&phoneno="+phoneno+"&kphjje="+kphjje+"&s_hjbhsje="+s_hjbhsje+"&hjse="+s_hjse+"&returnPage="+return_page+"&hsbz=1&xmdj="+kphjje+"&contractno="+contractno+"&id_no="+id_no+"&sm_code="+sm_code+"&chbz=1&s_xmmc="+s_xmmc+"&paynote=缴费";
...
<BODY>
<FORM action="s1352_print.jsp" method=post name=form>
...
<table cellspacing="0">
    <tbody>
        ...
        %>
            ...
            %>
            <input type="hidden" name="login_flag" value="<%=Integer.parseInt(result2[y][11])%>">
        </tr>
      <%
          }
      %>
