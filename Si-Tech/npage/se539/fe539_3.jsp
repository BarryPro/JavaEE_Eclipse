...
<%
	String opCode = "e539";	
	String opName = "变更基本接入号状态";	//header.jsp需要的参数   
...
<head>
	...
	{		
		...
		var packet = new AJAXPacket("fe539_modCfm.jsp","请稍候......");
   ...
   //恢复 
		...
		function doProcess(myPacket)
	{
		...
		var retMessage = myPacket.data.findValueByName("retMsg2");//声明返回的信息
		...
		if(retFlag=="1")
		{
				...
				if(retype.trim()=="h")
				{
				  ...
				  document.getElementById('description'+v_id.trim()).innerHTML="正常";
				...
				else 
				{
				  ...
				  document.getElementById('description'+v_id.trim()).innerHTML="暂停";
...
%>
        <td class="blue">
        <input id="operator1<%=i+1%>"  <%=stopFlag%> style="cursor:hand" type="button" value="暂停" class="b_text" onclick="queryMod(<%=i+1%>)">
		<input id="operator<%=i+1%>" <%=resumeFlag%> style="cursor:hand" type="button" value="恢复" class="b_text" onclick="queryMod1(<%=i+1%>)">	
	    </td>	
