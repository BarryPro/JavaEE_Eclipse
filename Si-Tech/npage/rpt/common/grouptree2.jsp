...
function saveTo(retGroupId,retGroupName)
{
	...
	//"rpt_f1640upg"
	/*2014/10/22 16:55:20 gaopeng 关于2147新营业厅操作统计报表的优化需求 
		调用方法默认赋值
	*/
	if("<%=execJsp%>" == "f1625_upg"){
		...
		var myPacket = new AJAXPacket("ajax_qryClassCode.jsp", "正在提交，请稍候......");
		var retGroupId = retGroupId;
...
function do_saveTo(packet){
	...
	if(code=="000000"){
		...
		if(available!="0"){
			window.opener.<%=grouptype%>.<%=groupId%>.value=groupId;     //将信息返回到调用的页面去
			window.opener.<%=grouptype%>.<%=groupName%>.value=groupName;
			window.close();
		}
		else{
			alert("该机构不属于装维点,不允许选择!",2);
			//return false;
		}
	}
}
