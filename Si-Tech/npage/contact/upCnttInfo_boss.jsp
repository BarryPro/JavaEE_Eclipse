...
<%
  ...
  if(retCodeForCntt.equals("0"))
  {
    retCodeForCntt="000000";
    retMsgForCntt="SUCCESS";
  }
    //if(!retCodeForCntt.equals("000000"))loginAccept="0";

	retCodeForCntt=retCodeForCntt+"#"+retMsgForCntt;
	
System.out.println("---------------------------------统一接触-------------------------------------------");
...
%>
	<wtc:sequence name="TlsPubSelBoss" key="sMaxSysAccept"  id="cnttLoginAccept"/>
<%
...
%>
	<wtc:pubselect name="TlsPubSelBoss" outnum="1">
		<wtc:sql> select to_char(cust_id) from dcustmsg where phone_no='?' </wtc:sql>
...
if(_isRecord)
{
    Map InfoMap = (Map)session.getAttribute("contactInfoMap");
		ContactInfo cInfo = (ContactInfo) InfoMap.get(_pageActivePhone);
		if(cInfo!=null)_lastContactSeq=cInfo.getContact_id(); 
		if(cInfo==null){
		...
		//-------------------------------------begin of 记录当前业务接触----------------------------
		try{
 			System.out.println("------------------------------------统一接触服务sUpCnttInfo------------------------------");
			System.out.println("_lastContactSeq|"+_lastContactSeq);
			System.out.println("opCode|"+opCode);
			System.out.println("opName|"+opName);
			System.out.println("loginAccept|"+loginAccept);
			System.out.println("retCodeForCntt|"+retCodeForCntt);
			System.out.println("_currentContactId|"+_currentContactId);
			System.out.println("_pageActivePhone|"+_pageActivePhone);
