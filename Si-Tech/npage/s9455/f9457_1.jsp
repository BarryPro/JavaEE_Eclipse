...
<%
    ...
    inParas[6] = iUserPwd;
%>
<wtc:service name="s9457Init" routerKey="region" routerValue="<%=regionCode%>" retcode="retCode1" retmsg="retMsg1" outnum="13">			
...
<%
	...
	if (result1.length>0 && retCode1.equals("000000"))
	{
		...
		vBeginTime = result1[0][10];
		vLevelId = result1[0][11];
		vBusiType = result1[0][12];
	}else{
%>
...
%>
		<script language=javascript>
		rdShowMessageDialog("查询BlackBerry个人资费出错<br>错误代码："+retCode2+"错误信息："+retMsg2);
		</script>
<%
 		}
%>
...
<title>BlackBerry个人业务用户信息变更</title>
...
function frmCfm()
{
 	if(document.all.iChangeType.value == "" || document.all.iLevelId.value == "")
 	{
 		rdShowMessageDialog("请选择有效的变更类型和资费类别!");
 		return;	
 	}
 	frm.submit();
	return true;
...
{
	var changeType= document.all.iChangeType.value;
...
<body>
<form name="frm" action="f9457_Cfm.jsp" method="post">
...
<table cellspacing="0">
	...
	<tr>
		...
		<td class="blue">开通时间</td>
		<td><input name="beginTime" type="text" value="<%=vBeginTime%>" class="InputGrey" readonly></td>
	</tr>

	<tr>
		<td class="blue">变更类型</td>
		<td><select name="iChangeType" onchange="selectLevel()">
			<option value="" selected="selected">--请选择--</option>
			<!--<option value="A">业务变更</option>-->
			<option value="B">资费变更</option>
			</select>
		</td>
		<td class="blue">资费类别</td>
	...
	<tr>
		<td class="blue">备注</td>
		<td colspan="3"><input  type="text" name="opNote" value="<%=vOpNote%>" size="60"></td>
	</tr>
