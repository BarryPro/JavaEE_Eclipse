...
<%
  ...
  //retList = co.callFXService("s7982Init", inputParsm, "39","phone",iPhoneNo);
%>
	<wtc:service name="s7982Init" routerKey="phone" routerValue="<%=iPhoneNo%>" retcode="retCode1" retmsg="retMsg1" outnum="46">			
...
<%
  ...
  {
	   retFlag = "1";
	   retMsg = "s7982Init查询号码基本信息为空!<br>errCode: " + errCode + "<br>errMsg+" + errMsg;
  }
  else if(errCode.equals("000000") && tempArr.length>0)
  {
	...
	<!--
  	 rdShowMessageDialog("错误代码：<%=errCode%>错误信息：<%=errMsg%>",0);
  		//window.location="f7981_login.jsp?opCode=<%=opCode%>&opName=<%=opName%>&activePhone=<%=iPhoneNo%>";
  	 history.go(-1);
  	//-->
  </script>
<%
...
<body>
	...
	<tr>
	    ...
	    <input type="hidden" name="iOpCode" value="7982">
		...
		<input type="hidden" name="do_note" value="<%=iPhoneNo%>"+"购TD固话赠话费冲正">
		...
		<input type="hidden" name="beforeOpCode" value="7981">
		<input type="hidden" name="backaccept" value="<%=iLoginNoAccept%>">

		<input type="hidden" name="return_page" value="/npage/bill/f7981_login.jsp?opName="购TD固话赠话费"&opCode=7981&activePhone=<%=iPhoneNo%>">	
