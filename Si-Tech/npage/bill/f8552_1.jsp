...
%>
	 <script language="JavaScript" >
	  ...
	  rdShowMessageDialog("此用户已在促销品统一付奖中进行<%=awardName%>领奖，请进行促销品统一付奖冲正，并确保奖品完好");
		location='f8551_login.jsp?opCode=8551&opName=购TD固话赠话费(铁通)';
		</script>
<%
		}
  }
...
%>
	<wtc:service name="s8552Qry" routerKey="phone" routerValue="<%=iPhoneNo%>" retcode="retCode1" retmsg="retMsg1" outnum="46">			
...
<%
  ...
  {
	   retFlag = "1";
	   retMsg = "s8552Qry查询号码基本信息为空!<br>errCode: " + errCode + "<br>errMsg+" + errMsg;
  }
  else if(errCode.equals("000000") && tempArr.length>0)
  {
	    ...
	    /*** ningtn add for pos start ***/	   	 
			payType       = tempArr[0][41];
			Response_time = tempArr[0][42];
			TerminalId    = tempArr[0][43];
			Rrn           = tempArr[0][44];
			Request_time  = tempArr[0][45];
	...
	<!--
  	 rdShowMessageDialog("错误代码：<%=errCode%>错误信息：<%=errMsg%>",0);
  		//window.location="f8551_login.jsp?opCode=<%=opCode%>&opName=<%=opName%>&activePhone=<%=iPhoneNo%>";
  	 history.go(-1);
  	//-->
  </script>
<%
...
<body>
	...
	<tr>
	    ...
	    <input type="hidden" name="iOpCode" value="8552">
		...
		<input type="hidden" name="do_note" value="<%=iPhoneNo%>"+"购TD固话赠话费(铁通)冲正">
		...
		<input type="hidden" name="beforeOpCode" value="8551">
		<input type="hidden" name="backaccept" value="<%=iLoginNoAccept%>">
		<!-- ningtn add   for POS缴费需求*****start*****-->
		...
		<!-- ningtn add   for POS缴费需求*****end*****-->	
		<input type="hidden" name="return_page" value="/npage/bill/f8551_login.jsp?opName="购TD固话赠话费(铁通)"&opCode=8551&activePhone=<%=iPhoneNo%>">	
