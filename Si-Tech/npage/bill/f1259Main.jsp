...
System.out.println("-----------------------------tempArr[0][19]------------------------"+tempArr[0][19]);
  ...
  /*******芦学琛20060301add,查询开户时间 *****  begin******/
	...
	inParas1[0]="select to_char(a.open_time,'YYYYMMDD HH24:MI:SS') "
		+"from dcustinnet a,dCustMsg b "
		+"where a.id_no=b.id_no and substr(b.run_code,2,1)<'a' "
		+"and b.phone_no=:v_phone_no";
	inParas1[1]="v_phone_no="+phoneNo;
	for ( int i=0; i<inParas1.length; i++ )
	{
		System.out.println("zhangyan add inParas1["+i+"]=["
			+inParas1[i]+"]  ");
	}
%>

<wtc:service name="TlsPubSelCrm" outnum="1" retmsg="retMsg2" retcode="retCode2" 
	routerKey="phone" routerValue="<%=phoneNo%>">
...
<%
	...
	System.out.println("zhangyan add showopentime=["+showopentime+"]");
...
<!--
  ...
  if((year_fee-prepay_fee)>0){
 	  rdShowMessageDialog("包年金额不能大于当前可用预存，请确认!");
	  return false;
  }
  ...
  function setParaForNext(){
    var iOpCode = "1259";//iOpCode    pay_type|year_fee|实缴预存款|“” 
...
{ 
		var flag_code_name = $.trim(document.all.flag_code_name.value);
    ...
    if(flag_code_name.length != 0){
    	sqlStr += " and flag_code_name like '%"+flag_code_name+"%'";
    }
...
{
    var path = "fPubSimpSer_1259.jsp?offerId=<%=rate_code%>&newOfferId="+document.all.new_rate_code.value;
    path = path + "&retQuence=" + retQuence;
    path = path + "&fieldName=" + fieldName + "&pageTitle=" + pageTitle;
    path = path + "&selType=" + selType + "&opCode=1259&newDateTime="+Math.random();  
...
function setgeftFlag(){
	if(document.all.isGeft.checked){
		document.all.geftFlag.value="1"	;
	}else{
		document.all.geftFlag.value="0"	;
	}
}
</script>
</head>
      ...
      <table cellspacing="0">
         ...
         <input type="text" class="button" name="new_rate_code" id="new_rate_code" onChange="changeRateCode()" size="8" maxlength="8" v_must=1 v_name="新套餐代码" >
			...
			<td id = "flagCodeTd" style="display:none">
			      ...
			      <input type="text" class="button" name="flag_code_name" size="18"  > 
          ...
          <tr>
          	<td class=blue colspan=4>	<input type="checkbox" id="isGeft" name="isGeft" onclick="setgeftFlag()">是否参与赠礼 </td>
        </tr> 
          <tr> 
            <td class="blue">备注：</td>
            <td colspan="3">
