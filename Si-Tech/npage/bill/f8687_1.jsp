    ...
    <%
      ...
      %>
				...
				<tr style="display:none"> 
				  ...
				  <input  type="hidden" name="s_spot" value="Yn40-->小区计费"  class="InputGrey" readonly>
		    </tr>
				<tr> 
      ...
      <input type="hidden" name="favorcode" size="20" maxlength=20 value="<%=result[0][23]%>">
		  ...
		  <input type="hidden" name="return_page" value="/npage/bill/f7898_login.jsp?activePhone=<%=phone%>&opName=TD商务固话主资费变更&opCode=7898">  
    	<!-------------------------------------------------------------->
		</table>
        <table cellSpacing="0">
          <tbody>
...
{ 
    ...
    sqlStr=sqlStr+"and a.op_code='8687' and c.op_code in ('8687','0000') and a.OLD_MODE ='<%=java.net.URLEncoder.encode(result[0][16])%>' and a.new_mode like '";
	...
	**/
	var sqlStr =" select a.offer_z_id, b.offer_name, decode(a.effect_type,'0', '0 24小时之内生效','2', '2 一般预约 ','3', '3 个性预约','1', '1 第二天生效'),1, b.offer_comments   ";
		sqlStr=sqlStr+" from product_offer_relation a, product_offer b, stgsalemode c where a.op_code = '8687'  and a.offer_z_id = b.offer_id and c.region_code='<%=regionCode%>' ";
		...
		sqlStr=sqlStr+" and sysdate between b.eff_date and b.exp_date and to_char(a.offer_z_id) = trim(c.mode_code) and c.op_code IN ('8687', '0000') ";
