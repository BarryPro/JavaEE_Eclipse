...
<!--
  ...
  function printfrmCfm(){
  	//alert(document.getElementById('kin_nos').value);
  	getAfterPrompt();
  	if(document.getElementById('kin_nos').value=="")
  	{
  		rdShowConfirmDialog('请输入要添加的亲情号码！');
  		document.getElementById('kin_nos').focus();
  		return false;
  	}
	else
	{
  ...
  function printInfo(printType)
  {
		...
		opr_info+="用户品牌："+document.all.oSmName.value+"  "+"办理业务：神州行助老爱心卡亲情号码添加"+"|";
		opr_info+="操作流水："+"<%=printAccept%>"+"|";		
		
		opr_info+="添加的亲情号码为："+document.getElementById('kin_nos').value+"|";
		
		opr_info+="生效时间： "+"当日"+"|";
		note_info1+="备注："+"|";
		note_info1+="每月增加亲情号码的数量只能是4次，即每月最多可增加4个亲情号码；"+"|";
 ...
 function setOpNote(){
	...
	{
	  document.frm.opNote.value = "操作员：<%=loginNo%> 为用户："+"<%=iPhoneNo%>"+"增加了亲情号码"; 
	}
	return true;
}
	...
	<div id="title_zi">亲情号码信息</div>
		...
		<TABLE cellSpacing="0">
          ...
          <tr>
			...
			<td colspan=3>
				<TEXTAREA NAME="kin_nos"  ROWS="1" COLS="60"></TEXTAREA>
