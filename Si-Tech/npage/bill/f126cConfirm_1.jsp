...
/*
	...
	*/
	...
	String chinaFee = ((String[][])(callView.view_sToChinaFee(WtcUtil.formatNumber(machine_fee,2)).get(0)))[0][2];//大写金额
 	/*
	String[] ret = impl.callService("s126b_Apply",paraAray,"2","phone",request.getParameter("phoneNo"));
...
function printBill(){
	 var infoStr="";                                                                         
	 infoStr+="<%=work_no%>  <%=printAccept%>"+"  预存赠机非按月扣费冲正"+"|";//工号                                      
	 ...
	 infoStr+="退购机款：  <%=WtcUtil.formatNumber(machine_fee,2)%>"+
		 "~~~机器型号：<%=machine_type%>"+"|";//项目

	 infoStr+="<%=work_name%>"+"|";//开票人
	 infoStr+=" "+"|";//收款人
	 
	 var dirtPage="/npage/obill/fa26b_login.jsp?opCode=126c%26activePhone="+<%=paraAray[1]%>;
...
<script language="JavaScript">
	   <%if(op_type.equals("9")){%>
	       rdShowMessageDialog("预存话费赠机冲正成功,下面将打印发票!");
           printBill();
	   <%}else{%>
		   rdShowMessageDialog("预存话费赠机冲正成功!");
		   location="fa26b_login.jsp?opCode=126c&activePhone"+paraAray[1];
	   <%}%>
</script>
<%
	}else{
%>   
<script language="JavaScript">
	rdShowMessageDialog("预存话费赠机冲正失败!<br>errCode:"+"<%=errCode%>"+"<br>errMsg:"+"<%=errMsg%>");
	history.go(-1);
</script>
<%}%>
<html>
