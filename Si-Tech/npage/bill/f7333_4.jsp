...
<%
	String exeTime = tempArr1[0][0].trim().substring(0,8);
...
<!--
  ...
  function printfrmCfm(){
  	getAfterPrompt();
  	//alert(document.frm.fam_prod_id.value);
  	setOpNote();//为备注赋值
  ...
  function printInfo(printType)
  {
		...
		if(document.all.parent_prod_id.value =="0")
		{
			prod_name="（家庭基本包）";
		}
		else
...
function f7328Init(obj){
	document.frm.fam_prod_id.value = obj.FamilyProd;
	document.frm.fam_prod_note.value = obj.FamilyNote.trim();
	document.all.parent_prod_id.value = obj.ParentProd.trim();
	qryMode();
}
function qryMode()
{
...
<div id="Operation_Table">
          ...
          <TBODY>
			...
			<tr>
				<td class="Grey" align=center>
		      		<input type="radio" name="radio" class="button" FamilyProd="<%=FamilyProdArr[i][0]%>" FamilyNote="<%=FamilyNoteArr[i][0]%>" ParentProd="<%=ParentProdArr[i][0]%>" onclick="f7328Init(this)">	</td>
 ...
 <input name="fam_prod_id" type="hidden" class="InputGrey" id="fam_prod_id" maxlength=8 > <!--家庭代码-->
 <input name="fam_prod_note" type="hidden" class="InputGrey" id="fam_prod_note" maxlength=8 >
