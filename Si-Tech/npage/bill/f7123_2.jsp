...
/****************由移动号码得到密码、机主姓名、 等信息 s1205Init***********************/
  ...
  String phoneNo = request.getParameter("chief_srv_no").trim(); /* 家长号码*/
  String home_no = request.getParameter("srv_no").trim(); /* 成员号码*/	
...
%>
	<wtc:service name="s7123Valid" routerKey="region" routerValue="<%=regionCode%>" retcode="retCode1" retmsg="retMsg1" outnum="27">			
...
//	}
	...
	paramIn[0] = "select a.offer_id,a.offer_id ||'--'||a.offer_name from product_offer a,region b,sregioncode c WHERE  a.offer_id = b.offer_id and sysdate between a.eff_date and a.exp_date AND b.group_id = c.group_id AND c.region_code='"+regionCode+"' and a.GROUP_TYPE_ID='10'";
...
//subButton.disabled = true;
  ...
  function printInfo(printType)
  {
		...
		/*begin huangrong and 在增加家庭成员及建立家庭时增加如下的说明信息 2011-3-16*/
		if("<%=openType%>"=="0" || "<%=openType%>"=="1")
		{
			note_info1+="家长有权自行取消家庭服务计划业务，解散家庭，无需事先征得家庭内成员的同意，家长解散家庭后，所有家庭成员内部通话将不再享受优惠，家庭成员可以自行退出家庭。同时如果家长离网状态，则家庭自动解散，所有家庭成员内部通话将不再享受优惠。"+"|";;
    }
    /*end huangrong and 在增加家庭成员及建立家庭时增加如下的说明信息 2011-3-16*/
...
function setOpNote(){
	...
	{
	  document.frm.opNote.value = "家庭服务计划变更--<%=op_note%> 家长号码:"+document.frm.phoneNo.value; 
	}
	return true;
}
//-->
...
<!-- ningtn 2011-8-3 14:30:23 扩大电子工单 -->
...
<!-- ningtn 2011-8-3 14:31:00 电子化工单扩大范围 -->
