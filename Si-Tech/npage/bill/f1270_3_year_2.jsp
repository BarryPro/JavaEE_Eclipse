...
*add by qiansheng 125a需要使用老版本的f1270_3_yeah.jsp
 ********************/
%>
...
<%
    ...
    //String iold_m_code = ReqUtil.get(request, "i16").substring(0, 8);     现主套餐代码（老）
    String iold_m_code = ReqUtil.get(request, "i16").substring(0, 8);
    iold_m_code=iold_m_code.substring(0,iold_m_code.indexOf("--")); //按固定位置截，换成按两个横杠截
    String inew_m_code = ReqUtil.get(request, "ip");                        //申请主套餐代码(新)
    ...
    String sqlStr2 = "";
    if (iop_code.equals("1255")) {
        titleStr = "包年申请";
        showFlag = "style=\"display:''\"";
    } else if (iop_code.equals("1257")) {
        titleStr = "包年冲正";
    } else if (iop_code.equals("1258")) {
        titleStr = "包年取消";
    } else if (iop_code.equals("1259")) {
        titleStr = "包年续签";
        showFlag = "style=\"display:''\"";
    } else if (iop_code.equals("125a")) {
        titleStr = "包年续签冲正";

    } else if (iop_code.equals("1201")) {
        titleStr = "随意行包年";
        showFlag = "style=\"display:''\"";
...
%>
		<wtc:service name="s1270MustPM" routerKey="region" routerValue="<%=regionCode%>" retcode="retCode1" retmsg="retMsg1" outnum="16" >
...
<% 	
   ...
   if(!retCode1.equals("000000")){
    	...
    	<script type="text/javascript">
    			var msg="错误代码：<%=retCode1%>  错误信息: <%=retMsg1%>" ;
		...
		if(result_must!=null&&result_must.length>0){
    		if (!ret_msg1.equals("")) {
	    		...
	    		ret_msg1 = result_must[0][1];		    			
		 		...
		 		/******
		 		//暂不使用SystemUtils这个类,服务直接返回了结果
		 		//若恢复此注释,请引入"com.sitech.boss.pub.util.*"
		    if (ret_msg1.equals("")) {
		    ...
		    mainModeCode = result_must[0][2];
		        ...
		        send_flag = "0 24小时之内生效";
		     		...
		     		System.out.println("1111111111111111111111111111111111111111"+mainModeCode);
		        send_flag = result_must[0][3];
		     }
		}
   	
   System.out.println(" mylog---result_must03 "  + result_must[0][3]);
    /****得到打印流水****/
    String printAccept = "";
%>
...
<%  
    System.out.println(" mylog--- flag"  + flag);
...
<TABLE cellSpacing="0">
    <TR>
        ...
        <TD>
            <input name="phone" value="<%=ReqUtil.get(request,"i1")%>" size="20" maxlength=11 v_must=1 v_type=int v_name=移动号码 class="InputGrey" readonly>
        </TD>
        ...
        <input type="hidden" name="next_main_cash" value="<%=ReqUtil.get(request,"i18").substring(0,ReqUtil.get(request,"i18").indexOf("--"))%>">
...
<TABLE cellSpacing="0">
    <TR>
        <TD width="16%" class="blue">集团客户类别</TD>
        <TD width="34%">
            <input name="group_type" size="20" value="<%=ReqUtil.get(request,"group_type")%>" class="InputGrey" readonly>
        </TD>
        <TD width="16%" class="blue">大客户类别</TD>
        <TD width="34%">
            <input name="ibig_cust" size="20" maxlength=20 value="<%=ReqUtil.get(request,"ibig_cust")%>"  class="InputGrey" readonly>
        </TD>
    </TR>
    <TR>
        ...
        <TD width="34%">
            <input name="i18" size="30" value="<%=ReqUtil.get(request,"i18")%>" class="InputGrey" readonly classCode="10431"  classValue="11">
        </TD>
    </TR>
...
<TABLE cellSpacing="0">
    <TBODY>
        <TR>
            ...
            <TD width="34%">
                <input type="text" name="ip" value="<%=ReqUtil.get(request,"ip")%>" size="30" class="InputGrey" readonly>
                <script>
...
<%	
                for (int i = 0; i < result_select.length; i++) {
                    out.println("<a Href='#' onClick='openwindow(" + '"' +
                            +i + '"' + "," + '"' + result_select[i][0] + '"' + "," + '"' + result_select[i][1] + '"' + ")'><font color='#0000FF'>" + result_select[i][0] + " " + result_select[i][1] + "</a>");
...
<TABLE cellSpacing="0">
    <TR>
        ...
        <TD colspan="3">
            <input name="sysnote" size="60" class="InputGrey" readonly>
        </TD>
    </TR>
...
%>
    ...
    %>
    <td <%=showFlag%>><input type="checkbox" name="checkId" checked
    ...
    %>
    <td <%=showFlag%>><input type="checkbox" name="checkId"
    ...
    %>
    <td <%=showFlag%>><input type="checkbox" name="checkId" disabled
                             onclick="if(document.all.checkId[<%=y%>+1].checked==true){
									 document.all.checkId[<%=y%>+1].checked=false;}
    ...
    <TBODY>
        <TR>
            <TD id="footer">
                ...
                <input name=kkkk class="b_foot" onClick="pageSubmit(1)" type=button value=返回>
            </TD>
        </TR>
    </TBODY>
</TABLE>
...
/****/
function frmCfm() {
    document.form1.action = "f1270_4_year.jsp";
    form1.submit();
    return true;
}
...
{  //显示打印对话框 
	...
	if("<%=iop_code%>"!="125a"){
		...
		var newmode=document.all.ip.value.substring(0, 8); //新主套餐代码
	...
	<%if(iop_code.equals("1255") || iop_code.equals("1257")|| iop_code.equals("1258")|| iop_code.equals("1259") || iop_code.equals("125a")){%>
	 var path = "<%=request.getContextPath()%>/npage/innet/hljBillPrint_jc_hw.jsp?DlgMsg=" + DlgMessage + "&submitCfm=" + submitCfm;
	<%}else{%>
	 var path = "<%=request.getContextPath()%>/npage/innet/hljBillPrint_jc.jsp?DlgMsg=" + DlgMessage + "&submitCfm=" + submitCfm;
	<%}%>
		var path = path  + "&mode_code="+mode_code+"&fav_code="+fav_code+"&area_code="+area_code+"&opCode=<%=opCode%>&sysAccept="+sysAccept+"&phoneNo=<%=phoneno%>&pType="+pType+"&billType="+billType+ "&printInfo=" + printStr;
...
<%}else if(iop_code.equals("1259")){%>
    retInfo = printInfo1259();
<%}else if(iop_code.equals("125a")){%>
    retInfo = printInfo125a();
<%}else if(iop_code.equals("1201")){%>
    retInfo = printInfo1201();
<%}%>
    return retInfo;
}
...
<%if(goodbz.equals("Y")){%>
    opr_info += "办理业务:" + "包年申请" + "  " + "操作流水: " + "<%=printAccept%>" + "  底线消费金额：" + "<%=modedxpay%>" + "元" + "|";
    ;
<%}else{%>
    opr_info += "办理业务:" + "包年申请" + "  " + "操作流水: " + "<%=printAccept%>" + "|";
<%}%>
    opr_info += "包年资费：" + codeChg(document.all.ip.value.trim()) + "  " + "生效时间：" + exeDate + "|";
    opr_info += "包年资费对应品牌：" + "<%=NsmName%>" + "|";
    opr_info += "到期后执行资费：" + "<%=matureCode%> <%=mature_Name%>" + "|";
	...
	{
		note_info2+=" "+"|";
		if(("<%=smCode%>" != "") && ("<%=m_smCode%>" != "<%=smCode%>"))
			note_info2+="包年资费生效后，您的品牌将由"+"<%=ReqUtil.get(request,"i8")%>"+"变更为"+"<%=NsmName%>"+"，现有积分（或M值）将于资费生效的次月初清零，请您及时兑换积分。"+"|";
		note_info2+=" "+"|";
	}
    note_info2 += "到期后执行资费描述：" + "|";
    note_info2 += "<%=note1.trim()%>" + "|";
...
function printInfo1257()
{
    ...
    opr_info += "业务受理时间：" + '<%=new java.text.SimpleDateFormat("yyyy-MM-dd HH:mm:ss", Locale.getDefault()).format(new java.util.Date())%>' + "  " + "用户品牌:" + "<%=ReqUtil.get(request,"i8")%>" + "|";
<%if(goodbz.equals("Y")){%>
    opr_info += "业务类型: 包年冲正" + "  " + "操作流水: " + "<%=printAccept%>" + "  底线消费金额：" + "<%=modedxpay%>" + "元" + "|";
    ;
<%}else{%>
    opr_info += "业务类型: 包年冲正" + "  " + "操作流水: " + "<%=printAccept%>" + "|";
<%}%>
    ...
    /**********描述类*********/
    if (document.all.modestr.value.length > 0) {
        note_info1 += "新资费生效时将被取消的可选资费:" + codeChg(document.all.modestr.value) + "|";
    } else {
        retInfo += " " + "|";
    }
<%if(goodbz.equals("Y")){%>
...
function printInfo1258()
{
    ...
    opr_info += "业务受理时间：" + '<%=new java.text.SimpleDateFormat("yyyy-MM-dd HH:mm:ss", Locale.getDefault()).format(new java.util.Date())%>' + "  " + "用户品牌：" + "<%=ReqUtil.get(request,"i8")%>" + "|";
<%if(goodbz.equals("Y")){%>
    opr_info += "办理业务:" + "包年取消" + "  " + "操作流水: " + "<%=printAccept%>" + "  底线消费金额：" + "<%=modedxpay%>" + "元" + "|";
    ;
<%}else{%>
    opr_info += "办理业务:" + "包年取消" + "  " + "操作流水: " + "<%=printAccept%>" + "|";
<%}%>
    opr_info += "取消的包年资费：" + "<%=ReqUtil.get(request,"i16")%>" + "|";
    opr_info += "取消后执行资费：" + document.all.ip.value + "  *" + "生效时间：" + exeDate + "|";
    opr_info += "取消后执行资费对应品牌：" + "<%=NsmName%>" + "|";
    note_info1 += "取消后执行资费描述：" + "|";
    note_info1 += "<%=note%>" + "|";
    /*******备注类**********/
    if ("<%=bdbz%>" == "Y") {
        note_info1 += "<%=bdts%>" + "|";
    ...
    if (document.all.modestr.value.length > 0) {
        note_info4 += " " + "|";
        note_info4 += "新资费生效时将被取消的可选资费:" + codeChg(document.all.modestr.value) + "|";
    } else {
...
<%if(goodbz.equals("Y")){%>
    opr_info += "办理业务:" + "包年续签" + "   操作流水: " + "<%=printAccept%>" + "  底线消费金额：" + "<%=modedxpay%>" + "元" + "|";
<%}else{%>
    opr_info += "办理业务:" + "包年续签" + "   操作流水: " + "<%=printAccept%>" + "|";
<%}%>
    opr_info += "原包年资费：" + codeChg("<%=ReqUtil.get(request,"i16")%>") + "|";
    opr_info += "包年续签资费：" + codeChg(document.all.ip.value) + "|";
    opr_info += "包年续签资费对应品牌：" + "<%=NsmName%>" + "|";
...
//随E行包年申请
function printInfo1201()
{
    //得到该业务工单需要的参数
    var tempStr = document.form1.iAddStr.value;
	  //iAddStr格式 pay_type|year_fee|prepay_fee|year_month|card_type|card_name 其中card_type、card_name用于随E行包年
    var year_fee = oneTokSelf(tempStr, "|", 3);//包年预存
    var year_month = oneTokSelf(tempStr, "|", 4);//包年周期
    var exeDate = "<%=exeDate.substring(0,8)%>";//生效日期
    var card_type = oneTokSelf(tempStr, "|", 5);//卡类型代码
    var card_name = oneTokSelf(tempStr, "|", 6);//卡类型名称
    ...
    cust_info += "客户地址：" + document.all.i5.value + "|";
    cust_info += "证件号码：" + document.all.i7.value + "|";
    cust_info += "卡类型：" + card_type + "--" + card_name + "|";

    /********受理类**********/
    opr_info += "用户品牌：" + "<%=ReqUtil.get(request,"i8")%>" + "*" + "办理业务:随意行包年" + "|";
<%if(goodbz.equals("Y")){%>
    opr_info += "操作流水: " + "<%=printAccept%>" + "       底线消费金额：" + "<%=modedxpay%>" + "元" + "|";
<%}else{%>
    opr_info += "操作流水: " + "<%=printAccept%>" + "|";
<%}%>
    opr_info += "资费类型：" + document.all.ip.value + "|";
