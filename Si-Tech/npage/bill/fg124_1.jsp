...
//******************得到下拉框数据***************************//
  //手机品牌
  String sqlAgentCode = "select  unique a.brand_code,trim(b.brand_name) from dphoneSaleCode a,schnresbrand b where a.region_code='" + regionCode + "' and a.sale_type='52' and a.brand_code=b.brand_code and a.valid_flag='Y'";
  String[] inParamA = new String[2];
  inParamA[0] = "select  unique a.brand_code,trim(b.brand_name) from dphoneSaleCode a,schnresbrand b where a.region_code=:region_code and a.sale_type='52' and a.brand_code=b.brand_code and a.valid_flag='Y'";
...
<%
  //手机类型
  String sqlPhoneType = "select unique a.type_code,trim(b.res_name), b.brand_code from dphoneSaleCode a,schnrescode_chnterm b where a.region_code='" + regionCode + "' and  a.type_code=b.res_code and a.brand_code=b.brand_code and a.sale_type='52' and a.valid_flag='Y'";
  String[] inParamB = new String[2];
  inParamB[0] = "select unique a.type_code,trim(b.res_name), b.brand_code from dphoneSaleCode a,schnrescode_chnterm b where a.region_code=:region_code and  a.type_code=b.res_code and a.brand_code=b.brand_code and a.sale_type='52'and a.valid_flag='Y'";
...
<%
  //营销代码
  String sqlsaleType = "select unique trim(a.sale_code),trim(a.sale_name), a.brand_code,a.type_code from dphoneSaleCode a where a.region_code='" + regionCode + "' and a.sale_type='52' and a.valid_flag='Y'";
  String[] inParamC = new String[2];
  inParamC[0] = "select unique trim(a.sale_code),trim(a.sale_name), a.brand_code,a.type_code from dphoneSaleCode a where a.region_code=:region_code and a.sale_type='52' and a.valid_flag='Y'";
...
<%
  //营销明细
  String sqlsaledet = "select to_char(to_number(sale_price)-to_number(prepay_gift)-to_number(prepay_limit)),prepay_gift,consume_term,prepay_limit,to_char(active_term),mon_base_fee,all_gift_price,sale_price,trim(sale_code),market_price from dphoneSaleCode  where region_code='" + regionCode + "' and sale_type='52' and valid_flag='Y'";
  String[] inParamD = new String[2];
  inParamD[0] = "select to_char(to_number(sale_price)-to_number(prepay_gift)-to_number(prepay_limit)),prepay_gift,consume_term,prepay_limit,to_char(active_term),mon_base_fee,all_gift_price,sale_price,trim(sale_code),market_price from dphoneSaleCode where region_code=:region_code and sale_type='52' and valid_flag='Y'";
...
<%
  ...
  //新主资费(修改部分)
  String sqlmodecode = "select a.mode_code,trim(a.sale_code),a.mode_code||'--'||c.mode_name from dChnResSalePlanModeRel a , dphoneSaleCode b,sbillmodecode c where a.sale_code=b.sale_code and b.region_code='" + regionCode + "' and b.sale_type='52'  and b.region_code=c.region_code and a.mode_code=c.mode_code";
  String[] inParamE = new String[2];
  // inParamE[0] = "select a.mode_code,trim(a.sale_code),a.mode_code||'--'||c.mode_name from dChnResSalePlanModeRel a , dphoneSaleCode b,sbillmodecode c where a.sale_code=b.sale_code and b.region_code=:region_code and b.sale_type='52' and b.region_code=c.region_code and a.mode_code=c.mode_code";
  inParamE[0] = "select a.mode_code,trim(a.sale_code),a.mode_code||'--'||c.offer_name from dChnResSalePlanModeRel a , dphoneSaleCode b,product_offer c where a.sale_code=b.sale_code and b.region_code=:region_code and b.sale_type='52' and trim(a.mode_code) = to_char(c.offer_id) and b.valid_flag = 'Y' and c.exp_date>sysdate";
	  		...
	  		if((document.all.m_smCode.value !="") && (document.all.m_smCode.value != document.all.oSmCode.value))
	  		{
		  		if(document.all.oSmCode.value == "gn"){
		  			...
		  			if(smzvalues=="3") {//非实名制全球通、动感地带客户转移为神州行客户
		  				rdShowMessageDialog("<%=readValue("g124","ps","jf",Readpaths)%>");
		  			}
		  		}
	  		}
...
function getbrand(){
 ...
 function checkAward()
 {
  ...
  function salechage(){
    ...
    document.all.do_note.value = "用户"+document.all.phoneNo.value+"办理购TD商务固话赠通信费(铁通)";
...
<body>
	...
	<table cellspacing="0">
      ...
      <td colspan="3">
				...
				<input name="Consume_Term" type="hidden" id="Consume_Term" />
...
<div name="licl" id="licl">
			<input type="hidden" name="iOpCode" value="g124">
			...
			<input type="hidden" name="return_page" value="/npage/bill/fg124_login.jsp">		
