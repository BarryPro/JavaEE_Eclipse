...
/****************由移动号码得到密码、机主姓名、 等信息 s1200InitEx***********************/
  ...
  //retList = impl.callFXService("s1200InitEx", paraAray1, "27","phone",phoneNo);
%>
  <wtc:service name="s1200Init" routerKey="phone" routerValue="<%=phoneNo%>" retcode="retCode1" retmsg="retMsg1" outnum="27" >
...
<%
  ...
  if(tempArr[0][1] == null)
  {
	...
	{
	   retFlag = "1";
	   retMsg = "s1200Init查询号码基本信息为空!<br>errCode: " + errCode + "<br>errMsg+" + errMsg;
    }    
  }else if(!(tempArr[0][1] == null))
  {
	if (errCode.equals("000000")){
	    ...
	    next_main_stream = tempArr[0][24];//预约资费开通流水
		  ...
		  next_begin = tempArr[0][25];	//免填单打印续签开始时间
	 
	}else{
		if(!retFlag.equals("1"))
		{
		   retFlag = "1";
	       retMsg = "s1200Init查询号码基本信息失败!<br>errCode: " + errCode + "<br>errMsg: " + errMsg;
		}
	}
  }
...
<!--
  ...
  function check(){
	...
	if(document.frm.new_rate_name.value =="" || next_rate_code == "" || document.frm.should_fee.value == "")
	{
	  rdShowMessageDialog("请点击查询按钮查询对应的资费信息!");
	  return false;
	}
...
{ 
  	//调用公共js得到方案代码
	document.frm.order_code.value = "";
    var pageTitle = "方案代码查询";
    var fieldName = "方案代码|方案名称|预存款|消费期限|月底线|新套餐代码|新套餐名称|";
    var sqlStr = "select a.order_code,a.order_name,a.prepay_fee,a.consume_term,a.mon_base_fee,a.mode_code,a.mode_code||'--'||c.mode_name from sMachOrderCode a,cBillBbChg b, sBillModeCode c where a.region_code=b.region_code and a.region_code=c.region_code  and b.op_code = 'k200' and a.mode_code=c.mode_code and a.mode_code=b.new_mode and a.region_code='<%=orgCode.substring(0,2)%>'  and b.district_code in ('<%=orgCode.substring(2,4)%>','99') and a.flag='1' and b.old_mode = '<%=java.net.URLEncoder.encode(rate_code)%>' and a.order_code like '" + codeChg("%"+document.frm.order_code.value+"%") + "'	order by a.order_code ";
    ...
    if(PubSimpSel(pageTitle,fieldName,sqlStr,selType,retQuence,retToField))
		document.frm.qryFlag.value = "1";
	getMidPrompt("10442",codeChg(document.getElementById("new_rate_code").value),"tdid");
...
function setOpNote(){
	...
	{
	  document.frm.opNote.value = "畅听套餐续签;号码:"+document.frm.phoneNo.value+";方案:"+document.frm.order_code.value; 
	}
	return true;
}
//-->
...
<form name="frm" method="post">
	<%@ include file="/npage/include/header.jsp" %>   
      ...
      <table cellspacing="0">
		  ...
		  <tr>
            ...
            <td colspan="3" id="tdid">
			  <input name="order_code" type="text"  id="order_code" size="8"  value="<%=order_code%>" readonly>
			  <input name="order_name" type="text"  id="order_name" size="18"  value="<%=order_name%>" readonly>			  
          ...
          <tr style="display:none"> 
            <td class="blue">备注</td>
            <td colspan="3">
             <input name="opNote" type="text" id="opNote" size="50" value="" onFocus="setOpNote();"> 
            </td>
          </tr>
		  <tr> 
            <td colspan="4"> 
  ...
  <input type="hidden" name="qryFlag">
  ...
  <input type="hidden" name="next_begin" value = "<%=next_begin%>">
