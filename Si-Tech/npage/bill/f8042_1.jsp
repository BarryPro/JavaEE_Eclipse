...
//	comImpl co=new comImpl();
	//手机品牌
	String sqlAgentCode = " select unique a.brand_code,trim(b.brand_name) from sPhoneSalCfg a,schnresbrand b where a.region_code='" + regionCode + "' and a.sale_type='10' and a.brand_code=b.brand_code and valid_flag='Y' and a.spec_type like 'P%'";
	//手机类型
	String sqlPhoneType = "select unique a.type_code,trim(b.res_name), b.brand_code from sPhoneSalCfg a,schnrescode_chnterm b where a.region_code='" + regionCode + "' and  a.type_code=b.res_code and a.brand_code=b.brand_code and a.valid_flag='Y' and a.sale_type='10' and a.spec_type like 'P%' ";
...
<%
	//营销代码
	String sqlsaleType = "select unique a.sale_code,trim(a.sale_name), a.brand_code,a.type_code from sPhoneSalCfg a where a.region_code='" + regionCode + "' and a.sale_type='10' and valid_flag='Y' and a.spec_type like 'P%' ";
...
<%
	//营销明细
	String sqlsaledet = "select BASE_FEE,FREE_FEE,favour_cost,CONSUME_TERM,MON_BASE_FEE,prepay_limit,sale_code from sPhoneSalCfg  where region_code='" + regionCode + "' and sale_type='10' and valid_flag='Y' and spec_type like 'P%'";
...
%>
	<wtc:pubselect name="sPubSelect"  routerKey="region" routerValue="<%=regionCode%>" outnum="7" retcode="retCode3" retmsg="retMsg3">
		<wtc:sql><%=sqlsaledet%></wtc:sql>
	</wtc:pubselect>
	<wtc:array id="saledetStr" scope="end" />
<%
	//新主资费(修改部分)
//	String sqlmodecode = "select a.mode_code,a.sale_code from dChnResSalePlanModeRel a , sPhoneSalCfg b,sbillmodecode c where a.sale_code=b.sale_code and b.region_code='" + regionCode + "' and b.sale_type='10'  and valid_flag='Y' and spec_type like 'P%' and b.region_code=c.region_code and a.mode_code=c.mode_code";
		String sqlmodecode = "select a.mode_code,a.sale_code from dChnResSalePlanModeRel a , sPhoneSalCfg b,product_offer c where a.sale_code=b.sale_code and b.region_code='" + regionCode + "' and b.sale_type='10'  and valid_flag='Y' and spec_type like 'P%' and to_number(a.mode_code)=c.offer_id and c.state = 'A'";
...
%>
	<wtc:pubselect name="sPubSelect"  routerKey="region" routerValue="<%=regionCode%>" outnum="2" retcode="retCode4" retmsg="retMsg4">
...
function checksec(){
	...
	var myPacket = new AJAXPacket("<%=request.getContextPath()%>/npage/bill/qrysecond.jsp","正在校验副卡信息，请稍候......");
	...
	//myPacket.data.add("iOpCode","8042");
	  		...
	  		if((document.all.m_smCode.value !="") && (document.all.m_smCode.value != document.all.oSmCode.value))
	  		{
		  		if(document.all.oSmCode.value == "gn"){
		  			...
		  			if(smzvalues=="3") {//非实名制全球通、动感地带客户转移为神州行客户
		  				rdShowMessageDialog("<%=readValue("8042","ps","jf",Readpaths)%>");
		  			}
		  		}
	  		}
 ...
 function frmCfm()
  {
	 ...
	 +document.frm.Mon_Base_Fee.value+"|"+document.frm.IMEINo.value+"|"+document.frm.secondphone.value+"|"+document.frm.secondname.value+"|"
...
function getbrand(){
 ...
 function checkAward()
 {
  ...
  function salechage(){
	...
	for ( x3 = 0 ; x3 < arrdetsalecode.length  ; x3++ )
   	{
      		if ( arrdetsalecode[x3] == document.all.sale_code.value )
      		{
        		...
        		//document.all.Mark_Subtract.value="0";
        		//为方便测试，积分扣减设为0，上线后注意修改  by   ningtn
    ...
    document.all.do_note.value = "用户"+document.all.phoneNo.value+"办理预存送手机、话费共分享";
...
<table cellspacing="0">
	...
	<tr>
		...
		<td>
			...
			<font color="orange">*</font>
			<input name="checksecond" class="b_text" type="button" value="副卡校验" onclick="checksec()">
		</td>
		<td class="blue">副卡姓名</td>
		<td>
	...
	<tr>
		<td class="blue">副卡品牌</td>
		<td>
			<input name="secondsmname" type="text" class="InputGrey" id="secondsmname" readonly>
		</td>
		<td class="blue">副卡资费</td>
		<td>
			<input name="secondmodename" type="text" class="InputGrey" id="secondmodename" readonly>
		</td>
	</tr>
	...
	<tr>
		<td class="blue">副卡预存</td>
		<td>
			<input name="secondprepay" type="text" class="InputGrey" id="secondprepay" readonly>
		</td>
		<td class="blue">副卡积分</td>
		<td>
			<input name="secondMarkPoint" type="text" class="InputGrey" id="secondMarkPoint" readonly>
		</td>
	</tr>
<!---------------------------副卡信息结束----------------------------------------------->
	<tr>
...
<div name="licl" id="licl">
			<input type="hidden" name="iOpCode" value="8042">
			<input type="hidden" name="opCode" value="8042">
			...
			<input type="hidden" name="return_page" value="/npage/bill/f8042_1.jsp">			
