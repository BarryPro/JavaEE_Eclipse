...
<%
    ...
    String iold_m_code = ReqUtil.get(request, "i16"); //.substring(0, ReqUtil.get(request, "i16"));        //现主套餐代码（老）
    ...
    System.out.println("mylog iop_code-- "+iop_code);
...
<%
    ...
    //sqlStr4 = "select mode_name from sbillmodecode where region_code='" + regionCode + "' and     mode_code='" + matureCode + "'";
     sqlStr4="select offer_name from product_offer where offer_id = to_number('" + matureCode + "')";
%>
...
<TABLE cellSpacing="0">
    <TR>
        ...
        <input type="hidden" name="next_main_cash" value="<%=ReqUtil.get(request,"i18").substring(0,8)%>">
...
<%
                ...
                */
				sqlStr =" SELECT b.offer_attr_type,d.mem_role_name,c.role_down_num,c.role_up_num "
						        ...
						        sqlStr +=" group by b.offer_attr_type,d.mem_role_name,c.role_down_num,c.role_up_num";
						        System.out.println("---------------sqlStr-------------===================="+sqlStr);
%>
...
%>
 ...
 if(data[y][1].equals("Y")&&data[y][11].equals("d")){
 		...
 		checkedFlag = "checked";
	 	disabledFlag = "";
 	}else if(data[y][1].equals("N")&&data[y][11].equals("0")){
 		checkedFlag = "checked";
	 	disabledFlag = "disabled";
    ...
    <TBODY>
        <TR>
            <TD id="footer">
                ...
                <input name=kkkk class="b_foot" onClick="history.go(-1)" type=button value=返回>
            </TD>
        </TR>
    </TBODY>
</TABLE>
...
function frmCfm() {
		...
		<%
			String newurl="";
			if(iop_code.equals("125a")){

				newurl="f1270_4_year.jsp";
			}else{

				newurl="fa270_q_year.jsp";
...
function openwindow(theNo, p, q)
{
    ...
    var ipV1 = document.all.ip.value;
    var ip = "";
    if(ipV1.indexOf(" ")!=-1){
    		ip = ipV1.substring(0,ipV1.indexOf(" "));
    	}else{
    		ip = ipV1;
    	}
...
function senddata()
{
    ...
    //alert("document.all.R1["+i+"].value|"+document.all.R1[i].value+"\n"+"document.all.R11["+i+"].value|"+document.all.R11[i].value);
        if((document.all.R1[i].value=="Y"&&document.all.R11[i].value=="d")||(document.all.R1[i].value=="N"&&document.all.R11[i].value=="0")||(document.all.R1[i].value=="Y"&&document.all.R11[i].value!="d"&&document.all.checkId[i].checked==false)||(document.all.R1[i].value=="N"&&document.all.R11[i].value=="1"&&document.all.checkId[i].checked==true))
        {
