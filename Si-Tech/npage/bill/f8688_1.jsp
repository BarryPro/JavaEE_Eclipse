...
<%
  ...
  if(phone==null||phone.length()==0){
  	phone = request.getParameter("srv_no");
  }
...
<form action="" method=post name="form1">
	<%@ include file="/npage/include/header.jsp" %>
      ...
      <table cellspacing="0">
			 <tr>
				  ...
				  <td class="blue" width="15%">证件类型名称</td>
				  <td>
				    <input name="i6" size="30" type="text" maxlength=30 value="<%=result[0][6]%>" class="InputGrey" readonly>
				  </td>
       </tr>
	   <tr style="display:none">
				  <td class="blue">&nbsp;</td>
				  <td>
				    <input name="i5" size="20" type="hidden" maxlength=30 value="<%=result[0][5]%>" class="InputGrey" readonly>
				  <td class="blue">&nbsp;</td>
				  <td>
				    <input name="i7" size="20" type="hidden" maxlength=30 value="<%=result[0][7]%>" class="InputGrey" readonly>
				  </td>
       </tr>
       <tr>
          ...
          <td class="blue">下月主套餐</td>
			 ...
			 <tr style="display:none">
		      <td class="blue">&nbsp;</td>
				  <td>
				    <input name="ibig_cust" type="hidden" size="20" value="<%=ibig_cust_ls%>"  class="InputGrey" readonly>
				    <input type="hidden" name="ibig_cust_1" type="hidden" size="20" maxlength=20 value="<%=i21%>" readonly>
    ...
    <%
             			String sqlStr = "select distinct a.BSM_CODE ,a.sm_code ,b.band_name from snewsmcode a,band b where a.sm_code=b.sm_code";
		%>
      ...
      %>
				...
				<tr style="display:none"> 
				  <td class="blue">业务类型</td>
				  <td>
				  <input type="text" name="s_city" value="zn-->神州行"  class="InputGrey" readonly>
          </td>
          <td class="blue">套餐类别</td>
			      <input type="hidden"  name="subi20" value="<%=subi20%>" >
          <td>
          		 <input type="text" name="s_spot" value="Yn40-->小区计费"  class="InputGrey" readonly>
				  </td>
		    </tr>
				<tr>
      ...
      <input type="hidden" name="favorcode" size="20" maxlength=20 value="<%=result[0][23]%>">
		  ...
		  <input type="hidden" name="return_page" value="/npage/bill/f7981_login.jsp?activePhone=<%=phone%>&opName=预存话费赠TD固话&opCode=7981">
    	<!-------------------------------------------------------------->
		</table>
        <table cellSpacing="0">
          <tbody>
...
function getBankCode()
{
    ...
    sqlStr=sqlStr+"from cBillBBChg a,sBillModeCode b,sTGSaleMode c where a.region_code='<%=subi20%>' and a.district_code in ('<%=disCode%>','99')";
    sqlStr=sqlStr+"and a.op_code='8688' and c.op_code in('8688','0000') and a.OLD_MODE ='<%=java.net.URLEncoder.encode(result[0][16])%>' and a.new_mode like '";
    sqlStr=sqlStr+codeChg("%"+(document.all.modecodetmp1.value)+"%").trim()+"' and b.mode_name like '%"+(document.all.iq.value).trim();
    sqlStr=sqlStr+"%' and b.region_code=a.region_code and a.region_code=c.region_code and a.new_mode=c.mode_code and b.mode_code=a.new_mode and a.POWER_RIGHT <= <%=aftertrim%> and b.POWER_RIGHT <= <%=aftertrim%>";
	...
	**/
	var sqlStr =" select a.offer_z_id, b.offer_name, decode(a.effect_type,'0', '0 24小时之内生效','2', '2 一般预约 ','3', '3 个性预约','1', '1 第二天生效'),1, b.offer_comments   ";
		sqlStr=sqlStr+" from product_offer_relation a, product_offer b, stgsalemode c where a.op_code = '8688'  and a.offer_z_id = b.offer_id and c.region_code='<%=regionCode%>' ";
		...
		sqlStr=sqlStr+" and sysdate between b.eff_date and b.exp_date and to_char(a.offer_z_id) = trim(c.mode_code) and c.op_code IN ('8688', '0000') ";
...
function judge_area()
 {
	...
	myPacket.data.add("modeCode",(document.all.mainModeCode.value).trim());
...
function submitNext()
{
	...
	if(document.all.flagCodeTd.style.display=="")
	{
		if(document.all.flag_code.value.trim().len()==0)
		{
			rdShowMessageDialog("小区代码不能为空,请选择!");
			return false;	
		}	
	}
  //if(thelength())
