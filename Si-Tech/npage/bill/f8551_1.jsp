...
//******************得到下拉框数据***************************//
	...
	inParamA[0] = "select  unique a.brand_code,trim(b.brand_name) from dphoneSaleCode a,schnresbrand b where a.region_code=:region_code and a.sale_type='31' and a.brand_code=b.brand_code and a.valid_flag='Y'";
	inParamA[1] = "region_code="+regionCode;
	System.out.println("inParamA[0]====="+inParamA[0]);
%>
...
<%
  	...
  	inParamB[0] = "select unique a.type_code,trim(b.res_name), b.brand_code from dphoneSaleCode a,schnrescode_chnterm b where a.region_code=:region_code and  a.type_code=b.res_code and a.brand_code=b.brand_code and a.sale_type='31'and a.valid_flag='Y'";
  	inParamB[1] = "region_code=" + regionCode;
  	System.out.println("inParamB[0]====="+inParamB[0]);
%>
...
<%
  	...
  	inParamC[0] = "select unique a.sale_code,trim(a.sale_name), a.brand_code,a.type_code from dphoneSaleCode a where a.region_code=:region_code and a.sale_type='31' and a.valid_flag='Y'";
  	inParamC[1] = "region_code=" + regionCode;
  	System.out.println("inParamC[0]====="+inParamC[0]);
%>
...
<%
	...
	inParamD[0] = "select to_char(to_number(sale_price)-to_number(prepay_gift)-to_number(prepay_limit)),prepay_gift,consume_term,prepay_limit,to_char(active_term),mon_base_fee,all_gift_price,sale_price,sale_code,market_price from dphoneSaleCode where region_code=:region_code and sale_type='31' and valid_flag='Y'";
	inParamD[1] = "region_code=" + regionCode;
	System.out.println("inParamD[0]====="+inParamD[0]);
%>
...
<%
	...
	inParamE[0] = "select a.mode_code,a.sale_code,a.mode_code||'--'||c.offer_name from dChnResSalePlanModeRel a , dphoneSaleCode b,product_offer c where a.sale_code=b.sale_code and b.region_code=:region_code and b.sale_type='31' and trim(a.mode_code) = to_char(c.offer_id) and b.valid_flag ='Y'";
	  		...
	  		if(document.all.oSmCode.value == "gn"){
	  			...
	  			//rdShowMessageDialog("该操作涉及到品牌变更，您的现有积分或M值将于品牌变更生效时失效，请您在新资费生效前及时兑换。");
		  			...
		  			if(smzvalues=="3") {//非实名制全球通、动感地带客户转移为神州行客户
		  				rdShowMessageDialog("<%=readValue("8551","ps","jf",Readpaths)%>");
		  			}
		  		}
  		}
  	}
...
function salechage()
{
    ...
    document.all.do_note.value = "用户"+document.all.phoneNo.value+"办理购TD固话赠话费(铁通)";
...
<div name="licl" id="licl">
			<input type="hidden" name="iOpCode" value="8551">
			...
			<input type="hidden" name="return_page" value="/npage/bill/f8551_login.jsp">		
