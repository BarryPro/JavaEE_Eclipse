...
/*		String iDICCID = "";
	...
	public OutDTO cfm(InDTO inParam) {
		...
		long contractNo = inDto.getContractNo();
		/* 缴费确认必要校验 */
		cfmCheck(inDto.getOpCode(), inDto.getPayMoney() ,contractNo,inDto.getIsDoweInv());
		...
		if (inDto.getIsDoweInv() != null && inDto.getIsDoweInv().equals("R")) {
			...
			if (!record.isPreInv(contractNo, inDto.getPreloginAccept(), inDto.getPayMoney())) {
				...
				throw new BusiException(AcctMgrError.getErrorCode("8020", "00006"), "没有预开发票记录!");
			}
	...
	protected void cfmCheck(String opCode, long payMoney,long contractNo,String isDoweInv) {
		...
		if(("a".equals(flag)|| "b".equals(flag))&& !"R".equals(isDoweInv) ){
			log.info("账户号码："+contractNo + "该账户有预开发票且没有回收，请先做预开发票回收");
			throw new BusiException(AcctMgrError.getErrorCode(opCode, "00007"), "该账户有预开发票且没有回收，请在2300模块先做预开发票回收");
		}
	}
