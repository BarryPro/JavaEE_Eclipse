...
public class SRemainFeeQry extends AcctMgrBaseService implements IRemainFeeQry {
    ...
    public OutDTO jtQuery(InDTO inParam) {
		...
		ruleNumMap.put("JT_CONTRACT_NO", contractNo);
		ruleNumMap.put("OPER_PHONE", phoneNo);
		long ruleNum = balance.getTransFeeLimitNum(ruleNumMap);
		String flag="";
		if(ruleNum>0) {
			String totalDate = DateUtil.format(new Date(), "yyyyMMdd");
    ...
    public OutDTO qryKdRemain(InDTO inParam) {
        ...
        //取当月应收费用
        long totalMonthFee = billAccount.getKdMonthFee(prcId, regionCode);
